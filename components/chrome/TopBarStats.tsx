...
export function TopBarStats() {
  ...
  return (
    <div
      className="hud-surface ml-auto flex scrollbar-none items-stretch justify-between overflow-x-auto p-1 max-md:ml-0 max-md:grid max-md:w-full max-md:flex-[1_1_100%] max-md:grid-cols-2 max-md:overflow-x-visible max-md:[&>*:nth-child(even)]:border-r-0 md:justify-stretch md:overflow-x-visible"
      id="tbStats"
    >
      {chips.map((c) => (
        <Chip key={c.label} {...c} />
...
export function TopBarTerm() {
  ...
  return (
    <small
      id="tbTerm"
      className="mt-0.5 block text-[10px] font-medium tracking-[.06em] text-ink-faint uppercase max-md:text-[9px] max-sm:max-w-[38vw] max-sm:overflow-hidden max-sm:text-ellipsis max-sm:whitespace-nowrap"
    >
