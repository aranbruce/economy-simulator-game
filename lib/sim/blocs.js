...
export function blocById(id) {
  return BLOC_TEMPLATES[id] || null;
}

export function blocForCountry(countryId, blocMember = {}) {
  const bid = blocMember[countryId];
  if (!bid) return null;
  return BLOC_TEMPLATES[bid] || null;
}

export function countriesInBloc(blocId, blocMember) {
  return Object.keys(blocMember).filter((c) => blocMember[c] === blocId);
}

export function isCustomsUnion(bloc) {
