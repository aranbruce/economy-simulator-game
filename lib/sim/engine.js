...
function renderPanel() {
    ...
    if (tab === "bill") {
        ...
        host.querySelectorAll("[data-bloc-create-name]").forEach((inp)=>{
                inp.addEventListener("keydown", (ev)=>{
                    ...
                    if (ev.key === "Escape") {
                        ev.preventDefault();
                        if (G.draft.blocCreate) inp.value = G.draft.blocCreate.name || "";
                        render();
                        inp.blur();
                    }
                });
                inp.addEventListener("blur", ()=>{
                    if (!G.draft.blocCreate) return;
                    const v = (inp.value || "").trim().slice(0, 34);
                    const tpl = CUSTOM_BLOC_TEMPLATES[G.draft.blocCreate.template];
                    G.draft.blocCreate.name = v || (tpl ? tpl.name : "");
                    render();
                });
            });
...
function blocMembershipPanelHtml() {
    ...
    if (bid) {
        ...
        if (acc) {
            ...
            const myCustom = G.customBlocs && Object.values(G.customBlocs).filter((b)=>b && b.founder === player);
            if (myCustom && myCustom.length) {
                h += '<div class="eyebrow mt" style="margin-top:10px">Your custom blocs</div>';
                for (const cb of myCustom) {
                    h += accessionCardHtml(cb.id);
                }
            }
        }
        ...
        h += '</div>';
        if (G.draft.blocCreate) {
            h += '<div class="eff">Staged: found <input data-bloc-create-name="1" type="text" value="' + esc(G.draft.blocCreate.name || "") + '" maxlength="34" class="bloc-name-input" style="margin:6px 0;width:100%;font:inherit" aria-label="Name of your trade bloc"/></div>';
        }
    }
    return h;
}
