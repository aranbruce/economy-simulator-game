...
/** Light automatic fiscal rule for AI seats — keep books from exploding. */ function applyAiFiscalRule(law, econ) {
    if (!law || !econ || !law.spend) return;
    const def = econ._lastDeficit;
    if (def == null || !Number.isFinite(def)) return;
    if (def > 7) {
        for (const d of DEPTS){
            if (law.spend[d.id] > d.min + 0.15) law.spend[d.id] = Math.max(d.min, law.spend[d.id] - 0.08);
        }
        if (law.taxes && law.taxes.vat && law.taxes.vat.on && law.taxes.vat.rate < 24) law.taxes.vat.rate += 0.25;
...
function realmLawKey(role) {
    ...
    for (const [legacy, country] of Object.entries(LEGACY_ROLE_MAP)){
        if (country === id) return legacy;
    }
    return id;
}
...
const R_TFP = 0.18; // points of annual TFP growth per unit of (R/R0 - 1)
...
/* One quarter of the expenditure side. Every component is a level indexed so
   ...
   all at once. */ function expenditureStep(e, law, E, revShare, modSpend, g, det) {
    ...
    for (const p of partners){
        ...
        const pulse = partnerPulse[p.id] || 0;
        const rel = relMap[p.id] != null ? relMap[p.id] : 50;
    ...
    const gravityRest = tradeRestShare(homeRole) * shareX / 100 * e.worldY * Math.pow(compBase, ELAST_X);
    const restBlend = g && g.q != null ? Math.min(1, Math.max(0, g.q) / 4) : 1;
    const restX = e.clearedFlows && e.clearedFlows.rest != null ? gravityRest * (1 - restBlend) + e.clearedFlows.rest * restBlend : gravityRest;
    bilat.rest = restX;
    Xstar += restX;
   ...
   ================================================================== */ function step(g, law, prevLaw, det) {
    ...
    /* A creditor state pays no risk premium; it earns a return roughly in line
     with world rates on what it holds. */ if (e.debt < 0) {
        ...
        e.riskPremium = 0;
    } else e.yield = clamp(e.rate + 0.3 + Math.pow(Math.max(0, e.debt - 80), 1.35) * 0.012 + Math.pow(Math.max(0, deficit - 3), 1.40) * 0.20 + e.riskPremium - (law.policies.fiscalRule ? 0.5 : 0), 0.2, 25);
    e.riskPremium *= 0.7;
    e.effInterest += (e.yield - e.effInterest) * MATURITY_PASS;
