...
function createCustomBloc(name, templateId) {
    ...
    if (!tpl) return null;
    const player = playerCountryId();
    if (countryBlocId(player)) return null;
    if (G.blocAccession || (G.blocAccessionByCountry && G.blocAccessionByCountry[player])) {
        return null;
    }
...
function enactHumanSeatOnSnapshot(g, seatId, draft, opts) {
    ...
    try {
        ...
        for (const dealId of Object.keys(g.draft.deals)) {
            ...
            parkInboundDealProposal(g, seatId, d.partner, dealId);
        }
...
function paintTradePanel(host) {
    ...
    host.querySelectorAll("[data-bloc-withdraw]").forEach((b)=>b.onclick = ()=>{
            ...
            G.draft.blocAccession = null;
            G.draft.blocCreate = null;
            if (G.blocAccessionByCountry && player) delete G.blocAccessionByCountry[player];
            render();
        });
