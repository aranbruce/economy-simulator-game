...
/* Elasticity of taxable income, rising with income: the top of the tail has the
   ...
   Laffer fudge on income. */ const ETI_OF = (inc) =>
...
function missionOptionHint(opt, ctx) {
  ...
  if (opt.fac) {
    const f = Object.entries(opt.fac)
      .filter((e) => e[1])
...
function dealsWith(pid, law) {
  ...
  return Object.keys(L.deals).filter(
    (id) => L.deals[id] && DEAL_BY_ID[id] && DEAL_BY_ID[id].partner === pid,
  );
}
...
function shareSameCustomsUnion(partnerId, homeRole, blocMember) {
  ...
  const bloc =
    blocById(pb) ||
    Object.values((G && G.customBlocs) || {}).find((b) => b.id === pb);
...
const taxGroup = (id) =>
  SYNTH_GRP[id] || (TAX_BY_ID[id] ? TAX_BY_ID[id].grp : "other");
...
/* ==================================================================
  ...
  for (const f of FACTIONS) {
    ...
    g.fac[f.id] = clamp(
      g.fac[f.id] + (t - g.fac[f.id]) * 0.2 + (shock[f.id] || 0) + mA,
      2,
      96,
    );
  }
...
function updateMapLabel() {
  ...
  box.innerHTML =
    ...
    "</b>" +
    "<span>" +
    MAP_METRICS.map((m) => m.name + " " + m.fmt(st[m.id])).join(" &middot; ") +
    "</span>";
}
function updateMap() {
  if (Map2D.ctx) {
...
function spark(key) {
  let w = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 44,
...
function termReviewScore() {
  ...
  const record =
    (recent.reduce((a, r) => a + (r.growth || 0), 0) / n) * 2.5 -
...
/* ---- tab panels ---- */ function globeFallbackHtml() {
  ...
  try {
    img =
      ...
      worldCanvas().toDataURL("image/png") +
      '">';
  } catch (err) {
    img = "";
  }
...
function paintTradePanel(host) {
  ...
  for (const r of ["europe", "americas", "asia", "africa", "gulf", "oceania"]) {
    ...
    partnerCards +=
      ...
      '</div><div class="cards">';
    partnerCards += list
      .map((p) => partnerTradeCardHtml(p, bilat, bilatTotal))
      .join("");
    partnerCards += "</div>";
  }
  host.innerHTML =
    ...
    dealsHtml = dealsForPartner(p, G.homeRole)
      .map((d) => {
        ...
        return (
          ...
          d.terms.map((t) => "&middot; " + t).join("<br>") +
          "</div>" +
          (unmet.length && !signed
  ...
  if (lock.mode !== "none") {
    ...
    for (const bid in usedBlocs) {
      ...
      h +=
        ...
        leverHtml(
          "tariffBloc:" + bid,
          (bloc ? bloc.name : bid) + " (" + usedBlocs[bid] + " countries)",
          val,
          0,
          25,
          1,
...
function dealBlockers(d) {
  ...
  if (n.deal && !D.deals[n.deal])
    out.push("requires " + DEAL_BY_ID[n.deal].name.toLowerCase());
  needBlockers(n, D, {
    partner: d.partner,
  }).forEach((b) => out.push(b));
    ...
    opts: [
      ...
      },
      {
        ...
        f: () => {
          ...
          const custom = bid && G.customBlocs[bid];
          if (custom)
            custom.members = custom.members.filter((m) => m !== G.eventFocus);
        },
      },
      {
...
function briefingImpactParts(im, meta) {
  ...
  const facs = Object.entries(im.fac || {})
...
function showMpInboundDealProposal(prop) {
  ...
  const terms =
    Array.isArray(prop.terms) && prop.terms.length
      ? "<ul>" +
        prop.terms.map((t) => "<li>" + esc(t) + "</li>").join("") +
        "</ul>"
      : "";
  const body =
    "<p>" +
...
function fullEffects(imp, fac, cost, ch) {
  ...
  for (const k in imp || {}) {
    if (!imp[k]) continue;
    bits.push(
      (IMP_NAMES[k] || k) + " " + sgn(imp[k], Math.abs(imp[k]) < 1 ? 2 : 1),
    );
  }
  for (const k in ch || {}) {
    if (!ch[k]) continue;
    bits.push(
      (IMP_NAMES[k] || k) + " " + sgn(ch[k], Math.abs(ch[k]) < 1 ? 2 : 1),
    );
  }
  const f = Object.entries(fac || {})
    .filter((e) => e[1])
    .sort((a, b) => b[1] - a[1]);
  const facTxt = f
    .map((param) => {
...
function beginCoachStep(stepIndex) {
  if (!G) return;
  ...
  G.coach = {
    ...
    startQ: step.trackStartQ ? G.q : G.q,
  };
...
/* Opening / restart country picker. Tests and the reference HTML keep calling
   newGame() directly; the Next shell registers a setup overlay instead. */ let _onSetup =
  null;
