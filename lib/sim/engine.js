import { REALM_LAW, applyRealmLawOverlay } from "./realmLaws.js";
...
import { clearBilateralTrade, seatsFromWorld, worldSeatIds } from "./worldTrade.js";
import { stepCurrencyAreas } from "./fxAreas.js";
import { metricRamp } from "./metricRamp.js";
import { relationColour } from "./partners.js";
import {
  LEDGER_DECAY, LEDGER_FLOOR, ENVOY_SLOTS, ENVOY_TARGET, ENVOY_ASSIGN_PC, ENVOY_RECALL_PC,
  ULTIMATUM_PC, ULTIMATUM_CD, ULTIMATUM_WAIT, ULTIMATUM_JITTER, DIPLO_POLICY_KEYS, DIPLO_SPHERES, DIPLO_CAMPS,
  ensureDiploStocks as ensureDiploStocksCore, seedRelBase, addDiploLedger, setSanctionStance,
  clearSanctionStance, decayDiploLedger, maybeClearSanctionStance, spherePatronsForPartner,
  buildRelationModifiers, canIssueUltimatum as canIssueUltimatumCore, emptyEnvoys, baseLabel,
  ultimatumDemandsFor, baseP, concedeP, partnerSelfInterest, interestTag, partnerTariffOnPlayer,
  applyUltimatumConcede, applyUltimatumDefy, ultimatumOutcomeImpacts, rollMissionEvent, formatMissionEventText, MISSION_EVENTS,
  sharedCamp, VISIT_DURATION, beginActiveVisit, visitQuartersLeft, isVisitActive,
  diploMapMarkers, hasFormalProtest,
} from "./diplomacy.js";
import { queueDrawerPartnerScroll } from "../scrollDrawerPartner.js";
...
/* UK 2025-26: total managed expenditure 44.8% of GDP, receipts 40.4%, deficit
   ...
   residual so the opening fiscal position stays put. UN dues (0.02, the
   default-on unMember policy) are likewise carved out of the residual. */ const OTHER_SPEND = 7.68, OTHER_REV = 12.25, TERM_LEN = 20;
/* Political regime per seat. Opening pin on NATION_PROFILE; live player polity
   ...
   use authoritarian. */ const POLITY = {
    democracy: {
        termLen: 20, loseAt: 44, kind: "election", label: "Democracy",
        capitalRegen: 1.0, coupOn: "approval", coupFloor: 20, coupQuarters: 4,
        changePc: 40,
        blurb: "Competitive elections on a five-year clock. Capital recovers at full rate."
    },
    hybrid: {
        termLen: 20, loseAt: 30, kind: "managed", label: "Hybrid regime",
        capitalRegen: 0.85, coupOn: "approval", coupFloor: 20, coupQuarters: 4,
        changePc: 36,
        blurb: "Managed ballots and thinner mandates. Capital recovers a little slower."
    },
    authoritarian: {
        termLen: 40, loseAt: 28, kind: "congress", label: "Authoritarian",
        capitalRegen: 0.55, coupOn: "patriots", coupFloor: 28, coupQuarters: 3,
        changePc: 48,
        blurb: "Party congress on a decade clock. Slow capital recovery; elite coup risk."
    }
};
...
const REL_POLITY = 6;
/** Collapse legacy "monarchy" pins into authoritarian. */ function normalisePolityId(key) {
    if (key === "monarchy") return "authoritarian";
    return key && POLITY[key] ? key : null;
}
/** Opening / partner pin from NATION_PROFILE only. */ function profilePolityId(role) {
    const id = resolveHomeRole(role != null ? role : "home");
    const p = id === "home" || id === "kingdom" ? NATION_PROFILE.kingdom : NATION_PROFILE[id];
    return normalisePolityId(p && p.polity) || "democracy";
}
/** Live polity id: player law.polity when present, else the profile pin. */ function polityIdOf(role) {
    const id = resolveHomeRole(role != null ? role : typeof G !== "undefined" && G && G.homeRole || "home");
...
function polityOf(role) {
    return POLITY[polityIdOf(role)];
}
function termLenOf(role) {
    return polityOf(role).termLen || TERM_LEN;
}
/** All live polity ids (ladder order). `from` is ignored — any system can
 *  stage any other; capital scales with distance via polityChangePc. */ function polityReachable(from) {
    return POLITY_LADDER.slice();
}
function polityCanReach(from, to) {
    return !!normalisePolityId(to);
}
/** Steps on the democracy–hybrid–authoritarian ladder. */ function politySteps(from, to) {
...
function polityChangePc(from, to) {
...
function coupMetric(fac, meta) {
    const m = meta || polityOf();
    if (m.coupOn === "patriots") return fac && fac.patriots != null ? fac.patriots : 50;
    return approvalOf(fac);
}
function reviewNoun(meta) {
    const m = meta || polityOf();
    return m.kind === "congress" ? "congress" : m.kind === "managed" ? "ballot" : "election";
}
function reviewStamp(meta) {
    const m = meta || polityOf();
    return m.kind === "congress" ? "Party congress" : m.kind === "managed" ? "Managed ballot" : "General election";
}
function careerHint(meta, sandbox) {
    const m = meta || polityOf();
    if (sandbox) return "Sandbox is on: you cannot be removed from office. Switch to Career any time from the top bar.";
    if (m.kind === "congress") {
...
function polityShiftIsLiberalising(shift) {
    if (!shift) return false;
    return polityRank(shift.to) < polityRank(shift.from);
}
function polityShiftIsTightening(shift) {
    if (!shift) return false;
    return polityRank(shift.to) > polityRank(shift.from);
}
/* --- The income distribution: twenty equal slices of the taxpaying
   population, gross annual income in pounds. Income tax and national insurance
   are integrated over this rather than approximated by a single rate, so
   thresholds matter as much as rates, and fiscal drag falls out for free: hold
   a threshold still while wages rise and the real tax take climbs on its own. */ const INCOME_DIST = [
    {
        w: 0.05,
        inc: 5000
    },
    ...
    /* The top five percent is a Pareto tail, not a point mass. Without this a new
     band above the old top slice would raise literally nothing, and the whole
     argument about taxing the rich would be unplayable. */ {
        w: 0.035,
        inc: 110000
    },
    {
...
/* Wage-setting / price-setting equilibrium unemployment, after Layard, Nickell
   ...
   wages falls directly on labour demand. */ const U_STAR = 4.1, WEDGE_ELAST = 0.06, LABOUR_COST_ELAST = 0.30, REPLACEMENT_ELAST = 0.06, SKILLS_ELAST = 0.05;
/* Natural real rate (r*) for the Taylor rule and the consumption real-rate term. */ const R_NEUTRAL = 3.4, R_NEUTRAL_REAL = 2.0;
/* Phillips curve: lambda is inertia, kappa the slope, anchor the weight on target */ const PI_TARGET = 2.0, PI_LAMBDA = 0.55, PI_KAPPA = 0.16, PI_ADJ = 0.09;
...
/* ---- Credibility as a stock ----
   ...
   weight expectations put on the target rather than on recent experience. */ const CRED_GAIN = 0.035, CRED_LOSS = 0.030;
/* ---- Effective lower bound ----
   ...
   it. Manual mode can go a little deeper for experimentation. */ const RATE_FLOOR = -1;
...
/* Profit shifting. The dominant behavioural response to corporate tax is not
   ...
   of the world. */ const WORLD_CORP = 21, SHIFT_ELAST = 0.018;
/* About a quarter of UK gilts are index-linked, so that portion cannot be
   ...
   interest bill. */ const INDEX_LINKED = 0.25, IL_REAL_COUPON = 0.6, MATURITY_PASS = 0.045;
const TAYLOR_PI = 0.6, TAYLOR_Y = 0.3, TAYLOR_SMOOTH = 0.26;
/* Okun */ const OKUN_BETA = 0.35, U_ADJ = 0.08;
...
/* Collection factors. The employer figure is reliefs and the employment
   ...
   the real headline and the collected share carries the difference. */ const EMPLOYER_COLLECT = 0.71, EMPLOYEE_COLLECT = 0.62;
/* Elasticity of taxable income, rising with income: the top of the tail has the
   ...
   Laffer fudge on income. */ const ETI_OF = (inc)=>0.05 + 0.55 * Math.pow(Math.max(0, Math.min(1, (inc - 40000) / 260000)), 0.7);
/* Above roughly a 45% combined marginal rate, activity stops being avoided and
   ...
   ETI above only bends the top of the distribution. */ const informality = (mtr)=>1.7 * Math.pow(Math.max(0, mtr - 0.45), 2);
/* Standard iso-elastic response: declared income moves with the net-of-tax
   ...
   60% and the bottom does not turn over until informality bites. */ const MTR_REF = 0.30;
function declaredFactor(inc, mtr) {
    const m = Math.min(0.95, Math.max(0, mtr));
    const e = ETI_OF(inc);
    return Math.max(0.15, Math.pow((1 - m) / (1 - MTR_REF), e) * (1 - informality(m)));
}
const DEF_INCOME = {
    on: true,
    allowance: 12570,
...
/* --- tax instruments. base = % of GDP raised per point of rate ---
   ...
   tax is abolishable; ones that start on still need a price for the reverse. */ const TAXES = [
    {
        id: "capGains",
        basis: "assets",
...
const TAX_BY_ID = Object.fromEntries(TAXES.map((t)=>[
        t.id,
        t
    ]));
   ...
   from the post-tax income distribution. */ const REGIMES = [
    {
        ...
        pc: 0,
        blurb: "Graduated rates on income, separate rates on capital. What {C} has always done.",
        mult: {},
        imp: {},
        fac: {}
    },
    {
        ...
        name: "Flat tax",
        pc: 34,
        blurb: "One rate on all income above an allowance. The additional rate is abolished and collection gets cheaper.",
        mult: {
            income: 0.88
        },
        imp: {
    ...
    },
    {
        ...
        pc: 32,
        blurb: "Tax what people spend, not what they earn. Income tax bases shrink, VAT and duties do the heavy lifting.",
        mult: {
            income: 0.72,
            consumption: 1.35
        },
    ...
    },
    {
        ...
        pc: 38,
        blurb: "Tax the unimproved value of land and lighten the load on work. Economists approve. Landowners do not.",
        mult: {
            income: 0.90,
            wealth: 2.4
        },
    ...
    },
    {
        ...
        pc: 28,
        blurb: "Labour income taxed progressively, capital income at a single flat rate. Broad base, fewer leaks.",
        mult: {
            wealth: 1.55,
            income: 0.95
        },
...
const REGIME_BY_ID = Object.fromEntries(REGIMES.map((r)=>[
        r.id,
        r
    ]));
   ...
   wages, not authored as a Gini override. */ const POLICIES = [
    {
        ...
        cost: 0.85,
        pc: 12,
        blurb: "Free places from nine months. Pulls a lot of second earners back into work.",
        ch: {
            part: 2.5,
            fertility: 2.6,
            tfp: 0.04
    ...
    },
    {
        ...
        cost: 5.2,
        pc: 32,
        blurb: "An unconditional payment to every adult, replacing most of the means-tested system.",
        ch: {
            replace: 2.6,
            mpcw: 6.0,
            part: -1.1
    ...
    },
    {
        ...
        cost: 0,
        pc: 18,
        blurb: "Lighter dismissal rules and fewer sectoral protections. Hiring gets easier both ways.",
        ch: {
            replace: -2.2,
            part: 0.8,
            tfp: 0.10
    ...
    },
    {
        ...
        cost: 0,
        pc: 22,
        blurb: "Presumption in favour of development. The single biggest thing you can do to trend growth, and the most hated.",
        ch: {
            kboost: 0.38,
            tfp: 0.05
        },
    ...
    },
    {
        ...
        cost: 0.35,
        pc: 10,
        blurb: "Pensions rise by the highest of earnings, prices or 2.5%. The cost compounds forever.",
        imp: {},
        fac: {
            pensioners: 9,
            workers: -2,
    ...
    },
    {
        ...
        cost: 1.0,
        pc: 18,
        blurb: "Put a slice of receipts away every year. Boring, and your successors will thank you.",
        imp: {},
        fac: {
            business: 3,
            patriots: 3,
    ...
    },
    {
        ...
        cost: 0.55,
        pc: 8,
        blurb: "Generous expensing that pulls private labs into the public research effort.",
    ...
    },
    {
        ...
        cost: 2.2,
        pc: 28,
        blurb: "Buy back the networks at regulated asset value. Expensive, popular, irreversible in practice.",
        imp: {
            srv: 3
        },
        ch: {
    ...
    },
    {
        ...
        cost: 0.9,
        pc: 14,
        blurb: "Six reactors and a small modular pipeline. Insulates you from the next energy shock.",
        imp: {
            env: 4
        },
        ch: {
    ...
    },
    {
        ...
        cost: 0,
        pc: 12,
        blurb: "Charge imports for the carbon they embody. Protects your industry, annoys everyone else's.",
        imp: {
            env: 3
        },
        ch: {
    ...
    },
    {
        ...
        cost: -0.25,
        pc: 12,
        blurb: "Fewer short sentences, more community disposals. Saves money, costs headlines.",
        imp: {
            cri: 3,
            lib: 6
        },
    ...
    },
    {
        ...
        cost: 0.3,
        pc: 22,
        blurb: "Every transaction visible to the revenue. Evasion collapses. So does privacy.",
        imp: {
            eva: -0.35,
            lib: -10
        },
    ...
    {
        id: "unMember",
        name: "United Nations membership",
        cat: "World",
        cost: 0.02,
        pc: 30,
        blurb: "A seat in the General Assembly, assessed dues, and a voice in the world's arguments. In force since the founding; walking out is cheap to do and expensive to be.",
        fac: {
            urban: 2,
            patriots: -1
        }
    ...
    },
    {
        ...
        cost: 0,
        pc: 10,
        blurb: "Debt must fall as a share of GDP within five years. Buys credibility, removes your excuses.",
        imp: {},
        fac: {
            business: 5,
            pensioners: 2
    ...
    },
    {
        ...
        cost: 0.15,
        pc: 14,
        blurb: "Licences for onshore shale. Cheaper gas and less import exposure, paid for in the countryside.",
        imp: {
            env: -6
        },
        ch: {
    ...
    },
    {
        ...
        cost: 1.1,
        pc: 10,
        blurb: "A temporary cap on household energy bills, paid from general taxation.",
        imp: {
            hlt: 0.2
        },
        ch: {
    ...
    },
    {
        ...
        cost: 0.45,
        pc: 10,
        blurb: "Guarantees and deposits for first-time buyers. Demand up; prices follow.",
        ch: {
            kboost: 0.15,
            hbuild: 0.2
        },
    ...
    },
    {
        ...
        cost: -0.4,
        pc: 12,
        blurb: "Sell the existing public housing stock to tenants and investors. Cash now, fewer homes later.",
        ch: {
            hbuild: -0.8,
            kboost: -0.1
        },
    ...
    },
    {
        ...
        cost: 0.4,
        pc: 8,
        blurb: "Funded places in technical colleges and apprenticeships. Builds skills with a lag.",
        ch: {
            hbuild: 0.55,
            tfp: 0.04,
            replace: -0.5,
    ...
    },
    {
        ...
        cost: 0,
        pc: 12,
        blurb: "Statutory protection for green belts and valued landscapes. Building gets harder.",
        ch: {
            kboost: -0.35,
            hbuild: -0.4
        },
    ...
    },
    {
        ...
        cost: 0.1,
        pc: 12,
        blurb: "Every job must carry guaranteed hours. Security for workers, rigidity for firms.",
        ch: {
            replace: 0.8,
            part: -0.3
        },
    ...
    },
    {
        ...
        cost: 0.05,
        pc: 10,
        blurb: "Landlords need a reason to end a tenancy. Security without a hard rent cap.",
        ch: {
            kboost: -0.08
        },
        fac: {
    ...
    },
    {
        ...
        cost: 0.85,
        pc: 10,
        blurb: "A dedicated fund to clear the backlog of elective procedures and outpatient waits.",
        imp: {
            srv: 5,
            hlt: 0.6
        },
    ...
    },
    {
        ...
        cost: -0.25,
        pc: 8,
        blurb: "Restrict the winter energy payment to lower incomes. Saves money; pensioners notice.",
        fac: {
            pensioners: -8,
            workers: 2,
            business: 1
    ...
    },
    {
        ...
        cost: 0.1,
        pc: 12,
        blurb: "No new licences for oil and gas extraction. Cleaner, and more exposed to import prices.",
        imp: {
            env: 5
        },
        ch: {
...
const POLICY_BY_ID = Object.fromEntries(POLICIES.map((p)=>[
        p.id,
        p
    ]));
/* --- what is legal, and on what terms --- */ const VICE = [
    {
        ...
        states: [
            ...
            },
            {
                ...
                blurb: "Possession is a civil matter. Supply stays criminal, so the market stays illegal.",
                imp: {
                    blk: 6,
                    cri: 1,
                    lib: 3,
            ...
            },
            {
                ...
                blurb: "Licensed retail, potency limits, and a duty you can actually collect.",
                imp: {
                    blk: -1,
                    cri: -4,
                    lib: 6,
    ...
    },
    {
        ...
        states: [
            ...
            },
            {
                id: "liberal",
                label: "Liberalised",
                blurb: "Light-touch licensing and open advertising. Receipts rise, so do harms.",
                imp: {
                    hlt: -1.4,
                    cri: 1
                },
        ...
        states: [
            {
                ...
                blurb: "Nobody born after a certain year may ever be sold tobacco. The duty base dies with the cohort.",
                imp: {
                    blk: 9,
                    hlt: 2.2,
                    lib: -5
        ...
        states: [
            {
                id: "banned",
                label: "Criminalised",
                blurb: "Purchase and sale both prosecuted. The trade continues out of sight.",
                imp: {
                    blk: 8,
                    cri: 5,
                    lib: -5,
            ...
            },
            {
                ...
                blurb: "Licensed premises, health checks, employment rights and a levy.",
                imp: {
                    blk: -2,
                    cri: -5,
                    lib: 6,
...
const MISSION_BY_ID = Object.fromEntries(MISSIONS.map((m)=>[
        m.id,
        m
    ]));
const MISSION_CD = 3;
...
function pruneInvalidDraftMissions(g) {
...
function seedGameRelBase(g) {
  ...
  for (const id of Object.keys(g.rel)) {
    ...
    polityOffset[id] = REL_POLITY * aff;
  }
...
function recallEnvoy(partnerId) {
...
function issueUltimatum(partnerId, demandId) {
  if (!G || !partnerId) return false;
  ensureDiploStocks(G.econ);
  ...
  const pick = demandId
    ? demands.find((d) => d.id === demandId) || { id: demandId, label: demandId }
    : demands[0];
...
function processUltimatums(g, det) {
  const state = g || G;
  ...
  for (const pid of Object.keys(state.ultimatums)) {
    ...
    const concede = det ? base >= 0.5 : Math.random() < concedeP(pid, u.demand, state, deps, false, meta);
    const impacts = ultimatumOutcomeImpacts(state, pid, u, concede, deps);
    if (concede) {
      notes.push(name + " conceded to our ultimatum.");
      if (!state.diploAlerts) state.diploAlerts = [];
      state.diploAlerts.unshift({
        kind: "ult_concede",
        partnerId: pid,
        demand: u.demand,
        label: u.label || u.demand || "your demand",
        impacts,
        q: state.q,
        expiresQ: state.q + 3,
      });
    } else {
      notes.push(name + " defied our ultimatum.");
      if (!state.diploAlerts) state.diploAlerts = [];
      state.diploAlerts.unshift({
        kind: "ult_defy",
        partnerId: pid,
    ...
    state.econ.ultimatumCd[pid] = ULTIMATUM_CD;
    delete state.ultimatums[pid];
  }
  if (state.diploAlerts) state.diploAlerts = state.diploAlerts.slice(0, 4);
  if (notes.length && state.brief) state.brief = notes.concat(state.brief).slice(0, 3);
}

function applyMissionEventOption(opt, ctx) {
  if (!G || !opt) return;
  ...
  if (opt.setRel) {
    for (const k in opt.setRel) {
      ...
      if (!pid) continue;
      G.rel[pid] = clamp((G.rel[pid] != null ? G.rel[pid] : 50) + opt.setRel[k], 5, 95);
    }
  }
  if (opt.fac && G.fac) {
    for (const f in opt.fac) G.fac[f] = clamp((G.fac[f] || 50) + opt.fac[f], 2, 96);
  }
  if (opt.relImpulse && partnerId) {
    G.econ.relImpulse[partnerId] = (G.econ.relImpulse[partnerId] || 0) + opt.relImpulse;
  }
  if (opt.capital) G.capital = clamp(G.capital + opt.capital, 0, 100);
  if (opt.ledger && partnerId) {
    addDiploLedger(G, partnerId, {
      id: "mission_evt_" + G.q + "_" + String(opt.ledger.label || "evt").slice(0, 12),
  ...
  if (opt.access && partnerId) {
    G.econ.partnerAccessEff[partnerId] = (G.econ.partnerAccessEff[partnerId] || 0) + opt.access;
  }
  if (opt.tariffCut && lockedTariff(G.law) == null) {
    G.law.tariff = Math.max(0, (G.law.tariff || 0) - opt.tariffCut);
  }
  if (opt.sanctionRival && rivalId) setSanctionStance(G, rivalId, { against: true });
}

function missionOptionHint(opt, ctx) {
    if (!opt) return "";
    const bits = [];
    const relLabel = (key) => {
        if (key === "_partner") return partnerById(ctx.partnerId)?.name || "Partner";
    ...
    if (opt.setRel) {
        for (const k in opt.setRel) {
            ...
            bits.push('<span class="' + (v > 0 ? "up" : "dn") + '">' + esc(relLabel(k)) + " relations " + (v > 0 ? "+" : "") + v + "</span>");
        }
    }
    if (opt.fac) {
        const f = Object.entries(opt.fac).filter((e) => e[1]).sort((a, b) => Math.abs(b[1]) - Math.abs(a[1]));
        for (const [fid, v] of f.slice(0, 3)) {
            const fn = (FACTIONS.find((x) => x.id === fid) || {}).name || fid;
            bits.push('<span class="' + (v > 0 ? "up" : "dn") + '">' + esc(fn) + (v > 0 ? " +" : " ") + v + "</span>");
        }
    }
    if (opt.capital) bits.push('<span class="' + (opt.capital > 0 ? "up" : "dn") + '">Capital ' + (opt.capital > 0 ? "+" : "") + opt.capital + "</span>");
    if (opt.relImpulse) bits.push('<span class="up">Relations drift +' + opt.relImpulse + '/Q</span>');
    if (opt.tariffCut) bits.push('<span class="up">Tariffs −' + opt.tariffCut + ' pts</span>');
    if (opt.access) bits.push('<span class="up">Market access +' + opt.access + '</span>');
    if (opt.sanctionRival && ctx.rivalId) bits.push('<span class="dn">Sanctions on ' + esc(ctx.rivalName || "rival") + '</span>');
    if (!bits.length) return "";
    return '<div class="impfac mission-hint">' + bits.join("") + "</div>";
}

function queueSummitVisitEvents(g) {
  ...
  for (const pid of Object.keys(g.activeVisits)) {
    ...
    g.missionEvents.push({ partnerId: pid, missionId: "summit", q: g.q, eventIndex: fired });
    v.eventsFired = fired + 1;
  }
}

function presentNextMissionEvent(onDone) {
  if (!G.missionEvents || !G.missionEvents.length) {
    onDone();
    return;
  }
  const item = G.missionEvents.shift();
  const ev = rollMissionEvent(item.missionId, item.partnerId, G, diploDeps(), false, item.eventIndex);
  if (!ev) {
    presentNextMissionEvent(onDone);
    return;
  }
  const pName = partnerById(item.partnerId)?.name || item.partnerId;
  let title = (ev.title || "").replace(/\{P\}/g, pName).replace(/\{R\}/g, ev.rivalName || "a rival");
  title = T(title);
  const body = "<p>" + formatMissionEventText(ev, G, diploDeps()) + "</p>" + '<p class="hint">Each choice shifts relations and factions. Expected impacts are listed under every option.</p>';
  const ctx = { partnerId: item.partnerId, rivalId: ev.rivalId, rivalName: ev.rivalName };
  const optsWithHints = ev.opts.map((o) => ({
    b: o.b,
    e: o.e,
    hint: missionOptionHint(o, ctx),
    f: () => {
      applyMissionEventOption(o, { partnerId: item.partnerId, rivalId: ev.rivalId, missionId: item.missionId });
      presentNextMissionEvent(onDone);
    },
  }));
...
function processActiveVisits(g, briefExtra) {
  if (!g || !g.activeVisits) return;
  const e = g.econ;
  if (!e) return;
  ensureDiploStocks(e);
...
function applySphereTrespassOnDeal(dealId, ratified) {
  if (!G || !dealId) return;
  const d = DEAL_BY_ID[dealId];
  ...
  for (const { patron, label } of spherePatronsForPartner(d.partner)) {
    ...
    if (ratified) {
      ...
      addDiploLedger(G, patron, {
        id: "sphere_" + dealId,
        label: "Trade deal in their sphere (" + (client ? client.name : d.partner) + ")",
        pts: -6,
      });
    }
  }
...
function sphereRiskHint(partnerId) {
  const patrons = spherePatronsForPartner(partnerId);
  if (!patrons.length) return "";
  const player = playerCountryId();
  const names = patrons.filter((p) => p.patron !== player).map((p) => {
    const x = partnerById(p.patron);
    return x ? x.name : p.patron;
  });
  if (!names.length) return "";
  return names[0] + " may see this as interference in their sphere (− relations)";
}
/** Partners still on the board for this game. Excludes the seat you occupy and
 *  The Kingdom when you are The Kingdom (ISO 826 already maps to "home"). */ function activePartners(homeRole) {
    const role = resolveHomeRole(homeRole != null ? homeRole : typeof G !== "undefined" && G && G.homeRole || "home");
...
/** Bilateral export weight for an active partner from the seat's trade matrix. */ function partnerShare(homeRole, partner) {
    const role = homeRole != null ? homeRole : typeof G !== "undefined" && G && G.homeRole || "home";
    const id = typeof partner === "string" ? partner : partner.id;
    const fb = typeof partner === "object" && partner ? partner.tradeShare : undefined;
    return shareFor(role, id, fb);
}
/** Rest-of-world residual so named active shares + rest still sum to 1. */ function tradeRestShare(homeRole) {
    const role = homeRole != null ? homeRole : typeof G !== "undefined" && G && G.homeRole || "home";
    const named = activePartners(role).reduce((s, p)=>s + partnerShare(role, p), 0);
    return Math.max(0, 1 - named);
}
/** Deals offered to this seat for a partner (hides Kingdom-as-outsider trees). */ function dealsForPartner(partner, homeRole) {
    const role = homeRole != null ? homeRole : typeof G !== "undefined" && G && G.homeRole || "home";
    const p = typeof partner === "string" ? partnerById(partner) : partner;
    if (!p || !p.deals) return [];
    return p.deals.filter((d)=>{
        if (d.homeOff && d.homeOff.includes(role)) return false;
        if (d.homeOnly && !d.homeOnly.includes(role)) return false;
        return true;
    });
}
function hasDeal(id, law) {
    const L = law || G && G.law;
    return !!(L && L.deals && L.deals[id]);
}
function dealsWith(pid, law) {
    const L = law || G && G.law;
    if (!L || !L.deals) return [];
    return Object.keys(L.deals).filter((id)=>L.deals[id] && DEAL_BY_ID[id] && DEAL_BY_ID[id].partner === pid);
}
/** Lowest lockTariff among ratified deals, or null if the lever is free. */ function lockedTariff(law) {
    const L = law || G && G.law;
    ...
    for(const id in L.deals){
        ...
        if (d && d.lockTariff != null) lock = lock == null ? d.lockTariff : Math.min(lock, d.lockTariff);
    }
    return lock;
}
...
function ensureTariffSchedule(law) {
...
function syncTariffHeadline(law, homeRole) {
    const L = law || G && G.law;
...
function playerCountryId(homeRole) {
    const role = resolveHomeRole(homeRole != null ? homeRole : typeof G !== "undefined" && G && G.homeRole || "home");
    return role === "home" ? "kingdom" : role;
}
/* ---- The United Nations ----
   Membership is law (`law.policies.unMember`, on everywhere by default), so
   leaving is an ordinary bill clause and AI seats inherit membership through
   baseLaw. The Security Council seats five permanent members; any other member
   realm can win an elected term through the `unSeatBid` event. The elected-seat
   expiry lives on econ (like the diplo stocks) so it rides every clone, save
   and preview path without new top-level state. */ const UN_P5 = [
    "united_states",
    "china",
    "russia",
    "france",
...
const UN_SEAT_QUARTERS = 8;
function isUnMember(law) {
    return !!(law && law.policies && law.policies.unMember);
}
function isP5(countryId) {
    return UN_P5.includes(countryId);
}
function unSeatQuartersLeft(g) {
    const st = g || G;
    if (!st || !st.econ || st.econ.unSeatUntil == null) return 0;
    return Math.max(0, st.econ.unSeatUntil - st.q);
}
function onSecurityCouncil(g) {
    const st = g || G;
    if (!st || !isUnMember(st.law)) return false;
    return isP5(playerCountryId(st.homeRole)) || unSeatQuartersLeft(st) > 0;
}
function countryBlocId(countryId, blocMember) {
    const bm = blocMember || G && G.blocMember || {};
    return bm[countryId] || null;
}
function blocMembers(blocId, blocMember) {
    const bm = blocMember || G && G.blocMember || {};
    return countriesInBloc(blocId, bm);
}
function shareSameCustomsUnion(partnerId, homeRole, blocMember) {
    ...
    const bloc = blocById(pb) || Object.values(G && G.customBlocs || {}).find((b)=>b.id === pb);
    return bloc && isCustomsUnion(bloc);
}
function effectiveTariff(partnerId, law, homeRole, blocMember) {
    const L = law || G && G.law;
    ...
    if (lock.mode === "cet" || lock.mode === "full") {
        ...
        return sched.cet != null ? sched.cet : (_blocById_defaultCet = (_blocById = blocById(lock.blocId)) === null || _blocById === void 0 ? void 0 : _blocById.defaultCet) !== null && _blocById_defaultCet !== void 0 ? _blocById_defaultCet : sched.default;
    }
    const pb = countryBlocId(partnerId, bm);
...
function tariffScheduleAverage(law, homeRole, blocMember) {
    const role = homeRole != null ? homeRole : G && G.homeRole || "home";
...
/* Trade-weighted effective import barrier: schedule/CU/CET minus phased deal cuts. */ function importTariffLevel(law, E, e, homeRole, blocMember) {
    const cutNow = (E.tariffCut || 0) + (e && e.tariffCutEff != null ? e.tariffCutEff : 0);
    const avg = tariffScheduleAverage(law, homeRole, blocMember);
    return Math.max(0, avg - cutNow);
}
/* Per-partner market-access targets from ratified deals and shared bloc membership. */ function partnerAccessTargets(law, homeRole, blocMember) {
    const role = homeRole != null ? homeRole : G && G.homeRole || "home";
    const bm = blocMember != null ? blocMember : G && G.blocMember || {};
    const out = {};
    for(const id in law.deals || {}){
        ...
        out[d.partner] = (out[d.partner] || 0) + d.ch.access;
    }
    ...
    if (bid) {
        ...
        if (bloc && bloc.accessBonus) {
            ...
            for (const p of activePartners(role)){
                if (countryBlocId(p.id, bm) === bid) out[p.id] = (out[p.id] || 0) + pts;
            }
        }
    }
    return out;
}
/* Phased trade-depth target: partner access plus tariff integration vs opening. */ function tradeExposureTarget(law, E, e, homeRole, blocMember) {
...
function tariffLocked(law, homeRole, blocMember) {
    const role = homeRole != null ? homeRole : G && G.homeRole || "home";
    const L = law || G && G.law;
    const player = playerCountryId(role);
    const bm = blocMember || G && G.blocMember || {};
    ...
    const bloc = blocById(bid) || G && G.customBlocs && G.customBlocs[bid];
    if (!bloc || !isCustomsUnion(bloc)) return {
        mode: "none"
    };
    const chair = bloc.chair || G && G.customBlocs && G.customBlocs[bid] && G.customBlocs[bid].founder;
    const sched = ensureTariffSchedule(L);
    const cet = sched.cet != null ? sched.cet : bloc.defaultCet;
    if (chair === player) return {
        mode: "cet",
        blocId: bid,
        cet
    };
...
function defaultAccessionForBloc(blocId) {
...
function inviteAcceptMin(blocId) {
    const spec = defaultAccessionForBloc(blocId);
    return spec.memberRelationMin || 45;
}
function blocInviteMemberApprovals(blocId, candidateId) {
    const spec = defaultAccessionForBloc(blocId);
    const min = spec.memberRelationMin || 45;
    const player = playerCountryId();
    return blocMembers(blocId).filter((m)=>m !== player).map((m)=>{
...
function blocInviteBlockers(blocId, candidateId) {
    const out = [];
    const player = playerCountryId();
    ...
    if (G.blocAccessionByCountry && G.blocAccessionByCountry[candidateId]) out.push("accession in progress");
    if (candidateId === player && G.blocAccession) out.push("accession in progress");
    const rel = G.rel[candidateId] != null ? G.rel[candidateId] : 50;
    if (rel < 40) out.push("relations with candidate " + rel.toFixed(0) + " (need 40+)");
    for (const a of blocInviteMemberApprovals(blocId, candidateId)){
        if (!a.ok) out.push("relations with " + a.name + " " + a.rel.toFixed(0) + " (need " + a.min + "+)");
    }
    return out;
}
function blocInviteIsStale(blocId, candidateId) {
    const blockers = blocInviteBlockers(blocId, candidateId);
    return blockers.some((b)=>b === "already in a bloc" || b === "invite pending" || b === "accession in progress");
}
function ensureDraftBlocState() {
    if (!G || !G.draft) return;
    if (!G.draft.blocInvite || typeof G.draft.blocInvite !== "object") G.draft.blocInvite = {};
}
...
/** Per-member relation gate for unanimous accession approval. */ function blocMemberApprovals(blocId) {
    const spec = blocAccessionSpec(blocId);
    const min = spec ? spec.memberRelationMin || 45 : 45;
    const player = playerCountryId();
    const bloc = blocByIdOrCustom(blocId);
    if (!bloc) return [];
    return (bloc.members || []).filter((m)=>m !== player).map((m)=>{
        const rel = G.rel[m] != null ? G.rel[m] : 50;
        const p = partnerById(m);
        return {
            id: m,
...
function needBlockers(need, law, ctx) {
    const out = [];
    const n = need || {};
    const D = law || G.draft || {};
    const partner = ctx && ctx.partner;
    (n.policyOff || []).forEach((p)=>{
        if (D.policies[p]) out.push("incompatible with " + POLICY_BY_ID[p].name.toLowerCase());
    });
    (n.policyOn || []).forEach((p)=>{
        if (!D.policies[p]) out.push("requires " + POLICY_BY_ID[p].name.toLowerCase());
    });
    (n.taxOff || []).forEach((t)=>{
        if (D.taxes[t] && D.taxes[t].on) out.push("requires abolishing the " + TAX_BY_ID[t].name.toLowerCase());
    });
    if (n.tariffMax != null) {
        const t = partner != null ? effectiveTariff(partner, D) : tariffScheduleAverage(D);
        if (t > n.tariffMax) out.push("tariff must be " + n.tariffMax + "% or lower");
    }
    if (n.tariffMin != null) {
        const t = partner != null ? effectiveTariff(partner, D) : tariffScheduleAverage(D);
        if (t < n.tariffMin) out.push("tariff must be at least " + n.tariffMin + "%");
    }
    for(const k in n.deptMin || {})if (D.spend[k] < n.deptMin[k]) out.push(k + " spending must be at least " + n.deptMin[k] + "%");
    return out;
}
/** Why a predefined-bloc accession step may be blocked. */ function blocJoinBlockers(blocId, phase) {
    const out = [];
    ...
    if (acc && acc.blocId !== blocId) out.push("withdraw your current application first");
    if (ph !== "apply") {
        if (!acc || acc.blocId !== blocId) out.push("application not filed");
        else if (ph === "align" && acc.step < 1) out.push("application not yet accepted");
        else if (ph === "accede" && acc.step < 2) out.push("alignment not complete");
    }
    if (ph === "apply" && spec && spec.chairRelationMin && chairId) {
        ...
        if (rel < spec.chairRelationMin) {
            ...
            out.push("relations with " + (chair ? chair.name : chairId) + " " + rel.toFixed(0) + " (need " + spec.chairRelationMin + "+)");
        }
    }
    ...
    if (ph === "accede") {
        ...
        if (spec && spec.chairRelationMin && chairId) {
            const rel = G.rel[chairId] != null ? G.rel[chairId] : 50;
            if (rel < spec.chairRelationMin) {
                out.push("chair relations " + rel.toFixed(0) + " (need " + spec.chairRelationMin + "+)");
            }
        }
    }
    return out;
}
function clearBilateralDeals(law) {
 ...
 *  via world trade clearing — no exogenous growth pulse. */ function applyBlocJoinNationEffects(state, countryId, blocId) {
    if (!state) return;
    if (state.world) {
        try {
...
/** When a partner joins the player's bloc, nudge phased access toward that market. */ function applyBlocJoinPlayerTrade(state, blocId, countryId) {
    ...
    const e = state.econ, law = state.law;
    if (!e || !law) return;
    const targets = partnerAccessTargets(law, role, state.blocMember);
    const target = targets[countryId] || 0;
    if (target <= 0) return;
...
/** Single join path: membership, player statute book, nation books, and player trade access. */ function finalizeBlocJoin(state, countryId, blocId, law) {
    ...
    if (custom && !custom.members.includes(countryId)) custom.members.push(countryId);
    const bloc = blocById(blocId) || custom;
    const player = state.homeRole === "home" ? "kingdom" : resolveHomeRole(state.homeRole || "home");
    const L = law || state.law;
    if (countryId === player && L) {
        if (bloc && isCustomsUnion(bloc)) {
            const sched = ensureTariffSchedule(L);
            if (sched.cet == null) sched.cet = bloc.defaultCet != null ? bloc.defaultCet : sched.default;
            syncTariffHeadline(L, state.homeRole);
        }
...
function blocDealMemberApprovals(blocId, relationMin) {
...
function blocExternalDealBlockers(partnerId, dealId) {
    const out = [];
    const player = playerCountryId();
    const bid = countryBlocId(player);
    if (!bid) out.push("not in a trade bloc");
    else if (!isBlocFounder()) out.push("only the bloc founder can negotiate external treaties");
    if (countryBlocId(partnerId)) out.push("partner trades through their bloc");
    const custom = bid && G.customBlocs[bid];
    const tpl = custom && CUSTOM_BLOC_TEMPLATES[custom.template];
    const min = tpl && tpl.externalDealRelationMin ? tpl.externalDealRelationMin : 42;
    const rel = G.rel[partnerId] != null ? G.rel[partnerId] : 50;
    if (rel < min) out.push("relations with partner " + rel.toFixed(0) + " (need " + min + "+)");
    if (bid) {
        for (const a of blocDealMemberApprovals(bid, min)){
            if (!a.ok) out.push(a.name + " withholds approval (" + a.rel.toFixed(0) + "/" + a.min + ")");
        }
    }
    if (dealId && DEAL_BY_ID[dealId]) {
        const d = DEAL_BY_ID[dealId];
        if (d.partner !== partnerId) out.push("deal does not match partner");
        if (d.need) needBlockers(d.need, G.draft, {
            partner: partnerId
        }).forEach((b)=>out.push(b));
    }
    return out;
}
function draftBlocSnapshotFrom(draft) {
    const d = draft || G.draft || {};
    const bm = clone(G.blocMember);
    const custom = clone(G.customBlocs);
    const player = playerCountryId();
    const bid0 = bm[player];
    if (d.blocLeave) {
        delete bm[player];
        if (bid0 && custom[bid0] && Array.isArray(custom[bid0].members)) {
            custom[bid0].members = custom[bid0].members.filter((m)=>m !== player);
            if (custom[bid0].members.length === 0) delete custom[bid0];
        }
    }
    if (d.blocAccession && d.blocAccession.phase === "accede" && d.blocAccession.blocId && !bm[player]) bm[player] = d.blocAccession.blocId;
    if (d.blocCreate && !bm[player]) {
        const tpl = CUSTOM_BLOC_TEMPLATES[d.blocCreate.template];
        if (tpl) {
...
function applyDraftBlocLaw(law, snap, draft) {
    const d = draft || G.draft || {};
    const player = playerCountryId();
    ...
    if (bid) {
        ...
        if (bloc && isCustomsUnion(bloc)) {
            law.tariffSchedule.cet = bloc.defaultCet != null ? bloc.defaultCet : law.tariffSchedule.cet;
...
function dissolveCustomBlocIfEmpty(blocId) {
    ...
    if (G.blocInvites) {
        for (const cid of Object.keys(G.blocInvites)){
            if (G.blocInvites[cid] && G.blocInvites[cid].blocId === blocId) delete G.blocInvites[cid];
        }
    }
    if (G.blocAccessionByCountry) {
        for (const cid of Object.keys(G.blocAccessionByCountry)){
            if (G.blocAccessionByCountry[cid] && G.blocAccessionByCountry[cid].blocId === blocId) delete G.blocAccessionByCountry[cid];
        }
    }
    if (G.blocAccession && G.blocAccession.blocId === blocId) G.blocAccession = null;
    for (const law of [
        G.law,
        G.draft
    ]){
        if (law && law.tariffSchedule && law.tariffSchedule.bloc && law.tariffSchedule.bloc[blocId] != null) {
            delete law.tariffSchedule.bloc[blocId];
        }
    }
    return true;
}
function leaveBloc(law) {
    const player = playerCountryId();
    const bid = countryBlocId(player);
    if (!bid) return false;
    ...
    if (custom && Array.isArray(custom.members)) {
        custom.members = custom.members.filter((m)=>m !== player);
    }
    dissolveCustomBlocIfEmpty(bid);
    const sched = ensureTariffSchedule(law || G.law);
    sched.cet = null;
    syncTariffHeadline(law || G.law);
    return true;
}
function createCustomBloc(name, templateId) {
    const tpl = CUSTOM_BLOC_TEMPLATES[templateId];
    if (!tpl) return null;
...
function processBlocInvites(g) {
    const state = g || (typeof G !== "undefined" ? G : null);
    if (!state || !state.blocInvites) return;
    const notes = [];
    const player = state.homeRole === "home" ? "kingdom" : resolveHomeRole(state.homeRole);
    for (const cid of Object.keys(state.blocInvites)){
        ...
        const bloc = blocById(inv.blocId) || state.customBlocs && state.customBlocs[inv.blocId];
        const spec = defaultAccessionForBloc(inv.blocId);
        const acceptMin = inviteAcceptMin(inv.blocId);
        if (cid === player) continue;
        if (state.q >= inv.expiresQ) {
            if (!state.blocMember[cid] && !(state.blocAccessionByCountry && state.blocAccessionByCountry[cid])) {
                const why = rel < acceptMin ? " — relations too cool (" + rel.toFixed(0) + ", need " + acceptMin + "+)" : "";
                notes.push((p ? p.name : cid) + " declines the bloc invitation" + why + ".");
            }
            delete state.blocInvites[cid];
            continue;
        }
        if (state.q >= (inv.sentQ != null ? inv.sentQ : inv.expiresQ - 4) + 1 && !state.blocMember[cid] && !(state.blocAccessionByCountry && state.blocAccessionByCountry[cid])) {
            if (rel >= acceptMin) {
                ...
                delete state.blocInvites[cid];
                notes.push((p ? p.name : cid) + " begins accession to " + (bloc ? bloc.name : inv.blocId) + ".");
            }
        }
    }
    if (notes.length && state.brief) state.brief = notes.concat(state.brief).slice(0, 3);
}
function processBlocAccessions(g) {
    const state = g || (typeof G !== "undefined" ? G : null);
    ...
    for (const cid of Object.keys(state.blocAccessionByCountry)){
        ...
        const p = partnerById(cid);
        const rel = state.rel[cid] != null ? state.rel[cid] : 50;
        const bloc = blocById(acc.blocId) || state.customBlocs && state.customBlocs[acc.blocId];
        if (state.blocMember[cid] === acc.blocId) {
            delete state.blocAccessionByCountry[cid];
            continue;
        }
        if (state.q - acc.sinceQ > maxQ || rel < willingnessMin) {
            delete state.blocAccessionByCountry[cid];
            notes.push((p ? p.name : cid) + " accession stalls and is withdrawn.");
            continue;
        }
        if (acc.step === 1 && state.q - acc.sinceQ >= 2) {
            acc.step = 2;
            acc.sinceQ = state.q;
        } else if (acc.step === 2 && state.q - acc.sinceQ >= 3) {
            finalizeBlocJoin(state, cid, acc.blocId, state.law);
            delete state.blocAccessionByCountry[cid];
            notes.push((p ? p.name : cid) + " joins " + (bloc ? bloc.name : acc.blocId) + ".");
        }
    }
    if (notes.length && state.brief) state.brief = notes.concat(state.brief).slice(0, 3);
}
function inviteToBloc(countryId, blocId) {
...
/** Pick an active partner. Optional score(p) → higher wins; filter(p) to exclude. */ function pickEventPartner(opts) {
    const o = opts || {};
    const pool = activePartners().filter((p)=>!o.filter || o.filter(p));
    if (!pool.length) return null;
    if (o.score) {
        let best = pool[0], bestS = o.score(best);
...
function ensureDiploStocks(e) {
    ensureDiploStocksCore(e);
}
/** Headlines for nation table / gravity from a full econ bag. */ function headlinesFromEcon(econ) {
    if (!econ) return nationStateFromProfile(NATION_PROFILE.kingdom);
    return {
        y: econ.gdp != null ? econ.gdp : 100,
        growth: econ._lastGrowth != null ? econ._lastGrowth : econ.trendGrowth != null ? econ.trendGrowth : 1,
...
function worldRoleForSeat(seatId) {
    return seatId === "kingdom" ? "home" : seatId;
}
/** Build G.world with settled econ+law for every non-player seat. */ function initWorldState(g) {
    if (!g) return;
    ...
    for (const id of worldSeatIds()){
        ...
        /* Align CU CET on AI statute books. */ applyDraftBlocLaw(g.world[id].law, {
...
function mirrorPlayerToWorld(g) {
    if (!g || !g.econ) return;
    const playerId = playerCountryId(g.homeRole);
    if (!g.world) g.world = {};
...
function syncNationsFromWorld(g, forceLive) {
    if (!g || !g.econ) return;
    const e = g.econ;
    ...
    for (const id of worldSeatIds()){
        ...
        if (atOpen && prof) {
            const fxBag = bag && bag.econ ? headlinesFromEcon(bag.econ) : nationStateFromProfile(prof);
            e.nations[id] = {
                y: 100,
                growth: prof.trend,
                debt: prof.debt0,
        ...
        else if (!e.nations[id] && prof) e.nations[id] = nationStateFromProfile(prof);
        ensureNationFx(e.nations[id]);
    }
}
 ...
 *  marginal projects are abroad. */ function debtFinanceWedge(debt) {
 ...
 *  number that would force a depression. */ function applyAiFiscalRule(law, econ) {
    ...
    /* Deficit tolerance tightens as debt leaves the seat's norm: still allow a
     ...
     cleared bandHi before reacting. */ const defTrigger = aboveBand > 30 ? 2.5 : aboveBand > 0 ? 3.5 : aboveTarget > 8 ? 4.5 : aboveTarget > 0 ? 5.5 : 7.0;
    /* ---- High side: prevent debt accumulation ----
     ...
     fall on its own). */ if (def > defTrigger || aboveTarget > 0 && def > 0.8 || aboveBand > 0 && def > -0.5) {
        const stress = aboveBand > 40 ? 1.45 : aboveBand > 0 ? 1.25 : aboveTarget > 20 ? 1.15 : 1;
        const cut = (aboveBand > 20 ? 0.055 : aboveTarget > 10 ? 0.045 : 0.035) * stress;
        for (const d of DEPTS){
            if (law.spend[d.id] > d.min + 0.15) law.spend[d.id] = Math.max(d.min, law.spend[d.id] - cut);
        }
        if (law.taxes && law.taxes.vat && law.taxes.vat.on && law.taxes.vat.rate < 25) {
    ...
    /* ---- Low side: prevent creditor / surplus spirals ----
     ...
     Do not ease on surplus alone when debt is already above target. */ if (debt < bandLo || def < -2.5 && debt < target) {
        const ease = debt < bandLo - 20 || def < -6 ? 1.6 : 1;
        for (const d of DEPTS){
            if (d.id === "health" || d.id === "education" || d.id === "infra" || d.id === "welfare") {
                if (law.spend[d.id] < d.max - 0.1) law.spend[d.id] = Math.min(d.max, law.spend[d.id] + 0.05 * ease);
            }
        }
        if (law.taxes && law.taxes.vat && law.taxes.vat.on && law.taxes.vat.rate > 16) {
            law.taxes.vat.rate = Math.max(16, law.taxes.vat.rate - 0.12 * ease);
        }
        if (law.income && law.income.uprate === false && debt < bandLo) law.income.uprate = true;
    }
}
/**
 ...
 */ function stepCountry(opts) {
    ...
    if (o.world && o.seedNations !== false) {
        ...
        syncNationsIntoEcon(g.econ, o.world, o.countryId || playerCountryId(role), o.playerEcon, playerId);
    }
...
function syncNationsIntoEcon(e, world, selfId, playerEcon, playerId) {
    if (!e) return;
    if (!e.nations) e.nations = {};
    for (const id of worldSeatIds()){
        if (id === selfId) {
        ...
        if (bag && bag.econ) e.nations[id] = headlinesFromEcon(bag.econ);
        else if (!e.nations[id] && NATION_PROFILE[id]) e.nations[id] = nationStateFromProfile(NATION_PROFILE[id]);
        ensureNationFx(e.nations[id]);
    }
}
function pairTariff(exporterId, partnerId, g) {
...
function pairAccess(exporterId, partnerId, g) {
    const bag = g.world && g.world[exporterId];
...
function refreshWorldTrade(g) {
    if (!g || !g.world) return null;
    const playerId = playerCountryId(g.homeRole);
    mirrorPlayerToWorld(g);
    const seats = seatsFromWorld(g.world, playerId, g.econ, (i, j)=>pairTariff(i, j, g), (i, j)=>pairAccess(i, j, g));
...
/** One-quarter synthetic mods so AI seats feel the player's active global pulses. */ function globalModsForAiSeat(playerEcon, partnerId) {
    const mods = [];
    ...
    const bilat = playerEcon.partnerShock && playerEcon.partnerShock[partnerId] || 0;
    const beta = NATION_PROFILE[partnerId] && NATION_PROFILE[partnerId].beta != null ? NATION_PROFILE[partnerId].beta : 0.8;
    if (w) {
        mods.push({
            world: w,
            q: 1
...
/** Advance every non-player seat with the full country macro, then FX areas + trade clear. */ function stepWorldPartners(g, det) {
    ...
    /* Keep opening-quarter partner Y at profile pins so Q1 trade does not jump
     when world bags first advance; live headlines sync after g.q increments. */ if (g.q > 0) syncNationsFromWorld(g, true);
 ...
 */
function resolveLockstepQuarter(g, humanSeatIds, submissions) {
    ...
    try {
        ...
        for (const id of worldSeatIds()) {
            ...
            if (isHuman && !g.politics[id]) {
                g.politics[id] = defaultMpPolitics(id, seat.role || worldRoleForSeat(id));
            }
            ...
            if (isHuman && !Array.isArray(seat.log)) {
                ...
                seat.log = id === leadId && g.log && g.log.length ? clone(g.log) : (seat.log || []);
            }
            ...
            const r = stepCountry({
                ...
                mods: isHuman ? (pol.mods || []) : globalModsForAiSeat(
                    g.world[leadId] && g.world[leadId].econ,
                    id
                )
            });
        ...
        refreshWorldTrade(g);
        g.q = (g.q || 0) + 1;
        if (g.q > 0 && g.q % termLenOf(g.homeRole) === 0) g.term = (g.term || 1) + 1;
...
function normalizeDiploPolitics(pol) {
    if (!pol) return pol;
    if (!Array.isArray(pol.envoys) || pol.envoys.length !== ENVOY_SLOTS) {
        const prev = Array.isArray(pol.envoys) ? pol.envoys.filter((x) => x != null) : [];
        pol.envoys = emptyEnvoys();
        for (let i = 0; i < Math.min(ENVOY_SLOTS, prev.length); i++) pol.envoys[i] = prev[i];
    }
    if (!pol.ultimatums || typeof pol.ultimatums !== "object") pol.ultimatums = {};
    if (!pol.activeVisits || typeof pol.activeVisits !== "object") pol.activeVisits = {};
    if (!Array.isArray(pol.missionEvents)) pol.missionEvents = [];
    if (!Array.isArray(pol.diploAlerts)) pol.diploAlerts = [];
    if (!pol.envoySpend || typeof pol.envoySpend !== "object") pol.envoySpend = {};
    return pol;
}
function cloneDiploPolitics(src) {
    const s = src || {};
    return {
        envoys: clone(Array.isArray(s.envoys) ? s.envoys : emptyEnvoys()),
        ultimatums: clone(s.ultimatums && typeof s.ultimatums === "object" ? s.ultimatums : {}),
        activeVisits: clone(s.activeVisits && typeof s.activeVisits === "object" ? s.activeVisits : {}),
        ...
        envoySpend: clone(s.envoySpend && typeof s.envoySpend === "object" ? s.envoySpend : {})
    };
}
function mountDiploOntoGame(g, pol) {
    if (!g || !pol) return;
...
function syncDiploPoliticsFromGame(pol, g) {
    if (!pol || !g) return;
 ...
 */
function diploActivityCapitalCost(pol, nextEnvoys, nextUltimatums) {
    let cost = 0;
    ...
    const nextUlt = nextUltimatums && typeof nextUltimatums === "object" ? nextUltimatums : prevUlt;
    for (const pid of Object.keys(nextUlt)) {
        const nu = nextUlt[pid];
        if (!nu || nu.status !== "pending") continue;
        const pu = prevUlt[pid];
        if (!pu || pu.status !== "pending" || pu.demand !== nu.demand) cost += ULTIMATUM_PC;
    }
    return { ok: true, cost, envoys: nextEnv, ultimatums: clone(nextUlt) };
}
/** Seed opening capital/fac/rel for each human seat on a room snapshot. */
function defaultMpPolitics(seatId, role, name) {
...
function syncMpPoliticsFromG(pol) {
    if (!pol || !G) return;
...
function mountMpSeatOnSnapshot(g, seatId) {
    const seat = g.world[seatId];
    if (!seat) return null;
    if (!g.politics) g.politics = {};
    if (!g.politics[seatId]) {
        g.politics[seatId] = defaultMpPolitics(seatId, seat.role || worldRoleForSeat(seatId));
    }
 ...
 */
function applyMpDiploAction(g, seatId, action, opts) {
    const o = opts || {};
    ...
    try {
        ...
        if (action === "assignEnvoy") {
            ...
            if (!ok) {
                if ((G.capital || 0) < ENVOY_ASSIGN_PC) error = "Need " + ENVOY_ASSIGN_PC + " capital";
                else if (G.envoys && G.envoys.indexOf(null) < 0) error = "All envoy slots are filled";
            ...
            if (!ok) {
                ...
                error = (check.reasons && check.reasons.length)
                    ? check.reasons.join("; ")
                    : (check.cooldown > 0 ? "cooldown " + check.cooldown + "Q" : "Could not issue ultimatum");
            }
        } else {
...
function applyMpDiploPatch(patch) {
    if (!G || !patch) return;
...
function restoreMpDiploUi(snap, g) {
    const state = g || G;
 ...
 */
function applyLocalMpDiploAction(action, opts) {
    const o = opts || {};
...
function clearQuarterEnvoySpend(g) {
    const state = g || G;
    if (!state) return;
    state.envoySpend = {};
    /* Next billClauses() will re-pin baseline to the post-Deliver roster. */
    state.envoysBaselineQ = null;
    const seatId = state.homeRole != null ? playerCountryId(state.homeRole) : null;
...
function clearMpLockedSubmission(g) {
    const state = g || G;
    if (state && state.mp) state.mp.lockedSubmission = null;
}

function mpSubmissionFingerprint(parts) {
...
function mpSubmissionDiverged(g) {
    const state = g || G;
    if (!state || !state.mp || !state.mp.waiting || !state.mp.lockedSubmission) return false;
...
function promoteMpMissionEvent(pol) {
    if (!pol || pol.pendingEvent) return false;
    if (!Array.isArray(pol.missionEvents) || !pol.missionEvents.length) return false;
...
function rollMpEventIntoPolitics(g, seatId, q) {
    const prev = G;
    try {
        G = g;
        const pol = mountMpSeatOnSnapshot(g, seatId);
 ...
 */
function applyMpEventChoice(g, seatId, optionIndex) {
    if (!g || !g.world || !g.world[seatId]) {
        return {
            ok: false,
            error: "Unknown seat"
...
function seedMpPolitics(snap, humans) {
    if (!snap) return snap;
    ...
    for (const h of humans || []) {
        ...
        if (snap.world && snap.world[seatId] && !Array.isArray(snap.world[seatId].log)) {
            const leadId = humans[0] && (humans[0].seatId || seatIdForRoleSafe(humans[0].role));
            snap.world[seatId].log = seatId === leadId && Array.isArray(snap.log)
                ? clone(snap.log)
                : [];
        }
    }
...
function seatIdForRoleSafe(role) {
 ...
 */
function computeBillCost(law, draft, ctx) {
    const o = ctx || {};
    const prev = G;
    ...
    try {
        ...
        return {
            ok: false,
            cost: 9999,
            error: err && err.message || "Bill costing failed",
            clauses: []
        };
    } finally {
        G = prev;
...
function validateMpSubmission(snap, seatId, draft, opts) {
    ...
    if (total > pol.capital) {
        return {
            ok: false,
            error: "Not enough political capital (" + total + " needed, " + Math.round(pol.capital) + " available)",
            cost: billCost,
            diploCost,
            capital: pol.capital
        };
 ...
 */
function applyNewMpUltimatums(g, prevUltimatums, nextUltimatums) {
 ...
 */
function enactHumanSeatOnSnapshot(g, seatId, draft, opts) {
    const seat = g.world[seatId];
    if (!seat) return {
            ok: false,
            error: "Unknown seat"
    ...
    try {
        ...
        if (g.draft.blocAccession) {
            ...
            if (ba.phase === "apply") g.blocAccession = {
                    blocId: ba.blocId,
                    step: 1
                };
            else if (ba.phase === "align") g.blocAccession = {
                    blocId: ba.blocId,
                    step: 2
                };
            else if (ba.phase === "accede") {
                joinBloc(ba.blocId, g.draft);
...
function exportGameSnapshot(g) {
    ...
    if (src.capital != null || src.fac) {
        ...
        snap.politics[sid] = normalizeDiploPolitics({
            ...
            pendingEvent: prevPol && prevPol.pendingEvent ? clone(prevPol.pendingEvent) : null,
 ...
 */
function hydrateGameSnapshot(snap, opts) {
    const o = opts || {};
    ...
    if (pol) {
        ...
        G.diploAlerts = diplo.diploAlerts;
...
/* Behavioural response of a tax base to its own rate, in the standard
   ...
   model cannot honour both. */ const baseResponse = (rate, def, eti)=>{
    const tau = Math.min(0.985, rate / 100), tau0 = def / 100;
...
/* ---- income tax and national insurance, integrated over the distribution ---- */ /* The personal allowance is the authoritative start of the first band; the UI
   ...
   famous 60% marginal band when the higher rate is 40%:
     MTR = statutory + 0.5 * statutory  while the taper runs. */ const TAPER_START = 100000, TAPER_RATE = 0.5;
function effectiveBands(law) {
    const I = law.income;
    const b = I.bands.map((x, i)=>i === 0 ? {
            from: I.allowance,
            rate: x.rate
        } : {
            from: x.from,
    ...
    for(let i = 1; i < b.length; i++)if (b[i].from < b[i - 1].from) b[i].from = b[i - 1].from;
...
function personalAllowance(y, law) {
...
/* Where the taxpayer's top band starts. Behavioural response applies only to
   ...
   declare less of the salary they were already paying basic rate on. */ function bandKink(bands, y) {
    let k = 0;
    for (const b of bands)if (y > b.from) k = b.from;
    return k;
}
function incomeTaxOn(y, bands) {
    let t = 0;
    for(let i = 0; i < bands.length; i++){
        const from = bands[i].from, to = i + 1 < bands.length ? bands[i + 1].from : Infinity;
        if (y > from) t += (Math.min(y, to) - from) * bands[i].rate / 100;
    }
    return t;
}
/* Extra marginal points from allowance withdrawal (0 outside the taper window). */ function taperMtrBoost(y, law) {
...
/* `floor` is the income tax personal allowance: NI bites where income tax does. */ function niOn(y, ni, floor) {
    const base = Math.max(0, y - floor);
    return {
        emp: ni.empOn === false ? 0 : base * ni.empRate / 100 * EMPLOYEE_COLLECT,
        er: ni.erOn === false ? 0 : base * ni.erRate / 100 * EMPLOYER_COLLECT
    };
}
function capitalTaxOn(yDiv, ySave, law) {
    const I = law.income;
...
function incomeYield(law, E, econ) {
    const ni = law.ni, wage = econ && econ.wageIndex || 1;
    const mult = (REGIME_BY_ID[law.regime].mult || {}).income || 1;
    const incomeOn = law.income.on !== false;
    let tax = 0, emp = 0, er = 0, meanBase = 0, capTax = 0;
    for (const slice of INCOME_DIST){
        ...
        const mtrLab = incomeOn ? marginalBand(bands, yLab) + taperMtrBoost(yLab, law) : 0;
        const mtr = (mtrLab + mtrNi) / 100;
        const kink = bandKink(bands, yLab);
        const declaredLab = Math.min(yLab, kink) + Math.max(0, yLab - kink) * declaredFactor(slice.inc, mtr);
        if (incomeOn) {
            tax += incomeTaxOn(declaredLab, bandsForIncome(declaredLab, law)) * slice.w;
    ...
    /* Avoidance responses to a tax's own rate already live in that tax's Laffer
     ...
     than the tax itself raised, which made its marginal yield negative. */ const evasion = 1 - clamp(E ? E.eva : 0, -0.7, 0.7) * 0.05;
...
/* Population-weighted marginal rate and progressivity. Declared-income effects
   ...
   additional rate. It now can: the first puts money where it is spent. */ const MPC_BOTTOM = 0.88, MPC_TOP = 0.22;
function mpcAt(income, median) {
    const rel = Math.log(Math.max(0.15, income / median)) / Math.log(9);
    return clamp(MPC_BOTTOM + (MPC_TOP - MPC_BOTTOM) * clamp(rel + 0.45, 0, 1), MPC_TOP, MPC_BOTTOM);
}
function incomeProfile(law, econ) {
    const ni = law.ni, wage = econ && econ.wageIndex || 1;
    const median = 30000 * wage;
    ...
    /* UBI is a flat adult transfer; min wage lifts the bottom of the earned
     ...
     than through an authored Gini coefficient. */ const ubi = pol.ubi ? 3200 * wage : 0;
    const minFloor = pol.minWage ? 22000 * wage : 0;
    let netW = 0, mpcW = 0;
    let meanMtr = 0, taxLow = 0, incLow = 0, taxTop = 0, incTop = 0;
    const slices = [];
    for (const s of INCOME_DIST){
        let y = s.inc * wage;
        if (minFloor && y < minFloor && y > 8000 * wage) y = minFloor * 0.55 + y * 0.45;
        const yLab = y * (1 - CAP_INCOME_SHARE);
        const yCap = y * CAP_INCOME_SHARE;
        const yDiv = yCap * DIV_OF_CAPITAL;
        const ySave = yCap * (1 - DIV_OF_CAPITAL);
        ...
        const liab = (incomeOn ? incomeTaxOn(yLab, bands) + capitalTaxOn(yDiv, ySave, law) : 0) + niOn(yLab, ni, floor).emp;
    ...
    return {
        ...
        netIncome: netW,
        progressivity: (incTop ? taxTop / incTop : 0) - (incLow ? taxLow / incLow : 0),
...
/* Gini coefficient of a discrete weighted income distribution:
   ...
   Scaled to the familiar 0–100 index the rest of the game uses. */ function giniFromSlices(slices) {
    let mean = 0, wSum = 0;
    ...
    for(let i = 0; i < slices.length; i++){
        for(let j = 0; j < slices.length; j++){
            absDiff += slices[i].w * slices[j].w * Math.abs(slices[i].net - slices[j].net);
        }
    }
    return clamp(absDiff / (2 * mean * wSum * wSum) * 100, 14, 68);
}
const MTR_BASE = 0.295, PROG_BASE = 0.30;
/* Real (CPI) value of the personal allowance against this seat's opening
   ...
   drives the ratio down; earnings growth is deliberately not in the deflator. */ function allowanceBase(econ, law) {
    if (econ && econ.allowBase > 0) return econ.allowBase;
    if (law && law.income && law.income.allowance > 0) return law.income.allowance;
    return DEF_INCOME.allowance;
}
function dragRatio(law, econ) {
...
const taxGroup = (id)=>SYNTH_GRP[id] || (TAX_BY_ID[id] ? TAX_BY_ID[id].grp : "other");
...
/* Copy carries a {C} token so the country can be named by the player. Applied
   ...
   briefing, card blurbs in the drawers, and floating press clippings. */ const T = (str)=>String(str).replace(/\{C\}/g, G && G.country || "United Kingdom").replace(/\{P\}/g, ()=>{
...
function baseLaw() {
    const law = {
        regime: "progressive",
        polity: "democracy",
        taxes: Object.fromEntries(TAXES.map((t)=>[
                t.id,
                {
                    on: !!t.on,
                    rate: t.def
        ...
        spend: Object.fromEntries(DEPTS.map((d)=>[
                d.id,
                d.def
            ])),
        income: clone(DEF_INCOME),
        ni: Object.assign({}, DEF_NI),
        /* Every seat opens inside the United Nations; leaving is a bill clause. */ policies: {
            unMember: true
        },
        vice: {
            cannabis: "banned",
 ...
 *  can carry its own spend / tax / policy book; fall back to legacy bloc keys. */ function realmLawKey(role) {
    const id = resolveHomeRole(role || "home");
    if (id === "home") return "home";
    return id;
}
/** Force vice duties off when the legality gate fails. */ function syncViceTaxes(law) {
...
/** Opening statute for a playable seat. `home` is the UK baseLaw unchanged. */ function lawForRole(role) {
    const law = baseLaw();
...
let _openingByRole = {};
/** Steady-state Taylor guess at zero gap — seed for non-home settle only. */ function taylorRateForInflation(pi) {
    const underlying = pi != null ? pi : PI_TARGET;
    return clamp(R_NEUTRAL_REAL + underlying + TAYLOR_PI * (underlying - PI_TARGET), RATE_FLOOR, 20);
}
...
/** Opening + equilibrium society pins for a playable seat. */ function societyPinsFromRole(role) {
    const id = resolveHomeRole(role || "home");
    const p = id === "home" || id === "kingdom" ? NATION_PROFILE.kingdom : NATION_PROFILE[id];
...
/** Structural supply + opening stocks for a playable seat. */ function seatSupplyFromRole(role) {
    const id = resolveHomeRole(role || "home");
 ...
 *  Bank rate from a Taylor guess (settle then lets Taylor run — see freeRate). */ function pinsForRole(role) {
    const id = resolveHomeRole(role || "home");
 ...
 *  with a 15%+ coupon on 250% debt. */ function pinOpeningHeadlines(e, law, pins, statutePin, opts) {
    ...
    /* Keep the seat's income/NI schedule, not a silent reset to UK defaults. */ const statute = statutePin || law;
...
/** Set otherRevAdj so balanceOf matches the profile deficit (after debt pin). */ function plugOpeningDeficit(e, law, deficit0) {
    if (deficit0 == null) {
        e.otherRevAdj = 0;
        return;
    }
...
/* Renormalise the level to GDP = 100 after a settle step. Scaling only at the
   end left K* and the consumption target behind and produced a fake Q1 slump. */ function renormaliseOpeningLevel(e, law) {
...
function syncLogRowToEcon(row, e, g, law) {
    if (!row) return;
 ...
 *  is then wiped). Keeps real rates coherent for high-CPI seats. */ function applySettleTaylor(e, pin) {
    const underlying = pin.inflation != null ? pin.inflation : PI_TARGET;
    const target = R_NEUTRAL_REAL + underlying + TAYLOR_PI * (underlying - PI_TARGET);
    e.rate = clamp(e.rate + (clamp(target, RATE_FLOOR, 20) - e.rate) * TAYLOR_SMOOTH, RATE_FLOOR, 20);
...
/* Pin headline stocks each quarter and let the expenditure block clear at
   GDP = 100. FX is free (UIP at the Bank rate). Cached per homeRole. */ function settleOpeningEcon(pins, role) {
    ...
    /* Seat-specific public-service volume baseline: at opening statute and unit
     ...
     UK HEALTH+EDU composite. */ e.quality0 = (g.law.spend.health || 0) + (g.law.spend.education || 0) * 0.55;
    /* soc0 is the *observed* opening society layer. Opening statute already
     ...
     those opening impulses so steady state holds at soc0 until the law moves. */ {
        ...
        e.socOpen = {
            ...
            carbon: g.law.taxes.carbon && g.law.taxes.carbon.on ? g.law.taxes.carbon.rate : 0
        };
    }
    /* Pin FX baselines after settle so USD GDP opens at gdp0. Leave the live
     UIP level intact; only the denominator of the valuation formula is set. */ e.fx0 = e.fx != null ? e.fx : 1;
    if (!e.nations) {
        e.nations = {};
        for(const id in NATION_PROFILE)e.nations[id] = nationStateFromProfile(NATION_PROFILE[id]);
    }
...
function openingSnapshot(role) {
    const key = role || "home";
    if (!_openingByRole[key]) _openingByRole[key] = settleOpeningEcon(pinsForRole(key), key);
...
function openingBrief(pins, country) {
    const debt = pins.debt;
    const def = pins.deficit0;
    const pi = pins.inflation;
    let fiscal;
    if (def == null) {
        fiscal = "Growth is thin, prices are still running hot, and debt is the highest in a generation.";
    } else if (def < 1.5 && debt < 40) {
        fiscal = "The books open near balance, with public debt still light by peer standards.";
    } else if (def >= 6 || debt >= 110) {
        fiscal = "You inherit a wide deficit and debt already above " + debt.toFixed(0) + "% of GDP.";
    } else if (pi >= 5) {
        fiscal = "Prices are still running hot at " + pi.toFixed(1) + "%, and the fiscal inheritance is uncomfortable.";
    } else {
        fiscal = "Debt sits near " + debt.toFixed(0) + "% of GDP, with the deficit around " + (def != null ? def.toFixed(1) : "4.5") + "% — workable, not comfortable.";
    }
    return [
...
function newGame(opts) {
    ...
    const country = typeof o.country === "string" && o.country.trim() ? o.country.trim().slice(0, 34) : "United Kingdom";
    const homeRole = resolveHomeRole(o.homeRole || "home");
    const homeIso = String(o.homeIso || "826").padStart(3, "0");
    const homeScale = typeof o.homeScale === "number" && o.homeScale > 0 ? o.homeScale : null;
    ...
    G.draft.blocInvite = {};
    G.draft.blocExternalDeal = null;
    applyDraftBlocLaw(G.law, {
        blocMember: G.blocMember,
        customBlocs: G.customBlocs
    }, G.draft);
    applyDraftBlocLaw(G.draft, {
        blocMember: G.blocMember,
        customBlocs: G.customBlocs
    }, G.draft);
    initWorldState(G);
    ...
    render();
    renderPress();
    /* Opening morning note is first-class, not buried in the Budget drawer. */ if (o.silent) {
        bump();
        return;
    }
    despatch("Morning note", "Permanent Secretary", briefingHtml(G.brief) + '<p class="hint" style="margin-top:10px">' + careerHint(polityOf(G.homeRole), G.sandbox) + '</p>' + (!G.coachDone ? '<p class="hint">Try staging one change in Budget, Taxes or Policies, open the forecast with Deliver, then confirm.</p>' : ''), [
        {
            b: "To work",
...
/* ---- Consumption: Campbell-Mankiw ----
   ...
   MPC, as before, redistribution had no effect on aggregate demand at all. */ const RULE_OF_THUMB = 0.40; // share of households spending current income
...
/* Constrained households take a smaller share of market income than of
   ...
   basic-rate tax cut. */ const RT_MARKET_SHARE = 0.26, RT_TRANSFER_SHARE = 0.66;
...
/* Amplifies the distributional signal into the constrained share. The raw
   ...
   ready-reckoners on relative demand from transfers vs rate cuts. */ const MPC_INCIDENCE = 0.65, MPC_DIST_BASE = 0.4661;
...
const I_ACCEL = 0.10; // short-run accelerator on top of the stock model
...
const EDU_RND_SPILL = 0.10; // share of education above baseline that feeds R
/* The bare first-order condition implies more capital than firms actually hold,
   ...
   That is the position the UK is actually in. */ const UC_BASE = 3.4 - 2.0 + DEPREC + 25 * CORP_WEDGE;
const KSTAR_SCALE = K0 / (ALPHA * 100 / (UC_BASE / 100));
/* ==================================================================
   3. AGGREGATION  (what the statute book does to the country)
   ================================================================== */ function taxAvailable(t, law) {
    if (!t.req) return true;
    const [viceId, ...allowed] = t.req;
    return allowed.includes(law.vice[viceId]);
}
/* First-round CPI contribution from a change in duties, VAT or the tariff. */ function indirectTaxDelta(law, prevLaw) {
    let d = 0;
    for (const t of TAXES){
        const pass = INDIRECT_PASS[t.id];
        if (pass == null) continue;
        const a = prevLaw.taxes[t.id], b = law.taxes[t.id];
        if (!a || !b) continue;
...
/* Keys allowed on content `imp`. Anything that moves growth, inflation or the
   NAIRU must go through `ch` (or a tax base / price wedge) instead. */ const IMP_KEYS = new Set([
    "gini",
    "srv",
    "lib",
    "cri",
...
function aggregate(law, homeRole, blocMember) {
    /* `ch` carries effects that act through a real channel: labour supply, the
     ...
     indicators and a few fiscal/trade bookkeeping fields — never pot/gro/inf. */ const E = {
        gini: 0,
        srv: 0,
        lib: 0,
        cri: 0,
    ...
    const add = function(imp, fac) {
        let k = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : 1, ch = arguments.length > 3 ? arguments[3] : void 0;
        if (imp) for(const key in imp){
            if (!IMP_KEYS.has(key)) continue;
            if (key in E) E[key] += imp[key] * k;
        }
        if (ch) for(const key in ch){
            if (!CH_KEYS.has(key) && key !== "tariffCut" && key !== "access") continue;
            if (key in E) E[key] += ch[key] * k;
        }
        if (fac) {
            for(const f in fac)if (f in E.fac) E.fac[f] += fac[f] * k;
        }
    };
    const R = REGIME_BY_ID[law.regime];
    ...
    for(const id in law.deals){
       ...
       targets so only DEAL_PHASE of the gap closes each quarter. */ const imp = d.imp ? Object.assign({}, d.imp) : null;
...
/* How far a tax's own base has moved from its opening level. */ function taxBaseIndex(t, econ) {
...
/* Aggregate MPC implied by how post-tax income is distributed. */ function distMPC(law, econ) {
    const p = incomeProfile(law, econ);
    return p.distMPC;
}
function revenue(law, E, econ) {
    const R = REGIME_BY_ID[law.regime], mult = R.mult || {};
    const by = {};
    let total = 0;
    for (const t of TAXES){
        const s = law.taxes[t.id];
        if (!s || !s.on || !taxAvailable(t, law)) continue;
        if (R.kills && R.kills.includes(t.id)) continue;
        let base = t.base * (mult[t.grp] || 1);
        if (t.grp === "vice") base *= 1 - E.blackLevel / 100;
        base *= taxBaseIndex(t, econ);
        const y = Math.max(0, s.rate * base * baseResponse(s.rate, t.def, t.eti) * (1 - clamp(E.eva, -0.7, 0.7) * 0.05));
        by[t.id] = y;
        total += y;
    }
    const inc = incomeYield(law, E, econ || (typeof G !== "undefined" && G ? G.econ : null));
...
/* The welfare line is generosity, not a total. What it costs depends on how many
   ...
   Chancellor does anything. */ const CYCLICAL_WELFARE = 0.30;
/* Receipts are about 1.3 times as elastic as output over the cycle. */ const REV_GAP_ELAST = 0.30;
...
const CORP_LAG = 0.30; // corporation tax is paid in arrears
const CGT_LAG = 0.25;
...
function welfareCost(law, econ) {
    const u = econ && econ.unemployment || U_STAR;
    /* Triple lock: the state pension rises by max(earnings growth, CPI, 2.5%)
     each year. Cost = baseline pension share × (index − 1) × old-age caseload. */ const lockOn = !!(law.policies && law.policies.tripleLock);
    const idx = econ && econ.pensionIndex || 1;
    const oldLoad = (econ && econ.popOld || POP_OLD0) / POP_OLD0;
    const lockCost = lockOn ? PENSION_GDP_SHARE * Math.max(0, idx - 1) * oldLoad : 0;
...
/* ---- Service level per department ----
   ...
   score steady and let the cost follow, which is the honest way round. */ function deptUnitCost(econ) {
    return (econ && econ.realWage || 1) / (econ && econ.pubProd || 1);
}
function deptNeed(id, econ) {
    const pop = (econ && econ.L || 100) / 100;
    const dep = econ && econ.dependency || DEP_0;
    const ageing = id === "health" ? 1 + AGEING_HEALTH * (dep - DEP_0) : 1;
    return pop * ageing;
}
function serviceScore(id, law, econ) {
    const d = DEPTS.find((x)=>x.id === id);
    if (!d || TRANSFER_DEPTS[id]) return null;
    const gdp = econ && econ.gdp || 100;
    const volume = law.spend[id] / 100 * gdp / deptUnitCost(econ);
    return clamp(55 * (volume / deptNeed(id, econ)) / (d.def / 100 * 100), 0, 100);
}
/* The budget that would hold a given score: cost follows the standard, rather
   than the standard following whatever the budget happens to buy. */ function spendForScore(id, score, econ) {
    const d = DEPTS.find((x)=>x.id === id);
    const gdp = econ && econ.gdp || 100;
...
/* Hold mode freezes a service standard. The slider still edits % of GDP, so the
   ...
   otherwise a drag that only updated spend snaps back to the old hold on step. */ function syncServiceHolds(law, econ) {
    if (!law || !law.mode) return;
    if (!law.hold) law.hold = {};
    for (const d of DEPTS){
        if (TRANSFER_DEPTS[d.id]) continue;
...
/* What a claimant actually receives: the bill divided by the caseload, which
   rises with unemployment and with the old-age dependency ratio. */ function welfareAdequacy(law, econ) {
    const u = econ && econ.unemployment || U_STAR;
    const dep = econ && econ.dependency || DEP_0;
    const caseload = 1 + (u - U_STAR) * 0.055 + AGEING_PENSION * Math.max(0, dep - DEP_0);
    const idx = econ && econ.pensionIndex || 1;
...
function spending(law, E, econ) {
    let prog = 0;
    for (const d of DEPTS)prog += d.id === "welfare" ? welfareCost(law, econ) : law.spend[d.id];
    prog += OTHER_SPEND + E.spend;
    const il = IL_REAL_COUPON + Math.max(-2, econ.inflation !== undefined ? econ.inflation : 2);
...
function balanceOf(law, econ) {
    const E = aggregate(law);
    const rev = revenue(law, E, econ), sp = spending(law, E, econ);
    return {
        E,
        rev,
        sp,
...
/* ---- Potential output from a production function ----
   ...
   only affect employment through the wage-bargaining wedge. */ const PART_ELAST = 0.15, PART_ADJ = 0.09;
...
const A0 = 100 / (Math.pow(K0, ALPHA) * Math.pow(KG0, ALPHA_G) * Math.pow(100, 1 - ALPHA - ALPHA_G));
function humanCapital(law, econ) {
    ...
    /* Stock dominates long-run supply. A thin contemporaneous flow keeps the
     education slider visible in potentialLevel / projections immediately. */ return clamp(stock + flow * 0.25, 0.72, 1.45);
}
function humanCapitalTarget(law, E, econ) {
    const edu = law && law.spend && law.spend.education || 4.1;
    const skillsBoost = E && E.hbuild ? E.hbuild * 0.08 : 0;
    const mig = E && E.migrate ? E.migrate * 0.015 : 0;
    /* Steady-state index consistent with education spend and policy feeders. */ return clamp(H_CAP0 + MINCER * (edu - 4.1) + skillsBoost + mig, 0.72, 1.45);
}
function stepHumanCapital(law, E, econ) {
    const e = econ;
    if (e.hCap == null) e.hCap = H_CAP0;
    const edu = law.spend && law.spend.education || 4.1;
    const invest = H_BUILD * Math.max(0, edu - EDU_FLOOR) / 100 + (E && E.hbuild || 0) * 0.012 + Math.max(0, E && E.migrate || 0) * 0.002;
    const decay = DEPREC_H / 100 * Math.max(0, e.hCap - 0.85);
    e.hCap = clamp(e.hCap + (invest - decay) / 4, 0.72, 1.45);
    /* Soft pull toward the target so a sustained education cut does scar. */ const target = humanCapitalTarget(law, E, e);
...
/* R&D effort as a share of GDP: the research budget, a small spillover from
   ...
   fiscal drain while the knowledge stock is still catching up. */ function researchEffort(law, E) {
    const spend = law.spend && law.spend.research || 0;
    const edu = law.spend && law.spend.education || 4.1;
    const spill = EDU_RND_SPILL * Math.max(0, edu - 4.1);
    return spend + spill + (E && E.rndEffort || 0);
}
function knowledgeTfp(econ) {
    const R = econ && econ.R != null ? econ.R : R0;
    return R_TFP * (R / R0 - 1);
}
/* Relative income vs the frontier (federated ≈ 1). Catch-up TFP growth for
   seats below the frontier — Barro-style log gap, not a per-realm tfpTrend. */ function tfpCatchupRate(econ) {
    const y = econ && econ.yRel != null ? econ.yRel : 1;
    return CATCHUP_K * Math.max(0, Math.log(1 / Math.max(0.08, clamp(y, 0.08, 1.6))));
}
function baselineTfpGrowth(econ) {
    return TFP_FRONTIER + tfpCatchupRate(econ);
}
/** Pin opening relative-income / potential baselines once per seat. */ function ensureYRelPins(econ) {
    if (!econ) return;
    if (econ.yRel0 == null) econ.yRel0 = econ.yRel != null ? econ.yRel : 0.88;
    if (econ.potential0 == null || !(econ.potential0 > 0)) econ.potential0 = econ.potential != null ? econ.potential : 100;
}
/** Live relative income: opening yRel scaled by own potential path vs the
 *  frontier (US) potential path. Catch-up fades as a seat converges. */ function liveYRel(econ, frontierEcon) {
    ensureYRelPins(econ);
    ...
    const fr = (frontierEcon.potential || 100) / Math.max(1e-6, frontierEcon.potential0);
    return clamp(econ.yRel0 * own / Math.max(1e-6, fr), 0.08, 1.6);
}
function refreshSeatYRel(econ, frontierEcon) {
    if (!econ) return;
 ...
 *  every seat's yRel so catch-up is endogenous. */ function refreshWorldYRel(g) {
    if (!g || !g.econ) return;
    const frontierId = "united_states";
    let frontier = null;
    const playerId = playerCountryId(g.homeRole);
    if (playerId === frontierId) frontier = g.econ;
    else if (g.world && g.world[frontierId] && g.world[frontierId].econ) frontier = g.world[frontierId].econ;
...
/* Births per working-age head per quarter, moved by fertility policy and a
   thin welfare-generosity link. */ function effectiveBirthRate(econ, law, E) {
    const base = econ && econ.birthRate != null ? econ.birthRate : BIRTH_RATE;
    const fert = E && E.fertility || 0;
    const wel = law && law.spend && law.spend.welfare != null ? law.spend.welfare : 13.3;
    const welBoost = 1 + WELFARE_FERT * Math.max(0, wel - 13.3);
    return clamp(base * (1 + FERTILITY_CH * fert / 100) * welBoost, 0.0004, 0.014);
}
/* Annualised labour-force growth from migration and the youth ratio.
   Full AGE_OUT would treat retirement as a labour-supply collapse; the labour
   index already absorbs dependency via NAIRU and ageing drag on L. Seat
   differences come from migBase and popChild, not a labourTrend table. */ function impliedLabourGrowth(econ, E) {
    const e = econ || {};
    const pw = Math.max(1, e.popWork != null ? e.popWork : POP_WORK0);
    const pc = e.popChild != null ? e.popChild : POP_CHILD0;
    const mig = (e.migBase != null ? e.migBase : MIG_BASE) + (E && E.migrate || 0);
    const childGap = pc / pw - POP_CHILD0 / POP_WORK0;
    return clamp(mig + 0.30 + childGap * 3.0, -1.5, 6);
}
function labourGrowthRate(econ, E) {
    /* Prefer demography-implied growth for the accounting identity. Tracking
     quarter-to-quarter ΔL whipsaws after settle renormalisation. */ return impliedLabourGrowth(econ, E);
}
/* Effective labour input: the labour force less structural unemployment, times
   human capital. Migration policy moves the force itself. */ function labourInput(law, E, econ) {
    const uStar = econ && econ.nairu || U_STAR;
    const part = econ && econ.participation || 1;
    const force = (econ && econ.L || 100) * (1 + (E.labour || 0) / 100) * part;
    return force * (1 - uStar / 100) / (1 - U_STAR / 100) * humanCapital(law, econ);
}
function potentialLevel(law, E, econ) {
    const K = econ && econ.K || K0;
    const KG = econ && econ.KG || KG0;
    const A = econ && econ.A || A0;
    return A * Math.pow(Math.max(1, K), ALPHA) * Math.pow(Math.max(1, KG), ALPHA_G) * Math.pow(Math.max(1, labourInput(law, E, econ)), 1 - ALPHA - ALPHA_G);
}
/* Annualised trend growth, reported for display and used by the investment
   block. Derived from the production function rather than asserted. */ function potentialGrowth(law, E, econ) {
    const e = econ || (typeof G !== "undefined" && G ? G.econ : null);
    if (!e || !e.K) return 1.2;
    const now = potentialLevel(law, E, e);
    const gL = labourGrowthRate(e, E);
    const gA = baselineTfpGrowth(e) + (E.tfp || 0) + (e.tradeDepth || 0) * TRADE_TFP + (clamp(50 + (E.open || 0) + (e.openEff || 0), 5, 95) - 50) * 0.006 + knowledgeTfp(e);
    const gK = (e.I - DEPREC / 100 * e.K) / e.K * 100;
    const gKG = (law.spend.infra / 100 * e.gdp - DEPREC_G / 100 * e.KG) / e.KG * 100;
    return clamp(gA + ALPHA * gK + ALPHA_G * gKG + (1 - ALPHA - ALPHA_G) * gL, -2.5, 8);
}
function approvalOf(fac) {
...
/* ==================================================================
   ...
   and imports 31. They sum to 100 by construction. */ const SHARE_C = 61, SHARE_I = 17, SHARE_G = 22, SHARE_X = 31, SHARE_M = 31;
...
const OTHER_G = 1.8, OTHER_TRANSFER = 5.88; // splits OTHER_SPEND (research and UN dues carved out)
...
/* Consumption. Partial adjustment toward a target set by disposable income.
   ...
   C_RATE is the residual non-mortgage credit term. */ const C_ADJ = 0.34, C_RATE = 0.08, HH_TAX_SHARE = 0.70;
...
const C_CONTACT = 0.70; // consumption response to contact-capacity hit
...
const MORT_FIX = 0.10; // quarterly share of mortgages that reprice (~2.5y half-life)
...
const PD_HOUSE = 0.010; // PD points per point of annualised house-price fall
...
/* Investment: accelerator plus user cost of capital,
   ...
   competitiveness worsens the balance. */ const T_ADJ = 0.30, ELAST_X = 0.85, ELAST_M = 0.60;
/* Import content of the baseline spending programme, so only deviations count. */ const BASE_LEAK = 2.5 * 0.12 + 8.4 * 0.08 + 4.1 * 0.05 + 2.5 * 0.18 + 2.4 * 0.30 + 1.9 * 0.07 + 0.8 * 0.12;
const WORLD_GROWTH = 2.0, WORLD_INFL = 2.0, BASE_TARIFF = 3;
/* Fraction of a worldTfp pulse that spills into the home country's TFP growth. */ const WORLD_TFP_SPILL = 0.35;
/* Major world episodes: next one is scheduled 16–32 quarters (4–8 years) after the last ends. */ const MAJOR_GAP_MIN = 16, MAJOR_GAP_SPAN = 17;
/* Trade policy is not unilateral. Raise a tariff and partners raise theirs, with
   ...
   ratified agreement restrains it. */ const RETAL_SHARE = 0.75, RETAL_ADJ = 0.16, RETAL_ELAST = 1.1;
/* Bilateral exports scale gently with relations; cold partners also amplify
   tariff retaliation (trade-share weighted). */ const REL_TRADE = 0.22, RETAL_COLD = 0.55;
/* ---- Trade agreements take years, and then keep giving ----
   ...
   away from the frontier. */ const DEAL_PHASE = 0.055, TRADE_TFP = 0.0035;
...
/* A point of interest differential sustained over several years is worth a few
   per cent on the currency, which is the usual empirical range. */ const FX_UIP = 0.030; // fractional appreciation per point of differential
...
/* ---- Demography ----
   ...
   than permanently adding a labour fudge to trend growth. */ const DEP_0 = 0.30;
const POP_WORK0 = 100, POP_OLD0 = 30, POP_CHILD0 = 28;
/* Quarterly cohort rates, calibrated so dependency creeps from 0.30 toward
   about 0.37 over a few decades when migration is at baseline. */ const BIRTH_RATE = 0.0026; // births into child stock, per working-age head
const AGE_IN = 0.00305; // child → working age
const AGE_OUT = 0.00335; // working age → old
const DEATH_OLD = 0.0072; // deaths from old stock
...
function govDemandShares(law, econ) {
    let g = OTHER_G, tr = OTHER_TRANSFER;
    let leak = OTHER_G * IMPORT_CONTENT.other;
    for (const d of DEPTS){
        const amount = d.id === "welfare" ? welfareCost(law, econ) : law.spend[d.id];
        if (TRANSFER_DEPTS[d.id]) tr += amount;
        else {
            g += amount;
            leak += amount * (IMPORT_CONTENT[d.id] || 0.1);
        }
    }
    return {
        g,
...
/* One quarter of the expenditure side. Every component is a level indexed so
   ...
   all at once. */ function expenditureStep(e, law, E, revShare, modSpend, g, det) {
    ...
    /* Households pay the average rate on baseline income but the marginal rate on
     ...
     overstated multipliers by roughly a third. */ const hhTax = revShare / 100 * pot * HH_TAX_SHARE + MARGINAL_LEAK * (e.gdp - pot);
    ...
    /* The rate that matters for a spending or investment decision is ex ante:
     ...
     putting up with a VAT rise are not being stimulated by it. */ const realRate = e.rate - e.expect;
    /* Split disposable income between the two household types. A more equal
     ...
     lowers it, and demand follows. */ const equality = 1 + REDIST_SENS * (35 - (e.gini || 35)) / 35;
    const incidence = 1 + MPC_INCIDENCE * (incomeProfile(law, e).distMPC / MPC_DIST_BASE - 1);
    /* More households are constrained when the economy is weak, which is why the
     same budget buys more output in a slump than in a boom. */ const slack = clamp(-(e.gdp / pot - 1) * 100, -4, 4);
    const stateDep = 1 + SLACK_MPC * slack;
    const rtShare = clamp(RULE_OF_THUMB * equality * incidence * stateDep * (1 + (E.mpcw || 0) / 100), 0.15, 0.78);
    const market = e.gdp - hhTax;
    const ydRT = market * RT_MARKET_SHARE * (rtShare / RULE_OF_THUMB) + TR * RT_TRANSFER_SHARE * (rtShare / RULE_OF_THUMB);
    const ydPerm = yd - ydRT;
    /* Permanent income is a slow-moving average of the unconstrained households'
     disposable income, which is what gives consumption smoothing. */ e.permInc += (ydPerm - e.permInc) * PERM_ADJ;
    /* Mortgage fixation: the stock rate crawls toward Bank rate plus spread.
     ...
     conditions ration new mortgage lending. */ if (e.mortgageDebt == null) e.mortgageDebt = MORT_DEBT0;
    ...
    e.mortgageRate += (e.rate + MORT_SPREAD + (e.creditSpread || 0) * 0.35 - e.mortgageRate) * MORT_FIX;
    const pol = law.policies || {};
    /* Build cost: user cost of capital, eased by planning reform, raised by rent
     controls (developers face capped revenues) and by stamp-like asset taxes. */ const landRate = law.taxes.landTax && law.taxes.landTax.on ? law.taxes.landTax.rate : 0;
    const cgtGap = law.taxes.capGains && law.taxes.capGains.on ? Math.max(0, law.taxes.capGains.rate - 24) : 0;
    const buildCost = 100 * (1 + 0.012 * (realRate - R_REAL_NEUTRAL) + 0.010 * ((E.ucost || 0) + (e.modUcost || 0) + (e.creditSpread || 0)) - (pol.planning ? 0.085 : 0) + (pol.rentCtrl ? 0.055 : 0) + landRate * 0.008 + cgtGap * 0.0015);
    /* Housing demand vs stock → vacancy → rent. */ const gapFrac = e.gdp / pot - 1;
    const demandH = H0 * (1 + 0.55 * gapFrac + 0.12 * ((e.popWork || POP_WORK0) / POP_WORK0 - 1));
    const vacancy = clamp(e.housingStock / Math.max(1, demandH) - 1, -0.25, 0.35);
    let rentTarget = 100 * (1 - RENT_VAC_ELAST * vacancy + RENT_Y_ELAST * gapFrac);
    if (pol.rentCtrl) rentTarget *= 1 - RENT_CTRL_CUT;
    e.rentIndex += (rentTarget - e.rentIndex) * 0.14;
    e.rentIndex = clamp(e.rentIndex, 55, 180);
    /* Price = capitalised rent. */ const capRate = Math.max(0.015, CAP_RATE0 + (realRate - R_REAL_NEUTRAL) / 100 * 0.55 + (e.creditSpread || 0) / 100 * 0.4);
    ...
    /* Construction: private I^H responds to P/C; social housebuilding adds stock. */ const priceCost = e.housePrice / Math.max(40, buildCost);
    const privBuild = H_BUILD_BASE * Math.pow(Math.max(0.4, priceCost), H_BUILD_ELAST) * (pol.rentCtrl ? 0.72 : 1);
    const pubBuild = (E.hbuild || 0) * 0.35;
    const hInvest = (privBuild + pubBuild) / 100 * e.housingStock;
    e.housingInvest = hInvest;
    e.housingStock = Math.max(70, e.housingStock * (1 - H_DEPREC / 400) + hInvest);
    e.buildCost = buildCost;
    e.vacancy = vacancy;
    /* Credit: NIM − loan losses → bank capital; leverage → spread; deposit-run
     risk gates mortgage supply. */ if (e.bankLoans == null) e.bankLoans = BANK_LOANS0;
    ...
    const ltv = (e.mortgageDebt || MORT_DEBT0) / Math.max(40, e.housePrice) * 100;
    const hpFall = Math.max(0, -(e.housePriceGrowth || 0));
    const pd = clamp(0.004 + PD_U * uGap + PD_LTV * Math.max(0, ltv - 70) / 10 + PD_HOUSE * hpFall + Math.max(0, e.riskPremium) * 0.01, 0.001, 0.12);
    const corpLoans = Math.max(20, e.bankLoans - e.mortgageDebt);
    e.bankLoans = clamp(0.55 * e.mortgageDebt + 0.45 * (corpLoans + e.I * 0.8), 40, 160);
    const nim = BANK_NIM * (e.bankLoans / BANK_LOANS0) * (1 + ((e.mortgageRate || e.rate) - MORT_NEUTRAL) * 0.04);
    const losses = BANK_LGD * pd * e.bankLoans * 0.25;
    e.bankCapital += nim - losses - CREDIT_SOV * Math.max(0, e.riskPremium) * (e.bankLeverage / BANK_LEV0) * 0.12;
    e.bankCapital += (BANK_CAP0 - e.bankCapital) * 0.025;
    e.bankCapital = clamp(e.bankCapital, 2, 16);
    /* Deposit flight when spreads and unemployment rise. */ const runPressure = Math.max(0, e.creditSpread) * 0.15 + uGap * 0.04 + Math.max(0, e.riskPremium) * 0.1;
    e.depositRun = clamp(e.depositRun * 0.7 + runPressure, 0, 8);
    e.bankDeposits = clamp(e.bankDeposits * (1 - e.depositRun / 400) + (e.bankLoans * 0.92 - e.bankDeposits) * 0.06, 30, 180);
    e.bankLeverage = clamp(e.bankLoans / Math.max(2, e.bankCapital) * (BANK_CAP0 / BANK_LOANS0) * BANK_LEV0, 6, 28);
    e.creditSpread = Math.max(0, (e.bankLeverage - BANK_LEV_TARGET) * CREDIT_PHI + Math.max(0, e.riskPremium) * 0.25 + e.depositRun * 0.12 + Math.max(0, -(e.housePriceGrowth || 0)) * 0.035 - WEALTH_COLLAT * Math.max(-2, ((e.privateWealth || PRIVATE_WEALTH0) - PRIVATE_WEALTH0) / 10));
    /* Endogenous bank stress: force a credit crunch and fiscal recap when leverage
     is high and capital is thin. Scripted bank events are gated while stressed. */ if (e.bankStressCd == null) e.bankStressCd = 0;
    if (e.bankStressCd > 0) e.bankStressCd -= 1;
    if (!det && !e.bankStress && e.bankStressCd <= 0 && e.bankLeverage >= BANK_STRESS_LEV && e.bankCapital <= BANK_STRESS_CAP) {
        e.bankStress = true;
        ...
        e._bankStressNote = "A mid-sized lender failed the stress test. Deposit flight forced a fiscal recapitalisation and credit is scarce.";
    } else if (e.bankStress && e.bankCapital >= BANK_CAP0 - 0.5 && e.creditSpread < 1.2) {
        e.bankStress = false;
    }
    if (e.bankStress) {
        e.creditSpread = Math.max(e.creditSpread, 1.6);
    }
    const creditSupply = clamp(1 - MORT_CREDIT_ELAST * e.creditSpread / 4 - e.depositRun * 0.06, 0.35, 1.15);
    const rateFactor = Math.pow((1 + MORT_NEUTRAL / 100) / (1 + Math.max(0.2, e.mortgageRate) / 100), 1.15);
    const mortDesired = MORT_DEBT0 * (e.housePrice / 100) * rateFactor * (pol.rentCtrl ? 0.96 : 1) * creditSupply * (1 - cgtGap * 0.004 - landRate * 0.01);
    ...
    const wealthImpulse = C_WEALTH * (e.housePrice / 100 - 1) * HH_HOUSING_WEALTH / 100 * pot + C_FIN_WEALTH * ((e.privateWealth || PRIVATE_WEALTH0) - PRIVATE_WEALTH0) / 100 * pot;
    if (e.uncertainty == null) e.uncertainty = 0;
    if (e.contactHit == null) e.contactHit = 0;
    if (e.residCarry == null) e.residCarry = 0;
    const target = C_AUTO / 100 * pot + MPC_RT * ydRT + MPC_PERM * e.permInc - C_RATE * (realRate - R_REAL_NEUTRAL) / 100 * pot - C_MORT * mortService / 100 * pot + wealthImpulse - C_UNC * e.uncertainty / 100 * pot - C_CONTACT * e.contactHit / 100 * pot + RESID_C_ADJ * e.residCarry / 100 * pot;
    e.C += (target - e.C) * C_ADJ;
    e.rtShare = rtShare;
    // --- I: desired capital stock from the production function's FOC
    const corpRate = law.taxes.corpTax && law.taxes.corpTax.on ? law.taxes.corpTax.rate : 0;
    const fiscalUc = debtFinanceWedge(e.debt);
    e.fiscalFinance = fiscalUc;
    const userCost = Math.max(1.2, realRate + DEPREC + corpRate * CORP_WEDGE + (E.ucost || 0) + (e.modUcost || 0) + (e.creditSpread || 0) * CREDIT_I_PASS + e.uncertainty * 0.08 + fiscalUc);
    e.ucSmooth += (userCost - e.ucSmooth) * UC_SMOOTH;
    const gapPct = (e.gdp / pot - 1) * 100;
    const Kstar = KSTAR_SCALE * ALPHA * e.gdp / (Math.max(1.2, e.ucSmooth) / 100);
    const trendG = e.trendGrowth || 1.2;
    const Istar = (DEPREC + trendG) / 100 * e.K + K_ADJ * (Kstar - e.K) + I_ACCEL / 100 * gapPct * pot + (E.kboost || 0) / 100 * pot + (E.rndEffort || 0) / 100 * pot - I_UNC * e.uncertainty / 100 * pot;
    e.I += (Istar - e.I) * 0.30;
    e.Kstar = Kstar;
    e.userCost = userCost;
    ...
    /* Retaliation builds toward a share of whatever tariff wall you put up, less
     whatever your treaties hold back. Poor relations make retaliation worse. */ ensureDiploStocks(e);
    const bm = g && g.blocMember || {};
    const relMap = g && g.rel || {};
    const homeRole = g && g.homeRole;
    let coldW = 0, coldSum = 0, provokeW = 0, provokeSum = 0;
    ...
    const retalTarget = provoke * RETAL_SHARE * restraint * (1 + RETAL_COLD * cold);
    e.retaliation += (retalTarget - e.retaliation) * RETAL_ADJ;
    /* World demand is the trade-share × absolute-GDP basket of partners, plus a
     rest-of-world residual. Opening worldY is normalised to 100 via worldDemand0. */ refreshWorldY(e, homeRole);
    ...
    /* Competitiveness runs on underlying prices, not headline. VAT is
     ...
     every VAT change look like a real appreciation, which it is not. */ e.priceP *= 1 + (e.inflation - e.vatEcho * 0.9) / 400;
    /* Uncovered interest parity, with a risk premium. */ const worldRate = (e.worldRate != null ? e.worldRate : WORLD_RATE) + (e.modWorldRate || 0);
    ...
    /* Deal channels phase in. Statutory tariff and non-deal openness still bite
     immediately; only the deal's open / tariffCut / access crawl via DEAL_PHASE. */ if (e.openEff == null) e.openEff = 0;
    ...
    /* Competitiveness is indexed to the opening settings, so a fresh game starts
     ...
     makes exports dearer and imports cheaper. */ const openNow = (E.open || 0) + e.openEff;
    const cutNow = (E.tariffCut || 0) + e.tariffCutEff;
    const avgTariff = tariffScheduleAverage(law, homeRole, bm);
    const compBase = e.worldP / e.priceP * (1 + openNow / 200) * (1 + BASE_TARIFF / 100) / (1 + Math.max(0, avgTariff - cutNow) / 100) / Math.max(0.5, e.fx);
    ...
    /* A `world` shock is a synchronised foreign-demand pulse: it hits every
     ...
     residual. Without this, "global recession" barely moved X. */ const worldPulse = e.worldShock || 0;
    const bilat = {};
    let Xstar = 0;
    ...
    for (const p of partners){
        ...
        const effT = effectiveTariff(p.id, law, homeRole, bm);
        const comp = compBase * (1 + BASE_TARIFF / 100) / (1 + Math.max(0, effT - cutNow) / 100);
        const access = e.partnerAccessEff && e.partnerAccessEff[p.id] || 0;
        const beta = NATION_PROFILE[p.id] && NATION_PROFILE[p.id].beta != null ? NATION_PROFILE[p.id].beta : 0.8;
        const pulse = (partnerPulse[p.id] || 0) + worldPulse * beta;
        const rel = relMap[p.id] != null ? relMap[p.id] : 50;
        const relFac = 1 + REL_TRADE * (rel - 50) / 50;
        let Xi;
        const gravityXi = partnerShare(homeRole, p) * shareX / 100 * e.worldY * Math.pow(Math.max(0.4, Yi / 100), GRAVITY_Y) * Math.pow(comp, ELAST_X) * (1 + pulse / 100) * (1 + access / 200) * relFac;
        if (e.clearedFlows && e.clearedFlows[p.id] != null) {
            /* Phase cleared trade in over a year so Q1 does not jump when world bags wake. */ const blend = g && g.q != null ? Math.min(1, Math.max(0, g.q) / 4) : 1;
            const clearedXi = e.clearedFlows[p.id] * (1 + pulse / 100) * (1 + access / 400) * relFac;
    ...
    const gravityRest = tradeRestShare(homeRole) * shareX / 100 * e.worldY * Math.pow(compBase, ELAST_X) * (1 + worldPulse / 100);
    const restBlend = g && g.q != null ? Math.min(1, Math.max(0, g.q) / 4) : 1;
    const restCleared = e.clearedFlows && e.clearedFlows.rest != null ? e.clearedFlows.rest * (1 + worldPulse / 100) : null;
    const restX = restCleared != null ? gravityRest * (1 - restBlend) + restCleared * restBlend : gravityRest;
    bilat.rest = restX;
    Xstar += restX;
    const domestic = e.C + e.I + G;
    /* Imports pulled in by domestic demand, plus the direct import content of the
     government's own purchases. */ const Mstar = shareM / (shareC + shareI + SHARE_G) * domestic * Math.pow(compBase, -ELAST_M) + (leak - BASE_LEAK) / 100 * pot;
...
/* ==================================================================
   4. THE ENGINE
   ================================================================== */ function step(g, law, prevLaw, det) {
    const e = g.econ;
    const E = aggregate(law, g.homeRole, g.blocMember), Ep = aggregate(prevLaw, g.homeRole, g.blocMember);
    ...
    /* Event mods are structural channels only: world demand, cost of capital,
     ...
     map into the uncertainty stock. */ let mS = 0, mA = 0, mP = 0, mW = 0, mUC = 0, mPart = 0, mLab = 0, mMig = 0, mToT = 0, mUnc = 0, mContact = 0;
    let mSrv = 0, mHlt = 0, mCri = 0, mWorldInfl = 0, mWorldRate = 0, mWorldTfp = 0;
    e.partnerShock = {};
    g.mods = g.mods.filter((m)=>m.q > 0);
    g.mods.forEach((m)=>{
        /* Legacy demand/growth → precautionary uncertainty (sign: cut demand = more uncertainty). */ const dem = (m.demand || 0) + (m.growth || 0);
        if (dem) mUnc += -dem;
        ...
        if (m.worldPartner && m.partner) {
            e.partnerShock[m.partner] = (e.partnerShock[m.partner] || 0) + m.worldPartner;
        }
        m.q--;
    });
    ...
    /* Partner macros and trade clear before expenditure so foreign demand is live.
     Settle / AI seats skip the world loop to avoid recursion. */ if (g.world && !g._settling && !g._aiSeat) {
    ...
    /* The old fiscal-impulse term lived here. It is gone: G enters the identity
     ...
     which is where multipliers come from in the first place. */ const revShare = revenue(law, E, e).total;
    const prevY = e.gdp;
    ...
    /* Elevated gilt yields hit borrowers as a credit-spread shock attributed to
     private demand so sectoral balances still close. */ const yieldShock = -Math.pow(Math.max(0, e.yield - 6.0), 1.3) * 0.45;
    ...
    /* Potential is the production function evaluated on the current stocks, not a
     ...
     supply to the demand it was failing to meet. */ const hyst = HYSTERESIS * clamp((e.gdp / e.potential - 1) * 100, -4, 4);
    e.A *= 1 + (baselineTfpGrowth(e) + (E.tfp || 0) + (e.tradeDepth || 0) * TRADE_TFP + (clamp(50 + (E.open || 0) + (e.openEff || 0), 5, 95) - 50) * 0.006 + knowledgeTfp(e) + hyst + mP) / 400;
    /* ---- Demography: cohort stocks, then labour force ----
     ...
     additive on trend growth. Fertility policy moves the birth rate. */ if (e.popWork == null) {
        e.popWork = POP_WORK0;
        e.popOld = POP_OLD0;
    ...
    const mig = (migBase + (E.migrate || 0) + mMig + (E.labour || 0) * 0.35 + mLab * 0.5) / 400 * e.popWork;
    ...
    const ageingDrag = (e.dependency - DEP_0) * 55;
    /* Labour force tracks working-age population; residual labour channel is for
     conscription-style temporary withdrawals, not migration. */ const prevL = e.L;
    e.L *= 1 + ((e.popWork / POP_WORK0 - e.L / 100) * 12 + (E.labour || 0) * 0.4 - ageingDrag * 0.15) / 400;
    ...
    /* Participation margin: people enter and leave the workforce as the reward
     for working changes. */ const netWage = (1 - prof.meanMtr) / (1 - MTR_BASE);
    const partTarget = (1 + PART_ELAST * (netWage - 1)) * (1 + ((E.part || 0) + mPart) / 100);
    ...
    /* Catch-up uses live yRel. Refresh after potential moves so seats that have
     ...
     partner bags (already advanced this quarter). */ if (g.world && !g._settling && !g._aiSeat) refreshWorldYRel(g);
    else if (g._aiSeat && g.world) {
        const frontierId = "united_states";
        const playerId = playerCountryId(g.homeRole);
        let frontier = null;
        if (playerId === frontierId) frontier = g.econ;
        else if (g.world[frontierId] && g.world[frontierId].econ) frontier = g.world[frontierId].econ;
    ...
    /* ---- Expectations-augmented Phillips curve ----
     ...
     which is what lets inflation de-anchor if it is allowed to run. */ const newGap = (e.gdp / e.potential - 1) * 100;
    /* First-round price-level shift from duties/VAT and from the effective import
     ...
     cuts move CPI the same way statutory schedule changes do. */ const importT = importTariffLevel(law, E, e, g.homeRole, g.blocMember);
    const prevImportT = e.importTariff != null ? e.importTariff : importT;
    const dTax = indirectTaxDelta(law, prevLaw) + (importT - prevImportT) * TARIFF_PASS;
    e.vatEcho = e.vatEcho * 0.6 + dTax;
    /* Credibility earned or lost, and it sets how strongly expectations are
     anchored to the target rather than to recent experience. */ const miss = Math.abs(e.inflation - PI_TARGET);
    e.credibility = clamp(e.credibility + (miss < 1 ? CRED_GAIN * (1 - e.credibility) : -CRED_LOSS * Math.min(3, miss - 1)), 0.05, 0.97);
    const anchor = 0.10 + 0.55 * e.credibility;
    e.expect += ((1 - anchor) * e.inflation + anchor * PI_TARGET - e.expect) * PI_ADJ;
    const shockNow = e.supplyShock * (1 - clamp(E.resilience, 0, 0.8));
    const dEr = ((law.ni.erOn === false ? 0 : law.ni.erRate) - (prevLaw.ni.erOn === false ? 0 : prevLaw.ni.erRate)) * 0.06;
    /* Inertia carries *underlying* inflation forward, not the indirect-tax echo. A
     ...
     the Bank then tightened into for years. */ /* Wage bargaining, then unit labour costs, then prices. */ const slack = e.nairu - e.unemployment;
    const gL = labourGrowthRate(e, E);
    ...
    /* Import prices: world inflation, FX, terms-of-trade shocks, and the level of
     the effective import tariff relative to the opening rate (P_m' = P_m (1+τ)). */ const importInf = WORLD_INFL + (e.modWorldInfl || 0) - (e.fx - 1) * 100 * 0.6 + (e.modToT || 0) + (importT - BASE_TARIFF) * TARIFF_IMPORT;
    e.ulc = ulc;
    e.importTariff = importT;
    const inertia = e.inflation - e.vatEcho * 0.6;
    e.inflation = clamp(PI_LAMBDA * inertia + (1 - PI_LAMBDA) * (ULC_PASS * ulc + IMPORT_PASS * importInf + (1 - ULC_PASS - IMPORT_PASS) * e.expect) + PI_KAPPA * (1 + PI_CONVEX * Math.max(0, newGap)) * newGap + shockNow + e.vatEcho + dEr, -4, 45);
    e.supplyShock *= 0.55;
    /* ---- Taylor rule ----
       i = r* + pi + a_pi*(pi - pi*) + a_y*gap
     with partial adjustment, because policy rates move in steps.
     Manual mode (sandbox-style toggle) pins the rate and skips the rule. */ if (g.rateManual) {
        e.rate = clamp(g.manualRate != null ? g.manualRate : e.rate, MANUAL_RATE_MIN, 20);
    } else {
       ...
       once through a rate rise it should never have caused. */ const underlying = e.inflation - e.vatEcho * 0.75;
        const target = R_NEUTRAL_REAL + underlying + TAYLOR_PI * (underlying - PI_TARGET) + TAYLOR_Y * newGap;
        e.rate = clamp(e.rate + (clamp(target, RATE_FLOOR, 20) - e.rate) * TAYLOR_SMOOTH, RATE_FLOOR, 20);
    }
    e.atBound = e.rate <= RATE_FLOOR + 0.02;
    /* ---- Okun's law around a wage-setting equilibrium ----
       du = -beta*(g - g*) + adjustment toward u*
     u* itself comes from the wage-setting block below. */ const erGap = (law.ni.erOn === false ? 0 : law.ni.erRate) - DEF_NI.erRate;
    /* Track how much of the employer NI deviation has not yet passed into wages.
     A rate change lands unshifted and decays toward the wage bill from there. */ e.erBorne += erGap - e.erBornePrev;
    e.erBornePrev = erGap;
    e.erBorne *= 1 - WAGE_SHIFT;
    const shifted = erGap - e.erBorne;
    const wedgePP = (prof.meanMtr - MTR_BASE) * 100 + shifted;
    const nairu = U_STAR + WEDGE_ELAST * wedgePP + LABOUR_COST_ELAST * e.erBorne + REPLACEMENT_ELAST * (law.spend.welfare - 13.3 + (E.replace || 0)) - SKILLS_ELAST * (law.spend.education - 4.1) * 0.35 - 4.0 * ((e.hCap != null ? e.hCap : H_CAP0) - H_CAP0) + E.nairu;
    e.nairu = nairu;
    e.unemployment = clamp(e.unemployment - OKUN_BETA * (growth - pg) / 2 + (nairu - e.unemployment) * U_ADJ, 1.6, 25);
    /* ---- Debt dynamics ----
       db = (r - g)/(1 + g) * b - primary balance
     the standard accumulation identity, plus a convex risk premium in the
     yield: markets ignore the debt level until they abruptly do not. */ const rev = revenue(law, E, e), sp = spending(law, E, e);
    /* ---- Growth is not fiscally neutral ----
     ...
     a deficit was impossible, which is not how public finances work. */ const gapNow = e.gdp / e.potential - 1;
    const pot = e.potential;
    ...
    /* Debt may go negative. Sustained surpluses turn the liability into a stock of
     ...
     Inflating your way out of the debt works, but on three quarters of it. */ const ilCost = IL_REAL_COUPON + Math.max(-2, e.inflation);
    const blended = (1 - INDEX_LINKED) * e.effInterest + INDEX_LINKED * ilCost;
    e.blendedRate = blended;
    e.debt = clamp(e.debt * (1 + (blended - nominalGrowth) / 400) + primaryDeficit / 4 - e.fund * 0.02, -200, 400);
    ...
    /* ---- Sectoral balances ----
     ...
     instead of quietly distorting the accounts. */ const trShare = govDemandShares(law, e).tr;
    ...
    /* Force the Godley identity for the stock that accumulates; keep the
     measurement gap as an explicit residual that feeds back into C. */ e.balPrivate = -(e.balGovt + e.balForeign);
    ...
    /* Net private financial wealth accumulates the identity-consistent private
     balance and feeds consumption and credit collateral. */ if (e.privateWealth == null) e.privateWealth = PRIVATE_WEALTH0;
    ...
    /* A creditor state still earns on its assets, but private finance does not
     keep getting cheaper with every extra point of net assets — see debtFinanceWedge. */ if (e.debt < 0) {
        ...
        /* Sovereign risk is always priced off excess debt above the seat's own
         ...
         pays more as debt climbs above that pin. */ const anchor = e.debtAnchor != null ? e.debtAnchor : 80;
        const debtForRisk = 80 + Math.max(0, e.debt - anchor);
        e.yield = clamp(e.rate + 0.3 + Math.pow(Math.max(0, debtForRisk - 80), 1.35) * 0.012 + Math.pow(Math.max(0, deficit - 3), 1.40) * 0.20 + e.riskPremium - (law.policies.fiscalRule ? 0.5 : 0), 0.2, 25);
    }
    e.riskPremium *= 0.7;
    /* High-debt openers (Japan) carry a long-maturity, financially-repressed
     ...
     live play then marches it to 4–5% in a decade, blowing the debt ratio. */ const maturityPass = e.debtAnchor != null && e.debtAnchor >= 150 ? MATURITY_PASS * 0.22 : MATURITY_PASS;
    ...
    /* Headline CPI index — same inflation factor the Uprate toggle applies to
     cash thresholds, so real allowance holds when uprating and falls when frozen. */ e.cpiIndex = (e.cpiIndex || 1) * (1 + Math.max(0, e.inflation) / 400);
    if (law.income.uprate) {
        const up = 1 + Math.max(0, e.inflation) / 400;
        law.income.allowance *= up;
        law.income.bands.forEach((b)=>b.from *= up);
    }
    // society
    /* ---- Public services: volume against need, not an assumed decay ----
     ...
     growth and sent fast seats' tax take toward 60%+ of GDP. */ const gapPct = e.gdp / e.potential - 1;
    const shareC0 = e.shareC != null ? e.shareC : SHARE_C;
    e.baseConsumption = e.C / Math.max(1e-6, shareC0);
    /* Wage-bill proxy: participation and unemployment, not the GDP index. */ e.baseWages = (e.participation || 1) * (1 - (e.unemployment - U_STAR) / 100 * 1.4);
    ...
    /* Profits booked here rather than abroad. A rate above the world average
     ...
     than a static costing suggests. */ const cRate = law.taxes.corpTax && law.taxes.corpTax.on ? law.taxes.corpTax.rate : 0;
    ...
    const assetTarget = Math.max(0.25, 1 / (1 + (realRateNow - R_REAL_NEUTRAL) / ASSET_RATE)) * (1 + gapPct * 0.8);
    e.assetPrice += (assetTarget - e.assetPrice) * ASSET_ADJ;
    /* Asset-tax base tracks real asset prices (and the gap), not GDP level. */ e.baseAssetsRaw = e.assetPrice;
    e.baseAssets = e.baseAssets + (e.baseAssetsRaw - e.baseAssets) * CGT_LAG;
    e.fuelVolume *= 1 - (FUEL_EROSION + CARBON_EROSION * (law.taxes.carbon.on ? law.taxes.carbon.rate : 0)) / 400;
    /* Regional divergence, driven by what each place is exposed to. */ const tradeG = (acct.X - (e.prevX || acct.X)) / Math.max(1, acct.X) * 400;
    const publicG = (govDemandShares(law, e).g - (e.prevG || govDemandShares(law, e).g)) * 4;
    stepRegions(e, law, growth, clamp(tradeG, -12, 12), clamp(publicG, -6, 6), (e.ucSmooth || UC_BASE) - UC_BASE);
    e.prevX = acct.X;
    ...
    /* Departments set to hold their service level have their budget uprated to
     ...
     people who deliver the service, nor a growing population. */ for (const d of DEPTS){
        ...
        if (mode === "service" && !TRANSFER_DEPTS[d.id]) {
            ...
            law.spend[d.id] = clamp(law.spend[d.id] * (1 - realGrowth / 400), d.min, d.max);
        }
    }
    ...
    const sTarget = clamp(services0 * quality / Math.max(0.5, quality0) - Math.max(0, e.inflation - 2) * 1.2 + E.srv + E.hlt * 2 + (e.modSrv || 0) + (e.modHlt || 0) * 2, 0, 100);
    ...
    /* Pension caseload rises with the dependency ratio, so a fixed share of GDP
     ...
     the triple lock argues about. */ /* Triple lock: ΔP = max(wage growth, CPI, 2.5%) while the policy is on. */ if (e.pensionIndex == null) e.pensionIndex = 1;
    ...
    const carbonNow = law.taxes.carbon && law.taxes.carbon.on ? law.taxes.carbon.rate : 0;
    e.health = clamp(e.health + (health0 + (E.hlt - soc0.hlt) * 6 + (e.modHlt || 0) * 6 + (e.services - services0) * 0.4 - Math.max(0, e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 1.1 - Math.max(0, e.dependency - DEP_0) * 18 - e.health) * 0.15, 0, 100);
    e.crime = clamp(e.crime + (crime0 + (E.cri - soc0.cri) * 1.6 + (e.modCri || 0) * 1.6 + (E.blackLevel - soc0.blk) * 0.35 - (law.spend.justice - soc0.justice) * 7 + (e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 2.2 + Math.max(0, e.gini - gini0) * 0.18 - e.crime) * 0.16, 0, 100);
    e.liberty = clamp(e.liberty + (liberty0 + (E.lib - soc0.lib) - e.liberty) * 0.25, 0, 100);
    e.env = clamp(e.env + (env0 + (E.env - soc0.env) + (law.spend.environment - soc0.envSpend) * 6 + (carbonNow - soc0.carbon) * 0.5 - e.env) * 0.14, 0, 100);
    ...
    /* Inequality is derived from the post-tax income distribution (and UBI / min
     ...
     still nudge the path, but authored Gini overrides are no longer the core. */ const distGini = prof.gini != null ? prof.gini : 35;
    const gTarget = distGini + (e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 0.35 + E.gini - (prof.progressivity - PROG_BASE) * 6;
    ...
    for (const p of activePartners(g.homeRole)){
        ...
        /* Deal stress ladder: cold relations freeze access, then force a withdrawal. */ const partnerDeals = dealsWith(p.id, law);
        if (partnerDeals.length) {
            let floor = 28;
            for (const id of partnerDeals){
                const need = DEAL_BY_ID[id].need && DEAL_BY_ID[id].need.relation || 0;
                if (need) floor = Math.max(floor, need - 12);
            }
            if (g.rel[p.id] < floor) e.dealStress[p.id] = (e.dealStress[p.id] || 0) + 1;
            else e.dealStress[p.id] = Math.max(0, (e.dealStress[p.id] || 0) - 0.5);
            if ((e.dealStress[p.id] || 0) >= 4) {
                const softest = partnerDeals.slice().sort((a, b)=>(DEAL_BY_ID[a].pc || 0) - (DEAL_BY_ID[b].pc || 0))[0];
                if (softest) {
                    delete law.deals[softest];
                    if (g.draft && g.draft.deals) delete g.draft.deals[softest];
                    briefExtra.push(DEAL_BY_ID[softest].name + " with " + p.name + " collapsed under diplomatic strain.");
    ...
    const common = 40 + realIncome * 2.6 - Math.max(0, e.inflation - 3) * 2.0 - (e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 2.6 + (e.services - svcRef) * 0.40;
    const extra = {
        business: (pg - 1.5) * 5,
        workers: -(e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 2.0 - (e.gini - giniRef) * 0.30,
        pensioners: -Math.max(0, e.inflation - 2) * 2.2 + (e.services - svcRef) * 0.25,
        urban: (e.liberty - libRef) * 0.22 + (e.env - envRef) * 0.14,
        rural: -(e.crime - crimeRef) * 0.14,
        patriots: -(e.openness - openRef) * 0.14 - (e.crime - crimeRef) * 0.20
    };
    const basicRate = law.income.on === false ? 0 : law.income.bands[0].rate;
    const topRate = law.income.on === false ? 0 : law.income.bands[law.income.bands.length - 1].rate;
    extra.workers -= (basicRate - 20) * 0.55;
    extra.business -= (topRate - 45) * 0.22;
    extra.urban += (prof.progressivity - PROG_BASE) * 10;
    extra.rural -= (basicRate - 20) * 0.30;
    const fiscalDrag = clamp(1 - dragRatio(law, e), -0.4, 0.6);
    extra.workers -= fiscalDrag * 16 + ((law.ni.empOn === false ? 0 : law.ni.empRate) - DEF_NI.empRate) * 0.9;
    ...
    /* The empirical result on employer payroll taxes is that most of the burden
     ...
     employers, and only a little shows up as structural unemployment above. */ extra.business -= erGap * 0.55;
    extra.workers -= erGap * 0.60;
    const shock = billShock(law, prevLaw);
    for (const f of FACTIONS){
        const t = common + E.fac[f.id] + (extra[f.id] || 0);
        g.fac[f.id] = clamp(g.fac[f.id] + (t - g.fac[f.id]) * 0.20 + (shock[f.id] || 0) + mA, 2, 96);
    }
    ...
    /* Base regen plus a mild approval bonus. Tuned so a mid-term chancellor can
     ...
     cheaper than legislative pace. */ const regen = polityOf(g.homeRole).capitalRegen;
    g.capital = clamp(g.capital + (3.2 + (appr - 45) * 0.10) * regen, 0, 100);
...
/* one-off popularity jolt from what the bill actually did */ function billShock(law, prevLaw) {
    const s = {
        business: 0,
        workers: 0,
        pensioners: 0,
    ...
    for (const t of TAXES){
        const a = prevLaw.taxes[t.id], b = law.taxes[t.id];
        if (!a || !b) continue;
        const wasOn = a.on && taxAvailable(t, prevLaw), isOn = b.on && taxAvailable(t, law);
        let d = (isOn ? b.rate : 0) - (wasOn ? a.rate : 0);
        if (!d) continue;
        const weight = t.grp === "income" ? 0.55 : t.grp === "consumption" ? 0.40 : 0.25;
        for(const f in s)s[f] -= d * weight * 0.5;
        if (t.fac) {
            for(const f in t.fac)if (f in s) s[f] += (t.fac[f] || 0) * d * 0.25;
        }
    }
    const rateNow = (law)=>law.income.on === false ? 0 : law.income.bands[0].rate;
    const topNow = (law)=>law.income.on === false ? 0 : law.income.bands[law.income.bands.length - 1].rate;
    const dBasic = rateNow(law) - rateNow(prevLaw);
    const dAllow = (law.income.allowance - prevLaw.income.allowance) / 1000;
    const dEmp = (law.ni.empOn === false ? 0 : law.ni.empRate) - (prevLaw.ni.empOn === false ? 0 : prevLaw.ni.empRate);
    const dEr = (law.ni.erOn === false ? 0 : law.ni.erRate) - (prevLaw.ni.erOn === false ? 0 : prevLaw.ni.erRate);
    ...
    const dTop = law.income.bands.length === prevLaw.income.bands.length || law.income.on === false || prevLaw.income.on === false ? topNow(law) - topNow(prevLaw) : 0;
    s.business -= dTop * 1.1;
    s.workers += dTop * 0.5;
    for (const dp of DEPTS){
        ...
        for(const f in s)s[f] += d * pop * 0.55;
    }
    if (prevLaw.polity && law.polity && prevLaw.polity !== law.polity) {
        const pf = polityShockFac(prevLaw.polity, law.polity);
        for (const f in pf)if (f in s) s[f] += pf[f];
    }
    return s;
}
function qLabel(g, i) {
    return "Y" + (Math.floor(i / 4) + 1 + (g.term - 1) * 5) + " Q" + (i % 4 + 1);
}
...
function announceQuarterAdvance() {
    return new Promise((resolve) => {
        ...
        if (tb) {
            ...
            window.setTimeout(() => tb.classList.remove("q-advance"), QUARTER_FLASH_MS + 100);
        }
...
/* ==================================================================
   ...
   the Bank without mutating live G. */ function simulate(law, quarters, opts) {
    const o = opts || {};
    ...
    else if (rateManual && manualRate != null) sim.econ.rate = clamp(manualRate, rateLo, 20);
    const rows = [];
...
/** Forward simulation that applies staged bloc join/leave/create on top of a draft law. */ function simulateFromDraft(draft, quarters, opts) {
    const snap = draftBlocSnapshotFrom(draft);
    const law = clone(draft);
    applyDraftBlocLaw(law, snap, draft);
    return simulate(law, quarters, Object.assign({}, opts || {}, {
        blocMember: snap.blocMember,
        customBlocs: snap.customBlocs
    }));
}
function project(quarters) {
...
/* ---- The forecast is not the outturn ----
   ...
   forecast, which is what makes the decision a judgement rather than a lookup. */ const FC_GROWTH_SD = 0.75, FC_BAL_SD = 0.55, FC_INF_SD = 0.45;
function withForecastError(p) {
    /* Sandbox is the mode for seeing how the model works, so the forecast is the
     ...
     which is deciding on numbers that will turn out to be wrong. */ if (G.sandbox) return {
        rows: p.rows,
        end: p.end,
        err: {
            growth: 0,
    ...
    const eg = gauss() * FC_GROWTH_SD, eb = gauss() * FC_BAL_SD, ei = gauss() * FC_INF_SD;
...
/* The law you would have if only clause `i` of the current bill passed. Built by
   ...
   machinery the bill itself does rather than a parallel description of it. */ function lawWithOnlyClause(i) {
    const saved = G.draft;
...
/* Compound the path's quarterly growth rates into total % growth over the
   ...
   before compounding. Over four quarters this is year-on-year GDP growth. */ function pathGrowth(rows) {
    let y = 1;
...
/* Difference a law makes against a baseline, after `quarters` quarters.
   ...
   quarter's annualised pace, which whipsaws after a level jump. */ function impactOf(law, baseline, quarters) {
    const v = simulateFromDraft(law, quarters);
    const a = baseline.rows[baseline.rows.length - 1], b = v.rows[v.rows.length - 1];
    const out = {
        head: {},
        fac: {},
    ...
    IMPACT_ROWS.forEach((r)=>{
        out.head[r.k] = r.k === "growth" ? pathGrowth(v.rows) - pathGrowth(baseline.rows) : b[r.k] - a[r.k];
    });
    FACTIONS.forEach((f)=>{
        out.fac[f.id] = v.end.fac[f.id] - baseline.end.fac[f.id];
    });
    out.revenue = revenue(law, aggregate(law)).total - revenue(G.law, aggregate(G.law)).total;
    return out;
}
/* Pin the base rate versus leaving it with the Bank, same horizon as the bill
   impact panel. Uses current law so the strip isolates the monetary choice. */ function impactOfRatePin(quarters) {
    const q = quarters || 4;
    const pin = clamp(G.manualRate != null ? G.manualRate : G.econ.rate, MANUAL_RATE_MIN, 20);
    ...
    const a = base.rows[base.rows.length - 1], b = v.rows[v.rows.length - 1];
    const out = {
        head: {},
        fac: {},
    ...
    IMPACT_ROWS.forEach((r)=>{
        out.head[r.k] = r.k === "growth" ? pathGrowth(v.rows) - pathGrowth(base.rows) : b[r.k] - a[r.k];
    });
    FACTIONS.forEach((f)=>{
        out.fac[f.id] = v.end.fac[f.id] - base.end.fac[f.id];
    });
    return out;
}
function rateImpactHtml() {
    ...
    return '<div class="impblock rate-impact">' + '<div class="imphead">Four quarters at ' + im.pin.toFixed(2) + '% vs the Bank</div>' + '<div class="hint" style="margin:0 0 6px">Same law, pinned rate against the Taylor rule from today\'s starting point.</div>' + (empty ? '<div class="impfac">No measurable move over four quarters.</div>' : chips + impactFactions(im)) + '</div>';
}
function projectionWarnings(p) {
    const w = [], last = p.rows[p.rows.length - 1], first = p.rows[0], e = p.end.econ;
    if (last.yield > 7.5) w.push("Gilt yields reach " + last.yield.toFixed(1) + "%. The debt market is repricing {C}.");
    if (last.debt > G.econ.debt + 4) w.push("Debt rises " + (last.debt - G.econ.debt).toFixed(1) + " points of GDP over four quarters and keeps climbing.");
    if (last.balance < -7) w.push("The deficit settles near " + Math.abs(last.balance).toFixed(1) + "% of GDP, well beyond what the market has tolerated before.");
    if (e.inflation > 5) w.push("Inflation is still " + e.inflation.toFixed(1) + "% a year out, so expect the Bank to keep tightening.");
    if (e.inflation < 0.5) w.push("Inflation drops to " + e.inflation.toFixed(1) + "%. Deflation risk is real.");
    if (first.growth < -0.4) w.push("The first quarter contracts. Fiscal tightening lands before anything else does.");
    if (last.unemployment > G.econ.unemployment + 0.8) w.push("Unemployment rises " + (last.unemployment - G.econ.unemployment).toFixed(1) + " points on this plan.");
    if (last.approval < 32) {
        const meta = polityOf(G.homeRole);
        if (meta.coupOn === "patriots") {
            w.push("Approval falls to " + last.approval.toFixed(0) + "%. The party apparatus will notice if loyalty frays.");
        } else {
            w.push("Approval falls to " + last.approval.toFixed(0) + "%. Your parliamentary party will notice.");
        }
    }
    if (e.services < 40) w.push("Public services degrade to " + e.services.toFixed(0) + " on the composite index.");
    if (G.draft.policies.fiscalRule && last.balance < -4) w.push("This plan breaches your own statutory debt rule.");
    if (!w.length) w.push("Nothing in this plan trips a Treasury warning. The forecast is still a forecast.");
    return w;
}
/* ==================================================================
   6. THE BILL  (diff between what is law and what you propose)
   ================================================================== */ const dp = (t)=>t.step && t.step < 1 ? 1 : 0;
const fmtRate = (t, r)=>r.toFixed(dp(t)) + "%";
const rateCost = (t, d)=>Math.max(1, Math.ceil(Math.abs(d) / t.max * 25));
function billClauses() {
    pruneDraftBlocInvites();
    const L = G.law, D = G.draft, out = [];
    if (D.regime !== L.regime) out.push({
    ...
    if ((D.polity || "democracy") !== (L.polity || "democracy")) {
        ...
        if (polityCanReach(from, to)) {
            out.push({
                label: "Change the political system: " + (POLITY[to] && POLITY[to].label || to),
    ...
    for (const t of TAXES){
        const a = L.taxes[t.id], b = D.taxes[t.id];
        if (a.on !== b.on) {
            out.push({
                label: (b.on ? "Introduce " : "Abolish ") + t.name + (b.on ? " at " + fmtRate(t, b.rate) : ""),
    ...
    if (D.income.on !== false) {
        if (Math.abs(D.income.allowance - L.income.allowance) > 1) {
            out.push({
                label: "Personal allowance " + money(L.income.allowance) + " to " + money(D.income.allowance),
                pc: Math.max(1, Math.ceil(Math.abs(D.income.allowance - L.income.allowance) / 1800)),
                undo: ()=>{
        ...
        if (D.income.uprate !== L.income.uprate) {
            out.push({
                label: D.income.uprate ? "Uprate thresholds with inflation each year" : "Freeze thresholds in cash terms",
                pc: 8,
                undo: ()=>{
        ...
        if (D.income.bands.length !== L.income.bands.length) {
            out.push({
                label: "Restructure the rate bands: " + L.income.bands.length + " to " + D.income.bands.length,
                pc: 12,
                undo: ()=>{
            ...
            D.income.bands.forEach((b, i)=>{
                const a = L.income.bands[i];
                if (Math.abs(a.rate - b.rate) > 1e-9) out.push({
                    label: (BAND_NAMES[i] || "Band " + (i + 1)) + " rate " + a.rate + "% to " + b.rate + "%",
                    pc: Math.max(1, Math.ceil(Math.abs(b.rate - a.rate) / 3)),
                    undo: ()=>{
                        b.rate = a.rate;
                    }
                });
                if (i > 0 && Math.abs(a.from - b.from) > 1) out.push({
                    label: (BAND_NAMES[i] || "Band " + (i + 1)) + " threshold " + money(a.from) + " to " + money(b.from),
        ...
        for(const k in CAP_LABEL){
            const a = L.income[k], b = D.income[k];
            ...
            out.push({
                label: CAP_LABEL[k] + " " + a.toFixed(0) + "% to " + b.toFixed(0) + "%",
    ...
    for (const d of DEPTS){
        const mL = (L.mode || {})[d.id] || "share", mD = (D.mode || {})[d.id] || "share";
        if (mL !== mD) {
            const label = {
        ...
        if (Math.abs(diff) > 1e-9) out.push({
            ...
            undo: ()=>{
                ...
                /* Spend and hold travel together in service mode — undoing one
                 without the other leaves step() enforcing a stale standard. */ if (((D.mode || {})[d.id] || "share") === "service") {
                    if (!D.hold) D.hold = {};
    ...
    }
    {
        ...
        if (D.tariffSchedule.cet !== L.tariffSchedule.cet && tLock.mode !== "none") {
            out.push({
                label: "Common external tariff " + (L.tariffSchedule.cet || tLock.cet) + "% to " + D.tariffSchedule.cet + "%",
                pc: Math.max(2, Math.ceil(Math.abs((D.tariffSchedule.cet || 0) - (L.tariffSchedule.cet || 0)) * 1.2)),
                undo: ()=>{
        ...
        for(const bid in D.tariffSchedule.bloc){
            const a = L.tariffSchedule.bloc[bid], b = D.tariffSchedule.bloc[bid];
            if (a !== b) {
        ...
        for(const cid in D.tariffSchedule.country){
            const a = L.tariffSchedule.country[cid], b = D.tariffSchedule.country[cid];
            if (a !== b) {
        ...
        if (D.tariffSchedule.default !== L.tariffSchedule.default && tLock.mode === "none") {
            out.push({
                label: "Default external tariff " + L.tariffSchedule.default + "% to " + D.tariffSchedule.default + "%",
                pc: Math.max(1, Math.ceil(Math.abs(D.tariffSchedule.default - L.tariffSchedule.default) * 0.4)),
                undo: ()=>{
                    D.tariffSchedule.default = L.tariffSchedule.default;
                    syncTariffHeadline(D);
                }
    ...
    if (G.draft.blocAccession && !countryBlocId(playerCountryId())) {
        const ba = G.draft.blocAccession;
        const bloc = blocByIdOrCustom(ba.blocId);
        const spec = blocAccessionSpec(ba.blocId);
        const steps = spec && spec.steps ? spec.steps : {
            apply: 8,
            align: 14,
            accede: 16
        };
        const labels = {
            apply: "Application to join ",
            align: "Accession alignment with ",
            accede: "Accession treaty with "
        };
        const pcs = {
            apply: steps.apply,
            align: steps.align,
        ...
        if (labels[ba.phase]) {
            out.push({
                label: labels[ba.phase] + (bloc ? bloc.name : ba.blocId),
                pc: pcs[ba.phase] || 12,
                undo: ()=>{
                    G.draft.blocAccession = null;
                }
    ...
    if (G.draft.blocCreate) {
        const tpl = CUSTOM_BLOC_TEMPLATES[G.draft.blocCreate.template];
        out.push({
    ...
    for(const cid in G.draft.blocInvite || {}){
        ...
        const bloc = bid && (blocById(bid) || G.customBlocs[bid]);
        out.push({
            label: "Propose " + (c ? c.name : cid) + " join " + (bloc ? bloc.name : "bloc"),
            pc: 12,
            undo: ()=>{
    ...
    for (const p of POLICIES){
        const a = !!L.policies[p.id], b = !!D.policies[p.id];
        if (a !== b) out.push({
    ...
    for(const id in DEAL_BY_ID){
        const a = !!L.deals[id], b = !!G.draft.deals[id];
        if (a !== b) {
            const d = DEAL_BY_ID[id];
            out.push({
...
const billCost = ()=>billClauses().reduce((a, c)=>a + (c.sunk ? 0 : c.pc), 0);
const isFiscalOnly = (cl)=>cl.every((c)=>!/Ratify|Withdraw|Enact|Repeal|Cannabis|Psychedelics|Gambling|Alcohol|Tobacco|Adult|summit|protest|Restrictive|State visit|Formal protest|envoy/i.test(c.label));
/* ==================================================================
   7. RENDERING
...
/* --- seamless 3D value noise --- */ function hash3(i, j, k) {
...
/* The country is built from overlapping lobes rather than one radial blob,
   ...
   of a circle. Offshore islands are separate lobes; the lake is a negative one. */ const LOBES = [
    ...
    [
        ...
        0.20,
        0.30
    ] // waist, keeps north and south joined
];
const ISLES = [
    [
        -0.74,
...
function lobeField(list, dx, dy) {
    let f = 0;
    for (const L of list){
        const ex = (dx - L[0]) / L[2], ey = (dy - L[1]) / L[2];
...
/* >0 is land. The noise is added in world space rather than radially, so the
   coast wanders the way a real one does instead of rippling around a circle. */ function countryField(dx, dy) {
    const warp = (fbm2(dx * 1.9 + 5.1, dy * 1.9 + 2.7, 4) - 0.5) * 0.52;
    const fine = (fbm2(dx * 6.2 + 17.3, dy * 6.2 + 9.1, 3) - 0.5) * 0.20;
    ...
    // inland water
    const lx = (dx - LAKE[0]) / LAKE[2], ly = (dy - LAKE[1]) / LAKE[2];
...
/* Region index per pixel: 255 means sea. Also returns a border mask and the
   pixel count per region, which is what the geometry tests assert on. */ function mapGeometry(W, H) {
    const idx = new Uint8Array(W * H), edge = new Uint8Array(W * H), shelf = new Uint8Array(W * H);
    ...
    for(let py = 1; py < H - 1; py++)for(let px = 1; px < W - 1; px++){
        const i = py * W + px, v = idx[i];
        if (v === 255) continue;
        if (idx[i - 1] !== v || idx[i + 1] !== v || idx[i - W] !== v || idx[i + W] !== v) edge[i] = 1;
    }
    /* Ocean is whatever the frame edge can reach. Any water it cannot reach is
     inland, and is drawn as a lake rather than as sea. */ const ocean = new Uint8Array(W * H), stack = [];
    const push = (i)=>{
    ...
    /* Continental shelf: how many dilation steps of sea from the nearest coast,
     which is what lets the water shade from shallow to deep. */ const RINGS = Math.max(3, Math.round(Math.min(W, H) * 0.045));
    let front = [];
    ...
    for(let ring = 1; ring <= RINGS && front.length; ring++){
        ...
        for (const i of front){
            ...
            for (const j of nb)if (!seen[j] && idx[j] === 255) {
                seen[j] = 1;
...
function openingFac(homeRole) {
    const id = resolveHomeRole(homeRole || "home");
    if (id === "home") return Object.assign({}, FAC_0);
    const p = NATION_PROFILE[id];
    return Object.assign({}, FAC_0, p && p.fac0 || {});
}
function openingRel(homeRole) {
    const id = resolveHomeRole(homeRole || "home");
    const row = REL_0[id] || REL_0.home;
    const out = {};
    for (const p of activePartners(id)){
        out[p.id] = row[p.id] != null ? row[p.id] : 50;
    }
    return out;
}
/** Opening GDP (USD bn) for a seat id, including the displaceable Kingdom. */ function gdp0ForSeat(role) {
    const id = resolveHomeRole(role || "home");
    if (id === "home") return NATION_PROFILE.kingdom.gdp0;
    const p = NATION_PROFILE[id];
    return p && p.gdp0 != null ? p.gdp0 : NATION_PROFILE.kingdom.gdp0;
}
/** ISO currency code for a playable seat (or partner nation). */ function currencyForSeat(role) {
    const id = resolveHomeRole(role || "home");
    if (id === "home") return NATION_PROFILE.kingdom.currency || "GBP";
    const p = NATION_PROFILE[id];
    return p && p.currency || "USD";
}
/** Fill missing FX fields on a partner nation state (older saves / lazy init). */ function ensureNationFx(n) {
    if (!n) return n;
    if (n.fx == null) n.fx = 1;
...
/** Currency strength index, 100 at opening. Home uses e.fx; partners use n.fx. */ function fxDisplayIndex(role, g) {
    ...
    const n = ensureNationFx(e.nations[role] || (e.nations[role] = nationStateFromProfile(NATION_PROFILE[role] || NATION_PROFILE.kingdom)));
 ...
 *  dollars; depreciation shrinks the USD figure. */ function realmGdpBn(role, g) {
    const G = g || (typeof getG === "function" ? getG() : null);
    ...
    const n = ensureNationFx(e.nations[role] || (e.nations[role] = nationStateFromProfile(NATION_PROFILE[role] || {
        trend: 1,
        debt0: 50,
        deficit0: 3,
        inflation0: 2.2
...
/** Implied rest-of-world GDP (bn) matching the average of the named trade basket. */ function restGdp0ForRole(homeRole) {
    let named = 0, namedShare = 0;
    for (const p of activePartners(homeRole)){
        const w = partnerShare(homeRole, p);
...
/** Trade-share × absolute-GDP foreign demand, including the rest residual. */ function worldDemandBn(e, homeRole) {
    if (!e.nations) {
        e.nations = {};
        for(const id in NATION_PROFILE)e.nations[id] = nationStateFromProfile(NATION_PROFILE[id]);
    }
...
/** Recompute worldY from the partner GDP basket. Pins worldDemand0 on first use. */ function refreshWorldY(e, homeRole) {
    if (e.worldRestY == null) e.worldRestY = 100;
...
/** Rest-of-world residual grows at the exogenous world rate (+ event shocks). */ function stepWorldRest(e) {
    if (e.worldRestY == null) e.worldRestY = 100;
    e.worldRestY *= 1 + (WORLD_GROWTH + (e.worldShock || 0)) / 400;
}
/** Format USD bn as $3.6tn / $420bn for the realm card. */ function fmtGdpBn(bn) {
...
function nationStateFromProfile(p) {
    return {
        y: 100,
        growth: p.trend,
        debt: p.debt0,
...
function stepNations(e, det, g) {
    ...
    const partnerPulse = e.partnerShock || {};
    for(const id in NATION_PROFILE){
        const p = NATION_PROFILE[id], s2 = ensureNationFx(e.nations[id]);
        ...
        s2.growth = s2.growth * 0.55 + (p.trend + worldPulse * p.beta + worldTfpPulse * p.beta + bilat + shock) * 0.45;
        s2.y *= 1 + s2.growth / 400;
        s2.deficit = clamp(s2.deficit * 0.85 + (p.deficit0 - (s2.growth - p.trend) * 0.7) * 0.15, -12, 18);
        s2.debt = clamp(s2.debt * (1 + (2.6 - s2.growth - s2.inflation) / 400) + s2.deficit / 4, 0, 400);
        s2.inflation = clamp(s2.inflation * 0.8 + (piStar + (s2.growth - p.trend) * 0.35) * 0.2, -3, 30);
        const fxUip = p.fxUip != null ? p.fxUip : FX_UIP;
        const worldRate = p.worldRate != null ? p.worldRate : WORLD_RATE;
        const proxyRate = clamp(worldRate + (s2.inflation - WORLD_INFL) * 0.85, RATE_FLOOR, 20);
        const stress = (s2.debt - p.debt0) * 0.02 + Math.max(0, s2.deficit - p.deficit0) * 0.15;
        s2.riskPremium = Math.max(0, (s2.riskPremium || 0) * 0.7 + Math.max(0, stress));
        const fxTarget = 1 + fxUip * (proxyRate - worldRate) - FX_RISK * s2.riskPremium;
...
/* ---------- Regions diverge ----------
   ...
   place years later. */ const REGION_STICKY = 0.045, REGION_OKUN = 0.30;
/* Places do not start level. A region built on heavy industry carries higher
   ...
   which is the inheritance rather than the policy. */ function initRegions(e) {
    return REGIONS.map((R)=>({
            y: 100 * (0.88 + R.prosper * 0.12),
            u: clamp(e.unemployment * (0.72 + R.beta * 0.28), 1.5, 14)
        }));
}
function stepRegions(e, law, growth, tradeGrowth, publicGrowth, ucGap) {
    if (!e.regions) e.regions = initRegions(e);
    const trend = e.trendGrowth || 1.0;
    const hpG = e.housePriceGrowth || 0;
    e.regions.forEach((r, i)=>{
        const R = REGIONS[i];
        const local = trend + (growth - trend) * R.beta + tradeGrowth * (R.trade - 1) * 0.55 + publicGrowth * R.publicShare * 1.5 - ucGap * R.housing * 0.30 + hpG * R.housing * 0.22;
        r.y *= 1 + local / 400;
        r.u = clamp(r.u - REGION_OKUN * (local - trend) / 2 + (e.unemployment - r.u) * REGION_STICKY, 0.8, 30);
    });
}
/* ---------- what each region is feeling ---------- */ function regionStats(r) {
    const e = G.econ;
    ...
    /* A region that has fallen behind is a region whose people are less impressed,
     so local conditions feed back into local politics. */ appr += (st.y / 100 - e.gdp / 100) * 55 - (st.u - e.unemployment) * 2.4;
    const urbanShare = r.mix.urban || 0;
    const prosper = r.prosper * (st.y / 100) * (1 - (e.gini - (e.gini0 != null ? e.gini0 : 35)) * 0.006);
    const services = clamp(e.services + (urbanShare - 0.3) * 10 - (r.prosper < 0.85 ? 6 : 0), 0, 100);
    return {
...
let mapMetric = "approval", mapHover = -1;
/* Green → yellow → orange → red via shared metricRamp. No blue.
   `good` flips which end is which. */ function ramp(t) {
    return metricRamp(t);
}
function regionColour(r, metric) {
    const m = MAP_METRICS.find((x)=>x.id === metric) || MAP_METRICS[0];
    const v = regionStats(r)[m.id];
...
function paintMap() {
    ...
    for(let y = 1; y < H - 1; y++)for(let x = 1; x < W - 1; x++){
        ...
        if (idx[i - 1] === 255 || idx[i + 1] === 255 || idx[i - W] === 255 || idx[i + W] === 255) {
            const p = i * 4;
            d[p] *= 0.55;
            d[p + 1] *= 0.55;
            d[p + 2] *= 0.58;
    ...
    M.ctx.stroke();
    M.ctx.font = "600 " + (W * 0.020 | 0) + "px -apple-system, system-ui, sans-serif";
    M.ctx.textAlign = "center";
    M.ctx.textBaseline = "middle";
    for (const p of activePartners()){
        const b = PARTNER_BEARING[p.id];
        if (!b) continue;
        const rad = (b.a - 90) * Math.PI / 180;
        const rr = Math.min(W, H) * 0.462;
        const ax = W / 2 + Math.cos(rad) * rr, ay = H / 2 + Math.sin(rad) * rr;
        ...
        M.ctx.fillText(b.label, ax, ay - W * 0.030);
    }
    M.ctx.restore();
}
...
function updateMapLabel() {
    ...
    if (mapHover < 0) {
        ...
        box.innerHTML = '<b>' + esc(G.country) + '</b><span>' + m.name + ' by region</span>';
        return;
    }
    const r = REGIONS[mapHover], st = regionStats(r);
    box.innerHTML = '<b>' + esc(r.name) + '</b>' + '<span>' + MAP_METRICS.map((m)=>m.name + " " + m.fmt(st[m.id])).join(" &middot; ") + '</span>';
}
function updateMap() {
    if (Map2D.ctx) {
        paintMap();
...
function mapFallbackHtml() {
    return '<div class="globe-fallback"><p>The map could not be drawn here. ' + 'Everything else in the game is unaffected.</p></div>';
}
const TABS = [
    {
        id: "budget",
...
function spark(key) {
    let w = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 44, h = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : 13;
    const vals = G.log.map((r)=>r[key]);
    if (vals.length < 2) return "";
    const mn = Math.min(...vals), mx = Math.max(...vals), rg = mx - mn || 1;
    const pts = vals.map((v, i)=>(i / (vals.length - 1) * w).toFixed(1) + "," + (h - (v - mn) / rg * h).toFixed(1)).join(" ");
    return '<svg width="' + w + '" height="' + h + '" aria-hidden="true"><polyline points="' + pts + '" fill="none" stroke="#eed69a" stroke-width="1.2"/></svg>';
}
function dialHtml(k, v, unit, delta, warn, sk, invert) {
    const cls = delta == null ? "" : invert ? delta < 0 ? "up" : "dn" : delta > 0 ? "up" : "dn";
    const dt = delta == null ? "&nbsp;" : sgn(delta, 1);
    return '<div class="dial' + (warn ? " warn" : "") + '"><div class="k">' + k + '</div><div class="v">' + v + '<small>' + unit + '</small></div><div class="d ' + cls + '">' + dt + '</div>' + (sk ? spark(sk) : "") + '</div>';
}
const ICONS = {
    ...
    percent: '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3"><circle cx="5" cy="5" r="2"/><circle cx="11" cy="11" r="2"/><path d="M12.5 3.5l-9 9"/></svg>',
    scroll: '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3"><path d="M4 2.5h7.5v11H4z"/><path d="M6 5.5h3.5M6 8h3.5M6 10.5h2"/></svg>',
    people: '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3"><circle cx="6" cy="5.5" r="2.2"/><path d="M2.4 13c.3-2.3 1.8-3.6 3.6-3.6S9.3 10.7 9.6 13"/><path d="M10.6 4.2a2.1 2.1 0 010 4M11 9.6c1.6.2 2.7 1.5 3 3.4"/></svg>',
    globe: '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3"><circle cx="8" cy="8" r="6"/><path d="M2 8h12M8 2c1.8 2 1.8 10 0 12M8 2C6.2 4 6.2 12 8 14"/></svg>',
    ...
    chart: '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3"><path d="M2.5 13.5h11"/><path d="M4 11V7.5M7 11V4M10 11V8.5M13 11V6"/></svg>',
    clock: '<svg width="15" height="15" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.3"><circle cx="8" cy="8" r="6"/><path d="M8 4.5v4l2.5 2.5"/></svg>',
    close: '<svg width="10" height="10" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path d="M4 4l8 8M12 4l-8 8"/></svg>'
};
...
function termReviewScore() {
    ...
    const record = recent.reduce((a, r)=>a + (r.growth || 0), 0) / n * 2.5 - Math.max(0, e.inflation - 3) * 1.5 - (e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 1.5 + (e.services - (e.services0 != null ? e.services0 : 55)) * 0.15;
...
function chip(k, v, unit, delta, state, invert, kind) {
    const cls = delta == null ? "" : invert ? delta < 0 ? "up" : "dn" : delta > 0 ? "up" : "dn";
    // Always reserve the delta row so chips without a change stay the same height.
    const d = delta == null ? '<div class="d" aria-hidden="true">&nbsp;</div>' : '<div class="d ' + cls + '">' + sgn(delta, 2) + '</div>';
    const title = kind === "bank" ? G.rateManual ? ' title="Pinned by you. Turn Bank mode back on in the Bill panel to return the rate to the Taylor rule."' : ' title="Set by the Bank, not by you. It follows a Taylor rule: it rises when inflation is above target or output is above potential, and it is smoothed, so it moves in steps. Pin it yourself in the Bill panel."' : kind === "trend" ? ' title="Annualised potential growth from TFP, capital and labour — not outturn GDP."' : '';
    return '<div class="chip ' + (state || "") + (kind ? " " + kind : "") + '"' + title + '><div class="k">' + k + '</div>' + '<div class="v">' + v + (unit ? '<small>' + unit + '</small>' : '') + '</div>' + d + '</div>';
}
function renderChrome() {
    const n = G.log.length, last = n ? G.log[n - 1] : null, prev = n > 1 ? G.log[n - 2] : null;
    const d = (k)=>last && prev ? last[k] - prev[k] : null;
    const e = G.econ, appr = approvalOf(G.fac);
    /* Balance must match the Deficit reading elsewhere (deficit = −balance).
     ...
     which ignores those and drifts from the books the game actually ran. */ const balShow = last ? last.balance : balanceOf(G.law, e).balance;
    const nb = $("nameBtn");
    ...
    if (tb) {
        ...
        const electHint = left <= 4 ? " · " + noun + " score ~" + therm.toFixed(0) + (therm <= polityOf().loseAt ? " (at risk)" : "") : "";
        tb.textContent = [
            "First",
            "Second",
            "Third",
            ...
            "Fifth",
            "Sixth"
        ][Math.min(G.term - 1, 5)] + " term \u00b7 " + qLabel(G, G.q) + " \u00b7 " + noun + " in " + left + "Q" + electHint;
    }
    const mode = $("tbMode");
    if (mode) {
        mode.innerHTML = '<button type="button" class="mode-toggle" id="modeCareer" aria-pressed="' + !G.sandbox + '">Career</button>' + '<button type="button" class="mode-toggle" id="modeSandbox" aria-pressed="' + !!G.sandbox + '" title="Cannot be removed from office">Sandbox</button>';
        const setMode = (sb)=>{
            G.sandbox = sb;
    ...
    if (stats) {
        ...
        const left = electionQuartersLeft();
        const therm = electionThermometer();
        const reviewLabel = polityOf().kind === "congress" ? "Congress" : polityOf().kind === "managed" ? "Ballot" : "Election";
        stats.innerHTML = chip("Growth", fmt(gShow, 1), "%", d("growth"), last && last.growth < 0 ? "alert" : "") + chip("Trend", (e.trendGrowth != null ? e.trendGrowth : 0).toFixed(1), "%", null, "", null, "trend") + chip("Inflation", e.inflation.toFixed(1), "%", d("inflation"), e.inflation > 4 || e.inflation < 0 ? "alert" : "", true) + chip("Base rate", e.rate.toFixed(2), "%", d("rate"), e.atBound ? "alert" : "", null, "bank") + chip("Unemp.", e.unemployment.toFixed(1), "%", d("unemployment"), e.unemployment > 6 ? "alert" : "", true) + chip("Balance", fmt(balShow, 1), "%", d("balance"), balShow < -6 ? "alert" : balShow >= 0 ? "good" : "") + chip("Debt", e.debt.toFixed(0), "%", d("debt"), e.debt > 120 ? "alert" : "", true) + chip("Yield", e.yield.toFixed(2), "%", d("yield"), e.yield > 6.5 ? "alert" : "", true) + chip("Approval", appr.toFixed(0), "%", null, appr < 30 ? "alert" : appr > 55 ? "good" : "") + chip("Capital", Math.round(G.capital), "", null, G.capital < 12 ? "alert" : "") + chip(reviewLabel, String(left), "Q", null, left <= 4 ? therm <= polityOf().loseAt ? "alert" : "" : "", null);
    }
    const cl = billClauses(), cost = billCost(), afford = cost <= G.capital;
    const dock = $("dockTabs");
    if (dock) {
        dock.innerHTML = TABS.map((t)=>{
            ...
            return '<button class="dock-btn" data-tab="' + t.id + '" aria-label="' + t.name + '" aria-expanded="' + (t.id === tab) + '">' + (ICONS[t.icon] || "") + '<span>' + t.name + '</span>' + pip + '</button>';
        }).join("");
    ...
    if (bill) {
        ...
        $("billCost").textContent = cl.length ? cl.length + " \u00b7 " + cost + " cap" : "empty";
        bill.onclick = ()=>{
    ...
    if (db) {
        if (G.mp && G.mp.waiting) {
            ...
            db.textContent = h != null && n != null ? "Waiting " + n + "/" + h : "Waiting on others";
            db.title = "Edit anything to withdraw, or click here";
        } else {
            db.disabled = _quarterFlashLock || G.over || (cl.length > 0 && !afford);
            db.textContent = G.over ? "Term over" : cl.length === 0 ? "Next quarter" : afford ? "Deliver" : "No capital";
    ...
    if (dw) {
        const open = !!tab;
        dw.hidden = !open;
        dw.classList.toggle("wide", tab === "charts" || tab === "policies" || tab === "taxes");
        if (open) {
            const meta = tab === "bill" ? {
                name: "The Programme",
                sub: cl.length ? cost + " of " + Math.round(G.capital) + " capital" : "no clauses"
            } : tab === "trade" ? {
                name: "Trade",
...
function clausesIn(tabId, cl) {
    const pats = {
        budget: /points of GDP/,
        taxes: /tax|duty|levy|rate|tariff|Restructure|price|pricing/i,
        policies: /Enact|Repeal/,
        society: /Cannabis|Psychedelics|Gambling|Alcohol|Tobacco|Adult services|Change the political system/,
        trade: /Ratify|Withdraw|tariff|Propose|Leave|Found|join /i,
        diplomacy: /summit|State visit|Formal protest|Restrictive measures/i,
        charts: /(?!)/
    };
    return cl.some((c)=>pats[tabId] && pats[tabId].test(c.label));
}
/* ---- sliders ---- */ function leverHtml(id, name, val, min, max, stepv, decimals, note, cls, disabled) {
    return '<div class="lever ' + (cls || "") + '" data-lever="' + id + '">' + '<div class="top"><span class="nm">' + name + '</span><span class="val">' + val.toFixed(decimals) + '%</span><span class="chg"></span></div>' + '<input type="range" min="' + min + '" max="' + max + '" step="' + stepv + '" value="' + val + '" aria-label="' + esc(name) + '"' + (disabled ? " disabled" : "") + '>' + (note ? '<div class="yield">' + note + '</div>' : '') + '</div>';
}
function wireLevers(root, get, set, decimals) {
    root.querySelectorAll("[data-lever] input").forEach((inp)=>{
        const row = inp.closest("[data-lever]"), id = row.dataset.lever;
        const paint = ()=>{
            const v = get(id), base = row.dataset.base !== undefined ? +row.dataset.base : null;
            ...
            if (base !== null) {
                const d = v - base;
                c.textContent = Math.abs(d) < 0.049 ? "" : sgn(d, +row.dataset.dp || decimals);
...
/* ---- income tax and national insurance panel ---- */ function ctrlRow(name, val, min, max, step, disp, note, attrs) {
    return '<div class="taxrow" ' + (attrs || "") + '>' + '<div class="top"><span class="nm">' + name + '</span><span class="val">' + disp + '</span><span class="chg"></span></div>' + '<input type="range" min="' + min + '" max="' + max + '" step="' + step + '" value="' + val + '" aria-label="' + esc(name) + '">' + (note ? '<div class="yield">' + note + '</div>' : '') + '</div>';
}
/* Cash thresholds uprate with wages; keep real slider headroom constant so a
   long run never pins the thumb at a stale nominal ceiling. */ function thresholdSliderMax(baseMax, current, step) {
...
function incomePanelHtml() {
    const I = G.draft.income, N = G.draft.ni, E = aggregate(G.draft);
    ...
    let h = '';
    h += '<div class="eyebrow mt">Income tax <b>' + (incomeOn ? y.income.toFixed(2) + '% of GDP' : 'abolished') + '</b>' + (incomeOn && y.capital != null ? ' <span style="color:var(--ink-faint)">(labour ' + y.labour.toFixed(2) + ', capital ' + y.capital.toFixed(2) + ')</span>' : '') + ' <button class="btn ' + (incomeOn ? 'danger ' : '') + 'tiny" data-incometoggle="1" style="margin-left:8px">' + (incomeOn ? 'Abolish' : 'Reintroduce') + '</button></div>';
    h += '<div class="hint">' + (incomeOn ? 'Labour rates apply only above their own threshold, and only to wages and salaries.' + (flat ? ' The flat tax structure has collapsed every band into the first, and capital income is taxed at that same rate.' : '') + (G.sandbox ? ' Sandbox note: cutting the additional rate puts money where the MPC is low; with deficit finance the yield/FX channel can make the demand effect weakly negative — incidence is calibrated against relative demand from transfers and rate cuts.' : '') : 'Scrapped. About ' + incomeYield(withIncomeOn(G.draft, true), E, G.econ).income.toFixed(2) + '% of GDP forgone. National insurance still starts at the personal allowance.') + '</div>';
    if (!incomeOn) {
        h += '<div class="panel"><div class="hint" style="margin:0">The schedule is preserved so you can reintroduce it later. Abolishing costs 32 capital; bringing it back costs 28.</div></div>';
    } else {
        h += '<div class="panel">' + ctrlRow("Personal allowance", I.allowance, 0, thresholdSliderMax(30000, I.allowance, 250), 250, money(I.allowance), "real value " + Math.round(dr * 100) + "% of where it started" + (dr < 0.97 ? " &middot; <b class=\'warn-t\'>fiscal drag is raising taxes without a vote</b>" : ""), 'data-inc="allowance"') + '<div class="taxrow"><div class="top"><span class="nm">Threshold policy</span>' + '<span class="seg mini"><button data-uprate="1" aria-pressed="' + (I.uprate ? "true" : "false") + '">Uprate</button>' + '<button data-uprate="0" aria-pressed="' + (!I.uprate ? "true" : "false") + '">Freeze</button></span></div>' + '<div class="yield">Freezing thresholds raises real receipts every year without announcing anything. Voters work it out eventually.</div></div>' + '</div>';
        h += '<div class="panel">';
        I.bands.forEach((b, i)=>{
            const nm = BAND_NAMES[i] || "Band " + (i + 1);
            const top = i + 1 < I.bands.length ? money(I.bands[i + 1].from) : "upwards";
            const dim = flat && i > 0;
            h += '<div class="bandblock' + (dim ? " dim" : "") + '">' + '<div class="bandhead"><b>' + nm + '</b><span>' + money(b.from) + ' to ' + top + '</span>' + (I.bands.length > 1 && i === I.bands.length - 1 ? '<button class="btn danger tiny" data-delband="' + i + '">Remove</button>' : '') + '</div>' + ctrlRow(nm + " rate", b.rate, 0, 90, 1, b.rate + "%", null, 'data-band="' + i + '" data-field="rate"') + (i > 0 ? ctrlRow(nm + " starts at", b.from, 15000, thresholdSliderMax(300000, b.from, 1000), 1000, money(b.from), null, 'data-band="' + i + '" data-field="from"') : '') + '</div>';
        });
        if (I.bands.length < 6) h += '<div class="bandadd"><button class="btn" data-addband="1">Add a band</button>' + '<span class="pc">Restructuring the bands costs 12 capital</span></div>';
        h += '</div>';
        const dualCap = !!REGIME_BY_ID[G.draft.regime].dualCapital;
        if (!flat) {
            h += '<div class="eyebrow mt">Capital income <b>' + y.capital.toFixed(2) + '% of GDP</b></div>';
            h += '<div class="hint">' + (dualCap ? 'Tax on investment returns — dividends and savings interest — not on wealth itself. Both share one flat rate under this system. Selling an asset is capital gains tax.' : 'Tax on investment returns — dividends and savings interest — not on wealth itself, and not through the labour bands above. Selling an asset is capital gains tax.') + '</div>';
            h += '<div class="panel">';
            if (dualCap) {
                h += ctrlRow("Capital income rate", I.capitalRate != null ? I.capitalRate : 25, 0, 60, 1, (I.capitalRate != null ? I.capitalRate : 25) + "%", "one rate on dividends and savings interest alike", 'data-inc="capitalRate"');
            } else {
                h += ctrlRow("Dividend rate", I.divRate != null ? I.divRate : 33, 0, 60, 1, (I.divRate != null ? I.divRate : 33) + "%", "on dividends paid to shareholders from company profits", 'data-inc="divRate"');
                h += ctrlRow("Savings rate", I.saveRate != null ? I.saveRate : 20, 0, 60, 1, (I.saveRate != null ? I.saveRate : 20) + "%", "on interest from deposits, bonds and similar savings", 'data-inc="saveRate"');
            }
            h += '</div>';
        }
    } /* end incomeOn */ 
    h += '<div class="eyebrow mt">National insurance <b>' + (y.employee + y.employer).toFixed(2) + '% of GDP</b></div>';
    h += '<div class="hint">Two separate taxes wearing one name, and the difference is who really pays. Both start where income tax starts, so the personal allowance sets the floor for all three.</div>';
    const niHead = (name, on, yieldPct, key)=>'<div class="bandhead"><b>' + name + '</b>' + '<span>' + (on ? yieldPct.toFixed(2) + '% of GDP' : 'abolished') + '</span>' + '<button class="btn ' + (on ? 'danger ' : '') + 'tiny" data-nitoggle="' + key + '">' + (on ? 'Abolish' : 'Reintroduce') + '</button></div>';
    const floorTxt = money(effectiveBands(G.draft)[0].from);
    h += '<div class="panel">' + '<div class="bandblock' + (N.empOn ? '' : ' dim') + '">' + niHead("Employee", N.empOn, y.employee, "empOn") + (N.empOn ? ctrlRow("Employee rate", N.empRate, 0, 30, 0.5, N.empRate.toFixed(1) + "%", "on earnings above " + floorTxt + " &middot; comes out of the pay packet", 'data-ni="empRate"') : '<div class="hint" style="margin:2px 0 0">Scrapped. About ' + incomeYield(withNi(G.draft, "empOn", true), aggregate(G.draft), G.econ).employee.toFixed(2) + '% of GDP forgone, and every earner above the allowance keeps it.</div>') + '</div>' + '<div class="bandblock' + (N.erOn ? '' : ' dim') + '">' + niHead("Employer", N.erOn, y.employer, "erOn") + (N.erOn ? ctrlRow("Employer rate", N.erRate, 0, 30, 0.1, N.erRate.toFixed(1) + "%", "a tax on the job, not the pay &middot; every point adds roughly 0.06 to structural unemployment", 'data-ni="erRate"') : '<div class="hint" style="margin:2px 0 0">Scrapped. About ' + incomeYield(withNi(G.draft, "erOn", true), aggregate(G.draft), G.econ).employer.toFixed(2) + '% of GDP forgone, but hiring gets cheaper and structural unemployment falls.</div>') + '</div></div>';
    return h;
}
/* A throwaway copy of the law with one NI side switched back on, so the panel
   can quote what abolition actually costs. */ function withNi(law, key, val) {
    const c = clone(law);
    c.ni[key] = val;
    return c;
}
function withIncomeOn(law, on) {
    const c = clone(law);
    c.income.on = on;
    return c;
}
function wireIncomePanel(host) {
    const I = G.draft.income, N = G.draft.ni;
    const live = (el)=>{
        renderChrome();
    };
    host.querySelectorAll("[data-incometoggle]").forEach((b)=>{
        b.onclick = ()=>{
            I.on = !(I.on !== false);
            render();
        };
    });
    host.querySelectorAll("[data-nitoggle]").forEach((b)=>{
        b.onclick = ()=>{
    ...
    host.querySelectorAll('[data-inc="allowance"] input').forEach((inp)=>{
        inp.addEventListener("input", ()=>{
    ...
    host.querySelectorAll('[data-inc="divRate"] input, [data-inc="saveRate"] input, [data-inc="capitalRate"] input').forEach((inp)=>{
        const row = inp.closest("[data-inc]"), k = row.dataset.inc;
        inp.addEventListener("input", ()=>{
            const v = parseFloat(inp.value);
            I[k] = v;
    ...
    host.querySelectorAll('[data-band] input').forEach((inp)=>{
        const row = inp.closest("[data-band]"), i = +row.dataset.band, f = row.dataset.field;
        inp.addEventListener("input", ()=>{
            const v = parseFloat(inp.value);
            I.bands[i][f] = v;
    ...
    host.querySelectorAll('[data-ni] input').forEach((inp)=>{
        const row = inp.closest("[data-ni]"), k = row.dataset.ni;
        const isRate = k === "empRate" || k === "empUpper" || k === "erRate";
        inp.addEventListener("input", ()=>{
            const v = parseFloat(inp.value);
            N[k] = v;
            row.querySelector(".val").textContent = isRate ? v.toFixed(1) + "%" : money(v);
            live();
        });
        inp.addEventListener("change", render);
    });
    host.querySelectorAll("[data-uprate]").forEach((b)=>{
        b.onclick = ()=>{
            I.uprate = b.dataset.uprate === "1";
            render();
        };
    });
    host.querySelectorAll("[data-addband]").forEach((b)=>{
        b.onclick = ()=>{
    ...
    host.querySelectorAll("[data-delband]").forEach((b)=>{
        b.onclick = ()=>{
...
/* ---- charts ---- */ function lineChart(series, opts) {
    opts = opts || {};
    const h = opts.h || 168, w = 600, padL = 36, padR = 54, padT = 10, padB = 20;
    const len = series[0] ? series[0].data.length : 0;
    if (len < 2) return '<div class="empty">Not enough quarters recorded yet.</div>';
    let all = series.flatMap((s)=>s.data).filter((v)=>isFinite(v));
    ...
    for(let i = 0; i <= 4; i++){
        ...
        g += '<line x1="' + padL + '" y1="' + y.toFixed(1) + '" x2="' + (w - padR) + '" y2="' + y.toFixed(1) + '" stroke="#d6cdb8" stroke-width="1"/>' + '<text x="' + (padL - 5) + '" y="' + (y + 3.5).toFixed(1) + '" text-anchor="end" font-size="8.5" font-family="IBM Plex Mono, monospace" fill="#8d8474">' + v.toFixed(Math.abs(mx) > 60 ? 0 : 1) + '</text>';
    }
    if (opts.target != null) {
        const y = Y(opts.target);
        g += '<line x1="' + padL + '" y1="' + y.toFixed(1) + '" x2="' + (w - padR) + '" y2="' + y.toFixed(1) + '" stroke="#c0392f" stroke-width="1" stroke-dasharray="3 3"/>';
    }
    const lbl = G.log.map((r)=>r.label);
    for(let i = 0; i < len; i += Math.max(1, Math.ceil(len / 7))){
        g += '<text x="' + X(i).toFixed(1) + '" y="' + (h - 6) + '" text-anchor="middle" font-size="8" font-family="IBM Plex Mono, monospace" fill="#8d8474">' + (lbl[i] || "") + '</text>';
    }
    series.forEach((s)=>{
        const pts = s.data.map((v, i)=>X(i).toFixed(1) + "," + Y(v).toFixed(1)).join(" ");
        g += '<polyline points="' + pts + '" fill="none" stroke="' + s.color + '" stroke-width="' + (s.wide ? 2 : 1.4) + '"' + (s.dash ? ' stroke-dasharray="4 3"' : '') + '/>';
        const lv = s.data[len - 1];
        g += '<circle cx="' + X(len - 1).toFixed(1) + '" cy="' + Y(lv).toFixed(1) + '" r="2.2" fill="' + s.color + '"/>' + '<text x="' + (w - padR + 6) + '" y="' + (Y(lv) + 3).toFixed(1) + '" font-size="9" font-family="IBM Plex Mono, monospace" fill="' + s.color + '">' + lv.toFixed(1) + '</text>';
    });
    return '<svg viewBox="0 0 ' + w + ' ' + h + '" width="100%" height="' + h + '" role="img">' + g + '</svg>' + '<div class="legend">' + series.map((s)=>'<span><i style="background:' + s.color + '"></i>' + s.label + '</span>').join("") + (opts.targetLabel ? '<span><i style="background:#c0392f"></i>' + opts.targetLabel + '</span>' : '') + '</div>';
}
const COL = {
    ink: "#ffffff",
    ox: "#FF5A4E",
...
const chartBox = (title, cap, body)=>'<div class="chartbox"><h3>' + title + '</h3><div class="cap">' + cap + '</div>' + body + '</div>';
const col = (k)=>G.log.map((r)=>r[k]);
function compositionBar() {
    const E = aggregate(G.draft), rev = revenue(G.draft, E);
    const groups = {
        income: 0,
        consumption: 0,
        corporate: 0,
    ...
    for(const k in groups){
        const wdt = groups[k] / total * 100;
        bar += '<rect x="' + x.toFixed(2) + '%" y="0" width="' + wdt.toFixed(2) + '%" height="22" fill="' + cols[k] + '"/>';
        x += wdt;
    }
    return '<svg viewBox="0 0 100 22" width="100%" height="22" preserveAspectRatio="none">' + bar + '</svg>' + '<div class="legend">' + Object.keys(groups).map((k)=>'<span><i style="background:' + cols[k] + '"></i>' + names[k] + ' ' + groups[k].toFixed(1) + '</span>').join("") + '</div>';
}
/* ---- tab panels ---- */ function globeFallbackHtml() {
    let img = "";
    try {
        img = '<img alt="Map of the world of {C}" src="' + worldCanvas().toDataURL("image/png") + '">';
    } catch (err) {
        img = "";
    }
    return '<div class="globe-fallback">' + img + '<p>3D globe unavailable here, so this is the flat map. Everything else in the game is unaffected.</p></div>';
}
/* Share of GDP, indexed to inflation, or pegged to a service standard. */ /* Us against the rest of the world, on the numbers a Chancellor is judged on. */ function nationTableHtml() {
    ...
    ].concat(activePartners().filter((p)=>n[p.id]).map((p)=>({
            name: p.name,
    ...
    const cell = (v, d, inv)=>'<td class="' + ((inv ? -v : v) > 0 ? "pos" : (inv ? -v : v) < 0 ? "neg" : "") + '">' + fmt(v, d) + '</td>';
    return '<div class="ledger-wrap"><table class="ledger">' + '<thead><tr><th>Country</th><th>Growth</th><th>Inflation</th><th>Deficit</th><th>Debt</th></tr></thead><tbody>' + rows.map((r)=>'<tr' + (r.us ? ' style="background:rgba(212,175,105,.16)"' : '') + '>' + '<td>' + esc(r.name) + (r.us ? ' &middot; you' : '') + '</td>' + cell(r.growth, 1) + '<td>' + r.inflation.toFixed(1) + '</td>' + cell(-r.deficit, 1) + '<td>' + r.debt.toFixed(0) + '</td></tr>').join("") + '</tbody></table></div>';
}
function budgetModeHtml(d, transferOnly) {
    ...
    return '<div class="seg mini" style="margin-top:6px">' + opts.map((o)=>'<button data-mode="' + d.id + '" data-modeval="' + o[0] + '" aria-pressed="' + (o[0] === cur) + '">' + o[1] + '</button>').join("") + '</div>';
}
function panelHost() {
...
function renderPanel() {
    ...
    if (!host || !tab) return;
    const e = G.econ, E = aggregate(G.draft);
    if (tab === "bill") {
        const cl = billClauses();
        const dr = balanceOf(G.draft, G.econ), cur = balanceOf(G.law, G.econ);
        const delta = dr.balance - cur.balance;
        host.innerHTML = impactStripHtml() + (cl.length ? '<div id="clauses">' + cl.map((c, i)=>'<div class="clause"><button class="x" data-undo="' + i + '" title="Remove clause">' + ICONS.close + '</button>' + '<span>' + esc(c.label) + '</span><span class="cost">' + (c.sunk ? "paid" : c.pc) + '</span></div>').join("") + '</div>' : '<div class="empty-bill">No clauses yet. Change anything in Budget, Taxes, Policies, Society, Trade or Diplomacy ' + 'and it appears here to be costed before it becomes law.</div>') + '<div class="arith">' + '<div><span>Receipts</span><span>' + dr.rev.total.toFixed(1) + '</span></div>' + '<div><span>Departmental</span><span>' + dr.sp.prog.toFixed(1) + '</span></div>' + '<div><span>Debt interest</span><span>' + dr.sp.interest.toFixed(1) + '</span></div>' + '<div class="total"><span>' + (dr.balance >= 0 ? "Surplus" : "Deficit") + '</span>' + '<span class="' + (dr.balance >= 0 ? "pos" : "neg") + '">' + fmt(dr.balance, 1) + '% of GDP</span></div>' + (Math.abs(delta) > 0.049 ? '<div class="note" style="padding-top:5px">' + sgn(delta, 1) + ' points versus current law</div>' : '') + '</div>' + (G.sandbox ? impactPanelHtml(cl) : '') + '<div class="eyebrow mt">Rules</div>' + '<div class="panel" style="padding:11px 13px;margin-top:0">' + '<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">' + '<div style="flex:1;min-width:140px"><div style="font-weight:600;font-size:13.5px">Central Bank</div>' + '<div class="hint" style="margin:2px 0 0">' + (G.rateManual ? 'Base rate is pinned by you. No political capital. Forecasts use the same pin.' : 'The Bank follows a Taylor rule. Switch to Manual to pin the base rate for experimentation — free, no capital cost.') + '</div></div>' + '<span class="seg mini">' + '<button data-rate-mode="bank" aria-pressed="' + (!G.rateManual ? "true" : "false") + '">Bank</button>' + '<button data-rate-mode="manual" aria-pressed="' + (G.rateManual ? "true" : "false") + '">Manual</button>' + '</span>' + '</div>' + (G.rateManual ? '<div id="manualRateLever" style="margin-top:10px">' + leverHtml("manualRate", "Base rate", G.manualRate, MANUAL_RATE_MIN, 20, 0.25, 2, 'Applied immediately. Held every quarter until you return the rate to the Bank. Down to −2%; the Bank itself floors at −1%.') + '</div>' + rateImpactHtml() : '') + '</div>' + '<button class="reset" id="resetBtn2" style="margin-top:14px">Start a new government</button>';
        ...
        host.querySelectorAll("[data-rate-mode]").forEach((b)=>{
            b.onclick = ()=>{
        ...
        if (mrl) {
            wireLevers(mrl, ()=>G.manualRate, (id, v)=>{
        ...
        /* Keep held standards aligned with the live sliders before we paint. */ syncServiceHolds(G.draft, G.econ);
        /* Next year / Approval / This quarter live on Programme (and the
         morning-note despatch), not duplicated at the top of Budget. */
        host.innerHTML = '<div class="eyebrow">Departmental spending, % of GDP</div>' + '<div class="hint">Standing still is a cut: demand for services rises about one percent a year on its own.</div>' + '<div class="panel" id="deptLevers">' + DEPTS.map((d)=>{
            const held = G.draft.hold && G.draft.hold[d.id];
            if (TRANSFER_DEPTS[d.id]) {
                const bill = welfareCost(G.draft, G.econ), adeq = welfareAdequacy(G.draft, G.econ);
                return leverHtml(d.id, d.name, G.draft.spend[d.id], d.min, d.max, 0.1, 1, 'costs <b>' + bill.toFixed(2) + '%</b> of GDP at today&rsquo;s unemployment &middot; ' + 'per claimant ' + (adeq * 100).toFixed(0) + '% of the opening rate', "spend");
            }
            const score = held != null ? held : serviceScore(d.id, G.draft, G.econ);
            const holdCost = spendForScore(d.id, score, G.econ);
            return leverHtml(d.id, d.name, G.draft.spend[d.id], d.min, d.max, 0.1, 1, 'service level <b>' + score.toFixed(0) + '</b> / 100 &middot; ' + 'holding this level costs ' + holdCost.toFixed(2) + '% of GDP' + budgetModeHtml(d), "spend");
        }).join("") + '</div>' + '<div class="eyebrow mt">The ledger</div>' + '<div class="ledger-wrap">' + ledgerTable() + '</div>';
        host.querySelectorAll("[data-mode]").forEach((b)=>{
            ...
            b.onclick = ()=>{
                const id = b.dataset.mode, m = b.dataset.modeval;
        ...
        wireLevers(root, (id)=>G.draft.spend[id], (id, v)=>{
            G.draft.spend[id] = v;
            /* Hold mode freezes a service standard — keep it aligned with the slider. */
            if ((G.draft.mode || {})[id] === "service" && !TRANSFER_DEPTS[id]) {
                if (!G.draft.hold) G.draft.hold = {};
                G.draft.hold[id] = serviceScore(id, G.draft, G.econ);
            }
        }, 1);
        ...
        host.innerHTML = '<div class="eyebrow">The structure of the system</div>' + '<div class="hint">Changing the architecture is the biggest thing you can do, and the most expensive. Rates within a system are cheap by comparison.</div>' + '<div class="cards">' + REGIMES.map((r)=>{
            const on = G.draft.regime === r.id, isLaw = G.law.regime === r.id;
            return '<div class="card ' + (isLaw ? "on" : "") + ' ' + (on && !isLaw ? "staged" : "") + '"><h4>' + r.name + '<span class="cat">' + (isLaw ? "in force" : r.pc + " capital") + '</span></h4>' + '<p>' + T(r.blurb) + '</p>' + '<div class="foot">' + (on ? '<span class="pc">' + (isLaw ? "Current system" : "Staged in the bill") + '</span>' : '<button class="btn" data-regime="' + r.id + '">Adopt</button>') + '</div></div>';
        }).join("") + '</div>' + '<div class="eyebrow mt">Where the money comes from</div>' + compositionBar() + groups.map((param)=>{
            let [g, label] = param;
            if (g === "income") return incomePanelHtml();
            const list = TAXES.filter((t)=>t.grp === g);
            return '<div class="eyebrow mt">' + label + '</div><div class="panel" data-taxgrp="' + g + '">' + list.map((t)=>{
                const s = G.draft.taxes[t.id], avail = taxAvailable(t, G.draft);
                const killed = (REGIME_BY_ID[G.draft.regime].kills || []).includes(t.id);
                if (!avail) return '<div class="lever" style="opacity:.5"><div class="top"><span>' + t.name + '</span>' + '<span class="val" style="color:var(--ink-faint)">unavailable</span></div>' + '<div class="yield">Requires a change to the law on ' + VICE_BY_ID[t.req[0]].name.toLowerCase() + '.</div></div>';
                if (killed) return '<div class="lever" style="opacity:.5"><div class="top"><span>' + t.name + '</span>' + '<span class="val" style="color:var(--ink-faint)">abolished</span></div><div class="yield">Removed by the flat tax structure.</div></div>';
                if (!s.on) return '<div class="lever"><div class="top"><span style="color:var(--ink-soft)">' + t.name + '</span>' + '<span class="val" style="color:var(--ink-faint)">not levied</span>' + '<button class="btn" data-newtax="' + t.id + '" style="margin-left:8px">Introduce</button></div>' + '<div class="yield">Introducing it costs ' + (t.pc || 6) + ' political capital.</div></div>';
                const y = rev.by[t.id] || 0;
                return leverHtml(t.id, t.name, s.rate, 0, t.max, t.step || 1, dp(t), "raises " + y.toFixed(2) + "% of GDP" + (t.grp === "vice" ? " &middot; black market " + E.blackLevel.toFixed(0) + "%" : "") + ' <button class="btn danger" data-killtax="' + t.id + '" style="margin-left:8px;padding:2px 7px">Abolish</button>');
            }).join("") + '</div>';
        }).join("");
        wireIncomePanel(host);
        host.querySelectorAll("[data-regime]").forEach((b)=>b.onclick = ()=>{
                G.draft.regime = b.dataset.regime;
                render();
            });
        host.querySelectorAll("[data-newtax]").forEach((b)=>b.onclick = ()=>{
                const t = TAX_BY_ID[b.dataset.newtax];
                const s = G.draft.taxes[t.id];
                s.on = true;
                if (!(s.rate > 0)) s.rate = t.def > 0 ? t.def : t.step ? t.step * 5 : Math.round(t.max * 0.2);
                render();
            });
        host.querySelectorAll("[data-killtax]").forEach((b)=>b.onclick = ()=>{
                G.draft.taxes[b.dataset.killtax].on = false;
                render();
            });
        host.querySelectorAll("[data-taxgrp]").forEach((root)=>{
            root.querySelectorAll("[data-lever]").forEach((r)=>{
                const t = TAX_BY_ID[r.dataset.lever];
                r.dataset.base = G.law.taxes[t.id].on ? G.law.taxes[t.id].rate : 0;
                r.dataset.dp = dp(t);
            });
            wireLevers(root, (id)=>G.draft.taxes[id].rate, (id, v)=>{
                G.draft.taxes[id].rate = v;
            }, 0);
        });
    } else if (tab === "policies") {
        const cats = POLICY_CAT_ORDER.filter((c)=>POLICIES.some((p)=>p.cat === c)).concat([
            ...new Set(POLICIES.map((p)=>p.cat))
        ].filter((c)=>!POLICY_CAT_ORDER.includes(c)));
        const matchFilter = (p)=>{
            const isLaw = !!G.law.policies[p.id], staged = !!G.draft.policies[p.id];
        ...
        const killLine = (p)=>{
            ...
            const names = p.kills.map((id)=>POLICY_BY_ID[id] && POLICY_BY_ID[id].name || id);
            return '<div class="eff" style="color:var(--amber)">Cannot sit with ' + esc(names.join(", ")) + '</div>';
        };
        const cardHtml = (p)=>{
            const isLaw = !!G.law.policies[p.id], staged = !!G.draft.policies[p.id];
            /* Career and Sandbox both get signed qualitative effects; Sandbox also
         gets the full numeric channel list via fullEffects. */ const effects = G.sandbox ? fullEffects(p.imp, p.fac, p.cost, p.ch) : qualEffects(p.imp, p.fac, p.cost, p.ch);
            return '<div class="card ' + (isLaw ? "on" : "") + ' ' + (staged !== isLaw ? "staged" : "") + '">' + '<h4>' + p.name + '</h4><p>' + T(p.blurb) + '</p>' + effects + killLine(p) + '<div class="foot"><span class="pc">' + (staged === isLaw ? isLaw ? "In force" : p.pc + " capital" : "staged") + '</span>' + '<button class="btn ' + (staged ? "danger" : "") + '" data-pol="' + p.id + '">' + (staged ? "Repeal" : "Enact") + '</button></div></div>';
        };
        const filters = [
            [
                "all",
        ...
        host.innerHTML = '<div class="eyebrow">The statute book</div>' + '<div class="hint">Enacted policy stays in force until repealed, and repeal costs capital too. Costs are annual, in points of GDP.</div>' + '<div class="seg mini" style="margin:8px 0 12px" id="polFilter">' + filters.map((param)=>{
            let [id, lab] = param;
            return '<button type="button" data-polfilter="' + id + '" aria-pressed="' + (policyFilter === id) + '">' + lab + '</button>';
        }).join("") + '</div>' + cats.map((c)=>{
            const list = POLICIES.filter((p)=>p.cat === c && matchFilter(p));
            if (!list.length) return "";
            return '<div class="eyebrow mt">' + c + '</div><div class="cards">' + list.map(cardHtml).join("") + '</div>';
        }).join("");
        host.querySelectorAll("[data-polfilter]").forEach((b)=>b.onclick = ()=>{
                policyFilter = b.dataset.polfilter;
                render();
            });
        host.querySelectorAll("[data-pol]").forEach((b)=>b.onclick = ()=>{
        ...
        const facHtml = '<div class="eyebrow">Where you stand</div><div class="panel" style="padding:11px 13px">' + FACTIONS.map((f)=>{
            const v = G.fac[f.id], cls = v > 55 ? "good" : v < 38 ? "bad" : "";
            return '<div class="frow" style="grid-template-columns:110px 1fr 34px;margin-bottom:4px">' + '<span>' + f.name + ' <span style="color:var(--ink-faint);font-size:10px">' + Math.round(f.w * 100) + '%</span></span>' + '<span class="fb"><i class="' + cls + '" style="width:' + v.toFixed(0) + '%"></i></span>' + '<span class="fv">' + v.toFixed(0) + '</span></div>';
        }).join("") + '<div class="frow" style="grid-template-columns:110px 1fr 34px;border-top:1px solid var(--rule);padding-top:5px;margin-top:3px">' + '<span style="font-weight:600">Approval</span><span class="fb"><i style="width:' + approvalOf(G.fac).toFixed(0) + '%;background:var(--red)"></i></span>' + '<span class="fv" style="font-weight:600">' + approvalOf(G.fac).toFixed(0) + '</span></div></div>';
        const bars = [
            [
                "liberty",
                "Civil liberty",
        ...
        const polityHtml = '<div class="eyebrow mt">Political system</div>' + '<div class="hint">Stage any system. A neighbour costs the listed capital; leaping democracy ↔ authoritarian costs much more. Factions move when you deliver.</div>' + '<div class="cards">' + POLITY_LADDER.map((id)=>{
            const m = POLITY[id];
            ...
            const leap = steps > 1;
            const cat = isLaw ? "in force" : leap ? "leap · " + pc + " capital" : pc + " capital";
            return '<div class="card ' + (isLaw ? "on" : "") + ' ' + (on && !isLaw ? "staged" : "") + '"><h4>' + m.label + '<span class="cat">' + cat + '</span></h4>' + '<p>' + m.blurb + '</p>' + '<div class="foot">' + (on ? '<span class="pc">' + (isLaw ? "Current system" : leap ? "Staged leap — " + pc + " capital" : "Staged in the bill") + '</span>' : '<button class="btn" data-polity="' + id + '">' + (isLaw ? "Revert" : leap ? "Stage leap" : "Stage") + '</button>') + '</div></div>';
        }).join("") + '</div>';
        host.innerHTML = facHtml + '<div class="eyebrow mt">Social indicators</div>' + '<div class="panel" style="padding:11px 13px">' + bars.map((param)=>{
            let [k, n, v] = param;
            return '<div class="frow" style="grid-template-columns:150px 1fr 34px;margin-bottom:4px"><span>' + n + '</span>' + '<span class="fb"><i style="width:' + clamp(v, 0, 100).toFixed(0) + '%;background:' + (k === "crime" || k === "black" || k === "gini" ? v > 50 ? "var(--red)" : "var(--ink-soft)" : v > 55 ? "var(--green)" : "var(--ink-soft)") + '"></i></span>' + '<span class="fv">' + v.toFixed(0) + '</span></div>';
        }).join("") + '</div>' + polityHtml + '<div class="eyebrow mt">What is legal, and on what terms</div>' + '<div class="hint">Prohibition pushes a market underground: no receipts, more crime. Legalising creates a tax base you can then set a duty on, over on the Taxes tab.</div>' + '<div class="cards">' + VICE.map((v)=>{
            const cur = G.draft.vice[v.id], inLaw = G.law.vice[v.id];
            const st = v.states.find((s)=>s.id === cur);
            return '<div class="card ' + (cur !== inLaw ? "staged" : "on") + '"><h4>' + v.name + '<span class="cat">' + v.pc + ' capital</span></h4>' + '<div class="seg">' + v.states.map((s)=>'<button data-vice="' + v.id + '" data-state="' + s.id + '" aria-pressed="' + (s.id === cur) + '" class="' + (s.id === cur && s.id !== inLaw ? "staged" : "") + '">' + s.label + '</button>').join("") + '</div>' + '<p>' + T(st.blurb) + '</p>' + (G.sandbox ? fullEffects(st.imp, st.fac, 0) : qualEffects(st.imp, st.fac, 0)) + (v.tax ? '<div class="eff" style="color:var(--ink-faint)">' + (taxAvailable(TAX_BY_ID[v.tax], G.draft) ? "Duty available" : "No duty can be levied in this state") + '</div>' : '') + '</div>';
        }).join("") + '</div>';
        host.querySelectorAll("[data-polity]").forEach((b)=>b.onclick = ()=>{
                const id = b.dataset.polity;
                if (!polityCanReach(lawPolity, id)) return;
                G.draft.polity = id;
                render();
            });
        host.querySelectorAll("[data-vice]").forEach((b)=>b.onclick = ()=>{
                G.draft.vice[b.dataset.vice] = b.dataset.state;
                for (const t of TAXES)if (!taxAvailable(t, G.draft)) G.draft.taxes[t.id].on = false;
                render();
            });
    } else if (tab === "trade") {
        ensureDiploStocks(G.econ);
        ...
        const accessPhased = Object.values(pae).reduce((s, v)=>s + v, 0);
        const exposureTarget = tradeExposureTarget(G.draft, Eagg, G.econ, G.homeRole, G.blocMember);
        const bilat = G.econ.bilateralX || {};
        const bilatTotal = Object.keys(bilat).reduce((s, k)=>s + (bilat[k] || 0), 0) || 1;
        const tradeReadout = '<div class="hint">Treaty benefits phase in over about five years — openness, tariff cuts and market access crawl in; they do not jump on the day you ratify.</div>' + '<div class="hint">Retaliation ' + (G.econ.retaliation || 0).toFixed(1) + ' &middot; trade-weighted tariff ' + importT.toFixed(1) + '%' + ' &middot; partner access ' + accessPhased.toFixed(1) + ' of ' + (Eagg.access || 0).toFixed(1) + ' &middot; trade depth ' + (G.econ.tradeDepth || 0).toFixed(1) + ' of ' + exposureTarget.toFixed(1) + ' &middot; openness phased ' + (G.econ.openEff || 0).toFixed(1) + ' of ' + (Eagg.dealOpen || 0).toFixed(1) + '</div>';
        ...
        const fxPanel = '<div class="eyebrow">Currency</div>' + '<div class="panel" style="padding:12px 14px;margin-bottom:12px">' + '<div style="display:flex;align-items:baseline;gap:12px;flex-wrap:wrap">' + '<div style="font-size:28px;font-weight:650;letter-spacing:-.03em;line-height:1;color:' + fxColor + '">' + fxIdx.toFixed(1) + '</div>' + '<div style="flex:1;min-width:160px">' + '<div style="font-weight:650;font-size:14px">Currency strength (' + fxCode + ')</div>' + '<div class="hint" style="margin:3px 0 0">Versus the USD numeraire. Opening = 100. A stronger currency hurts exports and cheapens imports. The Bank and fiscal risk move it — you do not set it directly.</div>' + '</div></div></div>';
        ...
        for (const r of [
            ...
            partnerCards += '<div class="eyebrow mt">' + (COUNTRY_REGIONS[r] || r) + '</div><div class="cards">';
            partnerCards += list.map((p)=>partnerTradeCardHtml(p, bilat, bilatTotal)).join("");
            partnerCards += '</div>';
        }
        host.innerHTML = fxPanel + blocMembershipPanelHtml() + '<div class="eyebrow mt">Tariffs</div>' + '<div class="hint">Set rates by bloc or country. Partners in the same bloc share one lever. Customs-union members trade at zero internally.</div>' + tradeReadout + tariffSchedulePanelHtml() + '<div class="eyebrow mt">How {C} compares</div>' + nationTableHtml() + '<div class="eyebrow mt">Partners and agreements</div>' + partnerCards;
        wireTariffSchedule(host);
        host.querySelectorAll("[data-deal]").forEach((b)=>b.onclick = ()=>{
                const id = b.dataset.deal;
                if (G.draft.deals[id]) delete G.draft.deals[id];
                else G.draft.deals[id] = true;
                render();
            });
        host.querySelectorAll("[data-bloc-ext-deal]").forEach((b)=>b.onclick = ()=>{
                const dealId = b.dataset.blocExtDeal, pid = b.dataset.partner;
                if (G.draft.blocExternalDeal && G.draft.blocExternalDeal.dealId === dealId && G.draft.blocExternalDeal.partnerId === pid) G.draft.blocExternalDeal = null;
                else G.draft.blocExternalDeal = {
                    partnerId: pid,
                    dealId
                };
                render();
            });
        host.querySelectorAll("[data-bloc-acc]").forEach((b)=>b.onclick = ()=>{
                const id = b.dataset.blocId;
                const phase = b.dataset.blocAcc;
                if (blocJoinBlockers(id, phase).length && !(G.draft.blocAccession && G.draft.blocAccession.blocId === id && G.draft.blocAccession.phase === phase)) return;
                if (G.draft.blocAccession && G.draft.blocAccession.blocId === id && G.draft.blocAccession.phase === phase) G.draft.blocAccession = null;
                else G.draft.blocAccession = {
                    blocId: id,
                    phase
                };
                render();
            });
        host.querySelectorAll("[data-bloc-withdraw]").forEach((b)=>b.onclick = ()=>{
                G.blocAccession = null;
                G.draft.blocAccession = null;
                render();
            });
        host.querySelectorAll("[data-bloc-leave]").forEach((b)=>b.onclick = ()=>{
                if (!G.draft.blocLeave) {
                    const ok = typeof window !== "undefined" && typeof window.confirm === "function"
                        ? window.confirm("Leave your trade bloc? Membership ends when you Deliver this quarter.")
                        : true;
                    if (!ok) return;
                }
        ...
        host.querySelectorAll("[data-bloc-create-unstage]").forEach((b)=>b.onclick = ()=>{
                G.draft.blocCreate = null;
                render();
            });
        host.querySelectorAll("[data-bloc-found-open]").forEach((b)=>b.onclick = ()=>{
                showBlocFoundModal();
            });
        host.querySelectorAll("[data-bloc-invite-open]").forEach((b)=>b.onclick = ()=>{
                const bid = countryBlocId(playerCountryId());
        ...
        for (const r of [
            ...
            const list = byRegion[r];
            if (!list || !list.length) continue;
            partnerCards += '<div class="eyebrow mt">' + (COUNTRY_REGIONS[r] || r) + '</div><div class="cards diplo-cards">';
            partnerCards += list.map((p)=>partnerDiploCardHtml(p)).join("");
            partnerCards += '</div>';
        }
        host.innerHTML = envoySummaryHtml() + unStatusPanelHtml() + '<div class="eyebrow mt">Partners</div>' + partnerCards;
        if (!G.draft.missions) G.draft.missions = {};
        pruneInvalidDraftMissions(G);
        host.querySelectorAll("[data-mission]").forEach((b)=>b.onclick = ()=>{
                const pid = b.dataset.partner, mid = b.dataset.mission;
                const already = G.draft.missions[pid] === mid;
                /* Allow unstage during a visit; only block newly staging sanctions. */
                if (!already && mid === "sanctionsPosture" && isVisitActive(G, pid)) return;
                if (already) delete G.draft.missions[pid];
                else G.draft.missions[pid] = mid;
                render();
            });
        host.querySelectorAll("[data-envoy-assign]").forEach((b)=>b.onclick = ()=>{
                const pid = b.dataset.envoyAssign;
                if (G.mp && typeof G.mp.onDiploAction === "function") {
                    /* Optimistic: local apply + queue; hard failures stay silent (buttons gate). */
                    Promise.resolve(G.mp.onDiploAction({ action: "assignEnvoy", partnerId: pid }));
                    return;
                }
                if (assignEnvoy(pid)) render();
            });
        host.querySelectorAll("[data-envoy-recall]").forEach((b)=>b.onclick = ()=>{
                const pid = b.dataset.envoyRecall;
                if (G.mp && typeof G.mp.onDiploAction === "function") {
                    Promise.resolve(G.mp.onDiploAction({ action: "recallEnvoy", partnerId: pid }));
                    return;
                }
                if (recallEnvoy(pid)) render();
            });
        host.querySelectorAll("[data-ultimatum]").forEach((b)=>b.onclick = ()=>{
                const raw = b.dataset.ultimatum || "";
                const colon = raw.indexOf(":");
                const pid = colon >= 0 ? raw.slice(0, colon) : raw;
                const demandId = colon >= 0 ? raw.slice(colon + 1) : null;
                if (G.mp && typeof G.mp.onDiploAction === "function") {
                    Promise.resolve(G.mp.onDiploAction({
        ...
        if (G.log.length < 2) {
            host.innerHTML = '<div class="empty">Deliver a bill or two. The charts need at least two quarters of data.</div>';
            return;
        }
        const preN = G.log.filter((r)=>r.pre).length;
        const chartNote = preN ? '<div class="hint">Includes the ' + preN + '-quarter run-up before your appointment.</div>' : '';
        const tr = G.econ.trendGrowth;
        const gapPts = G.econ.potential ? (G.econ.gdp / G.econ.potential - 1) * 100 : 0;
        host.innerHTML = chartNote + chartBox("Output against potential", "Index, 100 at the start of your term. The gap is cyclical pressure on prices. Trend (top bar) is how fast potential itself expands — currently " + (tr != null ? tr.toFixed(2) : "—") + "% a year; gap " + fmt(gapPts, 1) + " pts.", lineChart([
            {
                label: "Actual output",
                color: COL.ink,
                data: col("gdp"),
        ...
        ])) + '<div class="chartgrid">' + chartBox("Inflation and Bank rate", G.rateManual ? "Base rate is pinned by you. Inflation still answers to the gap and expectations." : "You set fiscal policy. The Bank answers with the rate.", lineChart([
            {
                label: "Inflation",
                color: COL.ox,
                data: col("inflation"),
            ...
            target: 2,
            targetLabel: "Target"
        })) + chartBox("Currency strength (" + currencyForSeat(G.homeRole) + ")", "Index versus the USD numeraire, 100 at term start. Stronger hurts exports and cheapens imports — watch net trade below.", lineChart([
            {
            ...
            target: 100,
            targetLabel: "Opening"
        })) + chartBox("Unemployment", "Okun's relationship: output above trend pulls people into work.", lineChart([
            {
                label: "Unemployment",
                color: COL.plum,
        ...
        ])) + chartBox("Debt", "Per cent of GDP. Interest compounds whether you are looking or not.", lineChart([
            {
                label: "Debt",
                color: COL.ink,
                data: col("debt"),
        ...
        ])) + chartBox("Gilt yield", "What the market charges {C} to borrow.", lineChart([
            {
                label: "Yield",
                color: COL.ox,
                data: col("yield"),
        ...
        ])) + '</div>' + chartBox("Approval by faction", "The overall number hides everything interesting.", lineChart(FACTIONS.map((f, i)=>({
                label: f.name,
                color: [
                    COL.blue,
                    COL.ox,
        ...
        ]))) + chartBox("The personal allowance in real terms", "Deflated by CPI and indexed to this seat's opening allowance. Uprate holds the line; Freeze lets inflation raise taxes without a vote.", lineChart([
            {
        ...
        })) + chartBox("Capital stock and the cost of capital", "Investment accumulates into the capital stock, which is an argument of potential output. Corporation tax and the policy rate move the user cost, and the user cost moves desired capital.", lineChart([
            {
                label: "Capital stock",
                color: COL.blue,
                data: col("K"),
        ...
        ])) + chartBox("Trend growth", "Annualised potential growth — the long-run score from TFP, capital and labour. Not outturn GDP. Latest " + (tr != null ? tr.toFixed(2) : "—") + "% a year.", lineChart([
            {
                label: "Trend growth %",
                color: COL.green,
                data: col("trend"),
        ...
        ])) + chartBox("Where output comes from", "The national accounts identity. Output is not a growth rate the model invents: it is the sum of these, less imports.", lineChart([
            {
                label: "Consumption",
                color: COL.blue,
                data: col("C"),
        ...
        ])) + chartBox("Net trade", "Exports less imports, in index points. Competitiveness runs on underlying prices, so a VAT change does not move it.", lineChart([
            {
                label: "Net trade",
                color: COL.ox,
                data: col("netTrade"),
            ...
            zero: true
        })) + chartBox("Receipts and spending", "Points of GDP. The distance between the lines is the deficit.", lineChart([
            {
                label: "Receipts",
                color: COL.green,
                data: col("rev"),
...
function interestTone(interest) {
    if (interest >= 0.25) return "for";
    if (interest <= -0.25) return "against";
    return "mixed";
}

function pendingUltimatumIds(g) {
    const state = g || G;
    if (!state || !state.ultimatums) return [];
    return Object.keys(state.ultimatums).filter((id) => state.ultimatums[id] && state.ultimatums[id].status === "pending");
}

function ultimatumQuartersLeft(g, partnerId) {
    const state = g || G;
...
function ultimatumWaitingCopy(ultLeft) {
    if (ultLeft <= 1) return "Their answer lands when you Deliver next quarter";
    return "They have " + ultLeft + "Q to respond · checked each time you Deliver";
}

function hasDiploAttention(g) {
    const state = g || G;
    if (!state) return false;
    if (pendingUltimatumIds(state).length) return true;
    return Object.keys(state.activeVisits || {}).some((id) => visitQuartersLeft(state, id) > 0);
}

function diploHudHtml(g) {
    const state = g || G;
    if (!state) return "";
    const chips = [];
    for (const id of pendingUltimatumIds(state)) {
        ...
        chips.push('<span class="diplo-hud-chip ult" title="Ultimatum — opens Diplomacy">' + esc(p ? p.name : id) + ' · ' + esc(label) + ' <b>' + left + 'Q</b></span>');
    }
    for (const id of Object.keys(state.activeVisits || {})) {
        const left = visitQuartersLeft(state, id);
        if (left <= 0) continue;
        const p = partnerById(id);
        chips.push('<span class="diplo-hud-chip visit" title="State visit — opens Diplomacy">' + esc(p ? p.name : id) + ' · visit <b>' + left + 'Q</b></span>');
    }
    return chips.join("");
}

function relationModifiersHtml(partnerId) {
    ensureDiploStocks(G.econ);
    seedGameRelBase(G);
    const { mods } = buildRelationModifiers(partnerId, G, G.draft || G.law, diploDeps());
    const rest = mods.filter((m) => m.kind !== "base" && Math.abs(m.pts) >= 0.5)
    ...
    for (const m of rest.slice(0, 5)) {
        ...
        const sign = m.pts > 0 ? "+" : "";
        h += '<span class="diplo-mod ' + tone + '" title="' + esc(m.label) + '">' + esc(m.label) + ' <b>' + sign + m.pts.toFixed(1) + '</b></span>';
    }
    if (rest.length > 5) {
        const other = rest.slice(5).reduce((s, m) => s + m.pts, 0);
        h += '<span class="diplo-mod other">+' + (rest.length - 5) + ' more <b>' + (other >= 0 ? "+" : "") + other.toFixed(1) + '</b></span>';
    }
    h += '</div>';
    return h;
}

function partnerDiploCardHtml(p) {
    ...
    const missionRow = MISSIONS.map((m)=>{
        ...
        if (visitBlock) {
            tip = m.id === "sanctionsPosture"
                ? 'Cannot stage sanctions during a state visit · ' + visitLeft + 'Q remaining'
                : 'State visit underway · ' + visitLeft + 'Q remaining';
        }
        return '<button type="button" class="diplo-mission' + (on ? " on" : "") + '" data-mission="' + m.id + '" data-partner="' + p.id + '"' + (blocked ? " disabled" : "") + ' title="' + esc(tip) + '"><span class="diplo-mission-name">' + (on ? "Unstage " : "") + missionShortLabel(m) + '</span><span class="diplo-mission-pc">' + m.pc + '</span></button>';
    }).join("");
    let envoyRow = "";
    if (envoyOn) {
        envoyRow = '<button type="button" class="btn diplo-envoy danger" data-envoy-recall="' + p.id + '">Recall envoy</button>';
    } else {
        ...
        envoyRow = '<button type="button" class="btn diplo-envoy" data-envoy-assign="' + p.id + '"' + (canAssign ? "" : " disabled") + ' title="' + assignTip + '">Assign envoy · ' + ENVOY_ASSIGN_PC + ' cap</button>';
    }
    let ultBody = "";
    let ultLeft = 0;
    let ultLabel = "";
    if (ultPending) {
        ...
        ultBody = '<div class="diplo-ult-waiting"><strong>Ultimatum issued</strong><span class="diplo-ult-demand">' + esc(ultLabel) + '</span><span class="diplo-ult-countdown">' + ultimatumWaitingCopy(ultLeft) + '</span><span class="diplo-ult-odds">Estimated ~' + ultOdds + '% chance they concede</span></div>';
    } else if (ultCd > 0) {
        ultBody = '<div class="diplo-ult-cooldown"><span class="diplo-ult-clock" aria-hidden="true">' + ICONS.clock + '</span><span>Cooldown ' + ultCd + 'Q</span></div>';
        if (ultCheck.reasons.length) {
            ultBody += '<div class="diplo-muted">' + esc(ultCheck.reasons.join(" · ")) + '</div>';
        }
    } else if (ultCheck.ok) {
        const demands = ultimatumDemandsFor(p.id, G, diploDeps());
        ultBody = demands.map((d) => {
            const pct = Math.round(d.baseP * 100);
            const tag = G.sandbox ? '<span class="diplo-demand-tag ' + interestTone(d.interest) + '">' + esc(d.interestLabel) + '</span>' : "";
            return '<button type="button" class="diplo-demand" data-ultimatum="' + p.id + ':' + d.id + '" title="Spend ' + ULTIMATUM_PC + ' capital immediately">' + '<span class="diplo-demand-main">' + esc(d.label) + '</span>' + '<span class="diplo-demand-meta"><span class="diplo-demand-odds">~' + pct + '%</span>' + tag + '</span></button>';
        }).join("");
    } else {
        ultBody = '<div class="diplo-muted">' + esc(ultCheck.reasons.join(" · ")) + '</div>';
    }
    let cardBanner = "";
    if (ultPending) {
        cardBanner += '<div class="diplo-ult-banner"><span class="diplo-ult-pulse"></span>Ultimatum live · ' + esc(ultLabel) + ' · ' + (ultLeft <= 1 ? "answer due next Deliver" : ultLeft + "Q to respond") + '</div>';
    }
    if (visitActive) {
        cardBanner += '<div class="diplo-visit-banner"><span class="diplo-visit-pulse"></span>State visit underway · ' + visitLeft + ' quarter' + (visitLeft === 1 ? "" : "s") + ' left</div>';
    }
    const ultSectionClass = ultPending ? " diplo-section ult-live" : " diplo-section";
    return '<div class="card diplo-card' + (stagedM ? " staged" : "") + (visitActive ? " visit-active" : "") + (ultPending ? " ult-active" : "") + '" id="partner-diplo-' + p.id + '" data-partner-card="' + p.id + '">' + cardBanner + '<div class="diplo-head">' + '<div class="diplo-title"><h4>' + p.name + '</h4><span class="cat">' + T(shareLabel(G.homeRole, p.id, p.tradeShare)) + '</span></div>' + '<div class="diplo-rel-badge ' + tone + '" style="--rel-col:' + relCol + '" title="Relations with ' + esc(p.name) + '">' + rel.toFixed(0) + '</div>' + '</div>' + (bloc ? '<div class="diplo-meta">' + esc(bloc.name) + '</div>' : '') + '<div class="diplo-rel-row"><div class="frow diplo-frow"><span>Relations</span><span class="fb"><i class="' + tone + '" style="width:' + rel.toFixed(0) + '%"></i></span><span class="fv">' + rel.toFixed(0) + '</span></div>' + '<div class="diplo-drift">Equilibrium <b>' + target.toFixed(0) + '</b></div></div>' + '<div class="diplo-section"><div class="diplo-section-h">Missions<span>Stage into bill</span></div><div class="diplo-missions">' + missionRow + '</div></div>' + '<div class="diplo-section"><div class="diplo-section-h">Envoy<span>+' + ENVOY_TARGET + '/Q while posted</span></div>' + envoyRow + '</div>' + '<div class="' + ultSectionClass.trim() + '"><div class="diplo-section-h">Ultimatum<span>' + (ultPending ? "Response pending" : ULTIMATUM_PC + " cap · immediate") + '</span></div><div class="diplo-ultimatums">' + ultBody + '</div></div>' + '</div>';
}

function envoySummaryHtml() {
    if (!G.envoys) G.envoys = emptyEnvoys();
    const slots = G.envoys.map((id, i) => {
        if (!id) return '<span class="diplo-slot empty">Slot ' + (i + 1) + ' · vacant</span>';
        const p = partnerById(id);
        return '<span class="diplo-slot filled">' + esc(p ? p.name : id) + '</span>';
    }).join("");
    return '<div class="diplo-summary panel"><div class="diplo-summary-h">Your envoys</div><div class="diplo-slots">' + slots + '</div><div class="hint" style="margin:6px 0 0">Assign costs ' + ENVOY_ASSIGN_PC + ' capital · missions stage into the bill · ultimatums spend capital immediately</div></div>';
}

function unStatusPanelHtml() {
    if (!isUnMember(G.law)) {
        return '<div class="diplo-summary panel"><div class="diplo-summary-h">United Nations</div><div class="hint" style="margin:6px 0 0">' + T("{C} has left the United Nations. Every relationship carries the chill; rejoin from the Policies drawer.") + '</div></div>';
    }
    const left = unSeatQuartersLeft();
    const council = isP5(playerCountryId())
        ? "Permanent Security Council seat — veto in hand."
        : left > 0
            ? "Elected Security Council seat — " + left + "Q remaining."
            : "Not currently on the Security Council. Seats come up for election from time to time.";
    return '<div class="diplo-summary panel"><div class="diplo-summary-h">United Nations</div><div class="hint" style="margin:6px 0 0">Member in good standing · ' + council + '</div></div>';
}

function partnerTradeCardHtml(p, bilat, bilatTotal) {
    ...
    if (playerInBloc && !founder) {
        dealsHtml = '<div class="eff" style="color:var(--ink-faint);display:block;margin-top:6px">Bilateral deals unavailable while in a trade bloc.</div>';
    } else if (playerInBloc && founder && !partnerInBloc) {
        dealsHtml = dealsForPartner(p, G.homeRole).map((d)=>{
            const signed = !!G.law.deals[d.id], staged = !!(G.draft.blocExternalDeal && G.draft.blocExternalDeal.dealId === d.id && G.draft.blocExternalDeal.partnerId === p.id);
            const unmet = blocExternalDealBlockers(p.id, d.id);
            return '<div style="border-top:1px solid var(--doc-3);padding-top:8px;margin-top:4px">' + '<div style="font-size:13px;font-weight:600">' + d.name + ' <span class="cat">bloc treaty</span></div>' + '<div class="eff" style="display:block;color:var(--ink-soft);margin:3px 0 5px">' + d.terms.map((t)=>"&middot; " + t).join("<br>") + '</div>' + (unmet.length && !signed ? '<div class="eff" style="color:var(--red);display:block">Blocked: ' + unmet.join("; ") + '</div>' : '') + '<div class="foot"><span class="pc">' + (signed ? "ratified" : d.pc + " capital") + '</span>' + '<button class="btn ' + (staged ? "danger" : "") + '" data-bloc-ext-deal="' + d.id + '" data-partner="' + p.id + '"' + (unmet.length && !staged && !signed ? " disabled" : "") + '>' + (staged ? "Unstage" : signed ? "Withdraw" : "Ratify as bloc") + '</button></div></div>';
        }).join("");
    } else {
        dealsHtml = dealsForPartner(p, G.homeRole).map((d)=>{
            const signed = !!G.law.deals[d.id], staged = !!G.draft.deals[d.id];
            const unmet = dealBlockers(d);
            if (playerInBloc) return "";
            const sphereHint = !signed && !unmet.length ? sphereRiskHint(p.id) : "";
            return '<div style="border-top:1px solid var(--doc-3);padding-top:8px;margin-top:4px">' + '<div style="font-size:13px;font-weight:600">' + d.name + '</div>' + '<div class="eff" style="display:block;color:var(--ink-soft);margin:3px 0 5px">' + d.terms.map((t)=>"&middot; " + t).join("<br>") + '</div>' + (unmet.length && !signed ? '<div class="eff" style="color:var(--red);display:block">Blocked: ' + unmet.join("; ") + '</div>' : '') + (sphereHint ? '<div class="eff" style="color:var(--amber);display:block">' + esc(sphereHint) + '</div>' : '') + '<div class="foot"><span class="pc">' + (signed ? "ratified" : d.pc + " capital") + '</span>' + '<button class="btn ' + (staged ? "danger" : "") + '" data-deal="' + d.id + '"' + (unmet.length && !staged ? " disabled" : "") + '>' + (staged ? signed ? "Withdraw" : "Unstage" : "Ratify") + '</button></div></div>';
        }).join("");
        if (playerInBloc && !founder) dealsHtml = "";
        else if (partnerInBloc && !playerInBloc) {
            dealsHtml = '<div class="eff" style="color:var(--ink-faint);display:block;margin-top:6px">Partner trades through ' + (bloc ? bloc.name : "their bloc") + ' — bilateral deals unavailable.</div>';
        }
    }
    return '<div class="card" id="partner-trade-' + p.id + '" data-partner-card="' + p.id + '"><h4>' + p.name + '<span class="cat">' + T(shareLabel(G.homeRole, p.id, p.tradeShare)) + '</span></h4>' + (bloc ? '<div class="eff" style="color:var(--ink-faint)">' + bloc.name + '</div>' : '') + '<p>' + p.blurb + '</p>' + '<div class="frow" style="grid-template-columns:70px 1fr 30px"><span style="font-size:11px">Relations</span>' + '<span class="fb"><i class="' + (rel > 60 ? "good" : rel < 38 ? "bad" : "") + '" style="width:' + rel.toFixed(0) + '%"></i></span><span class="fv">' + rel.toFixed(0) + '</span></div>' + '<div class="eff" style="display:block;color:var(--ink-soft);margin:4px 0">Exports ' + Xi.toFixed(1) + ' (' + sharePct + '%) · tariff ' + effectiveTariff(p.id, G.draft).toFixed(1) + '%</div>' + (stress >= 1 ? '<div class="eff" style="color:var(--red);display:block">Deal access suspended (stress ' + stress.toFixed(0) + ')</div>' : '') + dealsHtml + '</div>';
}
...
function tariffSchedulePanelHtml() {
    ...
    let h = "";
    if (lock.mode !== "none") {
        h += '<div class="hint">You are in a customs union. External tariff is ' + (sched.cet != null ? sched.cet : lock.cet) + '%.' + (lock.mode === "full" ? " Set in the bloc capital — you cannot change it." : "") + '</div>';
        if (lock.mode === "cet") {
            h += '<div class="panel" id="cetLever">' + leverHtml("tariffCet", "Common external tariff", sched.cet || lock.cet || 4, 0, 25, 1, 0, "Applies to all partners outside your customs union") + '</div>';
        }
    } else {
        h += '<div class="panel" id="defaultTariffLever">' + leverHtml("tariffDefault", "Default external tariff", sched.default, 0, 25, 1, 0, "Trade-weighted average " + tariffScheduleAverage(G.draft).toFixed(1) + "%") + '</div>';
        const usedBlocs = {};
        for (const p of activePartners()){
            const bid = countryBlocId(p.id);
        ...
        for(const bid in usedBlocs){
            ...
            h += '<div class="panel">' + leverHtml("tariffBloc:" + bid, (bloc ? bloc.name : bid) + " (" + usedBlocs[bid] + " countries)", val, 0, 25, 1, 0, "One rate for all members") + '</div>';
        }
        const byRegion = partnersByRegion();
        for(const r in byRegion){
            const lone = byRegion[r].filter((p)=>!countryBlocId(p.id));
            if (!lone.length) continue;
            h += '<div class="eyebrow mt">' + (COUNTRY_REGIONS[r] || r) + " (non-bloc)</div>";
            for (const p of lone){
                const val = sched.country[p.id] != null ? sched.country[p.id] : sched.default;
                h += '<div class="panel">' + leverHtml("tariffCountry:" + p.id, p.name, val, 0, 25, 1, 0, "Effective " + effectiveTariff(p.id, G.draft).toFixed(1) + "%") + '</div>';
            }
        }
    }
    return h;
}
function accessionPolicyChecklist(blocId, law) {
    const spec = blocAccessionSpec(blocId);
    if (!spec || !spec.need) return "";
    const need = spec.need;
    const items = [];
    (need.policyOff || []).forEach((p)=>{
    ...
    return '<div class="eff" style="display:block;margin:6px 0">' + items.map((it)=>'<span style="color:' + (it.ok ? "var(--green)" : "var(--red)") + '">' + (it.ok ? "✓ " : "✗ ") + esc(it.label) + '</span>').join("<br>") + '</div>';
}
function accessionPipelineSteps(cur, steps) {
    const labels = steps || [
        "Invite",
        "Accept",
        "Align",
    ...
    return '<div class="acc-pipe">' + labels.map((label, i)=>{
        const done = i < cur;
        const active = i === cur;
        const cls = done ? "done" : active ? "active" : "";
        return '<div class="acc-step ' + cls + '"><span class="acc-dot">' + (done ? "✓" : String(i + 1)) + '</span><span class="acc-label">' + esc(label) + '</span></div>' + (i < labels.length - 1 ? '<div class="acc-line' + (done ? " done" : "") + '"></div>' : "");
    }).join("") + '</div>';
}
function accessionStepIndicator(blocId) {
    const acc = G.blocAccession && G.blocAccession.blocId === blocId ? G.blocAccession : null;
...
function memberAccessionTrack(bid, cid) {
    ...
    if (inv && inv.blocId === bid) {
        ...
        return {
            ...
            status: waitingWindow ? "Invitation sent" : rel >= need ? "Considering invitation" : "Relations too cool",
            detail: waitingWindow ? "Response expected from next quarter · " + qLeft + "Q until expiry" : "Relations " + rel.toFixed(0) + "/" + need + (rel >= need ? " — likely to accept" : " — may decline"),
            qLeft,
            ok: rel >= need
        };
    }
    if (acc && acc.blocId === bid) {
        ...
        return {
            ...
            detail: qLeft + " quarter" + (qLeft === 1 ? "" : "s") + " remaining · relations " + rel.toFixed(0) + "/40",
            qLeft,
            ok: rel >= 40
        };
    }
...
function blocAccessionTrackerHtml(bid) {
    const cids = new Set();
    ...
    if (!cids.size) return "";
    let h = '<div class="eyebrow" style="margin-top:10px">Accession pipeline</div>';
    for (const cid of cids){
        const t = memberAccessionTrack(bid, cid);
        if (!t) continue;
        h += '<div class="card acc-track" style="margin-top:8px"><h4>' + esc(t.name) + '<span class="cat">' + esc(t.status) + '</span></h4>';
        h += accessionPipelineSteps(t.cur);
        h += '<div class="hint">' + esc(t.detail) + '</div>';
        if (!t.ok) h += '<div class="eff" style="color:var(--red);display:block">At risk — relations below the threshold</div>';
        h += '</div>';
    }
    return h;
}
...
function blocInviteCandidates(bid) {
    const out = [];
    for (const p of activePartners()){
...
function wireBlocInviteButtons(host) {
    host.querySelectorAll("[data-bloc-invite]").forEach((b)=>{
        b.type = "button";
        b.onclick = ()=>{
...
function accessionMemberTable(blocId) {
    const approvals = blocMemberApprovals(blocId);
    if (!approvals.length) return "";
    let h = '<div class="eyebrow" style="margin-top:6px">Member approval</div>';
    for (const a of approvals){
        h += '<div class="frow" style="grid-template-columns:90px 1fr 36px;margin:3px 0">' + '<span style="font-size:11px">' + esc(a.name) + '</span>' + '<span class="fb"><i class="' + (a.ok ? "good" : "bad") + '" style="width:' + a.rel.toFixed(0) + '%"></i></span>' + '<span class="fv" style="color:' + (a.ok ? "var(--green)" : "var(--red)") + '">' + a.rel.toFixed(0) + '</span></div>';
    }
    return h;
}
function accessionCardHtml(blocId) {
    const bloc = blocByIdOrCustom(blocId);
    if (!bloc) return "";
    const player = playerCountryId();
    if ((bloc.members || []).includes(player)) return "";
    const acc = G.blocAccession;
    const activeAcc = acc && acc.blocId === blocId;
    ...
    const curStep = activeAcc ? acc.step : 0;
    const phases = [
        "apply",
        "align",
        "accede"
    ];
    const curPhase = phases[Math.min(curStep, 2)];
    const staged = G.draft.blocAccession && G.draft.blocAccession.blocId === blocId ? G.draft.blocAccession.phase : null;
    const blockers = blocJoinBlockers(blocId, curPhase);
    const blocked = blockers.length > 0;
    let h = '<div class="card" style="margin-top:8px"><h4>' + bloc.name + '<span class="cat">' + (bloc.type === "customs_union" ? "Customs union" : "FTA") + '</span></h4>';
    h += accessionStepIndicator(blocId);
    if (curStep >= 1) h += accessionPolicyChecklist(blocId, G.draft);
    if (curPhase === "accede") h += accessionMemberTable(blocId);
    else if (curStep === 0) h += '<div class="hint">Member relations checked at accession.</div>';
    if (blocked) h += '<div class="eff" style="color:var(--red);display:block">' + esc(blockers[0]) + '</div>';
    const pc = steps[curPhase] || 12;
    const labels = {
        apply: "Stage application",
        align: "Stage alignment package",
        accede: "Stage accession treaty"
    };
    const unstaged = staged === curPhase;
    h += '<div class="foot"><span class="pc">' + pc + ' capital</span>' + '<button class="btn' + (staged === curPhase ? " danger" : "") + (blocked && !staged ? " disabled" : "") + '" data-bloc-acc="' + curPhase + '" data-bloc-id="' + blocId + '"' + (blocked && !staged ? ' title="' + esc(blockers.join("; ")) + '"' : "") + '>' + (staged === curPhase ? "Unstage" : labels[curPhase]) + '</button></div></div>';
    return h;
}
function inviteMemberTable(blocId, candidateId) {
...
function blocMembershipPanelHtml() {
    ...
    if (bid) {
        const bloc = blocById(bid) || G.customBlocs[bid];
        const members = blocMembers(bid).map((id)=>{
            var _partnerById;
            return ((_partnerById = partnerById(id)) === null || _partnerById === void 0 ? void 0 : _partnerById.name) || id;
        }).join(", ");
        h += '<div class="hint">Member of <b>' + (bloc ? bloc.name : bid) + '</b>' + (members ? " with " + members : "") + '.</div>' + '<div class="hint">Country-level bilateral deals are suspended while you are in a bloc.</div>' + '<div class="hint">Any member may propose a new member; every other member must approve. Proposals stage in your bill — use <b>Deliver</b> to send the invitation.</div>';
        if (isBlocFounder()) {
            h += '<div class="hint">As bloc founder you may ratify external treaties with non-bloc partners from their trade cards.</div>';
        } else if (G.customBlocs[bid]) {
            h += '<div class="hint">External treaties are negotiated by the bloc founder.</div>';
        } else {
            h += '<div class="hint">External treaties are negotiated by the bloc chair.</div>';
        }
        for(const cid in G.draft.blocInvite || {}){
            if (!G.draft.blocInvite[cid]) continue;
            const c = partnerById(cid);
            const blockers = blocInviteBlockers(bid, cid);
            h += '<div class="card staged-bloc-invite" id="bloc-staged-' + cid + '" data-bloc-staged="' + cid + '" style="margin-top:8px"><h4>Staged: propose ' + (c ? c.name : cid) + ' join ' + (bloc ? bloc.name : bid) + '<span class="cat">12 capital</span></h4>';
            h += '<div class="hint">Deliver this bill to send the invitation. After acceptance, accession runs automatically over several quarters.</div>';
            h += accessionPipelineSteps(0);
            h += inviteMemberTable(bid, cid);
            if (blockers.length) h += '<div class="eff" style="color:var(--red);display:block">' + esc(blockers[0]) + '</div>';
            else h += '<div class="eff" style="color:var(--green);display:block">Ready to deliver — every member approves.</div>';
            h += '<div class="foot"><button type="button" class="btn danger tiny" data-bloc-invite="' + cid + '">Unstage</button></div></div>';
        }
        h += blocAccessionTrackerHtml(bid);
        h += '<div class="bloc-leave-wrap"><button type="button" class="btn ' + (G.draft.blocLeave ? "danger" : "") + '" data-bloc-leave="1">' + (G.draft.blocLeave ? "Cancel leave" : "Leave") + '</button></div>';
        const inviteCandidates = blocInviteCandidates(bid);
        const inviteLabel = inviteCandidates.length ? "Invite a member (" + inviteCandidates.length + " eligible)" : "Invite a member";
        h += '<div style="margin-top:12px"><button type="button" class="btn" data-bloc-invite-open="1" style="width:100%;padding:10px 16px;font-size:15px;font-weight:600"' + (inviteCandidates.length ? "" : ' disabled title="No eligible partners — all are in blocs or already invited"') + '>' + inviteLabel + '</button></div>';
    } else {
        const acc = G.blocAccession;
        if (acc) {
            const b = blocByIdOrCustom(acc.blocId);
            h += '<div class="hint">Accession in progress: <b>' + (b ? b.name : acc.blocId) + '</b> (step ' + acc.step + ' of 2).</div>';
            h += '<button class="btn tiny" data-bloc-withdraw="1">Withdraw application</button>';
            h += accessionCardHtml(acc.blocId);
        } else {
            h += '<div class="hint">Joining a bloc takes three bills: application, policy alignment, and accession treaty. Every member must approve at the final step.</div>';
            for(const id in BLOC_TEMPLATES){
                h += accessionCardHtml(id);
            }
            for (const cb of Object.values(G.customBlocs || {})){
                if (cb && cb.founder === player) h += accessionCardHtml(cb.id);
            }
        }
        if (G.draft.blocCreate) {
            const tpl = CUSTOM_BLOC_TEMPLATES[G.draft.blocCreate.template];
            h += '<div class="card staged-bloc-create" style="margin-top:12px"><h4>Staged: found <b>' + esc(G.draft.blocCreate.name || (tpl ? tpl.name : "bloc")) + '</b><span class="cat">' + (tpl ? tpl.pc : 28) + ' capital</span></h4>';
            h += '<div class="hint">' + (G.draft.blocCreate.template === "deep_integration" ? "Customs union · zero internal tariffs · common external tariff locked at 5%" : "Free trade area · preferential internal rates · members keep independent tariff policy") + '</div>';
            h += '<div class="foot"><button type="button" class="btn danger tiny" data-bloc-create-unstage="1">Unstage</button></div></div>';
        } else {
            h += '<div style="margin-top:12px"><button type="button" class="btn" data-bloc-found-open="1" style="width:100%;padding:10px 16px;font-size:15px;font-weight:600">Found a trade bloc</button></div>';
        }
    }
    return h;
}
function tariffLeverValue(key, law, homeRole, blocMember) {
    ensureTariffSchedule(law);
    const sched = law.tariffSchedule;
    if (key === "tariffDefault") return sched.default;
    if (key === "tariffCet") {
        const lock = tariffLocked(law, homeRole, blocMember);
        return sched.cet != null ? sched.cet : lock.cet != null ? lock.cet : sched.default;
    }
...
function wireTariffSchedule(host) {
    ensureTariffSchedule(G.draft);
    const lock = tariffLocked(G.draft);
    const apply = (key, v)=>{
        if (key === "tariffDefault") G.draft.tariffSchedule.default = v;
        else if (key === "tariffCet") G.draft.tariffSchedule.cet = v;
        else if (key.startsWith("tariffBloc:")) G.draft.tariffSchedule.bloc[key.slice(11)] = v;
        else if (key.startsWith("tariffCountry:")) G.draft.tariffSchedule.country[key.slice(14)] = v;
        syncTariffHeadline(G.draft);
    };
    host.querySelectorAll("[data-lever]").forEach((lever)=>{
        ...
        wireLevers(lever.parentElement, ()=>tariffLeverValue(key, G.draft), (id, v)=>apply(key, v), 0);
    });
}
function dealBlockers(d) {
    const out = [], D = G.draft, n = d.need || {};
    if (countryBlocId(playerCountryId())) out.push("bilateral deals unavailable while in a trade bloc");
    if (d.partner && countryBlocId(d.partner)) out.push("partner trades through their bloc");
    if (n.relation && G.rel[d.partner] < n.relation) out.push("relations below " + n.relation);
    if (n.deal && !D.deals[n.deal]) out.push("requires " + DEAL_BY_ID[n.deal].name.toLowerCase());
    needBlockers(n, D, {
        partner: d.partner
    }).forEach((b)=>out.push(b));
    if (n.notInBloc && countryBlocId(playerCountryId())) out.push("requires leaving your trade bloc");
    if (n.inBloc && !countryBlocId(playerCountryId())) out.push("requires bloc membership");
    return out;
}
function ledgerTable() {
    if (!G.log.length) return '<div class="empty">No quarters recorded. Set out a bill and deliver it.</div>';
    const rows = G.log.slice(-12).map((r)=>{
        const c = (v, d)=>'<td class="' + (v > 0 ? "pos" : v < 0 ? "neg" : "") + '">' + fmt(v, d) + '</td>';
        return '<tr><td>' + r.label + '</td>' + c(r.growth, 1) + '<td>' + (r.trend != null ? r.trend.toFixed(1) : "—") + '</td><td>' + r.inflation.toFixed(1) + '</td><td>' + r.unemployment.toFixed(1) + '</td>' + c(r.balance, 1) + '<td>' + r.debt.toFixed(0) + '</td><td>' + r.yield.toFixed(2) + '</td><td>' + r.approval.toFixed(0) + '</td><td>' + Math.round(r.capital) + '</td></tr>';
    }).join("");
    return '<table class="ledger"><thead><tr><th>Quarter</th><th>Growth</th><th>Trend</th><th>Inflation</th><th>Unemp.</th><th>Balance</th><th>Debt</th><th>Yield</th><th>Approval</th><th>Capital</th></tr></thead><tbody>' + rows + '</tbody></table>';
}
...
/* ==================================================================
   ...
   ================================================================== */ function despatch(title, stamp, html, opts, cfg) {
    const titleEl = $("dpTitle"), stampEl = $("dpStamp"), bodyEl = $("dpBody"), box = $("dpOpts");
    ...
    opts.forEach((o)=>{
        ...
        b.innerHTML = "<b>" + T(esc(o.b)) + "</b>" + (o.e ? "<em>" + T(esc(o.e)) + "</em>" : "") + (o.hint || "") + extra;
        b.onclick = ()=>{
            const scrim = $("scrim");
...
/* Fiscal bank recapitalisation: debt rises, capital is restored. */ function recapitaliseBank(econ, capitalBoost, debtCost) {
    const boost = capitalBoost != null ? capitalBoost : 2;
    const cost = debtCost != null ? debtCost : boost * RECAP_COST;
    econ.debt += cost;
    econ.bankCapital = Math.min(16, Math.max(econ.bankCapital, BANK_CAP0) + boost);
...
function applyEventOption(opt) {
    if (!opt) return;
    if (opt.setRel) {
        for(const k in opt.setRel)G.rel[k] = clamp((G.rel[k] || 50) + opt.setRel[k], 0, 100);
    }
    if (opt.fac) {
    ...
    if (opt.shocks) {
        for (const s of opt.shocks){
            const ch = s.channel;
            const kind = EVENT_SHOCK_CHANNELS[ch];
            if (!kind) continue;
            const pts = s.points != null ? s.points : 0;
            const q = s.q != null ? s.q : 4;
            if (kind === "econ") {
                if (ch === "supplyShock") G.econ.supplyShock = (G.econ.supplyShock || 0) + pts;
                else if (ch === "riskPremium") G.econ.riskPremium = (G.econ.riskPremium || 0) + pts;
...
function scheduleNextMajorQ(fromQ, det) {
    const base = fromQ != null ? fromQ : 0;
    const gap = det ? 24 : MAJOR_GAP_MIN + Math.floor(Math.random() * MAJOR_GAP_SPAN);
    return base + gap;
}
function episodeDuration(ev, opt) {
...
function beginEpisode(ev, opt) {
    ...
    G.episode = {
        ...
        endText: ev.endText || "The pressures that defined this episode have eased.",
        startedQ: G.q,
        endsQ: G.q + dur,
...
function snapshotTariffSchedules() {
    ...
    if (G.world) {
        ...
        for (const id of Object.keys(G.world)){
            if (id === playerId) continue;
            const seat = G.world[id];
            if (seat && seat.law) world[id] = clone(ensureTariffSchedule(seat.law));
        }
    }
    return {
        player,
...
/** Raise default external tariffs for a trade-war episode. Skips CU locks and CU partners. */ function raiseTradeWarTariffs(delta, opts) {
    const o = opts || {};
    ...
    if (o.raisePlayer !== false && G.law) {
        if (lockedTariff(G.law) == null) {
            ensureTariffSchedule(G.law);
            G.law.tariffSchedule.default = clamp((G.law.tariffSchedule.default != null ? G.law.tariffSchedule.default : BASE_TARIFF) + d, 0, 45);
    ...
    if (G.world) {
        for (const id of Object.keys(G.world)){
            if (id === playerId) continue;
            if (playerBloc && countryBlocId(id, G.blocMember) === playerBloc) continue;
            const seat = G.world[id];
            if (!seat || !seat.law) continue;
            ensureTariffSchedule(seat.law);
            seat.law.tariffSchedule.default = clamp((seat.law.tariffSchedule.default != null ? seat.law.tariffSchedule.default : BASE_TARIFF) + d, 0, 45);
            syncTariffHeadline(seat.law, seat.role || worldRoleForSeat(id));
        }
    }
    return snap;
}
function restoreEpisodeTariffs(ep) {
...
function rollMajorEvent() {
    ...
    const pool = EVENTS.filter((e)=>e.major && e.cond && e.cond() && e.id !== G.lastEventId);
    if (!pool.length) return null;
    let total = pool.reduce((a, e)=>a + (e.w || 1), 0), r = G.sandbox ? 0 : Math.random() * total;
    for (const e of pool){
        r -= e.w || 1;
        if (r <= 0) {
...
const EVENTS = [
    /* ---- foreign relations ---- */ {
        ...
        resolve: ()=>{
            ...
            const inv = G.blocInvites && G.blocInvites[player];
            if (!inv) return null;
            const from = activePartners().find((p)=>p.id !== player && countryBlocId(p.id) === inv.blocId);
            return from ? {
                focus: from.id,
                blocId: inv.blocId
            } : {
        ...
        opts: [
            ...
            },
            {
                ...
                f: ()=>{
                    const player = playerCountryId();
                    const inv = G.blocInvites && G.blocInvites[player];
                    if (inv && G.eventFocus) G.rel[G.eventFocus] = clamp((G.rel[G.eventFocus] || 50) - 8, 0, 100);
                    delete G.blocInvites[player];
                }
            }
        ]
    ...
    },
    {
        ...
        cond: ()=>{
            ...
            const bid = countryBlocId(player);
            if (!bid) return false;
            return activePartners().some((p)=>countryBlocId(p.id) === bid && (G.rel[p.id] || 50) < 22);
        },
        resolve: ()=>{
            const player = playerCountryId();
            const bid = countryBlocId(player);
            const pool = activePartners().filter((p)=>countryBlocId(p.id) === bid && (G.rel[p.id] || 50) < 22);
            return pool.length ? {
                focus: pool.sort((a, b)=>(G.rel[a.id] || 50) - (G.rel[b.id] || 50))[0].id
            } : null;
        },
        opts: [
            {
                ...
                f: ()=>{
                    if (G.eventFocus) G.rel[G.eventFocus] = clamp((G.rel[G.eventFocus] || 50) + 14, 0, 100);
                }
            },
            {
                b: "Let them leave",
                ...
                f: ()=>{
                    ...
                    const custom = bid && G.customBlocs[bid];
                    if (custom) custom.members = custom.members.filter((m)=>m !== G.eventFocus);
                    if (bid) dissolveCustomBlocIfEmpty(bid);
                }
            },
            {
                ...
                f: ()=>{
                    if (G.eventFocus) G.rel[G.eventFocus] = clamp((G.rel[G.eventFocus] || 50) - 6, 0, 100);
                }
            }
        ]
    },
        ...
        opts: [
            {
                ...
                f: ()=>{
                    ...
                    const ok = typeof window !== "undefined" && typeof window.confirm === "function"
                        ? window.confirm("Leave your trade bloc now? Membership ends immediately when you confirm.")
                        : true;
                    if (!ok) return;
                    leaveBloc(G.law);
    ...
    },
    {
        ...
        cond: ()=>{
            ...
            if ((G.rel.united_states != null ? G.rel.united_states : 50) < 25) return false;
        ...
        opts: [
            {
                ...
                f: ()=>{
                    ...
                    addDiploLedger(G, "united_states", { id: "ultimatum_comply", label: "Complied on digital tax", pts: 8 });
                }
            },
            {
                ...
                f: ()=>{
                    ...
                    addDiploLedger(G, "united_states", { id: "ultimatum_half", label: "Compromised on digital tax", pts: 3 });
                }
            },
            {
                ...
                f: ()=>{
                    addDiploLedger(G, "united_states", { id: "ultimatum_refuse", label: "Defied digital-tax ultimatum", pts: -12 });
                }
            }
        ]
    },
            ...
            const target = pickEventPartner({
                filter: (p)=>{
                    ...
                    if ([
                        ...
                        "united_states",
                        "australia"
                    ].includes(p.id) && rel > 40) return false;
                    return dealsWith(p.id).length < 3;
                },
                score: (p)=>(52 - (G.rel[p.id] != null ? G.rel[p.id] : 50)) * 3 + partnerShare(G.homeRole, p) * 25
            });
            return !!target;
        },
        resolve: ()=>{
            ...
            const sponsors = activePartners().filter((p)=>[
                    "france",
                    "germany",
                    "united_states",
                    "australia"
                ].includes(p.id) && p.id !== (focus && focus.id) && (G.rel[p.id] != null ? G.rel[p.id] : 50) >= 40).map((p)=>p.id);
            return focus ? {
                focus: focus.id,
                sponsors
            } : null;
        ...
        opts: [
            {
                ...
                f: ()=>{
                    ...
                    addDiploLedger(G, tid, { id: "sanctions_full_" + tid, label: "Joined sanctions against them", pts: -18 });
                    setSanctionStance(G, tid, { against: true });
                    (G.eventSponsors || []).forEach((id)=>{
                        addDiploLedger(G, id, { id: "sanctions_with_" + id + "_" + G.q, label: "Stood with them on sanctions", pts: 8 });
            ...
            },
            {
                ...
                f: ()=>{
                    ...
                    (G.eventSponsors || []).slice(0, 2).forEach((id)=>{
                        G.rel[id] = clamp((G.rel[id] || 50) + 3, 0, 100);
                    });
                    addDiploLedger(G, tid, { id: "sanctions_sym_" + tid, label: "Symbolic sanctions against them", pts: -9 });
                    setSanctionStance(G, tid, { against: true });
                    (G.eventSponsors || []).slice(0, 2).forEach((id)=>{
                        addDiploLedger(G, id, { id: "sanctions_sym_with_" + id, label: "Soft support on sanctions", pts: 3 });
                        setSanctionStance(G, id, { with: true });
                    });
                }
            },
            {
                ...
                f: ()=>{
                    ...
                    if (tid) {
                        ...
                        addDiploLedger(G, tid, { id: "sanctions_out_" + tid, label: "Refused to sanction them", pts: 6 });
                    }
                    (G.eventSponsors || []).forEach((id)=>{
                        G.rel[id] = clamp((G.rel[id] || 50) - 14, 0, 100);
                        addDiploLedger(G, id, { id: "sanctions_abandon_" + id, label: "Abandoned them on sanctions", pts: -10 });
    ...
    },
    {
        id: "unSeatBid",
        w: 7,
        stamp: "United Nations",
        title: "A seat on the Security Council opens",
        text: "A rotating seat on the United Nations Security Council opens next session, and {C}'s ambassador believes the votes are there. Two years at the horseshoe table, and a hand raised on everything that reaches it.",
        cond: ()=>G.q > 2 && isUnMember(G.law) && !isP5(playerCountryId()) && unSeatQuartersLeft() === 0,
        opts: [
            {
                b: "Campaign hard for the seat",
                e: "Two years on the council",
                capital: -8,
                fac: {
                    patriots: 6,
                    urban: 3
                ...
                f: ()=>{
                    G.econ.unSeatUntil = G.q + UN_SEAT_QUARTERS;
                }
            },
            {
                b: "Buy the votes with aid pledges",
                e: "The seat, and the invoices",
                capital: -3,
                fac: {
                    patriots: 4,
                    business: -2
            ...
            {
                b: "Stay out of the race",
                e: "The chamber notes the absence",
                fac: {
                    patriots: -3,
                    urban: -1
                }
    ...
    },
    {
        ...
        w: 9,
        stamp: "United Nations",
        title: "A resolution reaches the Security Council",
        text: ()=>isP5(playerCountryId()) ? "A resolution censuring {P} has been tabled at the Security Council. {C} holds a permanent seat and a veto, and both sides have been in touch since breakfast." : "A resolution censuring {P} has been tabled at the Security Council while {C} holds an elected seat. The sponsors are counting hands; so is {P}.",
        cond: ()=>{
            if (G.q <= 4 || !onSecurityCouncil()) return false;
            return !!pickEventPartner({
                filter: (p)=>(G.rel[p.id] != null ? G.rel[p.id] : 50) < 46,
                score: (p)=>46 - (G.rel[p.id] != null ? G.rel[p.id] : 50)
            });
        },
        resolve: ()=>{
            const focus = pickEventPartner({
            ...
            const sponsors = activePartners().filter((p)=>p.id !== focus.id && profilePolityId(p.id) === "democracy" && (G.rel[p.id] != null ? G.rel[p.id] : 50) >= 42).slice(0, 4).map((p)=>p.id);
            return {
                focus: focus.id,
                sponsors
            };
        ...
        opts: [
            {
                b: "Vote for the resolution",
                e: "The sponsors bank it, {P} remembers",
                fac: {
                    urban: 3,
                    patriots: 2
                },
                f: ()=>{
                    const tid = G.eventFocus;
                    if (!tid) return;
                    G.rel[tid] = clamp((G.rel[tid] || 50) - 12, 0, 100);
                    addDiploLedger(G, tid, { id: "un_vote_for_" + tid + "_" + G.q, label: "Voted to censure them at the UN", pts: -10 });
                    (G.eventSponsors || []).forEach((id)=>{
                        G.rel[id] = clamp((G.rel[id] || 50) + 7, 0, 100);
                        addDiploLedger(G, id, { id: "un_vote_with_" + id + "_" + G.q, label: "Voted with them at the UN", pts: 5 });
                    });
                }
            },
            {
                b: "Abstain",
                e: "Nobody thanks an empty chair",
                capital: -3,
                f: ()=>{
                    const tid = G.eventFocus;
                    if (tid) G.rel[tid] = clamp((G.rel[tid] || 50) + 2, 0, 100);
                    (G.eventSponsors || []).forEach((id)=>{
                        G.rel[id] = clamp((G.rel[id] || 50) - 6, 0, 100);
                        addDiploLedger(G, id, { id: "un_abstain_" + id + "_" + G.q, label: "Sat out their resolution", pts: -4 });
                    });
                }
            },
            {
                b: "Veto the resolution",
                e: "One raised hand stops the chamber",
                available: ()=>isP5(playerCountryId()),
                fac: {
                    patriots: 6,
                    urban: -3
                },
                f: ()=>{
                    const tid = G.eventFocus;
                    if (tid) {
                        G.rel[tid] = clamp((G.rel[tid] || 50) + 14, 0, 100);
                        addDiploLedger(G, tid, { id: "un_veto_for_" + tid + "_" + G.q, label: "Vetoed the resolution against them", pts: 10 });
                    }
                    (G.eventSponsors || []).forEach((id)=>{
                        G.rel[id] = clamp((G.rel[id] || 50) - 11, 0, 100);
                        addDiploLedger(G, id, { id: "un_veto_against_" + id + "_" + G.q, label: "Vetoed their resolution", pts: -8 });
                    });
                }
            },
            {
                b: "Vote against",
                e: "On the record beside {P}",
                available: ()=>!isP5(playerCountryId()),
                fac: {
                    patriots: 4,
                    urban: -2
                ...
                f: ()=>{
                    ...
                    if (tid) {
                        G.rel[tid] = clamp((G.rel[tid] || 50) + 9, 0, 100);
                        addDiploLedger(G, tid, { id: "un_against_for_" + tid + "_" + G.q, label: "Voted against censuring them", pts: 7 });
                    }
                    (G.eventSponsors || []).forEach((id)=>{
                        G.rel[id] = clamp((G.rel[id] || 50) - 8, 0, 100);
                        addDiploLedger(G, id, { id: "un_against_" + id + "_" + G.q, label: "Voted against their resolution", pts: -6 });
                    });
                }
            }
        ]
    },
    {
        ...
        w: 6,
        stamp: "United Nations",
        title: "The peacekeeping assessment arrives",
        text: "The United Nations has priced its peacekeeping missions for the year, and {C}'s assessed share has landed on your desk — alongside a request for troops that nobody else wants to send.",
        cond: ()=>G.q > 2 && isUnMember(G.law),
        opts: [
            {
                b: "Pay in full and offer troops",
                e: "Blue helmets, {C}'s flag on them",
                fac: {
                    urban: 3,
                    patriots: 2,
                    business: -1
                ...
                f: ()=>{
                    activePartners().filter((p)=>profilePolityId(p.id) === "democracy").slice(0, 4).forEach((p)=>{
                        G.rel[p.id] = clamp((G.rel[p.id] || 50) + 3, 0, 100);
                    });
                }
            },
            {
                b: "Pay the assessment, decline the troops",
                e: "The cheque clears, quietly",
                capital: -1,
                shocks: [
                    {
                        channel: "spend",
            ...
            },
            {
                ...
                f: ()=>{
                    activePartners().filter((p)=>profilePolityId(p.id) === "democracy").slice(0, 4).forEach((p)=>{
                        G.rel[p.id] = clamp((G.rel[p.id] || 50) - 4, 0, 100);
                        addDiploLedger(G, p.id, { id: "un_arrears_" + p.id + "_" + G.q, label: "In arrears at the UN", pts: -3 });
                    });
                }
            }
        ]
    },
    {
        ...
        title: ()=>{
            ...
            if (meta.kind === "congress") return "A partner reshuffles its leadership";
        ...
        opts: [
            {
                ...
                f: ()=>{
                    ...
                    if (id) {
                        G.rel[id] = clamp((G.rel[id] || 50) + 14, 0, 100);
                        addDiploLedger(G, id, { id: "newgovt_court_" + id, label: "Courtship after change of government", pts: 8 });
                    }
                }
            },
            {
                ...
                f: ()=>{
                    const id = G.eventFocus;
                    if (id) G.rel[id] = clamp((G.rel[id] || 50) + ((!G.sandbox && Math.random() < 0.5) ? -12 : 5), 0, 100);
                }
            },
            {
                ...
                f: ()=>{
                    activePartners().forEach((p)=>{
                        ...
                        addDiploLedger(G, p.id, { id: "newgovt_crit_" + G.q, label: "Public criticism of foreign governments", pts: -4 });
                    });
                }
            }
        ]
    },
    {
        ...
        text: ()=>countryBlocId(playerCountryId()) === "continental_union" ? "The Gulf states have offered a standing swap line. It would steady the currency in a crisis. It also means someone else has a view on your budget." : "The Gulf states and the Continental Union have both offered a standing swap line. It would steady the currency in a crisis. It also means someone else has a view on your budget.",
        ...
        opts: [
            {
                ...
                available: ()=>countryBlocId(playerCountryId()) !== "continental_union" && activePartners().some((p)=>p.id === "france"),
                f: ()=>{
                    ["france", "germany"].forEach((id)=>{
                        if (!partnerById(id)) return;
                        addDiploLedger(G, id, { id: "swap_cu_" + id, label: "Accepted Continental swap line", pts: 6 });
                    });
                }
            },
            {
                ...
                f: ()=>{
                    ["saudi", "uae"].forEach((id)=>{
                        if (!partnerById(id)) return;
                        addDiploLedger(G, id, { id: "swap_gulf_" + id, label: "Accepted Gulf swap line", pts: 6 });
                    });
                }
            },
            {
        ...
        opts: [
            {
                ...
                f: ()=>{
                    ["france", "germany", "australia"].forEach((id)=>{
                        if (!partnerById(id)) return;
                        addDiploLedger(G, id, { id: "migration_share_" + id, label: "Burden-sharing agreement", pts: 6 });
                    });
                }
            },
            {
            ...
            },
            {
                ...
                f: ()=>{
                    activePartners().forEach((p)=>{
                        ...
                        addDiploLedger(G, p.id, { id: "migration_close_" + G.q, label: "Closed the border", pts: -6 });
                    });
                }
            }
        ]
        ...
        opts: [
            {
                ...
                f: ()=>{
                    ...
                    if (id) {
                        ...
                        addDiploLedger(G, id, { id: "espionage_expel_" + id, label: "Expelled their diplomats", pts: -14 });
                    }
                }
            },
            {
                ...
                f: ()=>{
                    ...
                    if (id) {
                        ...
                        addDiploLedger(G, id, { id: "espionage_quiet_" + id, label: "Quiet espionage row", pts: -5 });
                    }
    ...
    },
    {
        ...
        cond: ()=>{
            ...
            return !!pickEventPartner({
                ...
                score: (p)=>partnerShare(G.homeRole, p) * 20 + (dealsForPartner(p, G.homeRole).length - dealsWith(p.id).length) * 5
            });
        },
        resolve: ()=>{
            const p = pickEventPartner({
                filter: (p)=>dealsWith(p.id).length < dealsForPartner(p, G.homeRole).length,
                score: (p)=>partnerShare(G.homeRole, p) * 20 + (dealsForPartner(p, G.homeRole).length - dealsWith(p.id).length) * 5
            });
            return p ? {
                focus: p.id
        ...
        opts: [
            {
                ...
                f: ()=>{
                    ...
                    if (lockedTariff(G.law) == null) G.law.tariff = Math.max(0, G.law.tariff - 3);
                }
            },
            {
            ...
            },
            {
                ...
                f: ()=>{
                    activePartners().forEach((p)=>G.rel[p.id] = clamp(G.rel[p.id] + 7, 0, 100));
    ...
    },
    {
        ...
        cond: ()=>!(G.episode && (G.episode.id === "globalRecess" || G.episode.id === "creditCrunch")),
        opts: [
            {
    ...
    },
    {
        ...
        cond: ()=>!G.econ.bankStress && !(G.econ.bankStressCd > 0) && (G.econ.rate > 7.5 || G.econ.yield > 8.0) && G.econ.bankLeverage < BANK_STRESS_LEV - 0.5,
        opts: [
            {
            ...
            },
            {
                ...
                f: ()=>{
                    ...
                    G.econ.bankLoans = Math.max(40, (G.econ.bankLoans || BANK_LOANS0) * 0.96);
            ...
            },
            {
                ...
                f: ()=>{
                    ...
                    G.econ.bankLoans = Math.max(40, (G.econ.bankLoans || BANK_LOANS0) * 0.88);
    ...
    },
    {
        ...
        text: ()=>countryBlocId(playerCountryId()) === "continental_union" ? "A petition on the customs-union settlement has passed two million signatures. The backbenches are restive even though the tariff lever is already surrendered." : "A petition on your trade and border settlement has passed two million signatures. The backbenches are restive.",
    ...
    },
    {
        ...
        cond: ()=>G.law.tariff > 6 || G.law.taxes.digitalTax.rate > 5 || (G.econ.retaliation || 0) > 2.5,
        resolve: ()=>{
            const dt = G.law.taxes.digitalTax;
            let focus;
            if (dt.on && dt.rate > 5 && activePartners().some((p)=>p.id === "united_states")) focus = partnerById("united_states");
            else focus = pickEventPartner({
                score: (p)=>partnerShare(G.homeRole, p) * 30 + Math.max(0, 50 - (G.rel[p.id] != null ? G.rel[p.id] : 50))
            });
            return focus ? {
                focus: focus.id
            } : null;
        ...
        opts: [
            {
                ...
                f: ()=>{
                    ...
                    const id = G.eventFocus;
                    if (id) G.rel[id] = clamp((G.rel[id] || 50) + 8, 0, 100);
                    else activePartners().forEach((p)=>G.rel[p.id] = clamp(G.rel[p.id] + 6, 0, 100));
                }
            },
            {
                b: "Retaliate in kind",
                ...
                f: ()=>{
                    if (lockedTariff(G.law) == null) G.law.tariff = clamp(G.law.tariff + 5, 0, 25);
                    const id = G.eventFocus;
                    if (id) G.rel[id] = clamp((G.rel[id] || 50) - 12, 0, 100);
                    else activePartners().forEach((p)=>G.rel[p.id] = clamp(G.rel[p.id] - 9, 0, 100));
                }
            },
            {
    ...
    },
    {
        ...
        cond: ()=>!(G.episode && (G.episode.id === "aiBoom" || G.episode.id === "greenTransition")),
        opts: [
            {
            ...
            },
            {
                ...
                f: ()=>{
                    G.law.income.bands[0].rate = clamp(G.law.income.bands[0].rate - 1, 0, 90);
                }
            }
        ]
    },
    /* ---- Major world episodes (separate 4–8 year schedule) ---- */ {
        ...
        endText: "Order books abroad are filling again. The synchronised contraction has passed; what you enacted during it stays on the statute book.",
        ...
        endText: "The extraordinary productivity pulse abroad has normalised. Catch-up and diffusion continue at a quieter pace.",
        ...
        endText: "Import price pressure from abroad has eased. Domestic policy and the Bank still decide what happens next at home.",
        ...
        endText: "Oil and food prices abroad have come off their peaks. The terms-of-trade hit is fading.",
        ...
        endStamp: "Central bank",
        endText: "The burst of foreign easing has run its course. Exchange-rate and capital-flow effects from that episode are fading.",
        ...
        endStamp: "Foreign & Commonwealth",
        endText: "Partner tariff walls raised for this episode have come down. Your own schedule is restored unless you chose to keep the higher wall.",
        ...
        endStamp: "Foreign & Commonwealth",
        endText: "Activity in China has stabilised. The bilateral demand shock from that episode has lifted.",
        ...
        endStamp: "Treasury",
        endText: "Risk premia and foreign funding stress from the crunch have subsided. Domestic banks still carry whatever scars you left them with.",
        ...
        endText: "The burst of global clean-tech capex has levelled off. Productivity and price effects from that wave settle into the trend.",
    ...
    },
    {
        ...
        cond: ()=>{
            ...
            return age != null && age >= 2 && age <= 8 && polityShiftIsLiberalising(G.polityShift);
        },
        opts: [
            {
    ...
    },
    {
        ...
        cond: ()=>{
            const age = polityShiftAge();
            return age != null && age >= 2 && age <= 8 && polityShiftIsTightening(G.polityShift);
        },
        opts: [
            {
    ...
    },
    {
        ...
        cond: ()=>{
            const age = polityShiftAge();
            return age != null && age >= 2 && age <= 8 && !!G.polityShift && activePartners().length > 0;
        },
        opts: [
            {
    ...
    },
    {
        ...
        resolve: ()=>{
            const focus = pickEventPartner({
                filter: (p)=>(G.rel[p.id] != null ? G.rel[p.id] : 50) < 55,
                score: (p)=>55 - (G.rel[p.id] != null ? G.rel[p.id] : 50) + partnerShare(G.homeRole, p) * 20
            }) || pickEventPartner();
            const sponsors = activePartners().filter((p)=>[
                    "france",
                    "germany",
                    "united_states",
                    "australia"
                ].includes(p.id) && p.id !== (focus && focus.id)).map((p)=>p.id);
            return focus ? {
                focus: focus.id,
                sponsors
            } : null;
...
function prepareEvent(ev) {
    ...
    const opts = (ev.opts || []).filter((o)=>!o.available || o.available());
...
function rollEvent() {
    ...
    /* Guaranteed mid/late-term set-pieces so a 20-quarter term always has drama. */ if (G.q === 8 && !G.setPiece8) {
        G.setPiece8 = true;
    ...
    if (Math.random() > 0.20) return null; // and only occasionally even then
    /* Majors ride a separate 4–8 year track via rollMajorEvent. */ const pool = EVENTS.filter((e)=>!e.major && e.cond() && e.id !== G.lastEventId && e.id !== "setPieceFiscal" && e.id !== "setPieceAlly");
    if (!pool.length) return null;
    let total = pool.reduce((a, e)=>a + e.w, 0), r = G.sandbox ? 0 : Math.random() * total;
    for (const e of pool){
        r -= e.w;
        if (r <= 0) {
            const prepared = prepareEvent(e);
...
function writeBriefing(res) {
    const e = G.econ, b = [], E = res.E;
    if (res.growth < -0.5) b.push("The economy contracted this quarter. Expect the word recession in tomorrow's papers.");
    else if (res.growth > res.pg + 1.2) b.push("Output is running well ahead of what the economy can sustainably produce.");
    else if (res.growth > 0.4) b.push("Modest growth, broadly in line with trend.");
    else b.push("Growth has stalled. Nothing dramatic, just nothing happening.");
    if (e.inflation > 8) b.push("Inflation at " + e.inflation.toFixed(1) + "% is out of control, and wage demands are following it up.");
    else if (e.inflation > 4) b.push("Inflation is " + e.inflation.toFixed(1) + "%, and the Bank has taken rates to " + e.rate.toFixed(2) + "%.");
    else if (e.inflation < 0.5) b.push("Prices are barely moving. The Bank is more worried about deflation than you are.");
    if (e.yield > 7) b.push("Gilt yields hit " + e.yield.toFixed(1) + "%. The market is pricing in the possibility that you cannot pay.");
    else if (e.debt > 130) b.push("Debt is " + e.debt.toFixed(0) + "% of GDP, and interest alone costs " + res.sp.interest.toFixed(1) + "% of national income.");
    if (E.blackLevel > 34) b.push("Roughly " + E.blackLevel.toFixed(0) + "% of the trade in what you regulate now happens outside the law, and outside the tax base.");
    if (e.services < 35) b.push("Waiting lists and class sizes are the front page most mornings now.");
    if (e.unemployment > 8) b.push("Unemployment at " + e.unemployment.toFixed(1) + "% is the number your backbenchers quote at you.");
    /* Crisis runway — foreshadow terminal outcomes one or two quarters ahead. */ if (e.yield > 8.5 && e.debt > 110) b.unshift("The gilt market looks fragile. Another scare and the door may close.");
    if (e.inflation > 16) b.unshift("Inflation above sixteen percent is a political emergency in waiting.");
    const meta = polityOf(G.homeRole);
    const metric = coupMetric(G.fac, meta);
    if (G.lowRun >= 2 || metric < meta.coupFloor + 2) {
        if (meta.coupOn === "patriots") {
            b.unshift("The apparatus is restless. " + meta.coupQuarters + " quarters below " + meta.coupFloor + " among the patriots and you are removed.");
        } else {
            b.unshift("Letters are going in. Four quarters below twenty percent and your own side will remove you.");
        }
    }
    if (e.bankStress) b.unshift("The banking system is under stress. Credit is scarce and a fiscal recap may be forced.");
    const worst = FACTIONS.slice().sort((a, b2)=>G.fac[a.id] - G.fac[b2.id])[0];
    if (G.fac[worst.id] < 30) b.push(worst.name + " have effectively abandoned you, and they are " + Math.round(worst.w * 100) + "% of the electorate.");
    if (G.capital < 12) b.push("You have almost no political capital left. Ambitious legislation is off the table until that recovers.");
    const left = electionQuartersLeft();
    if (left <= 4 && left > 0) {
        const therm = electionThermometer();
        const noun = reviewNoun(meta);
        b.push(therm <= meta.loseAt
            ? (noun.charAt(0).toUpperCase() + noun.slice(1)) + " in " + left + " quarters. On today's numbers you would lose."
            : (noun.charAt(0).toUpperCase() + noun.slice(1)) + " in " + left + " quarters. The running score is still just about enough.");
    }
...
function pickBriefingEconomicRows(head) {
    const all = IMPACT_ROWS.filter((r)=>r.k !== "approval").map((r)=>({
            r,
            v: head[r.k] || 0
    ...
    all.filter((x)=>!seen.has(x.r.k)).sort((a, b)=>Math.abs(b.v) - Math.abs(a.v)).forEach((x)=>{
...
function pickBriefingApprovalRow(head) {
...
function briefingImpactClause(row, v) {
    const n = Math.abs(v).toFixed(row.dp);
    const up = v > 0;
    switch(row.k){
        case "growth":
            return up ? "growth about " + n + " points stronger" : "growth about " + n + " points weaker";
        case "trend":
            return up ? "the trend rate about " + n + " points higher" : "the trend rate about " + n + " points lower";
        case "potential":
            return up ? "capacity about " + n + " points higher" : "capacity about " + n + " points lower";
        case "balance":
            return up ? "the deficit about " + n + " points of GDP narrower" : "the deficit about " + n + " points of GDP wider";
        case "debt":
            return up ? "debt about " + n + " points of GDP higher" : "debt about " + n + " points of GDP lower";
        case "inflation":
            return up ? "inflation about " + n + " points higher" : "inflation about " + n + " points lower";
        case "unemployment":
            return up ? "unemployment about " + n + " points higher" : "unemployment about " + n + " points lower";
        case "yield":
            return up ? "gilts about " + n + " points dearer" : "gilts about " + n + " points easier";
        case "approval":
            return up ? "about " + n + " points gained in the polls" : "about " + n + " points off in the polls";
        case "services":
            return up ? "public services about " + n + " points stronger" : "public services about " + n + " points weaker";
        case "fx":
            return up ? "the pound about " + n + " points firmer" : "the pound about " + n + " points softer";
        default:
            return (up ? "higher " : "lower ") + row.name.toLowerCase() + " by about " + n + " points";
    }
}
function joinBriefingClauses(clauses) {
    if (!clauses.length) return "";
    if (clauses.length === 1) return clauses[0];
    if (clauses.length === 2) return clauses[0] + " and " + clauses[1];
    return clauses[0] + ", " + clauses[1] + ", and " + clauses[2];
}
function briefingFactionLine(facs) {
    const warm = facs.filter((e)=>e[1] > 0).slice(0, 2).map((e)=>factionName(e[0]));
    const cool = facs.filter((e)=>e[1] < 0).slice(0, 2).map((e)=>factionName(e[0]));
    if (warm.length && cool.length) {
        if (warm.length === 1 && cool.length === 1) {
            return warm[0] + " will take this better than " + cool[0] + ".";
        }
        return "Politically, " + warm.join(" and ") + " are with you; " + cool.join(" and ") + " are not.";
    }
    if (warm.length) {
        return warm.join(" and ") + (warm.length === 1 ? " should be on side." : " should both be on side.");
    }
    if (cool.length) {
...
function briefingImpactParts(im, meta) {
    ...
    const facs = Object.entries(im.fac || {}).filter((e)=>Math.abs(e[1]) > 0.3).sort((a, b)=>Math.abs(b[1]) - Math.abs(a[1]));
    ...
    if (!econRows.length && !approvalRow && !politics) {
        if (kind === "bill" && meta && meta.clauses && meta.clauses.length) {
            return {
                kind,
                quiet: "This barely moves the dial on a year-ahead view. Nothing the markets or the whips will lose sleep over.",
                economic: null,
                approval: null,
                politics: null
            };
    ...
    const tail = kind === "decision" ? ", over the next year, compared with doing nothing." : ", over the next year, compared with standing still.";
    let economic = null;
    if (econRows.length) {
        const body = joinBriefingClauses(econRows.map((x)=>briefingImpactClause(x.r, x.v)));
...
function briefingImpactLines(im, meta) {
...
function mergeBriefingImpact(brief, impactLines) {
    if (!impactLines || !impactLines.length) return brief || [];
    return impactLines.concat(brief || []).slice(0, 4);
}
function briefingHtml(brief, opts) {
    const o = opts || {};
    const impact = o.impact != null ? o.impact : G && G.briefImpact;
    let h = '<div class="briefing">';
    if (impact && (impact.quiet || impact.economic || impact.approval || impact.politics)) {
        h += '<div class="briefing-impact-grid">';
        if (impact.quiet) {
            h += '<section class="briefing-impact briefing-impact--econ briefing-impact--wide"><div class="briefing-label">Year-ahead view</div><p class="briefing-impact-line">' + esc(impact.quiet) + '</p></section>';
        } else {
            if (impact.economic) {
                h += '<section class="briefing-impact briefing-impact--econ"><div class="briefing-label">Next year</div><div class="briefing-impact-hint">Four-quarter total, not this quarter alone</div><p class="briefing-impact-line">' + esc(impact.economic) + '</p></section>';
            }
            if (impact.approval || impact.politics) {
                h += '<section class="briefing-impact briefing-impact--approval"><div class="briefing-label">Approval</div><div class="briefing-impact-hint">Four-quarter total, not this quarter alone</div>';
                if (impact.approval) h += '<p class="briefing-impact-line">' + esc(impact.approval) + '</p>';
                if (impact.politics) h += '<p class="briefing-politics">' + esc(impact.politics) + '</p>';
                h += '</section>';
            }
        }
        h += '</div>';
    ...
    const outturn = brief || [];
    if (outturn.length) {
        h += '<section class="briefing-outturn' + (impact && (impact.economic || impact.approval || impact.quiet) ? "" : " briefing-outturn--solo") + '">';
        if (impact && (impact.economic || impact.approval || impact.quiet)) h += '<div class="briefing-label">This quarter</div>';
        outturn.forEach((t)=>h += '<p>' + esc(t) + '</p>');
        h += '</section>';
    }
...
function showMpBriefing(opts) {
    ...
    const afterFlash = () => {
        ...
        writeBriefing(res);
        /* Ultimatum answers resolve inside lockstep with no solo Deliver press path —
         surface them on the morning note and as press clips. */
        const freshUlt = (G.diploAlerts || []).filter((a) => {
            if (!a || (a.kind !== "ult_concede" && a.kind !== "ult_defy")) return false;
            return a.q === G.q;
        });
        for (const a of freshUlt) {
            ...
            const line = a.kind === "ult_concede"
                ? name + " conceded to our ultimatum — " + demand + "."
                : name + " defied our ultimatum — " + demand + ".";
...
function showMpPendingEvent(opts) {
    if (!G || !G.mp) return false;
    const seatId = playerCountryId(G.homeRole);
    const pol = G.politics && G.politics[seatId];
    ...
    const present = () => {
        ...
        if (pend.episodeEnd) {
            ...
            despatch(T(ep && ep.endTitle || "The episode ends"), ep && ep.endStamp || "Cabinet Office", "<p>" + T(ep && ep.endText || "The pressures that defined this episode have eased.") + "</p>", [
                {
                    b: "Noted",
        ...
        if (pend.missionEvent) {
            ...
            let title = (ev.title || "").replace(/\{P\}/g, pName).replace(/\{R\}/g, ev.rivalName || "a rival");
            title = T(title);
            const body = "<p>" + formatMissionEventText(ev, G, diploDeps()) + "</p>" +
                '<p class="hint">Each choice shifts relations and factions.</p>';
            const ctx = { partnerId: pend.partnerId, rivalId: ev.rivalId, rivalName: ev.rivalName };
        ...
        despatch(ev.title, ev.stamp, "<p>" + T(ev.text) + "</p>", ev.opts.map((o, i)=>({
                b: o.b,
                e: o.e,
                previewTarget: o,
                f: ()=>choose({
...
function ultimatumPressImpactText(impacts) {
    if (!impacts) return "";
    const bits = [];
    if (impacts.diplomatic) bits.push("Diplomatically, " + lcFirst(impacts.diplomatic));
...
function composePress(input) {
    const clauses = input && input.clauses || [];
    ...
    const qIdx = input && input.q != null ? input.q : Math.max(0, (G && G.q ? G.q : 1) - 1);
    const date = G ? qLabel(G, qIdx) : "Q" + (qIdx + 1);
    const clips = [];
    if (clauses.length) {
        const byCost = clauses.slice().sort((a, b)=>b.pc - a.pc);
        let headline, lede;
        if (clauses.length === 1) {
            ...
            lede = byCost.slice(0, 2).map((c)=>c.label).join(". ") + ".";
        }
        if (typeof balDelta === "number" && Math.abs(balDelta) >= 0.05) {
            lede += balDelta > 0 ? " The books tighten by about " + Math.abs(balDelta).toFixed(1) + " points of GDP on delivery." : " The deficit widens by about " + Math.abs(balDelta).toFixed(1) + " points of GDP on delivery.";
        } else if (cost > 0) {
    ...
    if (event && option) {
        ...
        if (extras.length) lede = (lede ? lede.replace(/\s*$/, "") + " " : "") + extras.slice(0, 3).join("; ") + ".";
        if (!lede && event.text) lede = String(event.text).replace(/<[^>]+>/g, "").slice(0, 160);
        clips.push({
            kind: "event",
    ...
    for (const u of ultOutcomes) {
        ...
        if (u.kind === "ult_concede") {
            clips.push({
                ...
                lede: T(name + " has conceded to {C}'s ultimatum — " + demand + ". Foreign Office sources describe the climbdown as unconditional." + impact),
                kicker: date
            });
            ...
            clips.push({
                ...
                lede: T(name + " has rejected {C}'s ultimatum — " + demand + ". Ministers are returning to the diplomatic drawing board." + impact),
                kicker: date
            });
        }
    }
...
let _pressExpanded = null;
function clearAllPressFades() {
    _pressFade.forEach((t)=>clearTimeout(t));
    _pressFade.clear();
    _pressExpanded = null;
}
function schedulePressFade(id) {
    clearPressFade(id);
    if (typeof setTimeout !== "function") return;
    if (_pressExpanded === id) return;
    const host = typeof document !== "undefined" && document.getElementById("pressLayer") || $("pressLayer");
    if (!host) return;
    const t = setTimeout(()=>{
        dismissPress(id);
    }, PRESS_FADE_MS);
...
function expandPress(id) {
    if (!G || !G.press) return false;
    if (!G.press.some((c)=>c.id === id)) return false;
    clearPressFade(id);
    _pressExpanded = id;
    renderPress();
    return true;
}
function dismissPress(id) {
    if (!G || !G.press) return false;
    const i = G.press.findIndex((c)=>c.id === id);
    if (i < 0) return false;
    clearPressFade(id);
...
function clipBodyHtml(c) {
    return '<div class="clip-mast">' + esc(c.masthead) + '</div>' + '<div class="clip-kick">' + esc(c.kicker) + '</div>' + '<h4 class="clip-hed">' + esc(c.headline) + '</h4>' + '<p class="clip-lede">' + esc(c.lede) + '</p>';
}
function renderPress() {
    const host = $("pressLayer");
    if (!host) return;
    const clips = G && G.press ? G.press : [];
    const focused = _pressExpanded && clips.find((c)=>c.id === _pressExpanded);
    if (!focused) _pressExpanded = null;
    host.classList.toggle("is-focusing", !!focused);
    host.innerHTML = clips.map((c, i)=>{
        if (focused && c.id === focused.id) return "";
        const rot = (c.rot != null ? c.rot : 0).toFixed(2);
        const delay = (i * 0.05).toFixed(2);
        return '<article class="clipping" data-id="' + esc(c.id) + '" style="--clip-rot:' + rot + 'deg;--clip-delay:' + delay + 's" role="button" tabindex="0" aria-label="Open clipping">' + clipBodyHtml(c) + '</article>';
    }).join("");
    if (focused) {
        const rot = (focused.rot != null ? focused.rot : -1).toFixed(2);
        host.insertAdjacentHTML("beforeend", '<div class="press-focus" role="dialog" aria-modal="true" aria-label="Newspaper clipping">' + '<article class="clipping clipping-focus" style="--clip-rot:' + rot + 'deg">' + clipBodyHtml(focused) + '<button type="button" class="clip-dismiss">Dismiss</button>' + '</article>' + '</div>');
    ...
    host.querySelectorAll(".clipping:not(.clipping-focus)").forEach((el)=>{
...
function checkCrises(res) {
    const e = G.econ;
    if (e.yield > 10 && e.debt > 118) return gameOver("F", "The market closed the door", "{C} could not roll over its debt at any price you were willing to pay. An emergency programme was agreed with the international lender of last resort, and its conditions were not written in this building.");
    if (e.inflation > 22) return gameOver("F", "Prices ran away", "Inflation above twenty percent stopped being an economic statistic and became a political one. Savings evaporated, indexation spread, and the government fell before you could unwind any of it.");
    const meta = polityOf(G.homeRole);
    const metric = coupMetric(G.fac, meta);
    G.lowRun = metric < meta.coupFloor ? G.lowRun + 1 : 0;
    if (G.lowRun >= meta.coupQuarters) {
        if (meta.coupOn === "patriots") {
            return gameOver("E", "The apparatus removed you", meta.coupQuarters + " straight quarters below " + meta.coupFloor + " among the patriots. The security elite closed ranks, and you were offered a quiet exit.");
        }
        return gameOver("E", "Your own side removed you", "Four straight quarters below twenty percent. The letters went in, the meeting was short, and you were offered a peerage on the way out.");
    }
}
function grade() {
    const e = G.econ;
    ...
    const pct = s / 48;
    return pct > 0.80 ? "A" : pct > 0.66 ? "B" : pct > 0.50 ? "C" : pct > 0.34 ? "D" : "E";
}
function gameOver(g, title, text) {
    /* Sandbox keeps you in office. The crisis still happened and the briefing
     still says so; you simply are not removed for it. */ if (G.sandbox) {
        G.brief = [
            title + ". In another government this would have ended you, but you are still here.",
            text
        ].concat(G.brief).slice(0, 3);
        G.lowRun = 0;
    ...
    const rows = [
        ...
        [
            "Debt",
            e.debt.toFixed(0) + "% of GDP"
        ],
        [
            "Public services",
            e.services.toFixed(0) + " / 100"
        ],
        [
            "Civil liberty",
            e.liberty.toFixed(0) + " / 100"
        ],
        [
            "Inequality",
            e.gini.toFixed(1) + " Gini"
        ],
        [
            "Final approval",
    ...
    despatch(title, "Verdict", '<div class="verdict"><div class="grade">' + g + '</div><div class="lede">' + text + '</div><table>' + rows + '</table></div>', [
        {
...
function termReview() {
    ...
    if (score <= meta.loseAt && !G.sandbox) {
        if (meta.kind === "congress") {
            return gameOver(grade(), "You lost the congress", "The party had a view and expressed it. Your successor inherits debt at " + e.debt.toFixed(0) + "% of GDP, inflation at " + e.inflation.toFixed(1) + "% and a dossier you had better hope was polite.");
        }
        if (meta.kind === "managed") {
            return gameOver(grade(), "You lost the ballot", "Even a managed contest can go wrong. Your successor inherits debt at " + e.debt.toFixed(0) + "% of GDP, inflation at " + e.inflation.toFixed(1) + "% and a note in the desk drawer you had better hope was polite.");
        }
        return gameOver(grade(), "You lost the election", "The country had a view and expressed it. Your successor inherits debt at " + e.debt.toFixed(0) + "% of GDP, inflation at " + e.inflation.toFixed(1) + "% and a note in the desk drawer you had better hope was polite.");
    }
    G.term++;
    ...
    despatch(title, stamp, lede + "<p>A fresh mandate is worth real political capital. Spend it on something that outlasts you.</p>", [
        {
            b: "Continue",
...
function fullEffects(imp, fac, cost, ch) {
    const bits = [];
    if (cost) bits.push('<b>' + (cost > 0 ? "costs " : "saves ") + Math.abs(cost).toFixed(2) + '% GDP</b>');
    for(const k in imp || {}){
        if (!imp[k]) continue;
        bits.push((IMP_NAMES[k] || k) + " " + sgn(imp[k], Math.abs(imp[k]) < 1 ? 2 : 1));
    }
    for(const k in ch || {}){
        if (!ch[k]) continue;
        bits.push((IMP_NAMES[k] || k) + " " + sgn(ch[k], Math.abs(ch[k]) < 1 ? 2 : 1));
    }
    const f = Object.entries(fac || {}).filter((e)=>e[1]).sort((a, b)=>b[1] - a[1]);
    const facTxt = f.map((param)=>{
        let [k, v] = param;
        return cap1(k) + " " + sgn(v, 1);
    }).join(", ");
    return '<div class="eff">' + bits.join(" &middot; ") + '</div>' + (facTxt ? '<div class="eff" style="color:var(--ink-faint)">' + facTxt + '</div>' : '');
}
/** Career mode: signed qualitative effects without exact foresight numbers. */ function qualEffects(imp, fac, cost, ch) {
    const bits = [];
    if (cost) bits.push('<b>' + (cost > 0 ? "costs GDP" : "saves GDP") + '</b>');
    const pushSigned = (label, v)=>{
        if (!v) return;
        bits.push(label + (v > 0 ? " ↑" : " ↓"));
    };
    for(const k in imp || {})pushSigned(IMP_NAMES[k] || k, imp[k]);
    for(const k in ch || {})pushSigned(IMP_NAMES[k] || k, ch[k]);
    const f = Object.entries(fac || {}).filter((e)=>e[1]).sort((a, b)=>Math.abs(b[1]) - Math.abs(a[1]));
    const facTxt = f.slice(0, 3).map((param)=>{
        let [k, v] = param;
        return cap1(k) + (v > 0 ? " ↑" : " ↓");
    }).join(", ");
    return '<div class="eff">' + bits.join(" &middot; ") + '</div>' + (facTxt ? '<div class="eff" style="color:var(--ink-faint)">' + facTxt + '</div>' : '');
...
function impactChips(im) {
    return '<div class="impchips">' + IMPACT_ROWS.map((r)=>{
        const v = im.head[r.k];
        if (Math.abs(v) < 0.005) return '';
        const better = r.good === "up" ? v > 0 : v < 0;
        return '<span class="' + (better ? "up" : "dn") + '">' + r.name + ' ' + sgn(v, r.dp) + '</span>';
    }).filter(Boolean).join("") + '</div>';
}
function impactFactions(im) {
    const f = Object.entries(im.fac).sort((a, b)=>b[1] - a[1]).filter((e)=>Math.abs(e[1]) > 0.05);
    if (!f.length) return '<div class="impfac">No faction moves enough to notice.</div>';
    const best = f[0], worst = f[f.length - 1];
    return '<div class="impfac">' + (best[1] > 0 ? '<span class="up">' + cap1(best[0]) + ' ' + sgn(best[1], 1) + '</span>' : '') + (worst[1] < 0 ? '<span class="dn">' + cap1(worst[0]) + ' ' + sgn(worst[1], 1) + '</span>' : '') + (Math.abs(im.gini) > 0.05 ? '<span class="' + (im.gini < 0 ? "up" : "dn") + '">Inequality ' + sgn(im.gini, 2) + '</span>' : '') + '</div>';
}
...
function previewOption(opt, base) {
    const snap = {};
    MUTABLE.forEach((k)=>{
        snap[k] = G[k] === undefined ? undefined : clone(G[k]);
    });
    let im = null;
    try {
        applyEventOption(opt);
...
/* Running total for whatever is staged — shown in the bill drawer only.
   Cheap: one baseline plus one variant simulation. */ function impactStripHtml() {
    ...
    if (!cl.length) return '';
    if (!G.sandbox) {
        /* Career: qualitative signed summary from authored effects, not exact sims. */ const bits = cl.slice(0, 6).map((c)=>esc(c.label)).join(" · ");
        return '<div class="impstrip"><div class="impstrip-h">Staged: ' + cl.length + ' change' + (cl.length === 1 ? '' : 's') + ' &middot; ' + billCost() + ' capital</div>' + '<div class="eff">' + bits + '</div>' + '<div class="hint">Career mode shows the direction of travel. Turn on Sandbox for exact four-quarter forecasts.</div></div>';
    }
    let im;
    try {
    ...
    return '<div class="impstrip"><div class="impstrip-h">Staged: ' + cl.length + ' change' + (cl.length === 1 ? '' : 's') + ' &middot; ' + billCost() + ' capital</div>' + impactChips(im) + impactFactions(im) + '<div class="hint">Trend growth is annualised potential growth; four-quarter moves are small but compound. Growth is cumulative outturn GDP.</div></div>';
}
/* Every clause on its own, four quarters out, against passing nothing. */ function impactPanelHtml(cl) {
    if (!cl.length) return '';
    const base = simulate(G.law, 4);
    let h = '<div class="eyebrow mt">What each clause does on its own</div>' + '<div class="hint">Sandbox only. Each block is that clause alone over four quarters, measured against passing nothing at all. ' + 'Growth is cumulative GDP over the path, not the final quarter\'s pace. ' + 'Trend growth and Potential are the supply-side long-run score. ' + 'They will not add up to the whole bill, because these things interact.</div>';
    cl.forEach((c, i)=>{
        const im = impactOf(lawWithOnlyClause(i), base, 4);
        h += '<div class="impblock"><div class="imphead">' + esc(c.label) + '<span class="cost">' + c.pc + ' cap</span></div>' + (Math.abs(im.revenue) > 0.005 ? '<div class="impfac"><span class="' + (im.revenue > 0 ? "up" : "dn") + '">Receipts on impact ' + sgn(im.revenue, 2) + ' pts</span></div>' : '') + impactChips(im) + impactFactions(im) + '</div>';
    });
    if (cl.length > 1) {
        const whole = impactOf(clone(G.draft), base, 4);
        h += '<div class="impblock whole"><div class="imphead">The bill as a whole' + '<span class="cost">' + billCost() + ' cap</span></div>' + impactChips(whole) + impactFactions(whole) + '</div>';
    }
    return h;
}
function showBlocFoundModal() {
    ...
    const renderModal = ()=>{
        const tpl = CUSTOM_BLOC_TEMPLATES[selected];
        bodyEl.innerHTML = '<div style="margin-bottom:12px"><label style="display:block;font-size:12px;opacity:0.7;margin-bottom:4px">Bloc name</label>' +
            '<input id="blocFoundName" type="text" value="' + esc(tpl ? tpl.name : "") + '" maxlength="34" style="width:100%;font:inherit;padding:8px 12px;border-radius:8px;border:1px solid var(--ink-faint);background:var(--glass-bg,rgba(255,255,255,0.06));color:inherit" aria-label="Name of your trade bloc"/></div>' +
            '<div style="display:flex;flex-direction:column;gap:8px">' +
            Object.keys(CUSTOM_BLOC_TEMPLATES).map((tid)=>{
                const t = CUSTOM_BLOC_TEMPLATES[tid];
                const sel = tid === selected;
                return '<button type="button" class="bloc-type-opt' + (sel ? " selected" : "") + '" data-bloc-type="' + tid + '" style="text-align:left;padding:10px 14px;border-radius:10px;border:2px solid ' + (sel ? "var(--blue)" : "var(--ink-faint)") + ';background:' + (sel ? "rgba(10,132,255,0.1)" : "transparent") + ';cursor:pointer;color:inherit">' +
                    '<div style="font-weight:600;font-size:14px">' + esc(t.name) + ' <span style="opacity:0.6;font-weight:400">' + t.pc + ' capital</span></div>' +
                    '<div style="font-size:12px;opacity:0.7;margin-top:2px">' + (t.type === "customs_union" ? "Zero internal tariffs · common external tariff · stronger access bonus (+12%)" : "Preferential rates · members keep independent tariffs · moderate access bonus (+5%)") + '</div>' +
                    '</button>';
            }).join("") +
            '</div>';
        bodyEl.querySelectorAll("[data-bloc-type]").forEach((b)=>{
            b.onclick = ()=>{
                const nameInput = bodyEl.querySelector("#blocFoundName");
                const curName = nameInput ? nameInput.value.trim() : "";
                const prevTpl = CUSTOM_BLOC_TEMPLATES[selected];
                selected = b.dataset.blocType;
                renderModal();
                const newInput = bodyEl.querySelector("#blocFoundName");
                if (newInput && curName && curName !== (prevTpl ? prevTpl.name : "")) {
    ...
    confirmBtn.innerHTML = "<b>Found this bloc</b><em>Staged in your bill — Deliver to create</em>";
    confirmBtn.onclick = ()=>{
        const nameInput = bodyEl.querySelector("#blocFoundName");
...
function showBlocInviteModal(bid) {
    const scrim = $("scrim");
    const titleEl = $("dpTitle"), stampEl = $("dpStamp"), bodyEl = $("dpBody"), box = $("dpOpts");
    if (!titleEl || !stampEl || !bodyEl || !box) return;
    const bloc = blocById(bid) || G.customBlocs[bid];
    ...
    const syncConfirm = ()=>{
        ...
        confirmBtn.innerHTML = selected ? (selected.staged ? "<b>Already staged</b><em>See the bill panel above</em>" : "<b>Propose " + esc(selected.partner.name) + "</b><em>12 capital · staged in your bill</em>") : "<b>No candidate selected</b>";
    };
    confirmBtn.onclick = ()=>{
    ...
    const renderModal = ()=>{
        ...
        let listHtml = '<div class="hint" style="margin-bottom:10px">Choose a partner to invite. Every other bloc member must approve. The invitation stages in your bill — use <b>Deliver</b> to send it (12 capital).</div>';
        listHtml += '<div class="bloc-invite-list">';
        for (const r of regionOrder){
            const group = byRegion[r];
            if (!group || !group.length) continue;
            listHtml += '<div class="bloc-invite-region"><div class="eyebrow">' + esc(COUNTRY_REGIONS[r] || r) + '</div>';
            for (const c of group){
                const p = c.partner;
                const sel = selected && selected.partner.id === p.id;
                const relClass = c.rel > 60 ? "good" : c.rel < 38 ? "bad" : "";
                const status = c.staged ? "Staged in bill" : c.blockers.length ? c.blockers[0] : "Eligible";
                const statusCol = c.staged ? "var(--blue)" : c.blockers.length ? "var(--red)" : "var(--green)";
                listHtml += '<button type="button" class="bloc-invite-opt' + (sel ? " selected" : "") + '" data-invite-pick="' + p.id + '">' +
                    '<div class="bloc-invite-opt-head"><span class="bloc-invite-opt-name">' + esc(p.name) + '</span>' +
                    '<span class="bloc-invite-opt-rel"><span class="fb"><i class="' + relClass + '" style="width:' + c.rel.toFixed(0) + '%"></i></span>' +
                    '<span class="fv">' + c.rel.toFixed(0) + '</span></span></div>' +
                    '<div class="bloc-invite-opt-status" style="color:' + statusCol + '">' + esc(status) + '</div></button>';
            }
            listHtml += '</div>';
        }
        listHtml += '</div>';
        if (selected) {
            ...
            listHtml += '<div class="eyebrow">Preview: ' + esc(selected.partner.name) + '</div>';
            listHtml += accessionPipelineSteps(0);
            listHtml += inviteMemberTable(bid, cid);
            if (blockers.length) listHtml += '<div class="eff" style="color:var(--red);display:block;margin-top:6px">' + esc(blockers.join(" · ")) + '</div>';
            else listHtml += '<div class="eff" style="color:var(--green);display:block;margin-top:6px">Ready to stage — every member approves.</div>';
            listHtml += '</div>';
        } else if (!candidates.length) {
            listHtml += '<div class="hint" style="margin-top:10px">No eligible partners right now. Countries already in a bloc, with a pending invite, or mid-accession cannot be invited.</div>';
        }
        bodyEl.innerHTML = listHtml;
        bodyEl.querySelectorAll("[data-invite-pick]").forEach((b)=>{
            b.onclick = ()=>{
                const pick = candidates.find((c)=>c.partner.id === b.dataset.invitePick);
                if (pick) {
                    selected = pick;
                    renderModal();
                }
...
function projectionModal(onConfirm) {
    syncServiceHolds(G.draft, G.econ);
    const cl = billClauses(), cost = billCost();
    const truth = project(4);
    const p = withForecastError(truth);
    const warn = projectionWarnings(truth);
    const rows = p.rows.map((r)=>'<tr><td>' + r.label + '</td>' + '<td class="' + (r.growth > 0 ? "pos" : "neg") + '">' + fmt(r.growth, 1) + '</td>' + '<td>' + r.inflation.toFixed(1) + '</td><td>' + r.unemployment.toFixed(1) + '</td>' + '<td class="' + (r.balance >= 0 ? "pos" : "neg") + '">' + fmt(r.balance, 1) + '</td>' + '<td>' + r.debt.toFixed(0) + '</td><td>' + r.yield.toFixed(2) + '</td><td>' + r.approval.toFixed(0) + '</td></tr>').join("");
    const cur = balanceOf(G.law, G.econ).balance, draftBal = balanceOf(G.draft, G.econ).balance;
    const body = '<p style="color:var(--ink-soft);font-size:13px">Independent four-quarter forecast on the assumption this bill passes unchanged and nothing else happens. Something else always happens.</p>' + '<div class="proj"><div class="proj-scroll"><table><thead><tr><th>Quarter</th><th>Growth</th><th>Infl.</th><th>Unemp.</th><th>Balance</th><th>Debt</th><th>Yield</th><th>Appr.</th></tr></thead><tbody>' + rows + '</tbody></table></div></div>' + '<div style="margin-top:12px;font-family:var(--mono);font-size:11.5px;color:var(--ink-soft)">' + 'Balance on delivery ' + fmt(draftBal, 1) + '% of GDP (' + sgn(draftBal - cur, 1) + ' versus current law) &middot; ' + cl.length + ' clause' + (cl.length === 1 ? "" : "s") + ' &middot; ' + cost + ' of ' + Math.round(G.capital) + ' capital' + '</div>' + '<ul class="warnlist">' + warn.map((w)=>'<li>' + esc(w) + '</li>').join("") + '</ul>' + '<div class="hint" style="margin-top:12px">' + (p.exact ? 'Sandbox: this forecast is exact. It is the model itself, so the outturn will match it unless something else happens.' : 'This is a forecast, not a promise. Independent forecasts of borrowing miss by about a point of GDP a year out, ' + 'and by more on growth. The figures above carry an error of that size, so the outturn will not match them. ' + 'Turn on sandbox in the bill panel for an exact one.') + '</div>';
    const confirm = typeof onConfirm === "function" ? onConfirm : ()=>enact();
    despatch(cl.length ? "Forecast for this bill" : "Forecast on current policy", "Office for Budget Responsibility", body, [
        {
            b: cl.length ? "Confirm and deliver" : "Proceed to next quarter",
            e: cl.length ? "Enact every clause and move to the next quarter" : "Advance with no new legislation",
            f: confirm
        },
        {
            b: "Go back",
...
function applyDraftMissions(law, draft, econ, fac) {
    ...
    for(const pid in missions){
        ...
        if (m.id === "sanctionsPosture" && typeof G !== "undefined" && G && isVisitActive(G, pid)) {
            continue;
        }
        econ.missionCd[pid] = MISSION_CD;
        if (m.fac && fac) {
            for(const f in m.fac)fac[f] = clamp((fac[f] || 0) + m.fac[f], 2, 96);
        }
        if (m.id === "summit") {
        ...
        if (m.uncertainty) econ.uncertainty = (econ.uncertainty || 0) + m.uncertainty;
        if (m.id === "demarche") {
            addDiploLedger(gBag, pid, { id: "mission_demarche_" + (gBag.q || 0), label: "Formal protest", pts: -3 });
        } else if (m.id === "sanctionsPosture") {
            addDiploLedger(gBag, pid, { id: "mission_sanctions_" + (gBag.q || 0), label: "Restrictive measures", pts: -10 });
...
function enact() {
    ...
    if (G.draft.blocAccession && blocJoinBlockers(G.draft.blocAccession.blocId, G.draft.blocAccession.phase).length) return;
    if (G.draft.blocExternalDeal) {
        const bed = G.draft.blocExternalDeal;
    ...
    const balDelta = balanceOf(G.draft, G.econ).balance - balanceOf(G.law, G.econ).balance;
    G.capital -= cost;
    G.prevLaw = clone(G.law);
    ...
    /* Slider edits spend; hold must match before step() enforces the standard. */ syncServiceHolds(G.draft, G.econ);
    ...
    /* Measure the bill against holding still before any event shocks land. */ let billImpact = null;
    if (cl.length) {
        try {
    ...
    const proceed = (chosen)=>{
        /* Prefer the bill score; add the Cabinet choice only if there was no bill. */ G.briefImpact = null;
        if (billImpact) {
        ...
        if (G.draft.missions) G.draft.missions = {};
        writeBriefing(res);
        const freshUltOutcomes = (G.diploAlerts || []).filter((a) => a.q === G.q);
        ...
        Promise.resolve(announceQuarterAdvance()).then(() => {
            ...
            if (electionDue) termReview();
                ...
                /* Morning note after bills and Cabinet choices alike — impact lives here. */ despatch("Morning note", morningNoteStamp(), briefingHtml(G.brief), [
                    {
                        b: "Continue",
    ...
    const presentChoice = (ev, isMajor)=>{
        G.lastEventQ = G.q;
        G.lastEventId = ev.id;
        despatch(ev.title, ev.stamp, "<p>" + ev.text + "</p>", ev.opts.map((o)=>({
                b: o.b,
                e: o.e,
                previewTarget: o,
                f: ()=>{
...
/* Module bridge for React */ let _onState = null;
function setOnState(fn) {
    _onState = fn;
}
function bump() {
...
/* Opening / restart country picker. Tests and the reference HTML keep calling
   newGame() directly; the Next shell registers a setup overlay instead. */ let _onSetup = null;
function setOnSetup(fn) {
    _onSetup = fn;
}
...
function setOnTabChange(fn) {
    _onTabChange = fn;
}
function setTab(t, scrollPartner) {
    const prev = tab;
    tab = t;
...
export {
  FACTIONS, DEPTS, OTHER_SPEND, OTHER_REV, TERM_LEN, POLITY, POLITY_IDS, REL_POLITY,
  UN_P5, UN_SEAT_QUARTERS, isP5, isUnMember, onSecurityCouncil, unSeatQuartersLeft,
  ...
  LEDGER_DECAY, LEDGER_FLOOR, ENVOY_SLOTS, ENVOY_TARGET, ENVOY_ASSIGN_PC, ENVOY_RECALL_PC, ULTIMATUM_PC, ULTIMATUM_CD, ULTIMATUM_JITTER,
  DIPLO_POLICY_KEYS, DIPLO_SPHERES, DIPLO_CAMPS,
  ...
  countryBlocId, blocMembers, joinBloc, leaveBloc, createCustomBloc, inviteToBloc, finalizeBlocJoin, playerCountryId,
  stepCountry, initWorldState, syncNationsFromWorld, mirrorPlayerToWorld, stepWorldPartners, refreshWorldTrade,
  applyAiFiscalRule, resolveLockstepQuarter, exportGameSnapshot, hydrateGameSnapshot,
  seedMpPolitics, computeBillCost, validateMpSubmission, enactHumanSeatOnSnapshot,
  showMpBriefing, applyMpEventChoice, applyMpDiploAction, applyMpDiploPatch,
  announceQuarterAdvance, lockMpSubmission, clearMpLockedSubmission,
  snapshotMpDiploUi, restoreMpDiploUi, applyLocalMpDiploAction,
  canJoinBloc, blocJoinBlockers, blocMemberApprovals, blocInviteMemberApprovals, blocInviteBlockers, blocExternalDealBlockers, needBlockers, simulateFromDraft,
  BLOC_TEMPLATES, CUSTOM_BLOC_TEMPLATES, pickEventPartner, partnerById, prepareEvent,
  applyDraftMissions, ensureDiploStocks,
  addDiploLedger, setSanctionStance, clearSanctionStance, seedRelBase,
  relationModifiers, relationTarget, assignEnvoy, recallEnvoy, issueUltimatum,
  processUltimatums, spherePatronsForPartner, sphereRiskHint, canIssueUltimatum,
  applySphereTrespassOnDeal, emptyEnvoys, hasFormalProtest,
  ultimatumDemandsFor, baseP, concedeP, partnerSelfInterest, interestTag, partnerTariffOnPlayer,
  ultimatumOutcomeImpacts,
  rollMissionEvent, applyMissionEventOption, MISSION_EVENTS, queueSummitVisitEvents,
  VISIT_DURATION, beginActiveVisit, visitQuartersLeft, isVisitActive, diploMapMarkers,
  pendingUltimatumIds, ultimatumQuartersLeft, hasDiploAttention, diploHudHtml,
  gdp0ForSeat, realmGdpBn, fmtGdpBn, worldDemandBn, refreshWorldY, restGdp0ForRole,
  currencyForSeat, fxDisplayIndex, ensureNationFx,
  clearOpeningCache, openingFac, openingRel, REL_0, FAC_0,
  MUTABLE, IMPACT_ROWS, IMP_NAMES,
  clamp, baseLaw, lawForRole, realmLawKey, newGame, aggregate, revenue, spending, balanceOf,
  pinsForRole,
  ...
  spendForScore, syncServiceHolds, welfareCost, welfareAdequacy, taxAvailable,
  ...
  polityOf, polityIdOf, profilePolityId, polityAffinity, polityReachable, polityCanReach,
  polityChangePc, politySteps, POLITY_LEAP_EXTRA, polityShockFac, polityShiftAge, termLenOf, coupMetric, reviewNoun, reviewStamp, careerHint,
  electionQuartersLeft, electionThermometer, termReviewScore, termReviewDue, termReview, qualEffects,
  ...
  approvalOf, grade, gameOver, election, checkCrises, rollEvent, rollMajorEvent, writeBriefing,
  briefingImpactLines, mergeBriefingImpact, briefingImpactParts, briefingHtml,
  beginEpisode, endEpisode, scheduleNextMajorQ, raiseTradeWarTariffs, snapshotTariffSchedules,
  globalModsForAiSeat, WORLD_TFP_SPILL, MAJOR_GAP_MIN, MAJOR_GAP_SPAN,
  refreshWorldYRel, liveYRel,
  ...
  setOnState, bump, setOnSetup, requestSetup, setTab, getTab, setOnTabChange, getG,
