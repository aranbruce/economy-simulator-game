...
function resolveLockstepQuarter(g, humanSeatIds, submissions) {
    ...
    try {
        ...
        refreshWorldTrade(g);
        g.q = (g.q || 0) + 1;
        if (g.q > 0 && g.q % TERM_LEN === 0) g.term = (g.term || 1) + 1;
...
function newGame(opts) {
    ...
    despatch("Morning note", "Permanent Secretary", briefingHtml(G.brief) + '<p class="hint" style="margin-top:10px">' + (G.sandbox ? 'Sandbox is on: you cannot be removed from office. Switch to Career any time from the top bar.' : 'Career mode: lose an election, a party coup, or the bond market and it is over. Sandbox is one click away in the top bar.') + '</p>' + (!G.coachDone ? '<p class="hint">Try staging one change in Budget, Taxes or Policies, open the forecast with Deliver, then confirm.</p>' : ''), [
        {
            b: "To work",
...
/* ==================================================================
   ...
   ================================================================== */ function step(g, law, prevLaw, det) {
    ...
    /* Base regen plus a mild approval bonus. Tuned so a mid-term chancellor can
     stage a few medium bills without living permanently at the capital floor. */ g.capital = clamp(g.capital + 3.2 + (appr - 45) * 0.10, 0, 100);
...
function projectionWarnings(p) {
    ...
    if (last.approval < 32) w.push("Approval falls to " + last.approval.toFixed(0) + "%. Your parliamentary party will notice.");
...
/* --- the top bar --- */ /* `kind` marks a reading the Chancellor does not control, which is worth making
   ...
   it. */ /** Quarters remaining until the next scheduled election. */ function electionQuartersLeft() {
    if (!G || G.over) return 0;
    const into = G.q % TERM_LEN;
    return into === 0 && G.q > 0 ? TERM_LEN : TERM_LEN - into;
}
/** Rough running score the election() formula will use — for the thermometer. */ function electionThermometer() {
    const e = G.econ;
...
function renderChrome() {
    ...
    if (tb) {
        ...
        const electHint = left <= 4 ? " · election score ~" + therm.toFixed(0) + (therm <= 44 ? " (at risk)" : "") : "";
        tb.textContent = [
            "First",
            "Second",
            "Third",
            ...
            "Fifth",
            "Sixth"
        ][Math.min(G.term - 1, 5)] + " term \u00b7 " + qLabel(G, G.q) + " \u00b7 election in " + left + "q" + electHint;
    }
    const mode = $("tbMode");
    if (mode) {
...
function clausesIn(tabId, cl) {
    const pats = {
        ...
        society: /Cannabis|Psychedelics|Gambling|Alcohol|Tobacco|Adult services/,
...
function renderPanel() {
    ...
    if (tab === "bill") {
        ...
        }).join("") + '</div>' + '<div class="eyebrow mt">What is legal, and on what terms</div>' + '<div class="hint">Prohibition pushes a market underground: no receipts, more crime. Legalising creates a tax base you can then set a duty on, over on the Taxes tab.</div>' + '<div class="cards">' + VICE.map((v)=>{
...
const EVENTS = [
    ...
    },
    {
        ...
        w: 10,
        stamp: "Foreign & Commonwealth",
        title: "A partner changes government",
        text: "An election in {P} has produced a government with very different views about {C}. Everything previously agreed is now, in their words, open for discussion.",
...
function writeBriefing(res) {
    ...
    const appr = approvalOf(G.fac);
    if (G.lowRun >= 2 || appr < 22) b.unshift("Letters are going in. Four quarters below twenty percent and your own side will remove you.");
    ...
    if (left <= 4 && left > 0) {
        const therm = electionThermometer();
        b.push(therm <= 44 ? "Election in " + left + " quarters. On today's numbers you would lose." : "Election in " + left + " quarters. The running score is still just about enough.");
    }
...
const BRIEF_IMPACT_PIN = [
    "growth",
    "inflation"
];
const BRIEF_IMPACT_ORDER = [
    "growth",
    "inflation",
    "balance",
    "debt",
...
const BRIEF_IMPACT_MAX = 4;
function pickBriefingImpactRows(head) {
    const all = IMPACT_ROWS.map((r)=>({
            r,
            v: head[r.k] || 0
        })).filter((x)=>Math.abs(x.v) >= BRIEF_IMPACT_MIN);
    const picked = [], seen = new Set();
    BRIEF_IMPACT_PIN.forEach((k)=>{
    ...
    all.filter((x)=>!seen.has(x.r.k)).sort((a, b)=>Math.abs(b.v) - Math.abs(a.v)).forEach((x)=>{
        if (picked.length >= BRIEF_IMPACT_MAX) return;
        picked.push(x);
        seen.add(x.r.k);
    });
    const rank = (k)=>{
        const i = BRIEF_IMPACT_ORDER.indexOf(k);
        return i === -1 ? 99 : i;
    };
...
function briefingImpactClause(row, v) {
    ...
    switch(row.k){
        ...
        case "balance":
            return up ? "the books tighter by about " + n + " points of GDP" : "borrowing wider by about " + n + " points of GDP";
        case "debt":
...
function capBriefingSentence(body, kind) {
    const tail = kind === "decision" ? ", compared with doing nothing." : ", compared with standing still.";
...
function briefingImpactLines(im, meta) {
    ...
    if (!rows.length && !facs.length) {
        ...
        if (kind === "decision") {
            return [
                "This barely moves the dial on a year-ahead view."
            ];
        }
        return [];
    }
    const lines = [];
    if (rows.length) {
        const body = joinBriefingClauses(rows.map((x)=>briefingImpactClause(x.r, x.v)));
        lines.push(capBriefingSentence(body, kind));
    }
    const facLine = briefingFactionLine(facs);
...
function classifyBriefLines(brief) {
    const impact = [], politics = [], outturn = [];
    (brief || []).forEach((line)=>{
        if (/compared with (standing still|doing nothing)|barely moves the dial/i.test(line)) impact.push(line);
        else if (/pushback from|will take this better|are with you|should be on side|Politically,/i.test(line)) politics.push(line);
        else outturn.push(line);
    });
    return {
        impact,
        politics,
...
function briefingHtml(brief, opts) {
    const o = opts || {};
    const parts = classifyBriefLines(brief);
    let h = '<div class="briefing">';
    if (parts.impact.length) {
        h += '<section class="briefing-impact"><div class="briefing-label">Year-ahead view</div>';
        parts.impact.forEach((t)=>h += '<p class="briefing-impact-line">' + esc(t) + '</p>');
        h += '</section>';
    }
    if (parts.politics.length) {
        h += '<p class="briefing-politics">' + esc(parts.politics.join(" ")) + '</p>';
    }
    if (parts.outturn.length) {
        h += '<section class="briefing-outturn' + (parts.impact.length ? "" : " briefing-outturn--solo") + '">';
        if (parts.impact.length) h += '<div class="briefing-label">This quarter</div>';
        parts.outturn.forEach((t)=>h += '<p>' + esc(t) + '</p>');
        h += '</section>';
    }
...
function checkCrises(res) {
    ...
    const appr = approvalOf(G.fac);
    G.lowRun = appr < 20 ? G.lowRun + 1 : 0;
    if (G.lowRun >= 4) return gameOver("E", "Your own side removed you", "Four straight quarters below twenty percent. The letters went in, the meeting was short, and you were offered a peerage on the way out.");
}
function grade() {
    const e = G.econ;
    let s = 0;
...
function election() {
    ...
    const e = G.econ;
    const recent = G.log.slice(-8);
    const record = recent.reduce((a, r)=>a + r.growth, 0) / recent.length * 2.5 - Math.max(0, e.inflation - 3) * 1.5 - (e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 1.5 + (e.services - (e.services0 != null ? e.services0 : 55)) * 0.15;
    const score = approvalOf(G.fac) + record;
    if (score <= 44 && !G.sandbox) return gameOver(grade(), "You lost the election", "The country had a view and expressed it. Your successor inherits debt at " + e.debt.toFixed(0) + "% of GDP, inflation at " + e.inflation.toFixed(1) + "% and a note in the desk drawer you had better hope was polite.");
    G.term++;
    G.capital = clamp(G.capital + 22, 0, 100);
    const held = score <= 44;
    despatch(held ? "Returned on a technicality" : "Returned to office", "General election", (held ? "<p>On these numbers the country would have thrown you out. Sandbox mode is on, so you are returned anyway, with a majority you have not earned.</p>" : "<p>The government is returned with a working majority. You keep the red box, the debt and the demographic arithmetic.</p>") + "<p>A fresh mandate is worth real political capital. Spend it on something that outlasts you.</p>", [
        {
            b: "Continue",
...
function enact() {
    ...
    const proceed = (chosen)=>{
        /* Prefer the bill score; add the Cabinet choice only if there was no bill. */ const impactLines = [];
        if (billImpact) {
            briefingImpactLines(billImpact, {
                kind: "bill",
                clauses: cl
            }).forEach((line)=>impactLines.push(line));
        } else if (chosen && chosen.impact) {
            briefingImpactLines(chosen.impact, {
                kind: "decision"
            }).forEach((line)=>impactLines.push(line));
        }
        ...
        checkCrises(res);
        G.brief = mergeBriefingImpact(G.brief, impactLines);
        ...
        render();
        const electionDue = !G.over && G.q > 0 && G.q % TERM_LEN === 0;
        if (electionDue) election();
            ...
            /* Morning note after bills and Cabinet choices alike — impact lives here. */ despatch("Morning note", "Permanent Secretary", briefingHtml(G.brief), [
                {
                    b: "Continue",
                    e: electionQuartersLeft() + " quarters to the election",
                    f: ()=>render()
                }
            ]);
        }
...
export {
  ...
  briefingImpactLines, mergeBriefingImpact, briefingHtml,
