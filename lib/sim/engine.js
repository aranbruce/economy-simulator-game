...
function rateImpactHtml() {
    ...
    return '<div class="impblock" style="margin-top:10px;padding:10px 11px 0;border-top:1px solid rgba(0,0,0,.06)">' + '<div class="imphead">Four quarters at ' + im.pin.toFixed(2) + '% vs the Bank' + '<span class="cost">free</span></div>' + '<div class="hint" style="margin:0 0 6px">Same law, pinned rate against the Taylor rule from today\'s starting point.</div>' + (empty ? '<div class="impfac">No measurable move over four quarters.</div>' : chips + impactFactions(im)) + '</div>';
}
