...
/** Light automatic fiscal rule for AI seats — keep books from exploding.
 *  High-debt openers (Japan etc.) consolidate earlier and harder so they
 *  stagnate rather than collapse into the debt clamp. */ function applyAiFiscalRule(law, econ) {
    ...
    const debtStress = debt > 220 ? 2.2 : debt > 180 ? 1.5 : debt > 140 ? 1.15 : 1;
    const defTrigger = debt > 200 ? 2.5 : debt > 160 ? 4.5 : 7;
    if (def > defTrigger || debt > 180 && def > 1.5) {
        const cut = 0.08 * debtStress;
        for (const d of DEPTS){
            if (law.spend[d.id] > d.min + 0.15) law.spend[d.id] = Math.max(d.min, law.spend[d.id] - cut);
        }
        if (law.taxes && law.taxes.vat && law.taxes.vat.on && law.taxes.vat.rate < 28) {
            law.taxes.vat.rate = Math.min(28, law.taxes.vat.rate + 0.25 * Math.min(2.2, debtStress));
        }
        if (debt > 200 && law.income) law.income.uprate = false;
    } else if (def < -1 && econ.debt < 55) {
        for (const d of DEPTS){
            if (d.id === "health" || d.id === "education") {
                if (law.spend[d.id] < d.max - 0.1) law.spend[d.id] += 0.04;
            }
        }
    }
}
 ...
 *  seat's inflation instead of resetting to a UK pin every quarter. */ function pinOpeningHeadlines(e, law, pins, statutePin, opts) {
    ...
    const keepEff = freeRate ? e.effInterest : pin.effInterest;
...
/* ---- Trade agreements take years, and then keep giving ----
   ...
   `tariffCutEff` carry the phase-in; `TRADE_TFP` carries the dynamic gain. */ const DEAL_PHASE = 0.055, TRADE_TFP = 0.020;
...
/* One quarter of the expenditure side. Every component is a level indexed so
   ...
   all at once. */ function expenditureStep(e, law, E, revShare, modSpend, g, det) {
    ...
    const userCost = Math.max(1.2, realRate + DEPREC + corpRate * CORP_WEDGE + (E.ucost || 0) + (e.modUcost || 0) + (e.creditSpread || 0) * CREDIT_I_PASS + e.uncertainty * 0.08);
...
/* ==================================================================
   ...
   ================================================================== */ function step(g, law, prevLaw, det) {
    ...
    /* A creditor state pays no risk premium; it earns a return roughly in line
     with world rates on what it holds. */     if (e.debt < 0) {
        e.yield = clamp(e.rate + 0.3, 0.2, 25);
        e.effInterest += (e.yield - e.effInterest) * MATURITY_PASS;
        e.riskPremium = 0;
    } else {
        /* AI seats price debt risk off their opening anchor (Japan at 250% is
         not immediately a gilt crisis). The player still faces absolute debt. */ const anchor = e.debtAnchor != null ? e.debtAnchor : 80;
        const debtForRisk = g._aiSeat ? 80 + Math.max(0, e.debt - anchor) : e.debt;
    ...
    /* ---- Public services: volume against need, not an assumed decay ----
     ...
     how its own base has moved rather than by output. */ const gapPct = e.gdp / e.potential - 1;
    e.baseConsumption = e.C / SHARE_C;
    e.baseWages = e.gdp / 100 * ((e.realWage || 1) / (e.pubProd || 1) > 0 ? 1 : 1) * (e.participation || 1) * (1 - (e.unemployment - U_STAR) / 100 * 1.4);
    ...
    e.baseProfitsRaw = e.gdp / 100 * e.profitShare * e.profitShift;
    ...
    e.baseAssetsRaw = e.gdp / 100 * e.assetPrice;
