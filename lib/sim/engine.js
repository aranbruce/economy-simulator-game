...
function assignEnvoy(partnerId) {
  ...
  if (G.envoys.includes(partnerId)) return false;
...
const billCost = ()=>billClauses().reduce((a, c)=>a + c.pc, 0);
const isFiscalOnly = (cl)=>cl.every((c)=>!/Ratify|Withdraw|Enact|Repeal|Cannabis|Psychedelics|Gambling|Alcohol|Tobacco|Adult|summit|protest|Restrictive|State visit|Formal protest/i.test(c.label));
/* ==================================================================
   7. RENDERING
...
function renderChrome() {
    ...
    const cl = billClauses(), cost = cl.reduce((a, c)=>a + c.pc, 0), afford = cost <= G.capital;
    const dock = $("dockTabs");
    if (dock) {
    ...
    if (bill) {
        ...
        $("billLabel").textContent = cl.length && !isFiscalOnly(cl) ? "Programme" : "Finance bill";
    ...
    if (dw) {
        ...
        if (open) {
            const meta = tab === "bill" ? {
                name: cl.length && !isFiscalOnly(cl) ? "The Programme" : "The Finance Bill",
...
function renderPanel() {
    ...
    if (tab === "bill") {
        ...
        host.innerHTML = briefingHtml(G.brief, {
            footer: "Morning note \u00b7 Permanent Secretary"
        }) + '<div class="eyebrow">Departmental spending, % of GDP</div>' + '<div class="hint">Standing still is a cut: demand for services rises about one percent a year on its own.</div>' + '<div class="panel" id="deptLevers">' + DEPTS.map((d)=>{
        ...
        host.querySelectorAll("[data-envoy-assign]").forEach((b)=>b.onclick = ()=>{
                if (assignEnvoy(b.dataset.envoyAssign)) render();
            });
        host.querySelectorAll("[data-envoy-recall]").forEach((b)=>b.onclick = ()=>{
                if (recallEnvoy(b.dataset.envoyRecall)) render();
            });
        host.querySelectorAll("[data-ultimatum]").forEach((b)=>b.onclick = ()=>{
                ...
                if (issueUltimatum(pid, demandId)) {
                    render();
                    return;
                }
                const check = canIssueUltimatum(pid);
                const why = (check && check.reasons && check.reasons.length)
                    ? check.reasons.join("; ")
                    : (check && check.cooldown > 0 ? "cooldown " + check.cooldown + "Q" : "cannot issue");
                try {
                    if (typeof alert === "function") alert("Ultimatum not issued: " + why);
                } catch (err) { /* headless */ }
            });
    } else if (tab === "charts") {
        if (G.log.length < 2) {
...
function partnerDiploCardHtml(p) {
    ...
    const badges = [];
    if (visitActive) badges.push('<span class="diplo-badge visit">State visit · ' + visitLeft + 'Q</span>');
    if (envoyOn) badges.push('<span class="diplo-badge envoy">Envoy posted</span>');
    if (stagedM) badges.push('<span class="diplo-badge staged">Mission in bill</span>');
    if (ultPending) badges.push('<span class="diplo-badge ult">Ultimatum live</span>');
    if (cd > 0) badges.push('<span class="diplo-badge cd">Mission cooldown · ' + cd + 'Q</span>');
    ...
    if (envoyOn) {
        ...
        const canAssign = (G.capital || 0) >= ENVOY_ASSIGN_PC;
        envoyRow = '<button type="button" class="btn diplo-envoy" data-envoy-assign="' + p.id + '"' + (canAssign ? "" : " disabled") + ' title="' + (canAssign ? "Spend " + ENVOY_ASSIGN_PC + " capital · +" + ENVOY_TARGET + " relations per quarter" : "Need " + ENVOY_ASSIGN_PC + " capital") + '">Assign envoy · ' + ENVOY_ASSIGN_PC + ' cap</button>';
    }
    let ultBody = "";
    let ultLeft = 0;
    let ultLabel = "";
...
function showMpBriefing() {
    if (!G || !G.mp) return false;
    const seatId = playerCountryId(G.homeRole);
    const pol = G.politics && G.politics[seatId];
    if (pol && pol.pendingEvent) return showMpPendingEvent();
    const res = pol && pol.lastRes;
    if (!res || res.E == null) return false;
    writeBriefing(res);
    ...
    if (!G.brief || !G.brief.length) return false;
    despatch("Morning note", "Permanent Secretary", briefingHtml(G.brief), [
        {
            b: "Continue",
...
function showMpPendingEvent() {
    ...
    if (pending.episodeEnd) {
        const ep = G.episode || pol.episode;
    ...
    if (pending.missionEvent) {
        ...
        const pName = partnerById(pending.partnerId)?.name || pending.partnerId;
        let title = (ev.title || "").replace(/\{P\}/g, pName).replace(/\{R\}/g, ev.rivalName || "a rival");
        title = T(title);
        const body = "<p>" + formatMissionEventText(ev, G, diploDeps()) + "</p>" +
            '<p class="hint">Each choice shifts relations and factions.</p>';
        const ctx = { partnerId: pending.partnerId, rivalId: ev.rivalId, rivalName: ev.rivalName };
...
/* Running total for whatever is staged — shown in the bill drawer only.
   Cheap: one baseline plus one variant simulation. */ function impactStripHtml() {
    ...
    if (!G.sandbox) {
        ...
        return '<div class="impstrip"><div class="impstrip-h">Staged: ' + cl.length + ' change' + (cl.length === 1 ? '' : 's') + ' &middot; ' + cl.reduce((a, c)=>a + c.pc, 0) + ' capital</div>' + '<div class="eff">' + bits + '</div>' + '<div class="hint">Career mode shows the direction of travel. Turn on Sandbox for exact four-quarter forecasts.</div></div>';
    }
    let im;
    try {
    ...
    return '<div class="impstrip"><div class="impstrip-h">Staged: ' + cl.length + ' change' + (cl.length === 1 ? '' : 's') + ' &middot; ' + cl.reduce((a, c)=>a + c.pc, 0) + ' capital</div>' + impactChips(im) + impactFactions(im) + '<div class="hint">Trend growth is annualised potential growth; four-quarter moves are small but compound. Growth is cumulative outturn GDP.</div></div>';
}
/* Every clause on its own, four quarters out, against passing nothing. */ function impactPanelHtml(cl) {
    ...
    if (cl.length > 1) {
        ...
        h += '<div class="impblock whole"><div class="imphead">The bill as a whole' + '<span class="cost">' + cl.reduce((a, c)=>a + c.pc, 0) + ' cap</span></div>' + impactChips(whole) + impactFactions(whole) + '</div>';
    }
    return h;
}
function projectionModal(onConfirm) {
    syncServiceHolds(G.draft, G.econ);
    const cl = billClauses(), cost = cl.reduce((a, c)=>a + c.pc, 0);
...
function enact() {
    ...
    const proceed = (chosen)=>{
        ...
        if (electionDue) termReview();
            ...
            /* Morning note after bills and Cabinet choices alike — impact lives here. */ despatch("Morning note", "Permanent Secretary", briefingHtml(G.brief), [
                {
                    b: "Continue",
...
export {
  ...
  showMpBriefing, applyMpEventChoice,
