...
function blocAccessionSpec(blocId) {
    ...
    return bloc && bloc.accession ? bloc.accession : null;
}
...
/** Why a predefined-bloc accession step may be blocked. */ function blocJoinBlockers(blocId, phase) {
    ...
    if (ph === "apply" && spec && spec.chairRelationMin && bloc.chair) {
        const rel = G.rel[bloc.chair] != null ? G.rel[bloc.chair] : 50;
        if (rel < spec.chairRelationMin) {
            const chair = partnerById(bloc.chair);
            out.push("relations with " + (chair ? chair.name : bloc.chair) + " " + rel.toFixed(0) + " (need " + spec.chairRelationMin + "+)");
        }
    }
    ...
    if (ph === "accede") {
        ...
        if (spec && spec.chairRelationMin && bloc.chair) {
            const rel = G.rel[bloc.chair] != null ? G.rel[bloc.chair] : 50;
            if (rel < spec.chairRelationMin) {
                const chair = partnerById(bloc.chair);
...
export {
  ...
  setOnState, setOnDespatchChange, setOnDespatchShell, getDespatch, closeDespatch, notifyUi, bump, isDeliverLocked, setOnSetup, requestSetup, setTab, getTab, setOnTabChange, getG,
