...
function blocMembershipPanelHtml() {
    ...
    if (bid) {
        ...
        h += '<div class="eyebrow" style="margin-top:8px">Propose a new member</div>';
        ...
        for (const p of activePartners()){
            ...
            var _G_draft_blocInvite;
            const staged = !!((_G_draft_blocInvite = G.draft.blocInvite) === null || _G_draft_blocInvite === void 0 ? void 0 : _G_draft_blocInvite[p.id]);
            const blockers = blocInviteBlockers(bid, p.id);
            const tip = blockers.length ? blockers.join("; ") : staged ? "Remove from bill" : "Stage in bill (12 capital) — then Deliver";
            h += '<button type="button" class="btn tiny' + (staged ? " danger" : "") + '" data-bloc-invite="' + p.id + '" title="' + esc(tip) + '">' + (staged ? "Unstage " : "Propose ") + p.name + '</button>';
        }
        h += '</div>';
    } else {
        const acc = G.blocAccession;
