...
/* Political regime per seat. Fixed for the run — term reviews, capital regen,
   and elite-coup rules branch on this. REGIMES above is tax architecture only. */ const POLITY = {
    democracy: {
    ...
    monarchy: {
        termLen: 40, loseAt: 28, kind: "court", label: "Absolute monarchy",
        capitalRegen: 0.55, coupOn: "patriots", coupFloor: 28, coupQuarters: 3
    }
};
const POLITY_IDS = Object.keys(POLITY);
function polityIdOf(role) {
    ...
    const key = p && p.polity;
    return key && POLITY[key] ? key : "democracy";
}
...
function reviewNoun(meta) {
    const m = meta || polityOf();
    return m.kind === "congress" ? "congress" : m.kind === "court" ? "court review" : m.kind === "managed" ? "ballot" : "election";
}
function reviewStamp(meta) {
    const m = meta || polityOf();
    return m.kind === "congress" ? "Party congress" : m.kind === "court" ? "Court review" : m.kind === "managed" ? "Managed ballot" : "General election";
}
function careerHint(meta, sandbox) {
    ...
    if (m.kind === "court") {
        return "Career mode: keep the court's favour, survive succession pressure, or the bond market ends you. Political capital recovers slowly. Sandbox is one click away in the top bar.";
    }
    if (m.kind === "managed") {
...
/* Copy carries a {C} token so the country can be named by the player. Applied
   ...
   briefing, card blurbs in the drawers, and floating press clippings. */ const T = (str)=>String(str).replace(/\{C\}/g, G && G.country || "The Kingdom").replace(/\{P\}/g, ()=>{
...
function newGame(opts) {
    const o = opts || {};
    const country = typeof o.country === "string" && o.country.trim() ? o.country.trim().slice(0, 34) : "The Kingdom";
...
/* ==================================================================
   ...
   ================================================================== */ function step(g, law, prevLaw, det) {
    ...
    /* Base regen plus a mild approval bonus. Tuned so a mid-term chancellor can
     ...
     Authoritarian / monarchy seats recover capital more slowly — tenure is
...
function projectionWarnings(p) {
    ...
    if (last.approval < 32) {
        ...
        if (meta.coupOn === "patriots") {
            w.push(meta.kind === "court"
                ? "Approval falls to " + last.approval.toFixed(0) + "%. The court will notice if loyalty frays."
                : "Approval falls to " + last.approval.toFixed(0) + "%. The party apparatus will notice if loyalty frays.");
        } else {
...
/* Cold blue through to warm red, which reads the same way in every metric once
   `good` has been used to decide which end is which. */ function ramp(t) {
    t = clamp(t, 0, 1);
    const stops = [
        [
            10,
...
function renderChrome() {
    ...
    if (stats) {
        ...
        const left = electionQuartersLeft();
        const therm = electionThermometer();
        const reviewLabel = polityOf().kind === "congress" ? "Congress" : polityOf().kind === "court" ? "Court" : "Election";
...
const EVENTS = [
    ...
    },
    {
        ...
        text: "The Federated States have given {C} ninety days to drop the digital services tax, or face tariffs on {C}'s largest export categories.",
        cond: ()=>{
    ...
    },
    {
        ...
        title: ()=>{
            ...
            if (meta.kind === "court") return "A partner's palace succession";
        ...
        text: ()=>{
            ...
            if (meta.kind === "court") {
                return "A palace succession in {P} has produced a court with very different views about {C}. Everything previously agreed is now, in their words, open for discussion.";
            }
    ...
    /* ---- Major world episodes (separate 4–8 year schedule) ---- */ {
        ...
        text: "Labs and boardrooms from the Federated States to the Lotus Republic are shipping tools that raise measured output per hour. Foreign demand and the technology frontier are both moving.",
        cond: ()=>true,
        opts: [
            {
...
function writeBriefing(res) {
    ...
    if (G.lowRun >= 2 || metric < meta.coupFloor + 2) {
        if (meta.coupOn === "patriots") {
            b.unshift(meta.kind === "court"
                ? "The court is restless. " + meta.coupQuarters + " quarters below " + meta.coupFloor + " among the loyalists and the seals leave your hands."
                : "The apparatus is restless. " + meta.coupQuarters + " quarters below " + meta.coupFloor + " among the patriots and you are removed.");
        } else {
...
function checkCrises(res) {
    ...
    if (G.lowRun >= meta.coupQuarters) {
        if (meta.kind === "court") {
            return gameOver("E", "The court withdrew support", meta.coupQuarters + " straight quarters below " + meta.coupFloor + " among the loyalists. The meeting was short, and the seals left with someone else.");
        }
        if (meta.coupOn === "patriots") {
...
function termReview() {
    ...
    if (score <= meta.loseAt && !G.sandbox) {
        ...
        if (meta.kind === "court") {
            return gameOver(grade(), "The court moved against you", "Succession pressure broke your way of governing. Your successor inherits debt at " + e.debt.toFixed(0) + "% of GDP, inflation at " + e.inflation.toFixed(1) + "% and a court that has already chosen its favourite.");
        }
    ...
    } else if (meta.kind === "court") {
        title = held ? "Retained on a technicality" : "The court renews you";
        lede = held
            ? "<p>On these numbers the court would have chosen another. Sandbox mode is on, so you keep the seals anyway.</p>"
            : "<p>The court renews its confidence. You keep the seals, the debt and the demographic arithmetic.</p>";
...
export {
  FACTIONS, DEPTS, OTHER_SPEND, OTHER_REV, TERM_LEN, POLITY, POLITY_IDS,
  ...
  polityOf, polityIdOf, termLenOf, coupMetric, reviewNoun, reviewStamp, careerHint,
