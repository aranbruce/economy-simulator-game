        ...
        host.querySelectorAll("[data-bloc-leave]").forEach((b)=>b.onclick = ()=>{
                if (!G.draft.blocLeave) {
                    const ok = typeof window !== "undefined" && typeof window.confirm === "function"
                        ? window.confirm("Stage leaving your trade bloc? Membership will end when you Deliver this quarter.")
                        : true;
                    if (!ok) return;
                }
...
function blocMembershipPanelHtml() {
    ...
    if (bid) {
        ...
        h += '<button type="button" class="btn ' + (G.draft.blocLeave ? "danger" : "") + '" data-bloc-leave="1">' + (G.draft.blocLeave ? "Unstage leave" : "Stage leaving bloc") + '</button>';
