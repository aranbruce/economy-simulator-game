...
/* Permanent Secretary prose for what a bill or Cabinet choice does over four
   quarters against holding still. Same model as the sandbox impact chips, but
   written for the morning note rather than the newspaper. */ const BRIEF_IMPACT_MIN = 0.05;
function briefingImpactPhrase(row, v) {
    const n = Math.abs(v).toFixed(row.dp);
    const up = v > 0;
    switch(row.k){
        case "growth":
            return (up ? "lift growth by about " : "trim growth by about ") + n + " points";
        case "trend":
            return (up ? "raise trend growth by about " : "cut trend growth by about ") + n + " points";
        case "potential":
            return (up ? "raise potential by about " : "cut potential by about ") + n + " points";
        case "balance":
            return (up ? "improve the budget balance by about " : "widen the deficit by about ") + n + " points of GDP";
        case "debt":
            return up ? "add about " + n + " points of GDP to the debt" : "trim debt by about " + n + " points of GDP";
        case "inflation":
            return up ? "add about " + n + " points to inflation" : "take about " + n + " points off inflation";
        case "unemployment":
            return (up ? "raise unemployment by about " : "cut unemployment by about ") + n + " points";
        case "yield":
            return (up ? "push gilt yields up by about " : "ease gilt yields by about ") + n + " points";
        case "approval":
            return (up ? "lift approval by about " : "cut approval by about ") + n + " points";
        case "services":
            return (up ? "lift public services by about " : "cut public services by about ") + n + " points";
        case "fx":
            return (up ? "firm the currency by about " : "soften the currency by about ") + n + " points";
        default:
            return (up ? "raise " : "cut ") + row.name.toLowerCase() + " by about " + n + " points";
    }
}
function briefingImpactLines(im, meta) {
    ...
    if (!rows.length && !facs.length) {
        if (kind === "bill" && meta && meta.clauses && meta.clauses.length) {
            return [
                "Against holding still, the measures you delivered barely move the four-quarter score."
            ];
        }
        if (kind === "decision") {
            return [
                "Against leaving that choice alone, the four-quarter score barely moves."
            ];
        }
        return [];
    }
    const lines = [];
    if (rows.length) {
        const phrases = rows.map((x)=>briefingImpactPhrase(x.r, x.v));
        const joined = phrases.length === 1 ? phrases[0] : phrases.length === 2 ? phrases[0] + ", and " + phrases[1] : phrases[0] + ", " + phrases[1] + ", and " + phrases[2];
        const opener = kind === "decision" ? "Against leaving that choice alone, over four quarters it should " : "Against holding still, over four quarters the measures you delivered should ";
        lines.push(opener + joined + ".");
    }
    if (facs.length) {
        ...
        const bits = [];
        if (warm.length) bits.push(warm.join(" and ") + " warm to it");
        if (cool.length) bits.push(cool.join(" and ") + " do not");
        if (bits.length) lines.push(cap1(bits.join("; ")) + ".");
    }
    return lines.map(T);
}
...
function enact() {
    ...
    const proceed = (chosen)=>{
        ...
        if (electionDue) election();
            ...
            /* Morning note after bills and Cabinet choices alike — impact lives here. */ despatch("Morning note", "Permanent Secretary", '<div class="briefing">' + G.brief.map((t)=>'<p>' + t + '</p>').join("") + '</div>', [
                {
                    b: "Continue",
...
export {
  ...
  briefingImpactLines, mergeBriefingImpact,
