...
function newGame(opts) {
    ...
    despatch("Morning note", "Permanent Secretary", '<div class="briefing">' + G.brief.map((t)=>'<p>' + t + '</p>').join("") + '</div>' + '<p class="hint" style="margin-top:10px">' + careerHint(polityOf(G.homeRole), G.sandbox) + '</p>' + (!G.coachDone ? '<p class="hint">Try staging one change in Budget, Taxes or Policies, open the forecast with Deliver, then confirm.</p>' : ''), [
        {
            b: "To work",
...
function renderPanel() {
    ...
    if (tab === "bill") {
        ...
        host.innerHTML = impactStripHtml() + '<div class="briefing">' + G.brief.map((t)=>'<p>' + t + '</p>').join("") + '<div class="src">Morning note &middot; Permanent Secretary</div></div>' + '<div class="eyebrow">Departmental spending, % of GDP</div>' + '<div class="hint">Standing still is a cut: demand for services rises about one percent a year on its own.</div>' + '<div class="panel" id="deptLevers">' + DEPTS.map((d)=>{
...
function showMpBriefing() {
    ...
    despatch("Morning note", "Permanent Secretary", '<div class="briefing">' + G.brief.map((t)=>'<p>' + t + '</p>').join("") + '</div>', [
        {
            b: "Continue",
...
function enact() {
    ...
    const proceed = (chosen)=>{
        ...
        else if (!G.over && !(chosen && chosen.event) && !(chosen && chosen.episodeEnd) && G.brief && G.brief.length) {
            despatch("Morning note", "Permanent Secretary", '<div class="briefing">' + G.brief.map((t)=>'<p>' + t + '</p>').join("") + '</div>', [
                {
                    b: "Continue",
