...
/* ==================================================================
   ...
   ================================================================== */ function step(g, law, prevLaw, det) {
    ...
    for (const p of activePartners(g.homeRole)){
        let t = 50 + (law.tariff <= 3 ? 2 : -law.tariff * 0.9) + E.trade * 3;
        const impulse = e.relImpulse[p.id] || 0;
        t += impulse;
        let dealBonus = 0;
        for(const id in law.deals){
            if (law.deals[id] && DEAL_BY_ID[id] && DEAL_BY_ID[id].partner === p.id) dealBonus += 9;
        }
        const stressed = (e.dealStress[p.id] || 0) >= 1;
        t += stressed ? 0 : dealBonus;
        /* Partner-keyed trade sentiment on taxes (e.g. digital services → Federated). */ for (const tax of TAXES){
            if (!tax.trade || typeof tax.trade !== "object") continue;
            const st = law.taxes[tax.id];
            if (!st || !st.on) continue;
            const hit = tax.trade[p.id];
            if (hit) t += hit * st.rate;
        }
        t += REL_POLITY * polityAffinity(polityIdOf(g.homeRole), profilePolityId(p.id));
...
function renderChrome() {
    ...
    const dock = $("dockTabs");
    if (dock) {
        dock.innerHTML = TABS.map((t)=>'<button class="dock-btn" data-tab="' + t.id + '" aria-label="' + t.name + '" aria-expanded="' + (t.id === tab) + '">' + (ICONS[t.icon] || "") + '<span>' + t.name + '</span>' + (clausesIn(t.id, cl) ? '<span class="pip"></span>' : '') + '</button>').join("");
...
function partnerTradeCardHtml(p, bilat, bilatTotal) {
    ...
    const missionRow = MISSIONS.map((m)=>{
        ...
        const blocked = !on && cd > 0;
        const tip = m.name + ' · ' + m.pc + ' capital · relations ' + (m.impulse > 0 ? '+' : '') + m.impulse + (cd > 0 ? ' · cooldown ' + cd + 'q' : '');
        return '<button class="btn tiny' + (on ? " danger" : "") + '" data-mission="' + m.id + '" data-partner="' + p.id + '"' + (blocked ? " disabled" : "") + ' title="' + esc(tip) + '">' + (on ? "Unstage " : "") + m.name.split(" / ")[0] + ' (' + m.pc + ')</button>';
    }).join(" ");
    let dealsHtml = "";
...
/* ==================================================================
   ...
   ================================================================== */ function despatch(title, stamp, html, opts, cfg) {
    ...
    opts.forEach((o)=>{
        ...
        b.innerHTML = "<b>" + T(esc(o.b)) + "</b>" + (o.e ? "<em>" + T(esc(o.e)) + "</em>" : "") + extra;
        b.onclick = ()=>{
            const scrim = $("scrim");
    ...
    },
    {
        ...
        opts: [
            ...
            },
            {
                ...
                f: ()=>{
                    const tid = G.eventFocus;
                    if (tid) G.rel[tid] = clamp((G.rel[tid] || 50) + 10, 0, 100);
                    (G.eventSponsors || []).forEach((id)=>{
        ...
        opts: [
            {
                ...
                f: ()=>{
                    ...
                    if (id) G.rel[id] = clamp((G.rel[id] || 50) + 14, 0, 100);
                }
            },
            {
                b: "Wait and see",
            ...
            },
            {
                ...
                f: ()=>{
                    activePartners().forEach((p)=>G.rel[p.id] = clamp(G.rel[p.id] - 6, 0, 100));
                }
            }
        ]
    },
    ...
    },
    {
        ...
        opts: [
            ...
            },
            {
                ...
                f: ()=>{
                    activePartners().forEach((p)=>G.rel[p.id] = clamp(G.rel[p.id] - 10, 0, 100));
                }
            }
        ]
    },
        ...
        opts: [
            {
                ...
                f: ()=>{
                    const id = G.eventFocus;
                    if (id) G.rel[id] = clamp((G.rel[id] || 50) - 20, 0, 100);
                }
            },
            {
                ...
                f: ()=>{
                    const id = G.eventFocus;
                    if (id) G.rel[id] = clamp((G.rel[id] || 50) - 5, 0, 100);
...
function enact() {
    ...
    const proceed = (chosen)=>{
        ...
        if (clips.length) pushPress(clips);
        render();
