...
/* UK 2025-26: total managed expenditure 44.8% of GDP, receipts 40.4%, deficit
   ...
   residual so the opening fiscal position stays put. */ const OTHER_SPEND = 7.7, OTHER_REV = 12.25, TERM_LEN = 20;
...
function isHumanMpSeat(g, seatId) {
  return !!(g && g.politics && seatId && g.politics[seatId]);
}

function parkInboundUltimatum(g, fromId, toId, ult) {
...
function pushDiploAlert(state, alert) {
  if (!state) return;
  ...
  if (key && state.diploAlerts[0] && diploAlertKey(state.diploAlerts[0]) === key) {
    return;
  }
 ...
 */
function mergeDiploAlertsNoted(prevAlerts, nextAlerts) {
  if (!nextAlerts || !nextAlerts.length) return nextAlerts ? clone(nextAlerts) : [];
  const noted = new Set();
...
function diploOutcomeAlertsForBrief(alerts, q) {
  const qNow = q != null ? q : 0;
  return (alerts || []).filter((a) => {
    if (!a || !DIPLO_OUTCOME_KINDS[a.kind] || a.noted) return false;
    const aq = a.q != null ? a.q : qNow;
    return aq === qNow || aq === qNow - 1;
  });
}

function markDiploAlertsNoted(alerts) {
...
function finishOutboundUltimatum(issuerState, targetId, ult, concede) {
  const deps = diploDeps();
  const impacts = ultimatumOutcomeImpacts(issuerState, targetId, ult, !!concede, deps);
  ...
  if (issuerState.brief) issuerState.brief = [note].concat(issuerState.brief).slice(0, 3);
  return impacts;
}
...
function issueUltimatum(partnerId, demandId) {
  ...
  if (awaitHuman) {
    parkInboundUltimatum(G, playerCountryId(G.homeRole), partnerId, G.ultimatums[partnerId]);
  }
 ...
 */
function applyMpInboundUltimatumChoice(g, seatId, fromId, concede) {
  ...
  const inbound = targetPol.inboundUltimatums && targetPol.inboundUltimatums[fromId];
...
function timeoutInboundUltimatums(g) {
...
function parkInboundBlocInvite(g, fromId, toId, inv) {
...
function clearInboundBlocInvite(g, toId) {
  const pol = g && g.politics && g.politics[toId];
  if (!pol) return;
  pol.inboundBlocInvite = null;
}

function pendingInboundBlocInvite(pol) {
 ...
 */
function applyMpInboundBlocInviteChoice(g, seatId, accept) {
  if (!g || !g.world || !g.world[seatId]) {
    return { ok: false, error: "Unknown seat" };
  }
  ...
  try {
    ...
    if (accept) {
      ...
      if (fromId && G.rel) {
        G.rel[fromId] = clamp((G.rel[fromId] != null ? G.rel[fromId] : 50) - 2, 5, 95);
      }
      pushDiploAlert(G, {
    ...
    if (!accept && G.rel) {
      G.rel[seatId] = clamp((G.rel[seatId] != null ? G.rel[seatId] : 50) - 2, 5, 95);
    }
...
function timeoutInboundBlocInvites(g) {
  if (!g || !g.politics) return;
  const q = g.q || 0;
  const due = [];
  for (const toId of Object.keys(g.politics)) {
...
function pendingInboundDealProposal(pol) {
  const prop = pol && pol.inboundDealProposal;
  if (!prop || prop.status !== "pending") return null;
  return prop;
}

function parkInboundDealProposal(g, fromId, toId, dealId) {
...
function clearInboundDealProposal(g, toId) {
  const pol = g && g.politics && g.politics[toId];
  if (!pol) return;
  ...
  if (inbound && inbound.fromId && g.politics[inbound.fromId]) {
    ...
    if (fromPol.outboundDealProposal && fromPol.outboundDealProposal.toId === toId) {
 ...
 */
function applyMpInboundDealChoice(g, seatId, accept) {
  if (!g || !g.world || !g.world[seatId]) {
    return { ok: false, error: "Unknown seat" };
  }
  if (!g.politics || !g.politics[seatId]) {
  ...
  try {
    ...
    if (accept) {
      ...
      if (fromId && G.rel) {
        G.rel[fromId] = clamp((G.rel[fromId] != null ? G.rel[fromId] : 50) - 3, 5, 95);
      }
      pushDiploAlert(G, {
    ...
    if (!accept && G.rel) {
      G.rel[seatId] = clamp((G.rel[seatId] != null ? G.rel[seatId] : 50) - 3, 5, 95);
    }
    const p = partnerById(seatId);
    const name = p ? p.name : seatId;
    if (G.brief) {
...
function timeoutInboundDealProposals(g) {
  if (!g || !g.politics) return;
  const q = g.q || 0;
  const due = [];
  for (const toId of Object.keys(g.politics)) {
...
/** Why a predefined-bloc accession step may be blocked. */ function blocJoinBlockers(blocId, phase) {
    ...
    if (autoAcc && autoAcc.blocId !== blocId) out.push("withdraw your current application first");
    if (ph === "apply" && autoAcc && autoAcc.blocId === blocId) out.push("accession already in progress");
    if (ph !== "apply") {
        if ((!acc || acc.blocId !== blocId) && !(autoAcc && autoAcc.blocId === blocId)) out.push("application not filed");
        else if (ph === "align" && ((acc && acc.step < 1) || (autoAcc && autoAcc.step < 1))) out.push("application not yet accepted");
        else if (ph === "accede" && ((acc && acc.step < 2) || (autoAcc && autoAcc.step < 2))) out.push("alignment not complete");
    }
...
/** Single join path: membership, statute books, nation books, and player trade access. */ function finalizeBlocJoin(state, countryId, blocId, law) {
    ...
    const bagLaw = state.world && state.world[countryId] && state.world[countryId].law;
    ...
    if (L) {
        if (bloc && isCustomsUnion(bloc)) {
            ...
            const role = countryId === player ? state.homeRole : (state.world && state.world[countryId] && state.world[countryId].role);
...
function notifyBlocDeparture(state, leftId, blocId, blocName) {
...
function canCreateCustomBloc(state) {
    const g = state || G;
    if (!g) return false;
 ...
 */
function withdrawBlocAccession(state, seatId) {
    const g = state || G;
    if (!g) return false;
...
function processBlocAccessions(g) {
    ...
    for (const cid of Object.keys(state.blocAccessionByCountry)){
        ...
        if (state.q - acc.sinceQ > maxQ || (!acc.self && rel < willingnessMin)) {
            ...
            if (state.politics && state.politics[cid]) state.politics[cid].blocAccession = null;
        ...
        if (acc.step === 1 && state.q - acc.sinceQ >= 1) {
            ...
            const joinLaw = (state.world && state.world[cid] && state.world[cid].law)
...
function notifyBlocMembership(state, joinedId, blocId, blocName, invitedBy) {
    ...
    if (state.politics) {
        for (const seatId of Object.keys(state.politics)){
            if (countryBlocId(seatId, state.blocMember) === blocId) notifySeat(seatId);
        }
    }
...
function playerAccessionGate(state, blocId, phase) {
    const spec = blocAccessionSpec(blocId);
    ...
    if (phase === "accede") {
        ...
        for (const m of (bloc && bloc.members) || []) {
            ...
            if (rel < min) {
                const p = partnerById(m);
                return "relations with " + (p ? p.name : m) + " " + rel.toFixed(0) + " (need " + min + "+)";
            }
        }
        ...
        if (spec && spec.chairRelationMin && chairId) {
            ...
            if (rel < spec.chairRelationMin) {
                return "chair relations " + rel.toFixed(0) + " (need " + spec.chairRelationMin + "+)";
            }
        }
    }
    return null;
...
function beginPlayerBlocAccession(state, blocId) {
    if (!state || !blocId) return;
    const pid = state.homeRole === "home" ? "kingdom" : resolveHomeRole(state.homeRole || "home");
    if (!state.blocAccessionByCountry) state.blocAccessionByCountry = {};
...
function applyBlocAccessionDraft(state, ba) {
...
function inviteToBloc(countryId, blocId) {
    ...
    if (awaitHuman) parkInboundBlocInvite(G, fromId, countryId, G.blocInvites[countryId]);
    return true;
}
    ...
    try {
        ...
        for (const id of worldSeatIds()) {
            ...
            const r = stepCountry({
                ...
                blocAccessionByCountry: g.blocAccessionByCountry || (g.blocAccessionByCountry = {}),
...
function normalizeDiploPolitics(pol) {
    ...
    if (!pol.inboundUltimatums || typeof pol.inboundUltimatums !== "object") pol.inboundUltimatums = {};
 ...
 */
function isStaleMpUltimatumResubmit(pid, prevU, nextU, econ, q) {
    if (!nextU || nextU.status !== "pending") return false;
    if (prevU && prevU.status === "pending") return false;
    const cd =
        (econ && econ.ultimatumCd && econ.ultimatumCd[pid]) || 0;
    if (cd > 0) return true;
 ...
 */
function diploActivityCapitalCost(pol, nextEnvoys, nextUltimatums, opts) {
    let cost = 0;
...
function snapshotMpDiploUi(g) {
    ...
    return {
        ...
        accessionEntry:
            seatId && state.blocAccessionByCountry && state.blocAccessionByCountry[seatId]
...
function mpDraftBlocGateError() {
    ...
    if (G.draft.blocAccession && blocJoinBlockers(G.draft.blocAccession.blocId, "apply").length) {
        return "Cannot apply to join — " + blocJoinBlockers(G.draft.blocAccession.blocId, "apply")[0];
    }
    ...
    try {
        ...
        if (g.draft.blocCreate) {
            if (!canCreateCustomBloc()) {
                g.draft.blocCreate = null;
            } else if (!createCustomBloc(g.draft.blocCreate.name, g.draft.blocCreate.template)) {
                g.draft.blocCreate = null;
            }
        }
        for(const cid in g.draft.blocInvite || {}){
...
function exportGameSnapshot(g) {
    ...
    if (src.capital != null || src.fac) {
        ...
        snap.politics[sid] = normalizeDiploPolitics({
            ...
            inboundUltimatums: prevPol && prevPol.inboundUltimatums
                ? clone(prevPol.inboundUltimatums)
                : {},
            inboundBlocInvite: prevPol && prevPol.inboundBlocInvite
                ? clone(prevPol.inboundBlocInvite)
                : null,
            inboundDealProposal: prevPol && prevPol.inboundDealProposal
                ? clone(prevPol.inboundDealProposal)
                : null,
            outboundDealProposal: prevPol && prevPol.outboundDealProposal
                ? clone(prevPol.outboundDealProposal)
                : null,
            blocAccession: prevPol && prevPol.blocAccession
                ? clone(prevPol.blocAccession)
                : (src.blocAccession ? clone(src.blocAccession) : null),
...
function hydrateGameSnapshot(snap, opts) {
    ...
    const prevNotedAlerts = prevSeatPol && prevSeatPol.diploAlerts
...
/* Participation drag on the labour force from an ageing workforce, %/year.
   ...
   and the simulated labour force cannot disagree. */ function ageingLabourDrag(econ) {
...
/* Annualised labour-force growth implied by the cohort stocks: the same
   ...
   table. */ function impliedLabourGrowth(econ, E) {
    const e = econ || {};
...
const OTHER_G = 1.8, OTHER_TRANSFER = 5.9; // splits OTHER_SPEND (research carved out)
...
/* ==================================================================
   ...
   ================================================================== */ function step(g, law, prevLaw, det) {
    ...
    /* Wage-setting reference: each seat's own opening statute, captured on the
     ...
     UK-anchored rate it never legislated for. */ if (!e.wageRef) e.wageRef = {
    ...
    e.L *= 1 + ((e.popWork / POP_WORK0 - e.L / 100) * 12 + (E.labour || 0) * 0.4 - ageingLabourDrag(e)) / 400;
    ...
    /* Participation margin: people enter and leave the workforce as the reward
     for working changes. */ const netWage = (1 - prof.meanMtr) / (1 - wref.mtr);
    ...
    /* ---- Okun's law around a wage-setting equilibrium ----
     ...
     u* itself comes from the wage-setting block below. */ const erGap = (law.ni.erOn === false ? 0 : law.ni.erRate) - wref.er;
    ...
    const nairu = wref.nairu + WEDGE_ELAST * wedgePP + LABOUR_COST_ELAST * e.erBorne + REPLACEMENT_ELAST * (law.spend.welfare - wref.welfare + (E.replace || 0) - wref.replace) - SKILLS_ELAST * (law.spend.education - wref.education) * 0.35 - 4.0 * ((e.hCap != null ? e.hCap : H_CAP0) - wref.hCap) + E.nairu - wref.nairuCh;
    e.nairu = nairu;
...
function renderPanelInto(host, tabOverride) {
    if (!host) return;
    const prevTab = tab;
...
function paintBudgetPanel(host) {
    if (!host || !G) return;
    const e = G.econ;
...
function paintPoliciesPanel(host) {
    if (!host || !G) return;
...
function paintSocietyPanel(host) {
    if (!host || !G) return;
    const e = G.econ, E = aggregate(G.draft);
...
function paintTradePanel(host) {
    ...
    host.querySelectorAll("[data-bloc-acc]").forEach((b)=>b.onclick = ()=>{
            ...
            if (G.draft.blocAccession && G.draft.blocAccession.blocId === id && G.draft.blocAccession.phase === phase) {
                G.draft.blocAccession = null;
            } else {
...
function paintDiplomacyPanel(host) {
    if (!host || !G) return;
    ensureDiploStocks(G.econ);
    const byRegion = partnersByRegion();
    let partnerCards = "";
    ...
    host.innerHTML = envoySummaryHtml() + '<div class="eyebrow mt">Partners</div>' + partnerCards;
    if (!G.draft.missions) G.draft.missions = {};
    pruneInvalidDraftMissions(G);
...
function paintChartsPanel(host) {
    if (!host || !G) return;
    if (G.log.length < 2) {
...
function _renderPanelBody(host) {
    if (!host || !tab) return;
...
function partnerDiploCardHtml(p) {
    ...
    const missionRow = MISSIONS.map((m)=>{
        ...
        return '<button type="button" class="diplo-mission' + (on ? " on" : "") + '" data-mission="' + m.id + '" data-partner="' + p.id + '"' + (blocked ? " disabled" : "") + ' title="' + esc(tip) + '"><span class="diplo-mission-name">' + verb + missionShortLabel(m) + '</span><span class="diplo-mission-pc">' + m.pc + '</span></button>';
    }).join("");
    let envoyRow = "";
    if (envoyOn) {
...
function envoySummaryHtml() {
    ...
    return '<div class="diplo-summary panel"><div class="diplo-summary-h">Your envoys</div><div class="diplo-slots">' + slots + '</div><div class="hint" style="margin:6px 0 0">Assign costs ' + ENVOY_ASSIGN_PC + ' capital · missions go into the bill · ultimatums spend capital immediately</div></div>';
}
...
function accessionCardHtml(blocId) {
    ...
    h += '<div class="hint">One application bill starts accession. Alignment and the treaty then advance automatically each quarter — no further capital.</div>';
    h += '<div class="hint">Member relations and policy alignment are checked as stages complete.</div>';
    if (blocked) h += '<div class="eff" style="color:var(--red);display:block">' + esc(blockers[0]) + '</div>';
    const pc = steps.apply || 8;
    h += '<div class="foot"><span class="pc">' + pc + ' capital</span>' + '<button class="btn' + (staged === "apply" ? " danger" : "") + (blocked && staged !== "apply" ? " disabled" : "") + '" data-bloc-acc="apply" data-bloc-id="' + blocId + '"' + (blocked && staged !== "apply" ? ' title="' + esc(blockers.join("; ")) + '"' : "") + '>' + (staged === "apply" ? "Cancel" : "Join") + '</button></div></div>';
    return h;
}
...
function playerJoiningBloc() {
    ...
    if (player && G.blocAccessionByCountry && G.blocAccessionByCountry[player]) return true;
    return false;
}
function blocMembershipPanelHtml() {
    ...
    if (bid) {
        ...
        for(const cid in G.draft.blocInvite || {}){
            ...
            h += '<div class="card staged-bloc-invite" id="bloc-staged-' + cid + '" data-bloc-staged="' + cid + '" style="margin-top:8px"><h4>Propose ' + (c ? c.name : cid) + ' join ' + (bloc ? bloc.name : bid) + '<span class="cat">12 capital</span></h4>';
            ...
            h += '<div class="foot"><button type="button" class="btn danger tiny" data-bloc-invite="' + cid + '">Cancel</button></div></div>';
        }
        ...
        if (autoAcc || (acc && G.blocAccessionByCountry && G.blocAccessionByCountry[player])) {
            ...
            h += '<div class="hint">Accession in progress: <b>' + esc(b ? b.name : track.blocId) + '</b> — advances automatically each quarter (no further capital).</div>';
            h += '<button class="btn tiny" data-bloc-withdraw="1">Cancel joining</button>';
            if (t) {
                h += '<div class="card acc-track" style="margin-top:8px"><h4>' + esc(b ? b.name : track.blocId) + '<span class="cat">' + esc(t.status) + '</span></h4>';
                h += accessionPipelineSteps(t.cur);
            ...
            h += '<div class="hint">Accession in progress: <b>' + (b ? b.name : acc.blocId) + '</b> — advances automatically each quarter.</div>';
            h += '<button class="btn tiny" data-bloc-withdraw="1">Cancel joining</button>';
        } else {
            h += '<div class="hint">Joining a bloc takes one application bill. Alignment and the accession treaty then advance automatically each quarter — no further capital. Members must still approve at the final step.</div>';
        ...
        if (!playerJoiningBloc()) {
            if (G.draft.blocCreate) {
                const tpl = CUSTOM_BLOC_TEMPLATES[G.draft.blocCreate.template];
                h += '<div class="card staged-bloc-create" style="margin-top:12px"><h4>Found <b>' + esc(G.draft.blocCreate.name || (tpl ? tpl.name : "bloc")) + '</b><span class="cat">' + (tpl ? tpl.pc : 28) + ' capital</span></h4>';
                ...
                h += '<div class="foot"><button type="button" class="btn danger tiny" data-bloc-create-unstage="1">Cancel</button></div></div>';
            } else {
...
/* ==================================================================
   ...
   ================================================================== */ function despatch(title, stamp, html, opts, cfg) {
    ...
    prepared.forEach((o)=>{
        ...
        b.className = "opt";
        b.innerHTML = "<b>" + T(esc(o.b)) + "</b>" + (o.e ? "<em>" + T(esc(o.e)) + "</em>" : "") + (o.hint || "") + o.extra;
        b.onclick = ()=>{
            closeDespatch();
            const scrim = $("scrim");
        ...
        opts: [
            {
                ...
                f: ()=>{
                    ...
                    if (G.blocAccessionByCountry && G.blocAccessionByCountry[player]) {
                        G.blocAccessionByCountry[player].invitedBy =
                            inv.fromId || null;
                    }
 ...
 */
function mergeMpInboundAsksFromSnapshot(snap, seatId) {
    ...
    else if (local.pendingEvent && !remote.pendingEvent) local.pendingEvent = null;
    if (Array.isArray(remote.diploAlerts)) {
        local.diploAlerts = mergeDiploAlertsNoted(local.diploAlerts, remote.diploAlerts);
    }
...
function flushMpDiploPressAlerts() {
    ...
    const seatId = playerCountryId(G.homeRole);
    const pol = G.politics && G.politics[seatId];
    const alertSrc = (pol && pol.diploAlerts && pol.diploAlerts.length)
        ? pol.diploAlerts
        : G.diploAlerts;
    const fresh = diploOutcomeAlertsForBrief(alertSrc, G.q).filter(
        (a) => a && DIPLO_PRESS_ONLY_KINDS[a.kind]
    );
...
function showMpBriefing(opts) {
    ...
    const afterFlash = () => {
        ...
        for (const a of freshUlt) {
            ...
            if (!DIPLO_PRESS_ONLY_KINDS[a.kind]) {
...
function showMpInboundUltimatum(fromId, ult) {
    ...
    const body = "<p>" + T(name + " has issued an ultimatum against {C}: <b>" + esc(demand) +
        "</b>. Concede and their demand takes effect; defy and they may retaliate. Silence past the deadline counts as defiance.") + "</p>";
...
function showMpInboundBlocInvite(inv) {
    ...
    const p = partnerById(fromId);
    const name = p ? p.name : fromId;
    const bloc = blocById(inv.blocId) || (G.customBlocs && G.customBlocs[inv.blocId]);
    const blocName = bloc ? bloc.name : inv.blocId;
    const body = "<p>" + T(name + " has invited {C} to join <b>" + esc(blocName) +
        "</b>. Accept and accession begins — alignment and the final treaty still require legislation. Decline and the door stays open. Silence past the deadline counts as a polite decline.") + "</p>";
...
function showMpInboundDealProposal(prop) {
    ...
    const terms = Array.isArray(prop.terms) && prop.terms.length
        ? "<ul>" + prop.terms.map((t) => "<li>" + esc(t) + "</li>").join("") + "</ul>"
        : "";
    const body = "<p>" + T(name + " proposes to ratify <b>" + esc(dealName) +
...
function composePress(input) {
    const clauses = (input && input.clauses || []).filter((c) => {
    ...
    for (const u of ultOutcomes) {
        ...
        if (u.kind === "ult_concede") {
            ...
            clips.push({
                ...
                lede: T("{C} has conceded to " + name + "'s ultimatum — " + demand + "." + impact),
                kicker: date
            });
            ...
            clips.push({
                ...
                lede: T("{C} has defied " + name + "'s ultimatum — " + demand + "." + impact),
                kicker: date
            });
        } else if (u.kind === "bloc_invite_accept" || u.kind === "bloc_inbound_accept") {
            clips.push({
                kind: "ultimatum",
                masthead: "The Trade Gazette",
                headline: "Bloc invitation accepted",
                lede: T(u.kind === "bloc_invite_accept"
                    ? name + " accepts the invitation to " + demand + "."
                    : "{C} accepts " + name + "'s invitation to " + demand + "."),
                kicker: date
            });
        } else if (u.kind === "bloc_invite_decline" || u.kind === "bloc_inbound_decline") {
            clips.push({
                kind: "ultimatum",
                masthead: "The Trade Gazette",
                headline: "Bloc invitation declined",
                lede: T(u.kind === "bloc_invite_decline"
            ...
            clips.push({
                ...
                lede: T("{C} is now a full member of " + demand + ". Internal tariffs and market access take effect under the bloc's rules."),
                kicker: date
            });
            ...
            clips.push({
                ...
                lede: T(name + " is now a full member of " + demand + ". The accession pipeline is complete."),
                kicker: date
            });
            ...
            clips.push({
                ...
                lede: T("{C} has left " + demand + ". Preferential access and any common external tariff no longer apply."),
                kicker: date
            });
        ...
        } else if (u.kind === "deal_propose_accept" || u.kind === "deal_inbound_accept") {
            clips.push({
                kind: "ultimatum",
                masthead: "The Trade Gazette",
                headline: "Treaty accepted",
                lede: T(u.kind === "deal_propose_accept"
                    ? name + " accepts the treaty — " + demand + "."
                    : "{C} accepts " + name + "'s treaty — " + demand + "."),
                kicker: date
            });
        } else if (u.kind === "deal_propose_decline" || u.kind === "deal_inbound_decline") {
            clips.push({
                kind: "ultimatum",
                masthead: "The Trade Gazette",
                headline: "Treaty declined",
                lede: T(u.kind === "deal_propose_decline"
...
function showBlocFoundModal() {
    if (playerJoiningBloc()) return;
    whenDespatchShellReady(({ titleEl, stampEl, bodyEl, box })=>{
        ...
        const renderModal = ()=>{
            ...
            bodyEl.innerHTML = '<div style="margin-bottom:12px"><label style="display:block;font-size:12px;opacity:0.7;margin-bottom:4px">Bloc name</label>' +
                ...
                Object.keys(CUSTOM_BLOC_TEMPLATES).map((tid)=>{
                    ...
                    return '<button type="button" class="bloc-type-opt' + (sel ? " selected" : "") + '" data-bloc-type="' + tid + '" style="text-align:left;padding:10px 14px;border-radius:10px;border:2px solid ' + (sel ? "var(--accent,var(--blue))" : "var(--ink-faint)") + ';background:' + (sel ? "rgba(10,132,255,0.1)" : "transparent") + ';cursor:pointer;color:inherit">' +
        ...
        row.className = "opts";
        row.style.cssText = "display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:0;width:100%";
...
function showBlocInviteModal(bid) {
    whenDespatchShellReady(({ titleEl, stampEl, bodyEl, box })=>{
        ...
        const syncConfirm = ()=>{
            ...
            confirmBtn.innerHTML = selected ? (selected.staged ? "<b>Already in the bill</b><em>See the bill panel above</em>" : "<b>Propose " + esc(selected.partner.name) + "</b><em>12 capital · added to your bill</em>") : "<b>No candidate selected</b>";
        };
        confirmBtn.onclick = ()=>{
        ...
        const renderModal = ()=>{
            ...
            for (const r of regionOrder){
                ...
                for (const c of group){
                    ...
                    const status = c.staged ? "In the bill" : c.blockers.length ? c.blockers[0] : "Eligible";
            ...
            if (selected) {
                ...
                else listHtml += '<div class="eff" style="color:var(--green);display:block;margin-top:6px">Ready to propose — every member approves.</div>';
                listHtml += '</div>';
...
function enact() {
    ...
    if (G.draft.blocAccession && blocJoinBlockers(G.draft.blocAccession.blocId, "apply").length) return;
    if (G.draft.blocExternalDeal) {
        const bed = G.draft.blocExternalDeal;
    ...
    if (G.draft.blocCreate) {
        if (!createCustomBloc(G.draft.blocCreate.name, G.draft.blocCreate.template)) {
...
let _despatchOpen = null;
let _onDespatchChange = null;
let _onDespatchShell = null;
let _despatchShellOpen = false;

function setOnDespatchChange(fn) {
    _onDespatchChange = fn;
}
function setOnDespatchShell(fn) {
    _onDespatchShell = fn;
}
...
function whenDespatchShellReady(fn, tries = 0) {
    showDespatchShell();
    const titleEl = $("dpTitle"), stampEl = $("dpStamp"), bodyEl = $("dpBody"), box = $("dpOpts");
...
export {
  FACTIONS, DEPTS, OTHER_SPEND, OTHER_REV, TERM_LEN, POLITY, POLITY_IDS, POLITY_LADDER, REL_POLITY,
  ...
  countryBlocId, blocMembers, joinBloc, leaveBloc, createCustomBloc, canCreateCustomBloc, withdrawBlocAccession, inviteToBloc, finalizeBlocJoin, playerCountryId,
  ...
  showMpBriefing, mergeMpInboundAsksFromSnapshot, flushMpDiploPressAlerts, applyMpEventChoice, applyMpDiploAction, applyMpDiploPatch,
  diploOutcomeAlertsForBrief, markDiploAlertsNoted,
  applyMpInboundUltimatumChoice, timeoutInboundUltimatums,
  applyMpInboundBlocInviteChoice, timeoutInboundBlocInvites,
  applyMpInboundDealChoice, timeoutInboundDealProposals,
  ...
  composePress, pushPress, renderPress, dismissPress, dismissNewestPress, expandPress, getPressExpanded,
  dealBlockers, fullEffects, impactChips, impactFactions,
  impactStripHtml, impactPanelHtml, rateImpactHtml,
  ...
  incomePanelHtml, wireIncomePanel, lineChart, compositionBar, chartBox,
  nationTableHtml, budgetModeHtml, ledgerTable, clausesIn, isFiscalOnly,
  esc, fmt, sgn, clone, ICONS, despatch, enact, projectionModal,
  RATE_FLOOR, MANUAL_RATE_MIN,
  paintTaxesPanel, paintSocietyPanel, paintTradePanel, paintDiplomacyPanel, paintChartsPanel,
  paintBillPanel, paintBudgetPanel, paintPoliciesPanel,
  render, renderChrome, renderPanel, renderPanelInto, $, registerEl, T,
  setOnState, setOnDespatchChange, setOnDespatchShell, getDespatch, closeDespatch, hideDespatchShell, isDespatchShellOpen, notifyUi, bump, isDeliverLocked, setOnSetup, requestSetup, setTab, getTab, setOnTabChange, getG,
