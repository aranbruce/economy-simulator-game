...
function paintTradePanel(host) {
    ...
    host.querySelectorAll("[data-bloc-create]").forEach((b)=>b.onclick = ()=>{
            const tpl = CUSTOM_BLOC_TEMPLATES[b.dataset.blocCreate];
            G.draft.blocCreate = G.draft.blocCreate ? null : {
                name: tpl.name,
                template: b.dataset.blocCreate
            };
            render();
        });
...
function blocMembershipPanelHtml() {
    ...
    if (bid) {
        ...
        for(const tid in CUSTOM_BLOC_TEMPLATES){
            const t = CUSTOM_BLOC_TEMPLATES[tid];
            h += '<button class="btn tiny" data-bloc-create="' + tid + '">Found ' + t.name + ' (' + t.pc + ' cap)</button> ';
        }
        h += '</div>';
        if (G.draft.blocCreate) {
            h += '<div class="eff">Staged: found ' + G.draft.blocCreate.name + '</div>';
        }
    }
    return h;
}
...
export {
  ...
  setOnState, setOnDespatchChange, getDespatch, closeDespatch, notifyUi, bump, isDeliverLocked, setOnSetup, requestSetup, setTab, getTab, setOnTabChange, getG,
