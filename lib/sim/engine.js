...
import {
  BASE_INCOME_DIST, BASE_DIST_GINI, buildIncomeDist, incomeDistForRole,
  activeIncomeDist, distMedian, preTaxGini, clearIncomeDistCache,
} from "./incomeDist.js";
import {
...
/* Pin headline stocks each quarter and let the expenditure block clear at
   GDP = 100. FX is free (UIP at the Bank rate). Cached per homeRole. */ function settleOpeningEcon(pins, role) {
    ...
    /* Per-seat pre-tax shape; fixed for the run (not rewritten in step). */ g.econ.incomeDist = incomeDistForRole(homeRole);
