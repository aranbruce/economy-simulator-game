...
function renderPanel() {
    ...
    if (tab === "bill") {
        ...
        if (!G.draft.missions) G.draft.missions = {};
        for (const pid of Object.keys(G.draft.missions)) {
            if (!MISSION_BY_ID[G.draft.missions[pid]]) delete G.draft.missions[pid];
        }
        host.querySelectorAll("[data-mission]").forEach((b)=>b.onclick = ()=>{
                const pid = b.dataset.partner, mid = b.dataset.mission;
                if (mid === "sanctionsPosture" && isVisitActive(G, pid)) return;
