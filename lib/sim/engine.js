...
/* Diplomatic missions: one-shot bill clauses that add a decaying relImpulse
   rather than a permanent jump mean-reversion would eat in a quarter. */ const MISSIONS = [
    {
        id: "summit",
...
function assignEnvoy(partnerId) {
  ...
  let slot = G.envoys.indexOf(null);
  if (slot < 0) slot = 0;
...
function resolveLockstepQuarter(g, humanSeatIds, submissions) {
    ...
    try {
        for (const id of humans) {
            const draft = subs[id] && subs[id].draft;
            if (!draft) continue;
...
function validateMpSubmission(snap, seatId, draft) {
    ...
    if (priced.cost > pol.capital) {
        return {
            ok: false,
            error: "Not enough political capital (" + priced.cost + " needed, " + Math.round(pol.capital) + " available)",
            cost: priced.cost,
            capital: pol.capital
        };
    }
 ...
 */
function enactHumanSeatOnSnapshot(g, seatId, draft) {
    const seat = g.world[seatId];
    if (!seat) return {
            ok: false,
            error: "Unknown seat"
    ...
    if (!g.politics[seatId]) {
        g.politics[seatId] = {
            capital: 42,
            fac: openingFac(seat.role || worldRoleForSeat(seatId)),
            rel: openingRel(seat.role || worldRoleForSeat(seatId)),
            country: seatId
        };
    }
    const pol = g.politics[seatId];
    const role = seat.role || worldRoleForSeat(seatId);
    const check = validateMpSubmission(g, seatId, draft);
    ...
    try {
        ...
        pol.capital = clamp(pol.capital - check.cost, 0, 100);
        g.capital = pol.capital;
...
function billClauses() {
    ...
    /* Missions live on draft only; staging one is a one-shot clause. */ const missions = G.draft.missions || {};
...
const isFiscalOnly = (cl)=>cl.every((c)=>!/Ratify|Withdraw|Enact|Repeal|Cannabis|Psychedelics|Gambling|Alcohol|Tobacco|Adult|summit|concession|protest|Restrictive|State visit|Trade concession|Formal protest/i.test(c.label));
/* ==================================================================
   7. RENDERING
...
function clausesIn(tabId, cl) {
    const pats = {
        ...
        diplomacy: /summit|State visit|Trade concession|Formal protest|Restrictive measures/i,
        charts: /(?!)/
    };
...
function renderPanel() {
    ...
    if (tab === "bill") {
        ...
        host.innerHTML = impactStripHtml() + briefingHtml(G.brief, {
        ...
        host.innerHTML = impactStripHtml() + '<div class="eyebrow">The structure of the system</div>' + '<div class="hint">Changing the architecture is the biggest thing you can do, and the most expensive. Rates within a system are cheap by comparison.</div>' + '<div class="cards">' + REGIMES.map((r)=>{
        ...
        host.innerHTML = impactStripHtml() + '<div class="eyebrow">The statute book</div>' + '<div class="hint">Enacted policy stays in force until repealed, and repeal costs capital too. Costs are annual, in points of GDP.</div>' + '<div class="seg mini" style="margin:8px 0 12px" id="polFilter">' + filters.map((param)=>{
            let [id, lab] = param;
        ...
        host.innerHTML = impactStripHtml() + facHtml + '<div class="eyebrow mt">Social indicators</div>' + '<div class="panel" style="padding:11px 13px">' + bars.map((param)=>{
            let [k, n, v] = param;
        ...
        host.innerHTML = impactStripHtml() + envoySummaryHtml() + '<div class="eyebrow mt">Partners</div>' + partnerCards;
        if (!G.draft.missions) G.draft.missions = {};
        host.querySelectorAll("[data-mission]").forEach((b)=>b.onclick = ()=>{
                const pid = b.dataset.partner, mid = b.dataset.mission;
                if (G.draft.missions[pid] === mid) delete G.draft.missions[pid];
...
function missionShortLabel(m) {
    ...
    if (m.id === "concession") return "Concession";
...
function render(options) {
    ...
    const host = panelHost();
    if (host) host.querySelectorAll("[data-gobill]").forEach((b)=>{
        b.onclick = ()=>{
            setTab("bill");
        };
    });
...
/* A running total for whatever is staged, shown at the top of every drawer so
   you can see the consequence of a change without leaving the panel you made it
   in. Cheap: one baseline plus one variant simulation. */ function impactStripHtml() {
    const cl = billClauses();
    if (!cl.length) return '<div class="impstrip empty">Nothing staged. Change anything here and its effect appears in this strip.</div>';
    if (!G.sandbox) {
    ...
    return '<div class="impstrip"><div class="impstrip-h">Staged: ' + cl.length + ' change' + (cl.length === 1 ? '' : 's') + ' &middot; ' + cl.reduce((a, c)=>a + c.pc, 0) + ' capital ' + '<button class="btn tiny" data-gobill="1">Break it down</button></div>' + impactChips(im) + impactFactions(im) + '<div class="hint">Trend growth is annualised potential growth; four-quarter moves are small but compound. Growth is cumulative outturn GDP.</div></div>';
}
    ...
    for(const pid in missions){
        ...
        if (m.id === "concession") {
            if (typeof G !== "undefined" && G) {
                if (!G.missionEvents) G.missionEvents = [];
                G.missionEvents.push({ partnerId: pid, missionId: m.id, q: G.q });
            }
            continue;
        }
        econ.relImpulse[pid] = (econ.relImpulse[pid] || 0) + m.impulse;
        if (m.tariffCut && lockedTariff(law) == null) {
            law.tariff = Math.max(0, (law.tariff || 0) - m.tariffCut);
        }
        ...
        if (m.id === "demarche") {
            addDiploLedger(gBag, pid, { id: "mission_demarche_" + (gBag.q || 0), label: "Formal protest", pts: -5 });
