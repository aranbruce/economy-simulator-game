...
/* ==================================================================
   ...
   ================================================================== */ function step(g, law, prevLaw, det) {
    ...
    e.health = clamp(e.health + (health0 + E.hlt * 6 + (e.modHlt || 0) * 6 + (e.services - services0) * 0.4 - Math.max(0, e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 1.1 - Math.max(0, e.dependency - DEP_0) * 18 - e.health) * 0.15, 0, 100);
    e.crime = clamp(e.crime + (crime0 + E.cri * 1.6 + (e.modCri || 0) * 1.6 + E.blackLevel * 0.35 - (law.spend.justice - 1.9) * 7 + (e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 2.2 + Math.max(0, e.gini - gini0) * 0.18 - e.crime) * 0.16, 0, 100);
    e.liberty = clamp(e.liberty + (liberty0 + E.lib - e.liberty) * 0.25, 0, 100);
    e.env = clamp(e.env + (env0 + E.env + (law.spend.environment - 0.8) * 6 + law.taxes.carbon.rate * 0.5 - e.env) * 0.14, 0, 100);
    e.openness = clamp(openness0 + E.open, 5, 95);
