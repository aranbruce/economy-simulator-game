...
function rateImpactHtml() {
    ...
    return '<div class="impblock" style="margin-top:10px;padding:10px 0 0;border-top:1px solid rgba(0,0,0,.06)">' + '<div class="imphead">Four quarters at ' + im.pin.toFixed(2) + '% vs the Bank' + '<span class="cost">free</span></div>' + '<div class="hint" style="margin:0 0 6px">Same law, pinned rate against the Taylor rule from today\'s starting point.</div>' + (empty ? '<div class="impfac">No measurable move over four quarters.</div>' : chips + impactFactions(im)) + '</div>';
}
...
function renderChrome() {
    ...
    if (db) {
        if (G.mp && G.mp.waiting) {
            ...
            db.disabled = G.over || cl.length > 0 && !afford;
...
function endEpisode() {
    ...
    G.episode = null;
    G.nextMajorQ = scheduleNextMajorQ(G.q, false);
    return ep;
}
function rollMajorEvent() {
    ...
    if (!pool.length) return null;
    let total = pool.reduce((a, e)=>a + (e.w || 1), 0), r = Math.random() * total;
    for (const e of pool){
        r -= e.w || 1;
        if (r <= 0) {
    ...
    },
    {
        ...
        opts: [
            ...
            },
            {
                ...
                f: ()=>{
                    const id = G.eventFocus;
                    if (id) G.rel[id] = clamp((G.rel[id] || 50) + (Math.random() < 0.5 ? -12 : 5), 0, 100);
                }
            },
            {
...
function rollEvent() {
    ...
    if (!pool.length) return null;
    let total = pool.reduce((a, e)=>a + e.w, 0), r = Math.random() * total;
    for (const e of pool){
        r -= e.w;
        if (r <= 0) {
            const prepared = prepareEvent(e);
...
function enact() {
    ...
    const proceed = (chosen)=>{
        ...
        const res = step(G, G.law, G.prevLaw, false);
