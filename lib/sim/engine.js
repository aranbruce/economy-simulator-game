...
function briefingImpactClause(row, v) {
    ...
    switch(row.k){
        case "growth":
            return up ? "growth about " + n + " points stronger over the year ahead" : "growth about " + n + " points weaker over the year ahead";
        case "trend":
            return up ? "the trend rate about " + n + " points higher" : "the trend rate about " + n + " points lower";
        case "potential":
            return up ? "underlying capacity about " + n + " points higher" : "underlying capacity about " + n + " points lower";
        case "balance":
...
function briefingImpactLines(im, meta) {
    ...
    if (!rows.length && !facs.length) {
        if (kind === "bill" && meta && meta.clauses && meta.clauses.length) {
            return [
                "On the numbers, this package barely shifts the year-ahead view. Nothing the markets or the whips will lose sleep over."
            ];
        }
        if (kind === "decision") {
            return [
                "On the numbers, the decision barely shifts the year-ahead view."
            ];
        }
        return [];
    }
    ...
    if (rows.length) {
        ...
        const opener = kind === "decision" ? "On the central case against doing nothing, " : "On the central case against unchanged policy, ";
        lines.push(opener + body + ".");
    }
