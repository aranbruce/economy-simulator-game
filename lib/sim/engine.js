...
/** Light automatic fiscal rule for AI seats — target a debt band around each
 *  seat's opening anchor rather than consolidating without bound. */ function applyAiFiscalRule(law, econ) {
    ...
    /* High-debt openers (Japan) aim near their own norm, not a 60% Maastricht
     number that would force a depression. */ const target = clamp(anchor, 50, 260);
    const bandLo = target - Math.min(22, 12 + target * 0.12);
    const bandHi = target + Math.min(30, 14 + target * 0.14);
    /* Triggers are relative to the seat's band. Absolute "debt > 200" cut-offs
     put Japan into permanent austerity on its opening 4% deficit. */ const above = debt - bandHi;
    const debtStress = above > 40 ? 1.35 : above > 0 ? 1.15 : 1;
    const defTrigger = above > 40 ? 4.0 : above > 0 ? 5.5 : 7.5;
    /* Above the band, or a fat deficit: tighten — but gently at very high debt,
     where slashing demand raises the debt ratio via the denominator. */ if (def > defTrigger || above > 0 && def > 1.0) {
        const cut = (above > 40 ? 0.03 : 0.045) * debtStress;
        for (const d of DEPTS){
            if (law.spend[d.id] > d.min + 0.15) law.spend[d.id] = Math.max(d.min, law.spend[d.id] - cut);
        }
        if (law.taxes && law.taxes.vat && law.taxes.vat.on && law.taxes.vat.rate < 24) {
            law.taxes.vat.rate = Math.min(24, law.taxes.vat.rate + 0.12 * debtStress);
        }
        if (above > 20 && law.income) law.income.uprate = false;
        return;
    }
    /* Below the band, or a large surplus while still under the target: ease so AI
     seats do not accumulate −150% net assets. Do not ease on surplus alone when
     debt is already above target. */ if (debt < bandLo || def < -2.5 && debt < target) {
   ...
   ================================================================== */ function step(g, law, prevLaw, det) {
    ...
    /* A creditor state still earns on its assets, but private finance does not
     keep getting cheaper with every extra point of net assets — see debtFinanceWedge. */ if (e.debt < 0) {
        ...
        /* Sovereign risk is priced off excess debt above the seat's own norm for AI
         seats, opening settle, and high-debt player seats (Japan at 250% is not a
         UK gilt crisis). The Kingdom and similar still face absolute debt. */ const anchor = e.debtAnchor != null ? e.debtAnchor : 80;
        const useAnchor = g._aiSeat || g._settling || anchor >= 150;
        const debtForRisk = useAnchor ? 80 + Math.max(0, e.debt - anchor) : e.debt;
    ...
    const QUALITY_0 = (8.4 + 4.1 * 0.55) / 100 * 100; // baseline volume per unit of need
    const sTarget = clamp(55 * quality / QUALITY_0 - Math.max(0, e.inflation - 2) * 1.2 + E.srv + E.hlt * 2 + (e.modSrv || 0) + (e.modHlt || 0) * 2, 0, 100);
    e.services = clamp(e.services + (sTarget - e.services) * SVC_ADJ, 0, 100);
    e.svcQuality = quality / QUALITY_0;
    e.unitCost = unitCost;
    ...
    e.health = clamp(e.health + (55 + E.hlt * 6 + (e.modHlt || 0) * 6 + (e.services - 55) * 0.4 - Math.max(0, e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 1.1 - Math.max(0, e.dependency - DEP_0) * 18 - e.health) * 0.15, 0, 100);
    e.crime = clamp(e.crime + (CRIME_BASE + E.cri * 1.6 + (e.modCri || 0) * 1.6 + E.blackLevel * 0.35 - (law.spend.justice - 1.9) * 7 + (e.unemployment - 4.2) * 2.2 + Math.max(0, e.gini - 35) * 0.18 - e.crime) * 0.16, 0, 100);
    e.liberty = clamp(e.liberty + (58 + E.lib - e.liberty) * 0.25, 0, 100);
    e.env = clamp(e.env + (44 + E.env + (law.spend.environment - 0.8) * 6 + law.taxes.carbon.rate * 0.5 - e.env) * 0.14, 0, 100);
    e.openness = clamp(50 + E.open, 5, 95);
    ...
    const common = 40 + realIncome * 2.6 - Math.max(0, e.inflation - 3) * 2.0 - (e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 2.6 + (e.services - 55) * 0.40;
    const extra = {
        business: (pg - 1.5) * 5,
        workers: -(e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 2.0 - (e.gini - 35) * 0.30,
        pensioners: -Math.max(0, e.inflation - 2) * 2.2 + (e.services - 55) * 0.25,
        urban: (e.liberty - 58) * 0.22 + (e.env - 44) * 0.14,
        rural: -(e.crime - CRIME_BASE) * 0.14,
        patriots: -(e.openness - 50) * 0.14 - (e.crime - CRIME_BASE) * 0.20
    };
...
/* ---------- what each region is feeling ---------- */ function regionStats(r) {
    ...
    const prosper = r.prosper * (st.y / 100) * (1 - (e.gini - 35) * 0.006);
...
/** Rough running score the election() formula will use — for the thermometer. */ function electionThermometer() {
    ...
    const record = recent.reduce((a, r)=>a + (r.growth || 0), 0) / n * 2.5 - Math.max(0, e.inflation - 3) * 1.5 - (e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 1.5 + (e.services - 55) * 0.15;
...
function renderPanel() {
    ...
    if (tab === "bill") {
        ...
        host.innerHTML = impactStripHtml() + '<div class="briefing">' + G.brief.map((t)=>'<p>' + t + '</p>').join("") + '<div class="src">Morning note &middot; Permanent Secretary</div></div>' + '<div class="eyebrow">Departmental spending, % of GDP</div>' + '<div class="hint">Standing still is a cut: demand for services rises about one percent a year on its own.</div>' + '<div class="panel" id="deptLevers">' + DEPTS.map((d)=>{
            ...
            const score = serviceScore(d.id, G.draft, G.econ);
        ...
        host.querySelectorAll("[data-mode]").forEach((b)=>{
            b.onclick = ()=>{
                ...
                if (m === "service") G.draft.hold[id] = serviceScore(id, G.draft, G.econ);
...
function writeBriefing(res) {
    ...
    if (G.fac[worst.id] < 30) b.push(worst.name.toLowerCase() + " have effectively abandoned you, and they are " + Math.round(worst.w * 100) + "% of the electorate.");
...
function election() {
    ...
    const e = G.econ;
    const recent = G.log.slice(-8);
    const record = recent.reduce((a, r)=>a + r.growth, 0) / recent.length * 2.5 - Math.max(0, e.inflation - 3) * 1.5 - (e.unemployment - (e.nairu != null ? e.nairu : 4.2)) * 1.5 + (e.services - 55) * 0.15;
