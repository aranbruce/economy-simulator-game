...
export function ensureDiploStocks(e) {
  if (!e) return e;
...
export function beginActiveVisit(g, partnerId, missionId) {
...
export function visitQuartersLeft(g, partnerId) {
  const v = g && g.activeVisits && g.activeVisits[partnerId];
  if (!v) return 0;
  return Math.max(0, v.endsQ - (g.q || 0));
}

export function isVisitActive(g, partnerId) {
  return visitQuartersLeft(g, partnerId) > 0;
}

export function seedRelBase(e, relMap, opts) {
  ensureDiploStocks(e);
  if (!relMap) return;
...
export function addDiploLedger(g, partnerId, entry) {
...
export function setSanctionStance(g, partnerId, flags) {
...
export function decayDiploLedger(e) {
  ensureDiploStocks(e);
...
export function maybeClearSanctionStance(g) {
  if (!g || !g.econ) return;
  ensureDiploStocks(g.econ);
  const e = g.econ;
  for (const pid of Object.keys(e.sanctionStance)) {
    ...
    const ledgerGone = !(e.diploLedger[pid] || []).some((x) =>
...
export function spherePatronsForPartner(clientId) {
  const out = [];
...
export function sharedCamp(a, b) {
...
export function baseLabel(relBase) {
 ...
 */
export function buildRelationModifiers(partnerId, g, law, deps) {
  const {
  ...
  // Walking out of the United Nations chills every relationship at once.
  if (law.policies && !law.policies.unMember) {
    mods.push({ label: "Outside the United Nations", pts: UN_EXIT_DIPLO, kind: "live" });
  }
...
export function defaultUltimatumDemand(partnerId, g, deps) {
...
export function hasFormalProtest(e, partnerId) {
  const list = e && e.diploLedger && e.diploLedger[partnerId];
  if (!list || !list.length) return false;
  return list.some((x) => x && /Formal protest/.test(x.label || "") && (x.pts || 0) < 0);
}

export function canIssueUltimatum(partnerId, g, deps) {
  const reasons = [];
...
function partnerLawBag(g, partnerId, deps) {
...
function partnerBlocMember(g, partnerId) {
...
export function partnerTariffOnPlayer(partnerId, g, deps) {
...
function playerTariffOnPartner(partnerId, g, deps) {
...
function hasRatifiedDeal(partnerId, g, deps) {
...
function topPolicyDivergence(partnerId, g, deps) {
  const law = g.law || g.draft;
...
function leverageTerm(rel, gdpRatio) {
...
export function partnerSelfInterest(demandId, partnerId, g, deps, meta) {
  ...
  const totalX = Object.values(bilat).reduce((s, v) => s + (v || 0), 0) || 1;
  const exportShare = exportsToPartner / totalX;
...
export function demandFit(demandId, partnerId, g, deps, meta) {
...
function demandAvailable(demandId, partnerId, g, deps, meta) {
  const { partnerShare } = deps;
  const share = partnerShare ? partnerShare(g.homeRole, partnerId) : 0;
  if (demandId === "tariffCut") {
    const theirTar = partnerTariffOnPlayer(partnerId, g, deps);
...
export function interestTag(interest) {
  if (interest >= 0.25) return "In their interest";
  if (interest <= -0.25) return "Against their interest";
  return "Mixed";
}

export function baseP(partnerId, demandId, g, deps, meta) {
...
export function concedeP(partnerId, demandId, g, deps, det, meta) {
...
export function ultimatumDemandsFor(partnerId, g, deps) {
...
function stepPolityToward(from, to) {
...
export function applyUltimatumConcede(state, partnerId, ult, deps) {
...
export function applyUltimatumDefy(state, partnerId) {
  addDiploLedger(state, partnerId, {
...
export function ultimatumOutcomeImpacts(state, partnerId, ult, conceded, deps) {
  const e = state.econ;
  ensureDiploStocks(e);
...
export const MISSION_EVENTS = [
  ...
  },
  {
    ...
    cond: (pid, g, deps) => !!pickMissionRival(pid, g, deps),
    opts: [
      {
  ...
  },
  {
    ...
    cond: (pid, g) => (g.rel[pid] != null ? g.rel[pid] : 50) >= 48,
    opts: [
      {
  ...
  },
  {
    ...
    cond: (pid, g, deps) => !hasRatifiedDeal(pid, g, deps),
    opts: [
      {
...
function pickMissionRival(partnerId, g, deps) {
...
export function rollMissionEvent(missionId, partnerId, g, deps, det, eventIndex) {
  const pool = MISSION_EVENTS.filter((ev) => {
    if (!ev.missions.includes(missionId)) return false;
    if (ev.cond && !ev.cond(partnerId, g, deps)) return false;
    return true;
  });
...
export function formatMissionEventText(ev, g, deps) {
