 ...
 */
import { TRADE_MATRIX } from "./tradeMatrix.ts";
import { NATION_PROFILE } from "./nationProfiles.ts";

const MATRIX: Record<string, Record<string, number>> = TRADE_MATRIX;
...
/** Seat ids that have nation profiles (includes kingdom). */
export function worldSeatIds() {
...
/**
 * @param seats - { [id]: { y, shareX, shareM, gdp0, fx, fxPeer, tariff, access, relFac } }
 *   tariff[j] / access[j] optional per destination.
 * @returns { flows: { [i]: { [j]: number } }, totals: { [i]: { X: number, M: number } } }
 */
export function clearBilateralTrade(seats: Record<string, any>) {
  const ids = Object.keys(seats);
  const desired: Record<string, Record<string, number>> = {};
  for (const i of ids) {
    desired[i] = {};
    const si = seats[i];
  ...
  /* Import demand scale per seat from domestic absorption proxy. */
  const Mstar: Record<string, number> = {};
  for (const i of ids) {
    const si = seats[i];
  ...
  /* Average directed flows with reverse import claim so world roughly clears. */
  ...
  const totals: Record<string, { X: number; M: number }> = {};
  for (const i of ids) {
...
function shareMOf(s: any) {
  return (s && s.shareM != null ? s.shareM : 28) / 100;
}

/** Build seat snapshot map for clearing from world bags + player econ. */
export function seatsFromWorld(
  world: Record<string, any> | null | undefined,
  playerId: string,
  playerEcon: any,
