...
export function capitalShortfallHint(need, have) {
...
export function coachStepAt(i) {
  return COACH_STEPS[i] || null;
}

function taxRateUp(draft, law, id, minPts) {
...
function defaultTariffCut(draft, law, minPts) {
  const d =
 ...
 * @returns {{ id: string, label: string, done: boolean }[]|null}
 */
export function coachSubtasks(stepIndex, ctx) {
 ...
 *   rel?: object,
 *   blocMember?: object,
 *   blocAccession?: object,
 *   blocAccessionByCountry?: object,
 *   homeRole?: string,
 *   billLen?: number,
 * }} ctx
 */
export function coachAwaitSatisfied(stepIndex, ctx) {
  const step = COACH_STEPS[stepIndex];
