 ...
 */
import { realmByRole } from "../sim/realms.ts";
...
export function publicRoom(room: Room, viewerToken?: string | null) {
  const me: MpPlayer | undefined = viewerToken
...
export async function createRoom({
  ...
  /* One KV round-trip per attempt (SET NX). Old path did EXISTS/GET then SET. */
  for (let i = 0; i < 8; i++) {
    const c = code();
    const room: Room = {
      code: c,
    ...
    if (await saveRoomIfAbsent(room)) {
      return { room: publicRoom(room, hostTok), token: hostTok };
    }
  }
  return { error: "Could not allocate a room code", status: 500 };
}

export async function getRoom(codeStr: string, viewerToken?: string | null) {
...
export async function joinRoom(
  ...
  if (!(await saveRoomCas(room, baseVer))) {
    return { error: "Room changed — try again", status: 409 };
  }
 ...
 */
export async function unsubmitBill(codeStr: string, playerToken: string) {
  const room = await loadRoom(codeStr);
  if (!room) return { error: "Room not found", status: 404 };
 ...
 */
export async function leaveRoom(codeStr: string, playerToken: string) {
  const room = await loadRoom(codeStr);
  if (!room) return { error: "Room not found", status: 404 };
