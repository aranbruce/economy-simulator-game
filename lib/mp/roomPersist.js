...
function logKvFailure(kind, err) {
 ...
 */
export async function loadRoom(codeStr, opts = {}) {
  const key = String(codeStr || "").toUpperCase();
  if (!key) return null;
  const mem = roomsMap().get(key) || null;
  if (opts.preferMemory && mem) return mem;
  ...
  if (cfg) {
    try {
      ...
      if (raw == null || raw === "") {
        ...
        if (allowMemoryFallback()) return mem;
        roomsMap().delete(key);
        return null;
      }
 ...
 */
export async function saveRoomIfAbsent(room) {
