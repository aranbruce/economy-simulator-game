...
import { loadRoom, saveRoomCas } from "./roomPersist.ts";

const CONFLICT = { error: "Room changed — try again", status: 409 };

async function commitRoom(room: Room, baseVersion: number) {
...
export async function applyDiploAction(
  ...
  const result: any = applyMpDiploAction(room.snapshot, player.seatId, action, {
...
interface EventChoiceBody {
  inboundUltimatum?: boolean;
  fromId?: string;
  concede?: boolean;
